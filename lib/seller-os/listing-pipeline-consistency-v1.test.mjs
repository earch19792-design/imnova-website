...
test('normal read reevaluates the same immutable package after Shipping arrives without capture',async()=>{
 ...
 const before=await readSellOneLikeThisV1(request);assert.equal(before.consistency.PACKAGE_CONSISTENT,false)
 assert.ok(before.consistency.inconsistencies.includes('SOURCE_EVIDENCE_MATCH'))
