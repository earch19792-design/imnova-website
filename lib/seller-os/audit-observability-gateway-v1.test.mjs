...
test("Product Case fails closed without field receipts; downstream package is not source truth", async () => {
  ...
  tables.seller_os_commercial_trace_pricing_enrichment_jobs_v1 = [{
    job_id: "pricing-job", trace_id: "trace", receipt_digest: receiptDigest,
    mechanism_version:
      "COMMERCIAL_TRACE_PRICING_BROWSER_CAPTURE_V2_EXTENSION_1_2_40",
    generation: 2, completed_at: "2026-09-21T00:00:00Z" }]
  const withPricingReceipt=await readSellerOsProductCaseAuditV1({supabase,
    accountKey:'test',identityType:'LUNA_PRODUCT_ID',identity:'123',
    detailMode:'TRACE'})
  assert.equal(withPricingReceipt.PRODUCT_JOURNEY.find(x=>
    x.STAGE==='MARKET_RESEARCH').STATUS,'PROVEN')
  assert.ok(withPricingReceipt.TECHNICAL_TRACE.researchEvidenceIds.includes(
    receiptDigest))
  assert.equal(withPricingReceipt.safety.databaseBusinessWrites,0)
  assert.equal(withPricingReceipt.safety.marketplaceWrites,0)
})
