...
test("fresh Product Research heartbeat is not parked before its first capture", () => {
  const stamp = "2026-09-07T00:59:00.000Z"
  const result = evaluateSellerOsRuntimeCapabilityAssuranceV1({ now: NOW,
    runtimeHealth: runtimeHealth(), evidence: evidence({
      researchCapture: {},
      researchTasks: { pendingCount: 1, latestCapturedAt: null,
        latestProcessedAt: null },
      researchPlans: { latestUpdatedAt: stamp },
      workerLiveness: [
        { capability_id: "PRODUCT_RESEARCH_EXTENSION",
          heartbeat_source: "INDEPENDENT_WORKER_LIVENESS",
          physical_connection: "PROVEN_AVAILABLE", extension_identity_match: true,
          observed_at: "2026-09-07T00:59:00.000Z",
          fresh_until: "2026-09-07T01:04:00.000Z" },
        { capability_id: "PRODUCT_RESEARCH_BROWSER_WORKER",
          heartbeat_source: "INDEPENDENT_WORKER_LIVENESS",
          physical_connection: "PROVEN_AVAILABLE", extension_identity_match: true,
          observed_at: "2026-09-07T00:59:00.000Z",
          fresh_until: "2026-09-07T01:04:00.000Z" },
      ],
      workerLivenessReadAvailable: true,
    }) })
  for (const capabilityId of ["PRODUCT_RESEARCH_EXTENSION",
    "PRODUCT_RESEARCH_BROWSER_WORKER"]) {
    const worker = result.capabilityMatrix.find((entry) =>
      entry.capabilityId === capabilityId)
    assert.equal(worker.finalHealthState, "HEALTHY", capabilityId)
    assert.notEqual(result.checks.find((entry) => entry.invariantCode ===
      `CAPABILITY_EXPECTED_OUTPUT:${capabilityId}`).failureClass,
    "RUNNER_PARKED_AFTER_FAILURE")
  }
})
