...
import {visualExecutionScopeV1,visualScopeFailureDefinitelyUnsentV1} from './mayel-visual-execution-scope-v1.ts'
...
test('UNSUPPORTED_CHANGE_FAIL_CLOSED_PASS: no publication, spend, price, inventory, policy or identity authority',()=>{
 for(const action of ['NEW_LISTING_PUBLICATION','EBAY_ADS_SPEND','PRICE','QUANTITY','SKU','BRAND','INVENTORY']){
...
test('rejected independent proposals never block the safe asset selected by the explicit manifest',async()=>{
 const {task,asset}=fixture('ADD_SECONDARY')
 const rejected={...asset,id:'rejected-independent',status:'rejected',mayel_approval_status:'REJECTED',qa_result:{automaticStatus:'PASSED',humanReview:{decision:'REJECT',reason:'IDENTITY_DRIFT'}}}
 const result=await readDelegatedVisualAuthorityV1({supabase:{},accountKey:account,task,assets:[rejected,asset],grant})
 assert.equal(result.authorized,true)
 const badTask={...task,visual_manifest:{...task.visual_manifest,proposedOrderedImages:task.visual_manifest.proposedOrderedImages.map(e=>e.assetId?{...e,assetId:rejected.id}:e)}}
 assert.equal((await readDelegatedVisualAuthorityV1({supabase:{},accountKey:account,task:badTask,assets:[rejected,asset],grant})).authorized,false)
 assert.equal(rejected.qa_result.humanReview.reason,'IDENTITY_DRIFT')
})
test('NO_OWNER_ROUTINE_APPROVAL_PASS: real handoff creates one durable intent, repeats reuse it',async()=>{
 ...
 task.selection_signal.pendingGalleryDecision={contract:'MAYEL_PENDING_FULL_GALLERY_DECISION_V1',expectedManifestDigest:'older-generation',state:'REQUIRES_ATTENTION'}
...
test('OFFICIAL_READBACK_REQUIRED_PASS and NO_DUPLICATE_WRITE_PASS: uncertain commit reads, never replays write',async()=>{
 ...
 assert.equal(visualAssetStatusV1({...ui,officialReadback:true,readbackCompatible:true}).synced,true)
})
...
test('scoped runtime rejects mismatched item, manifest and absent delegation before outbox or eBay',async()=>{
 const {runDelegatedVisualScopedV1}=await import('./mayel-delegated-visual-scoped-run-v1.ts')
 const {task}=fixture()
 const input={accountKey:account,taskId:id,expectedItemId:item,expectedManifestDigest:task.visual_manifest_digest}
 const db={from(table){assert.ok(['ebay_mayel_visual_tasks_v1','seller_os_mayel_optimization_grants_v1'].includes(table));return {
  select(){return this},eq(){return this},is(){return this},limit(){return this},maybeSingle:async()=>({data:table==='ebay_mayel_visual_tasks_v1'?task:null,error:null})}}}
 await assert.rejects(runDelegatedVisualScopedV1({...input,supabase:db,expectedItemId:'366582671136'}),/SCOPE_CHANGED/)
 await assert.rejects(runDelegatedVisualScopedV1({...input,supabase:db,expectedManifestDigest:'sha256:'+'0'.repeat(64)}),/SCOPE_CHANGED/)
 await assert.rejects(runDelegatedVisualScopedV1({...input,supabase:db}),/OWNER_DELEGATION_REQUIRED/)
})
test('REPLACE_MAIN execution persists target hero and exact own package without rewriting the historical task',()=>{
 const {task,asset}=fixture();task.listing_package_id=null
 const proof={authority:'EXACT_LIVE_LINKED_PRODUCT_TRUTH_V1',taskId:id,itemId:item,accountKey:account,
  visualSourceDigest:task.source_image_set_digest,packageId:'00000000-0000-4000-8000-000000000005'}
 const r=visualExecutionScopeV1(task,[asset.public_url],proof)
 assert.equal(r.mainImageUrl,asset.public_url);assert.equal(r.listingPackageId,proof.packageId);assert.equal(task.listing_package_id,null)
 assert.throws(()=>visualExecutionScopeV1(task,[asset.public_url],{...proof,itemId:'other'}))
})
test('known local CHECK rejection resumes same intent only after readback; ambiguous or existing execution cannot retry',async()=>{
 const {task}=fixture();task.listing_package_id=null
 const input={reason:'MAYEL_VISUAL_PHASE_B_OWNER_APPROVAL_PERSIST_FAILED',dispatchCount:1,executionAbsent:true,manifestMatches:true,task}
 assert.equal(visualScopeFailureDefinitelyUnsentV1(input),true)
 for(const change of [{reason:'TIMEOUT'},{executionAbsent:false},{manifestMatches:false},{dispatchCount:0}])
  assert.equal(visualScopeFailureDefinitelyUnsentV1({...input,...change}),false)
 const events=[];let applied=false
 const deps={authority:async()=>({approved:true,reason:null}),quota:async()=>({open:true}),transition:async()=>{},markDispatch:async()=>events.push('durable'),
  readback:async()=>{events.push('readback');return {official:true,baseHash:'base',matchesIntent:applied,safetyPass:true,reason:null,receipt:{official:true}}},
  execute:async()=>{events.push('write');applied=true;return {writes:1,mediaWrites:0}},finish:async state=>events.push(state)}
 await executeOutboxOperationV1({state:'OFFICIAL_READBACK_REQUIRED',dispatchCount:1,baseHash:'base',kind:'IMAGE_SYNC',preDispatchFailureProven:true},deps)
 assert.deepEqual(events,['readback','durable','write','readback','SYNCED'])
})
test('Inventory acknowledgment cannot mark a stale LIVE gallery synchronized',()=>{
 const input={official:true,ownerApproved:true,baseListingCompatible:true,approvedManifestDigest:'same',currentManifestDigest:'same',
  expectedImages:['https://assets.example/approved.jpg'],currentImages:['https://assets.example/approved.jpg']}
 assert.equal(approvedVisualReadbackMatchesV1({...input,liveGalleryVerified:false}),false)
 assert.equal(approvedVisualReadbackMatchesV1({...input,liveGalleryVerified:true}),true)
})
