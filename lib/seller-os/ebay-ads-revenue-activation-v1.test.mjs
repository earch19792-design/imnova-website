...
const { readAdsRevenueActivationV1 } = await import('./ebay-ads-revenue-runtime-v1.ts')
import { classifyEconomicsBlockersV1 } from './ebay-economics-quota-hold-v1.ts'
...
test('canonical live plus historical exact GetItem keeps current price/stock and rejects conflicting presence',()=>{
 const pair=()=>{const i=fixture();i.raw.listings[0].source='EBAY_TRADING_GET_MY_EBAY_SELLING';
  i.raw.listings.push({...i.raw.listings[0],source:'EBAY_TRADING_GET_ITEM_READONLY',last_ebay_sync_at:'2026-09-09T00:00:00Z',ebay_price:99});return i;}
 for(const reverse of [false,true]){const i=pair();if(reverse)i.raw.listings.reverse();const r=buildAdsActivationListingV1(i);
  assert.equal(r.exactItemBinding,true);assert.equal(r.preview.SALE_PRICE,52.99);assert.equal(r.preview.TITLE,'Synthetic canary');assert.equal(r.singleListingAdsCanaryReady,true);}
 for(const mutate of [i=>i.raw.listings[1].ebay_sku='OTHER',i=>i.raw.listings[1].ebay_item_id='888888888888',
  i=>i.raw.listings[1].currency='EUR',i=>i.raw.listings[1].source='EBAY_TRADING_GET_MY_EBAY_SELLING',
  i=>i.raw.listings[1].last_ebay_sync_at=i.raw.listings[0].last_ebay_sync_at,
  i=>i.raw.listings[1].last_ebay_sync_at='2026-09-11T00:00:00Z',i=>i.raw.listings.push({...i.raw.listings[1]})]){
  const i=pair();mutate(i);const r=buildAdsActivationListingV1(i);assert.equal(r.exactItemBinding,false);assert.equal(r.singleListingAdsCanaryReady,false);assert.equal(r.preview.SALE_PRICE,null);}
 const empty=pair();empty.raw.listings[0].ebay_quantity=0;assert.equal(buildAdsActivationListingV1(empty).inStock,false);
 const stale=pair();stale.currentLiveFresh=false;assert.equal(buildAdsActivationListingV1(stale).singleListingAdsCanaryReady,false);
})
test('fresh exact base policy supersedes old category diagnosis without approving unresolved total fees',()=>{
 const i=fixture(),a=i.raw.feeHeads[0].authority;a.state='PENDING_ORDER_CONTEXT';i.raw.feeHeads[0].state=a.state;
 a.knownPreSaleBasis={salePrice:52.99};a.resolvedAuthority=null;a.resolutionBlockers=['TOTAL_FEE_BASIS_UNPROVEN'];
 a.components=[{type:'TAX_ON_FEES',status:'PENDING_AUTHORITY',pendingDependency:'CURRENT_ACCOUNT_FEE_TAX_AUTHORITY'}];
 i.raw.evidence.push({evidence_type:'EXPECTED_EBAY_FEE',value_amount:null,limitation_code:'OFFICIAL_CATEGORY_FEE_POLICY_NOT_CERTIFIED'});
 const r=classifyEconomicsBlockersV1(i);assert.equal(r.economicsProven,false);assert.equal(r.preview.EBAY_FEES,null);
 assert.ok(!r.reasons.includes('FEE_AUTHORITY:OFFICIAL_CATEGORY_POLICY_NOT_CERTIFIED'));
 assert.ok(r.reasons.includes('FEE_AUTHORITY:TOTAL_FEE_BASIS_UNPROVEN'));
 assert.ok(r.reasons.includes('FEE_AUTHORITY:PENDING_AUTHORITY:CURRENT_ACCOUNT_FEE_TAX_AUTHORITY'));
 for(const mutate of [a=>a.freshUntil='2026-09-09T00:00:00Z',a=>a.itemId='888888888888',a=>a.sku='OTHER',
  a=>a.marketplaceAccountKey='OTHER',a=>a.knownPreSaleBasis.salePrice=55]){
  const j=structuredClone(i);mutate(j.raw.feeHeads[0].authority);
  assert.ok(classifyEconomicsBlockersV1(j).reasons.includes('FEE_AUTHORITY:OFFICIAL_CATEGORY_POLICY_NOT_CERTIFIED'));}
})
...
test('same active rate is a no-op, not the required one Ads action',()=>{
 const i=fixture();Object.assign(i.official,{currentAdState:'ACTIVE',adId:'98765',currentAdRate:3})
...
test('explicit durable-only review forbids all official calls even when cached LIVE and economics are fresh',async()=>{
 const i=fixture()
 const supabase={from(){const q={select(){return q},eq(){return q},in(){return q},gte(){return q},order(){return q},then(resolve){return Promise.resolve({data:[]}).then(resolve)},limit(){return q},maybeSingle:async()=>({data:{current_live_source_state:'CURRENT_FRESH',last_certified_live_item_ids:[i.raw.itemId],last_certified_live_count:1,last_certified_live_observed_at:'2026-09-10T00:00:00Z',last_certified_live_fresh_until:'2026-09-10T00:20:00Z',last_certified_live_scope_id:'current-live:sha256:'+'a'.repeat(64),last_certified_live_source_authority:'EBAY_TRADING_GET_MY_EBAY_SELLING_PLUS_GET_ITEM_CERTIFICATION'}})};return q},rpc:async()=>({data:[i.raw]})}
 const r=await readAdsRevenueActivationV1({supabase,accountKey:i.accountKey,actorId:'TEST_ACTOR',itemIds:[i.raw.itemId],durableOnly:true,now})
 assert.equal(r.currentLiveState,'CURRENT_FRESH');assert.equal(r.rows[0].economicsProven,true)
 assert.equal(r.officialApiCalls,0);assert.equal(r.officialCandidatesExamined,0);assert.equal(r.resume.next,'WAIT_FOR_QUOTA_RECOVERY')
})
