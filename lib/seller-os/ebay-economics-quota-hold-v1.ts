...
export function classifyEconomicsBlockersV1(input: {accountKey:string;raw:unknown;now:Date}) {
  ...
  if(heads.length===1 && !row.ebayFeeAuthorityPass){
    ...
    for(const code of Array.isArray(a.resolutionBlockers)?a.resolutionBlockers:[]) if(typeof code==="string")
      reasons.push(`FEE_AUTHORITY:${code}`)
  ...
  if(!row.basePreSaleFeeProven && legacy?.limitation_code==="OFFICIAL_CATEGORY_FEE_POLICY_NOT_CERTIFIED") reasons.push("FEE_AUTHORITY:OFFICIAL_CATEGORY_POLICY_NOT_CERTIFIED")
