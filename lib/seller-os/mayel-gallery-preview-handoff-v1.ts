...
export async function confirmMayelGalleryPreviewV1(input: { supabase: SupabaseClient; accountKey: string;
  ...
  const taskRead = await input.supabase.from("ebay_mayel_visual_tasks_v1")
    ...
    .eq("id", input.taskId).eq("marketplace_account_key", input.accountKey).eq("assigned_operator_user_id", input.actorUserId).maybeSingle()
  ...
  if (taskRead.error || !task || task.visual_manifest_digest !== input.expectedDigest ||
      manifest?.galleryPolicy !== "REPLACE_APPROVED_SLOTS_ONLY" || !Array.isArray(manifest.slotReplacements))
    throw Error("MAYEL_GALLERY_PREVIEW_STALE")
  const order = replacementSlotOrderV1(task.current_image_set, manifest.slotReplacements)
  ...
  if (!preview || order.some((entry, position) => entry.kind === "MAYEL_ASSET"
  ...
  const assetIds = manifest.slotReplacements.map((r: { assetId: string }) => r.assetId)
  ...
  if (assets.error || assets.data?.length !== assetIds.length || assets.data.some(a => !visualAssetOwnerApprovedV1(a, task)))
