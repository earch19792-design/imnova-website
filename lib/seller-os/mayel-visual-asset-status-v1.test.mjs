...
test('NO_FALSE_SYNCED_MESSAGE_PASS excludes dispatch, timeout and unverified SYNCED',()=>{
 ...
 const done={...base,qaPassed:true,savedToSellerOS:true,ownerApproved:true,serverReceiptPresent:true,state:'SYNCED',officialReadback:true}
 assert.equal(visualAssetStatusV1(done).status,'SYNCED_WITH_EBAY')
 for(const k of ['qaPassed','savedToSellerOS','ownerApproved','serverReceiptPresent','officialReadback'])assert.equal(visualAssetStatusV1({...done,[k]:false}).synced,false)
})
test('PER_IMAGE_STATUS_PASS / PROGRESS_STEPPER_PASS six independent assets',()=>{
 const six=[base,{...base,qaPassed:true},{...base,savedToSellerOS:true},{...base,savedToSellerOS:true,qaPassed:true},
  ...
  {...base,qaPassed:true,savedToSellerOS:true,ownerApproved:true,serverReceiptPresent:true,state:'SYNCED',officialReadback:true}].map(visualAssetStatusV1)
...
test('IPAD_VISUAL_STATUS_PASS visible labels, touch targets, details remain closed',()=>{
 ...
 assert.match(progress,/minmax\(7rem,1fr\)/);assert.match(progress,/aria-hidden="true"/);assert.match(ui,/min-h-11/)
