...
import {galleryReorderDecisionV1} from './mayel-gallery-reorder-v1.ts'
...
test('autonomous reorder is an explicit permutation of the full official gallery, never removal or invention',()=>{
 const before=Array.from({length:6},(_,i)=>`https://i.ebayimg.com/${i}.jpg`),after=[before[2],before[0],before[1],...before.slice(3)]
 const result=galleryReorderDecisionV1(before,after)
 assert.equal(result.changed,true);assert.deepEqual(result.actions,['IMAGE_REORDER']);assert.equal(result.unauthorizedImageRemovalCount,0)
 assert.equal(result.qa.pass,true);assert.deepEqual(result.beforeGallery,before);assert.deepEqual(result.afterGallery,after)
 assert.equal(galleryReorderDecisionV1(before,before).changed,false)
 for(const bad of [after.slice(1),[...after,'https://i.ebayimg.com/new.jpg'],[after[0],...after.slice(0,5)],['https://evil.test/foreign.jpg',...after.slice(1)]])
  assert.throws(()=>galleryReorderDecisionV1(before,bad))
})
