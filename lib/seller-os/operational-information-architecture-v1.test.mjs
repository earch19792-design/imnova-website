...
test("canonical navigation has seven operational areas and preserves legacy route mapping", async () => {
  const navigation = await import("./navigation.ts")
  assert.deepEqual(navigation.SELLER_OS_PRIMARY_NAVIGATION.map((item) =>
    item.label), ["Dashboard", "Opportunities", "Listings", "StockGuard",
  ...
  assert.equal(navigation.sellerOsNavigationItem("publish").id, "opportunities")
  assert.equal(navigation.sellerOsNavigationItem("live").id, "listings")
})
...
test("Home consumes one shared read-only snapshot and freezes Publisher", () => {
  ...
  assert.match(home, /const lunaState = authority\.lunaCompactState/)
...
test("every cron executor is POST-only and GET uses the zero-write guard", () => {
  ...
  assert.ok(routes.includes("app/api/cron/luna-catalog-snapshot/route.ts"))
...
test("durable auditor learns mechanisms without product or marketplace authority", () => {
  ...
  assert.match(runtime, /runMayelVisualSafeRebaseRecoveryV1/)
  const mayelRecovery = read(
    "lib/ebay/ebay-mayel-visual-safe-rebase-runtime-v1.ts")
  assert.match(mayelRecovery, /SAFE_IDEMPOTENT_RUNTIME_RESUME/)
  assert.match(mayelRecovery, /claim_seller_os_operational_integrity_v1/)
  assert.match(mayelRecovery, /rebaseMayelVisualPhaseBPreviewV1/)
  assert.match(mayelRecovery, /marketplaceWrites:\s*0/)
})

test("Mayel rendering cannot acquire delegated work", () => {
  const workstation = read("app/admin/mayel-visual-workstation.tsx")
  const parsed = ts.createSourceFile("workstation.tsx", workstation,
    ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX)
  const effects = []
  function visit(node) {
    if (ts.isCallExpression(node) && node.expression.getText(parsed) === "useEffect") {
      effects.push(node.getText(parsed))
    }
    ts.forEachChild(node, visit)
  }
  visit(parsed)
  assert.ok(effects.length > 0, "effect inspection must not silently match nothing")
  assert.match(effects.join("\n"), /await load\(\)/)
  assert.doesNotMatch(effects.join("\n"),
    /ENSURE_NEXT_TASK|ENSURE_VISUAL_TASK|openVisualListing|onOpen\(/)
  assert.match(workstation, /onClick=\{\(\) => void onOpen\(listing\.ebayItemId\)\}/)
  assert.match(workstation, /onOpen=\{openVisualListing\}/)
})
