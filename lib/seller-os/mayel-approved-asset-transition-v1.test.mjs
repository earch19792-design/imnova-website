...
import {approvalGalleryV1,assertReviewIntentV1,savedSafeDecisionV1,recoverableVisualAssetV1,MAYEL_ASSET_TRANSITION_V1} from './mayel-approved-asset-transition-v1.ts'
...
test('an older unreviewed proposal cannot starve the safe pending decision',()=>{
 const unknown={...asset,id:'unknown',qa_result:{automaticStatus:'PASSED'}}
 const rejected={...asset,id:'rejected',status:'rejected',mayel_approval_status:'REJECTED'}
 assert.equal(recoverableVisualAssetV1([unknown,rejected,asset],task).id,asset.id)
 assert.equal(rejected.status,'rejected');assert.equal(unknown.mayel_approval_status,'PENDING')
})
...
test('FULL delegation removes routine approval status without converting unreviewed or rejected assets to safe',()=>{
 ...
 const resolved=visualAssetSyncViewV1({...asset,status:'approved',mayel_approval_status:'APPROVED',qa_result:{...asset.qa_result,transitionRecovery:{state:'REQUIRES_ATTENTION'}}},task,[],{active:true,authorized:true})
 assert.equal(resolved.state,'APPROVED_FOR_EBAY_SYNC');assert.equal(resolved.officialReadback,false)
...
test('real recovery refuses an unproven or rejected proposal without promotion, outbox or eBay',async()=>{
 ...
 for(const rejected of [false,true]){
  ...
  const r=await recoverApprovedAssetTransitionV1({supabase:db,accountKey:'account',taskId:task.id,itemId:task.ebay_item_id})
