...
async function setup(){const db=new PGlite();await db.exec(`create role anon;create role authenticated;create role service_role bypassrls;create schema auth;create table auth.users(id uuid primary key);
 ...
 // Reproduce actual hosted Supabase default privileges, not an empty PG ACL.
 ...
 await db.exec(migration('20260911055149_mayel_removal_review_explicit_service_acl_v1.sql'))
...
test('real durable producer/consumer: semantic removal beyond duplicate hashes, immutable/idempotent receipts and revalidation',async()=>{
 ...
 has_table_privilege('service_role','seller_os_mayel_removal_reviews_v1','DELETE') can_delete,
 has_table_privilege('service_role','seller_os_mayel_removal_reviews_v1','TRUNCATE') can_truncate,
 has_column_privilege('service_role','seller_os_mayel_removal_reviews_v1','checks','UPDATE') can_rewrite_qa,
 has_column_privilege('service_role','seller_os_mayel_removal_reviews_v1','revoked_at','UPDATE') can_revoke`)).rows[0]
 assert.deepEqual(acl,{anon:false,auth:false,can_delete:false,can_truncate:false,can_rewrite_qa:false,can_revoke:true})
