...
function database(f){
 ...
 const b=f.binding
 const tables={ebay_current_listing_packages_v1:{id:f.packageId,factory:f.factory,opportunity_id:b.OPPORTUNITY_ID,candidate_key:b.CANDIDATE_KEY,account_key:b.ACCOUNT_KEY,ownPrice:f.ownPrice,ownAspects:f.ownAspects,category:f.category},ebay_luna_opportunity_queue:{id:b.OPPORTUNITY_ID,candidate_key:b.CANDIDATE_KEY,supplier_product_id:b.PRODUCT_ID,supplier_variant_id:b.VARIANT_ID,supplier_sku:"OUR-SKU",shippingFamily:"market-family-v1:sha256:"+"e".repeat(64),productTruthDigest:f.productTruthDigest,truthFields:f.truthFields,canonicalReadiness:f.canonicalReadiness,requiredTruth:f.requiredTruth,aspectResolutions:f.aspectResolutions},seller_os_product_research_canonical_evidence_v2:f.reference}
 const db={from(table){const log={table,filters:[],limit:null};queries.push(log);const q={select(s){assert.ok(!s.includes("*"));return this},eq(k,v){log.filters.push([k,v]);return this},order(){return this},limit(n){log.limit=n;return this},abortSignal(){return this},retry(v){assert.equal(v,false);return this},async maybeSingle(){const row=table==="seller_os_profitability_frontier_snapshots"?shipping:tables[table];assert.equal(log.limit,1);return {data:row&&log.filters.every(([k,v])=>row[k]===v)?row:null,error:null}},then(resolve,reject){return Promise.resolve({data:[],error:null}).then(resolve,reject)},insert(){throw Error("WRITE")},update(){throw Error("WRITE")}};return q},rpc(name,args){if(name==="get_seller_os_latest_profitability_frontiers_v1"){
 assert.deepEqual(args.p_family_ids,["market-family-v1:sha256:"+"e".repeat(64)]);assert.equal(args.p_account_key,b.ACCOUNT_KEY)
 return {abortSignal(){return this},retry(v){assert.equal(v,false);return this},then(resolve,reject){return Promise.resolve({data:{frontiers:shipping?[{accountKey:b.ACCOUNT_KEY,marketplaceId:"EBAY_US",frontier:{familyId:shipping.family_id,lunaProductId:shipping.luna_product_id,lunaVariantId:shipping.luna_variant_id,lunaSku:shipping.luna_sku,shippingStatus:shipping.shipping_status,shippingValue:shipping.shipping_value,shippingCaptureEvidence:shipping.shippingEvidence}}]:[]},error:null}).then(resolve,reject)}}
 }assert.equal(name,"read_current_factory_keyword_handoff_v2_1");assert.equal(args.p_listing_package_id,f.packageId);assert.equal(args.p_variant_id,b.VARIANT_ID);const currentBinding={...f.keywordRead.BINDING,PACKAGE_ID:f.packageId};return Promise.resolve({data:{...f.keywordRead,BINDING:currentBinding,VALIDATION:{...f.keywordRead.VALIDATION,TRANSPORT_DIGEST:digest({BINDING:currentBinding,DECISION:f.keywordRead.DECISION})}},error:null})}}
 return {db,queries,mutateQuote(change){change(shipping)},fresh(){shipping={frontier_id:"frontier",family_id:"market-family-v1:sha256:"+"e".repeat(64),account_key:b.ACCOUNT_KEY,marketplace_id:"EBAY_US",luna_product_id:b.PRODUCT_ID,luna_variant_id:b.VARIANT_ID,luna_sku:"OUR-SKU",shipping_status:"SHIPPING_DURABLY_PERSISTED",shipping_value:4,shippingEvidence:{candidateId:"sha256:"+createHash("sha256").update(JSON.stringify({familyId:"market-family-v1:sha256:"+"e".repeat(64),productId:b.PRODUCT_ID,variantId:b.VARIANT_ID,sku:"OUR-SKU"})).digest("hex"),lunaProductId:b.PRODUCT_ID,lunaVariantId:b.VARIANT_ID,supplierSku:"OUR-SKU",canonicalDestinationMatch:true,canonicalDestinationFingerprint:destination.profileDigest,currency:"USD",quantity:1,shippingUsd:4,noPurchase:true,noCredentials:true,acquisitionMethod:"LUNA_PROTECTED_BROWSER_CHECKOUT_SHIPPING",evidenceDigest:digest("quote"),observedAt:"2026-09-10T14:00:00Z"}}}}
}
test("normal read attaches later fresh Shipping to same package; no claims, global scan or writes",async()=>{
 ...
 assert.equal(d.queries.some(q=>q.table==="seller_os_profitability_frontier_snapshots"),false)
...
test("certified frontier reader rejects wrong product and expired evidence without requesting another capture",async()=>{
 for(const change of [q=>q.shippingEvidence.lunaProductId="wrong",q=>q.shippingEvidence.lunaVariantId="wrong",q=>q.shippingEvidence.observedAt="2026-09-01T00:00:00Z",q=>q.shippingEvidence.canonicalDestinationMatch=false]){
  const f=fixture(),d=database(f);d.fresh();d.mutateQuote(change)
  const r=await readSellOneLikeThisV1({supabase:d.db,accountKey:f.binding.ACCOUNT_KEY,packageId:f.packageId,referenceItemId:f.reference.item_id,now:f.now})
  assert.equal(r.consistency.SHIPPING_STATUS,"WAITING_FOR_DATA")
  assert.equal(d.queries.some(q=>q.table==="seller_os_profitability_frontier_snapshots"),false)
 }
})

test("new CURRENT intake reports pending evidence without inventing a competitor reference",async()=>{
 const f=fixture();f.factory={version:'SELLER_OS_CURRENT_PUBLICATION_FACTORY_V1',packageId:f.packageId,generation:'current-generation',accountKey:f.binding.ACCOUNT_KEY,
 productId:f.binding.PRODUCT_ID,variantId:f.binding.VARIANT_ID,supplierSku:'OUR-SKU',authorityPolicy:'CURRENT_ONLY',reuseLegacyPreparation:false}
 const d=database(f),r=await readSellOneLikeThisV1({supabase:d.db,accountKey:f.binding.ACCOUNT_KEY,packageId:f.packageId,referenceItemId:'',now:f.now})
 assert.equal(r.referenceImport.pass,false);assert.equal(r.publicationGate.READY_TO_PUBLISH,false);assert.equal(r.previewPass,false)
 assert.equal(r.currentFactory.ownerActionRequired,false)
 const prior=fixture(),old=database(prior)
 await assert.rejects(readSellOneLikeThisV1({supabase:old.db,accountKey:prior.binding.ACCOUNT_KEY,packageId:prior.packageId,referenceItemId:'',now:prior.now}),/REFERENCE_INPUT_INVALID/)
})

test("settled CURRENT package consumes exact canonical specifics without legacy reference authority",async()=>{
 const f=fixture(),productTruthDigest=digest("truth-current")
 f.productTruthDigest=productTruthDigest
 f.factory={version:'SELLER_OS_CURRENT_PUBLICATION_FACTORY_V1',packageId:f.packageId,generation:'current-generation',accountKey:f.binding.ACCOUNT_KEY,
  productId:f.binding.PRODUCT_ID,variantId:f.binding.VARIANT_ID,supplierSku:'OUR-SKU',authorityPolicy:'CURRENT_ONLY',reuseLegacyPreparation:false}
 f.ownAspects={Brand:'Unbranded',Style:'Charm'}
 f.canonicalReadiness={ready:true,requiredItemSpecificsReady:true,categoryId:'261987',productTruthDigest,
  requiredItemSpecificsTruth:{evidenceDigest:f.requiredTruth.evidenceDigest}}
 const d=database(f),r=await readSellOneLikeThisV1({supabase:d.db,accountKey:f.binding.ACCOUNT_KEY,packageId:f.packageId,referenceItemId:'',now:f.now})
 assert.equal(r.referenceImport.pass,false);assert.equal(r.referenceImport.required,false)
 assert.equal(r.referenceImport.currentOnlyAuthorityUsed,true)
 assert.equal(r.itemSpecificsPass,true);assert.equal(r.blockers.includes('REFERENCE_STRUCTURE_UNPROVEN'),false)
 assert.deepEqual(r.preview.itemSpecifics,{Brand:'Unbranded',Style:'Charm'})
})
