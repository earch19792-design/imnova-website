...
function dbForWorkspace(phase, approved = true) {
  ...
  const db = { from(table) { calls.push(table)
    ...
    }] : table === "ebay_listing_image_assets" ? [canonical] : table === "seller_os_ipad_outbox_v1" ? [] : [
    ...
    for (const name of ["select", "eq", "neq", "in", "order", "limit"]) chain[name] = () => chain
    return chain
...
test("offline reload resolves queued/applied only for the exact manifest without Trading or generation", async () => {
  ...
  try {
    ...
    assert.equal(first.tradingCalls, 0); assert.equal(calls.length, 5)
