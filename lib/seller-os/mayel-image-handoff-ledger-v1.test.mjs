...
test("automatic image admission requires durable QA, exact origin and full human review; pending discovery skips completed manifests", async () => {
  ...
  try {
    ...
    await db.exec(readFileSync(new URL("../../supabase/migrations/20260909203958_assistant_image_source_derivative_binding_v1.sql", import.meta.url), "utf8"))
    ...
    assert.equal((await db.query("select public.seller_os_same_ebay_image_source_v1($1,$2) as bound", ["https://i.ebayimg.com/images/g/same/s-l140.png", source])).rows[0].bound, true)
    for (const ref of ["https://i.ebayimg.com/images/g/other/s-l140.png", "https://i.ebayimg.com.evil.test/images/g/same/s-l140.png", "https://i.ebayimg.com/images/g/same/s-l140.jpg", "https://i.ebayimg.com/images/g/same/s-l140.png?other=true"])
      assert.equal((await db.query("select public.seller_os_same_ebay_image_source_v1($1,$2) as bound", [ref, source])).rows[0].bound, false)
