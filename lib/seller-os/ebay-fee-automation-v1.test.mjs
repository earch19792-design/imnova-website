...
test('reusing source evidence never extends its freshness from the new calculation time',()=>{
 const i=fixture();delete i.resolutionInputs;i.context.observedAt='2026-09-09T15:00:00Z'
 const a=produceEbayFeeAuthorityV1(i)
 assert.equal(a.sourceObservedAt,'2026-09-09T15:00:00Z')
 assert.equal(a.freshUntil,'2026-09-09T21:00:00.000Z')
})
...
test('official policy updates are parsed without extending a stale snapshot or guessing changed structures',()=>{
 ...
 const officeHtml=html.replace('</table>','<tr><td>Select Business &amp; Industrial categories: Heavy Equipment Parts &amp; Attachments &gt; Heavy Equipment Printing &amp; Graphic Arts &gt; Commercial Printing Presses Restaurant &amp; Food Service &gt; Food Trucks, Trailers &amp; Carts</td><td>3%</td></tr></table>')
 assert.ok(parseCurrentOfficialFeePolicyV1(officeHtml,now).rules.some(r=>r.id==='BUSINESS_OFFICE_GENERAL'))
 assert.ok(!parseCurrentOfficialFeePolicyV1(officeHtml.replace('categories: Heavy','categories: Office Heavy'),now).rules.some(r=>r.id==='BUSINESS_OFFICE_GENERAL'))
...
test('current official payout currency removes only the proven conversion dependency',()=>{
 const i=fixture();delete i.resolutionInputs;i.context.listing.currency='USD';i.context.payoutCurrencyAuthority={status:'PROVEN',accountBindingExact:true,currency:'USD',source:'https://apiz.ebay.com/sell/finances/v1/seller_funds_summary',authorityClass:'CURRENT_PENDING_PAYOUT_FUNDS_CURRENCY',observedAt:now.toISOString()}
 const component=x=>produceEbayFeeAuthorityV1(x).components.find(c=>c.type==='CURRENCY_CONVERSION')
 assert.equal(component(i).amount,0);assert.equal(produceEbayFeeAuthorityV1(i).amount,null)
 for(const patch of [{currency:'EUR'},{accountBindingExact:false},{status:'UNPROVEN'},{observedAt:'2026-09-01T00:00:00Z'},{source:'INVOICE_CURRENCY'},{observedAt:'2027-01-01T00:00:00Z'}]){
  const j=structuredClone(i);j.context.payoutCurrencyAuthority={...j.context.payoutCurrencyAuthority,...patch};assert.equal(component(j).amount,null)
 }
})
