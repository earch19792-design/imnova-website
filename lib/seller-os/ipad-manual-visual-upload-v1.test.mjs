...
test('six manual ChatGPT outputs use existing QA, survive response loss, preserve source provenance, and await complete human review',async()=>{
 ...
 f.task.status='OWNER_PREVIEW_READY';f.task.visual_manifest_digest='approved-manifest';f.task.visual_manifest={proposedOrderedImages:assets.map(a=>({assetId:a.id}))}
 for(const a of assets){a.status='approved';a.approved_by=id(2);a.qa_result.humanReview={decision:'APPROVE'}}
