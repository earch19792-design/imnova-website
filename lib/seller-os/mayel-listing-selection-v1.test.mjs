...
function db(state=stored){
 const calls=[]
 return {calls,from(table){const filters=[];let itemIds=[];const q={select(){return q},eq(k,v){filters.push([k,v]);return q},in(k,v){itemIds=v;return q},order(){return q},limit(n){calls.push({table,filters,itemIds,limit:n});return q},maybeSingle(){return Promise.resolve({data:state,error:null})},then(resolve){return Promise.resolve({data:itemIds.map(id=>({ebay_item_id:id,title:'Saved '+id,ebay_sku:'SKU'+id})),error:null}).then(resolve)}};return q}}
}
