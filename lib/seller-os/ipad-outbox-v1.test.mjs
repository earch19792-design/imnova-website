...
function fixture(overrides={}){
 ...
 transition:async(state)=>{events.push(`state:${state}`);row.state=state},
...
test('EBAY_QUOTA_HOLD_DRAFT_PASS and POST_QUOTA_AUTO_RESUME_PASS: same durable operation resumes with fresh official preflight',async()=>{
 ...
 assert.deepEqual(f.events.slice(4),['authority','state:PENDING_EBAY_SYNC','quota','state:REVALIDATING','readback','authority','mark','write','state:OFFICIAL_READBACK_REQUIRED','readback','finish:SYNCED'])
})
test('STALE_DRAFT_REVALIDATION_PASS: drift, missing base and safety failure cannot write',async()=>{
 ...
 const f=fixture();setup(f);await executeOutboxOperationV1(f.row,f.d);assert.equal(f.row.state,'REQUIRES_ATTENTION');assert.ok(!f.events.includes('write'))}
})
test('DUPLICATE_SYNC_BLOCKED_PASS and UNKNOWN_COMMIT_READBACK_PASS: accepted response loss resolves by readback only',async()=>{
 ...
 assert.equal(f.row.state,'OFFICIAL_READBACK_REQUIRED');assert.equal(f.row.dispatchCount,1)
 ...
 assert.equal(g.row.state,'REQUIRES_ATTENTION');assert.ok(g.events.includes('readback'));assert.ok(!g.events.includes('write'))
})
test('draft image QA and an old Ads policy never grant spending authority',async()=>{
 ...
 assert.equal(f.row.state,'OWNER_APPROVAL_REQUIRED');assert.ok(!f.events.includes('quota'));assert.ok(!f.events.includes('write'))
...
test('OWNER approval must survive fresh preflight; a revoked approval cannot dispatch',async()=>{
 const f=fixture();const read=f.d.readback;f.d.readback=async()=>{const r=await read();f.set({approved:false});return r}
 await executeOutboxOperationV1(f.row,f.d);assert.equal(f.row.state,'REQUIRES_ATTENTION');assert.equal(f.row.dispatchCount,0);assert.ok(!f.events.includes('write'))
})
test('authority read outage never creates a pending eBay sync for an unapproved draft',async()=>{
 const f=fixture();f.d.authority=async()=>{throw Error('TEMPORARY_UPSTREAM_FAILURE')}
 await executeOutboxOperationV1(f.row,f.d);assert.equal(f.row.state,'OWNER_APPROVAL_REQUIRED');assert.equal(f.row.dispatchCount,0);assert.ok(!f.events.includes('quota'))
})

test('private draft preparation is recoverable without quota and stops before owner approval',async()=>{
 const f=fixture({state:'DRAFT'});f.d.authority=async()=>{throw Error('TEMPORARY_UPSTREAM_FAILURE')}
 await executeOutboxOperationV1(f.row,f.d);assert.equal(f.row.state,'DRAFT');assert.ok(!f.events.includes('quota'))
 f.d.authority=async()=>({approved:false,reason:'OWNER_VISUAL_REVIEW_REQUIRED'})
 await executeOutboxOperationV1(f.row,f.d);assert.equal(f.row.state,'OWNER_APPROVAL_REQUIRED');assert.equal(f.row.dispatchCount,0);assert.ok(!f.events.includes('quota'))
})
