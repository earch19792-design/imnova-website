...
const evidenceReadIndexMigration = readFileSync(
  "supabase/migrations/20260912154237_current_keyword_evidence_read_index.sql",
  "utf8",
)
...
test("CURRENT semantic revalidation stays inside the bounded evidence-read path", () => {
  assert.match(evidenceReadIndexMigration,
    /capture_batch_id,[\s\S]*marketplace_account_key,[\s\S]*source_listing_id,[\s\S]*created_at desc,[\s\S]*id/)
  assert.match(evidenceReadIndexMigration, /no authority semantics are relaxed/i)
})
