...
import {parseSellingFeeTaxPolicyV1,retainedSellingFeeTaxPolicyV1} from '../ebay/ebay-selling-fee-tax-policy-v1.ts'
...
test('1136_STALE_GENERATION_FAIL_CLOSED_PASS: wrong item, task and old manifest cannot poison current receipt',()=>{
 ...
 assert.notEqual(visualAssetSyncViewV1(asset,task,[old,row]).state,'SYNCED') // A flag without the matching durable receipt is not proof.
...
test('PUBLICATION_VS_LIVE_SYNC_SEPARATION_PASS / QUALITY_MENU_DISCOVERABILITY_REGRESSION_PASS',()=>{
 const menu=read('../../app/admin/ebay/mayel/revenue-engine.tsx')
 for(const label of ['Publicar nuevo listing','Listos para publicar','Esperando datos','Requieren revisión','Pendientes de sincronizar con eBay','Revisar cambios de listings activos','Listing Quality de eBay'])assert.ok(menu.includes(label))
 assert.match(menu,/menu === 0 && owner && <OwnerListingQualityReportControl/)
 for(const state of ['OWNER_APPROVAL_REQUIRED','DRAFT','PENDING_EBAY_SYNC','APPROVED_FOR_EBAY_SYNC','REVALIDATING','SYNCING','SYNCED','REQUIRES_ATTENTION'])assert.ok(['Mejora detectada','Mayel está revisando','Esperando actualización','Pendiente de eBay','Requiere atención'].includes(friendlyVisualSyncV1(state).label))
})
...
test('OTHER_VARIABLE_COST_POLICY_PRESENT is independent of incomplete shipping/fees',()=>{
 ...
 assert.equal(otherVariableCostPolicyPresentV1(p.marketplaceAccountKey,'999999999999'),true)
 assert.equal(otherVariableCostPolicyPresentV1('different-account','999999999999'),false)
})
...
test('fresh durable official tax policy survives a transient document read without extending expiry',()=>{
 const policy=parseSellingFeeTaxPolicyV1('Tax on eBay selling fees. In the US, this currently applies to sellers located in Hawaii, South Dakota, Texas, and Washington.',now)
 assert.equal(retainedSellingFeeTaxPolicyV1([{},policy],now),policy)
 assert.equal(retainedSellingFeeTaxPolicyV1([policy],new Date('2026-09-11T10:00:00Z')),null)
 assert.equal(retainedSellingFeeTaxPolicyV1([{...policy,source:'https://invalid.test'}],now),null)
 assert.equal(retainedSellingFeeTaxPolicyV1([{...policy,applicableStates:['TX']}],now),null)
 assert.equal(retainedSellingFeeTaxPolicyV1([{...policy,freshUntil:'2030-01-01T00:00:00Z'}],now),null)
})
