...
test('PUBLICATION_VS_LIVE_SYNC_SEPARATION_PASS / QUALITY_MENU_DISCOVERABILITY_REGRESSION_PASS',()=>{
 ...
 for(const state of ['OWNER_APPROVAL_REQUIRED','DRAFT','PENDING_EBAY_SYNC','APPROVED_FOR_EBAY_SYNC','REVALIDATING','SYNCING','SYNCED','REQUIRES_ATTENTION'])assert.ok(['Mejora detectada','En revisión','Pendiente de eBay','Requiere atención'].includes(friendlyVisualSyncV1(state).label))
})
