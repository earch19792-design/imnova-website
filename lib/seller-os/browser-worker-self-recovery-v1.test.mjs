...
const stableIdentityMigration = readFileSync(
  "supabase/migrations/20260907063433_browser_worker_stable_identity_v1.sql",
  "utf8")
const firstAttemptFairnessMigration = readFileSync(
...
const shippingManifest = JSON.parse(readFileSync(
  "tools/browser-extensions/luna-shipping-capture/manifest.json", "utf8"))
const shippingBundleLatest = readFileSync(
  "public/seller-os-tools/luna-shipping-capture-extension.zip")
const shippingBundleVersioned = readFileSync(
  "public/seller-os-tools/luna-shipping-capture-extension-v1.0.55.zip")
...
test("PASS_SHIPPING_IDLE_WORKER_HEARTBEAT_REMAINS_FRESH", () => {
  const result = projectSellerOsBrowserWorkerCapabilityV1({
    row: heartbeatRow({ capability_id: "LUNA_SHIPPING",
      worker_family: "LUNA_SHIPPING", extension_version: "1.0.55" }),
...
test("PASS_NEVER_ATTEMPTED_RESEARCH_PRECEDES_RETRY", () => {
  assert.match(firstAttemptFairnessMigration,
    /then plan\.worker_claim_count[\s\S]*?else ledger\.recovery_attempt_count/)
  assert.match(firstAttemptFairnessMigration,
    /plan\.created_at,[\s\S]*?plan\.id/)
  assert.match(firstAttemptFairnessMigration, /worker_claim_count < 5/)
  assert.match(firstAttemptFairnessMigration, /worker_next_retry_at/)
  assert.match(firstAttemptFairnessMigration, /pg_advisory_xact_lock/)
})
...
test("PASS_EXISTING_SHIPPING_PENDING_JOBS_RECOVERABLE", () => {
  assert.equal(shippingManifest.version, "1.0.55")
  assert.deepEqual(shippingBundleLatest, shippingBundleVersioned)
  assert.ok(shippingBundleVersioned.length > 0)
...
test("PASS_SHIPPING_CANONICAL_PROFILE_RESTORED_WITHOUT_OWNER_CLICK", () => {
  assert.match(shippingControl, /recoverCanonicalDestination\?\.\(true\)/)
  assert.match(shippingControl, /productionRuntimeAuthorized/)
  assert.match(shippingControl, /purpose: "CANONICAL_BIND_BOOTSTRAP"/)
  assert.match(shippingControl, /canonicalBindingRecoveryInFlight/)
  assert.match(shippingControl, /!canonicalDestinationMismatchProven/)
...
test("PASS_BROWSER_RESTART_RECOVERS_WITHOUT_OWNER_CLICK", () => {
  ...
  assert.match(researchRunner, /stableProductResearchWorkerId/)
  assert.match(shippingBackground,
    /workerVersion=\$\{[\s\S]*?EXTENSION_BUILD_VERSION/)
  assert.match(shippingBackground,
    /chrome\.tabs\.update\(existingWorkerTab\.id/)
  assert.match(researchRunner,
    /SELLER_OS_PRODUCT_RESEARCH_BROWSER_WORKER_V1/)
  assert.doesNotMatch(researchRunner,
    /product-research-browser:\$\{crypto\.randomUUID\(\)\}/)
  assert.match(researchService, /p_lease_seconds: 300/)
  assert.match(stableIdentityMigration,
    /capability\.worker_instance_id = p_worker_id/)
  assert.match(stableIdentityMigration, /ledger\.lease_owner = p_worker_id/)
  assert.match(stableIdentityMigration, /plan\.worker_lease_owner = p_worker_id/)
  assert.doesNotMatch(stableIdentityMigration,
...
test("PASS_COMPLETED_DOWNSTREAM_PLAN_DOES_NOT_STARVE_PENDING_RESEARCH", () => {
  const fairness = readFileSync(
    "supabase/migrations/20260907060211_browser_worker_research_claim_fairness_v1.sql",
    "utf8",
  )
  ...
  const claim = fairness.slice(
    fairness.indexOf("create or replace function public.claim_next"),
    fairness.indexOf("create or replace function public.release_live"),
  )
  assert.match(claim, /plan\.status = 'ACTIVE'/)
  assert.match(claim, /task\.status = 'PENDING'/)
  assert.doesNotMatch(claim, /plan\.status = 'COMPLETED'/)
  assert.doesNotMatch(runner, /RESUME_MARKET_REVALIDATION_DOWNSTREAM/)
  assert.match(runner, /if \(browserWorkerControl && autonomous\) continue/)
  assert.match(fairness, /recovery_attempt_count < 5/)
  assert.match(fairness, /worker_claim_count < 5/)
  assert.match(fairness, /interval '5 minutes'/)
  assert.match(fairness, /RECOVERY_POLICY_EXHAUSTED/)
})
