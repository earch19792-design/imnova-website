...
test("PASS_QUICK_PICK_RESEARCH_PLAN_IS_AUTO_CLAIMABLE", () => {
  ...
  assert.match(researchService,
    /\.in\("source_context", \["LIVE_LISTING_REVALIDATION",[\s\S]*"QUICK_PICK_RESEARCH_REQUIRED", "LUNA_PRE_RESEARCH"\]\)/)
...
test("PASS_EXISTING_SHIPPING_PENDING_JOBS_RECOVERABLE", () => {
  assert.equal(shippingManifest.version, "1.0.56")
  assert.ok(shippingBundleLatest.length > 0)
