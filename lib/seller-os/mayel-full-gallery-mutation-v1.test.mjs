...
import {buildFullGalleryMutationV1 as build,fullGalleryManifestMatchesV1,fullGalleryReadbackV1,existingGalleryMutationDecisionV1,contentGalleryReceiptMatchesV1} from './mayel-full-gallery-mutation-v1.ts'
...
test('Mayel position decision persists its reason and never requires OWNER slot selection',()=>{
 ...
 assert.throws(()=>decideMayelAssetPositionV1({assetId:id(2),role:'DETAIL',currentImages:current,manifest:{}}),/POSITION_EVIDENCE_REQUIRED/)
})
...
test('migration: durable removal addendum, immutable history, private ACL and full-gallery grant gate',async()=>{
 ...
 const sql=readFileSync(new URL('../../supabase/migrations/20260910192811_mayel_full_gallery_mutation_control_v1.sql',import.meta.url),'utf8')
 await db.exec(sql)
 await db.exec(readFileSync(new URL('../../supabase/migrations/20260910194454_mayel_pending_gallery_discovery_backoff_v1.sql',import.meta.url),'utf8'))
 await db.exec(readFileSync(new URL('../../supabase/migrations/20260910195125_mayel_gallery_pending_generation_scope_v1.sql',import.meta.url),'utf8'))
 const historical={pendingGalleryDecision:{contract:'MAYEL_PENDING_FULL_GALLERY_DECISION_V1',state:'REQUIRES_ATTENTION',expectedManifestDigest:'old'}}
 for(const [generation,expected] of [['old',false],['new',true]]){
  const r=await db.query('select seller_os_current_gallery_preparation_due_v1($1::jsonb,$2,now()) due',[JSON.stringify(historical),generation]);assert.equal(r.rows[0].due,expected)
 }

 for(const [next,state,expected] of [['2026-09-11T00:00:00Z','WAITING_FOR_CURRENT_GALLERY',false],['2026-09-09T00:00:00Z','WAITING_FOR_CURRENT_GALLERY',true],['bad','WAITING_FOR_CURRENT_GALLERY',false],[null,'REQUIRES_ATTENTION',false]]){
  const r=await db.query('select seller_os_pending_gallery_due_v1($1::jsonb,$2::timestamptz) due',[JSON.stringify({pendingGalleryDecision:{contract:'MAYEL_PENDING_FULL_GALLERY_DECISION_V1',state,nextAttemptAt:next}}),'2026-09-10T00:00:00Z']);assert.equal(r.rows[0].due,expected)
 }
...
test('quota hold persists the full decision before any upstream read; replay does not rewrite it',async()=>{
 const {saveFullMayelGalleryV1}=await import('./mayel-full-gallery-server-v1.ts')
 const task={id:base.visualTaskId,ebay_item_id:base.ebayItemId,marketplace_account_key:'account',assigned_operator_user_id:id(5),status:'OWNER_PREVIEW_READY',visual_manifest_digest:truth,current_image_set:current,visual_manifest:{},selection_signal:{productTruthSupported:true},product_truth_digest:truth,source_image_set_digest:truth,source_image_references:[]}
 const grant={id:id(4),account_key:'account',owner_user_id:id(5),scope:'FULL',status:'ACTIVE',revoked_at:null,contract_version:'MAYEL_AUTONOMOUS_LISTING_OPTIMIZATION_DELEGATION_V1',authority_digest:truth,allowed_actions:['SECONDARY_IMAGE_REPLACEMENT']}
 const a={id:id(2),status:'approved',mayel_approval_status:'APPROVED',mayel_output_role:'DETAIL',public_url:assets[0].publicUrl,output_sha256:assets[0].outputSha256,product_truth_digest:truth,source_image_set_digest:truth,qa_result:{automaticStatus:'PASSED',humanReview:{decision:'APPROVE',checks:{productIdentityPreserved:true,colorPreserved:true,shapePreserved:true,partCountPreserved:true,visibleLogosPreserved:true,roleMatchesOutput:true,noUnsupportedClaims:true,noInventedAccessories:true,noUnauthorizedText:true}}}}
 let saves=0,upstream=0;const db={from(table){let update=null;return {select(){return this},eq(){return this},is(){return this},not(){return this},limit(){return this},update(v){update=v;return this},async maybeSingle(){if(update){saves++;Object.assign(task,update);return {data:{id:task.id},error:null}}return {data:table==='ebay_mayel_visual_tasks_v1'?task:table==='seller_os_mayel_optimization_grants_v1'?grant:null,error:null}},then(r,j){assert.equal(table,'ebay_listing_image_assets');return Promise.resolve({data:[a],error:null}).then(r,j)}}}}
 const args={supabase:db,accountKey:'account',actorUserId:id(5),taskId:task.id,expectedManifestDigest:truth,expectedCurrentImages:current,decisions:replacement(),readGallery:async()=>{upstream++;assert.equal(task.selection_signal.pendingGalleryDecision.writeAuthority,false);return null}}
 const noop=await saveFullMayelGalleryV1({...args,decisions:keep()});assert.equal(noop.status,'NO_GALLERY_CHANGE');assert.equal(saves,0);assert.equal(upstream,0)
 for(let i=0;i<2;i++){const r=await saveFullMayelGalleryV1(args);assert.equal(r.status,'WAITING_FOR_DATA');assert.equal(r.marketplaceWrites,0)}
 assert.equal(saves,2);assert.equal(upstream,1);assert.equal(task.visual_manifest_digest,truth)
 assert.equal(task.selection_signal.pendingGalleryDecision.state,'WAITING_FOR_CURRENT_GALLERY')
 task.selection_signal.pendingGalleryDecision.nextAttemptAt="2026-01-01T00:00:00Z"
 const drift=await saveFullMayelGalleryV1({...args,readGallery:async()=>({authority:'CURRENT_OFFICIAL_ORDERED_IMAGE_SET',images:[...current].reverse(),digest:truth,observedAt:'2026-09-10T19:00:00Z'})})
 assert.equal(drift.status,'REQUIRES_ATTENTION');assert.equal(task.selection_signal.pendingGalleryDecision.state,'REQUIRES_ATTENTION')
})

test('strict official gallery verification never uses an external URL list to hide wrong displayed order',async()=>{
 const {verifyOfficialOrderedImageSetV1}=await import('../ebay/ebay-active-listing-image-revision-service.ts')
 const noFetch=async()=>{throw Error('NO_TRAFFIC_ALLOWED')}
 const expected=[...current],snapshot={pictureUrls:[...current].reverse(),externalPictureUrls:expected}
 assert.equal((await verifyOfficialOrderedImageSetV1(snapshot,expected,noFetch,true)).verified,false)
 assert.equal((await verifyOfficialOrderedImageSetV1({...snapshot,pictureUrls:expected},expected,noFetch,true)).verified,true)
 const {galleryReorderReadbackMatchesV1}=await import('../ebay/ebay-mayel-content-executor-v1.ts')
 assert.equal(await galleryReorderReadbackMatchesV1({galleryUrls:expected,protectedFields:{},official:snapshot},{afterGallery:expected,protectedBefore:{},galleryMutation:{galleryMutationContract:'MAYEL_FULL_GALLERY_MUTATION_V1'}}),false)
})

test('pending preparation reuses contractual retry time and the existing conservative outbox fallback',async()=>{
 const {nextOutboxAttemptAtV1}=await import('./ipad-outbox-contract-v1.ts');const now=Date.parse('2026-09-10T10:00:00Z')
 assert.equal(nextOutboxAttemptAtV1('2026-09-11T00:00:00Z',now),'2026-09-11T00:00:00Z')
 assert.equal(nextOutboxAttemptAtV1(null,now),'2026-09-10T10:15:00.000Z')
})

test('station projects pure removal/reorder receipts only for the exact task, generation and current order',()=>{
 const receipt={task_id:base.visualTaskId,item_id:base.ebayItemId,manifestDigest:truth,state:'SYNCED',official_readback:true,readbackItem:base.ebayItemId,readbackAuthority:'OFFICIAL_EBAY_CONTENT_READBACK_V1',afterGallery:current}
 const args={taskId:base.visualTaskId,itemId:base.ebayItemId,manifestDigest:truth,currentImages:current,receipt}
 assert.equal(contentGalleryReceiptMatchesV1(args),true)
 for(const change of [{task_id:id(9)},{item_id:'other'},{manifestDigest:'old'},{state:'SYNCING'},{official_readback:false},{readbackItem:'other'},{readbackAuthority:'INTERNAL'},{afterGallery:[...current].reverse()}])assert.equal(contentGalleryReceiptMatchesV1({...args,receipt:{...receipt,...change}}),false)
})
