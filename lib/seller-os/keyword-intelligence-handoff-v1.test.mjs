...
test("CURRENT package reader preserves a semantic decision across non-material refreshes",async()=>{
 const current={...binding,PACKAGE_ID:"00000000-0000-4000-8000-000000000003"}
 let calls=0
 const supabase={rpc:async(name,args)=>{calls++
  assert.equal(name,"read_current_factory_keyword_handoff_v2_1")
  assert.equal(args.p_listing_package_id,current.PACKAGE_ID)
  assert.equal(args.p_plan_id,current.PLAN_ID)
  return {data:{...wire(),BINDING:{...binding,PACKAGE_ID:current.PACKAGE_ID},
   VALIDATION:{CURRENT_INPUTS_MATCH:true,DECISION_DIGEST:"sha256:"+
    createHash("sha256").update(JSON.stringify(decision)).digest("hex")}},error:null}
 }}
 const budget={read:async x=>x.query(),close(){}}
 const got=await readKeywordDecisionHandoffV1({supabase,binding:current,budget})
 assert.equal(calls,1);assert.equal(got.STATUS,"READY")
})
test("CURRENT package reader rejects an invalid package binding before RPC",async()=>{
 let calls=0
 const got=await readKeywordDecisionHandoffV1({supabase:{rpc:async()=>{calls++}},
  binding:{...binding,PACKAGE_ID:"not-a-package"}})
 assert.equal(calls,0);assert.equal(got.STATUS,"UNAVAILABLE")
 assert.deepEqual(got.BLOCKERS,["KEYWORD_BINDING_REQUIRED"])
})
