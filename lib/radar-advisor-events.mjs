  ...
  if (hasAmbiguousStock) {
    return {
      event_intelligence_label:
        "Validacion manual",
      event_intelligence_summary:
        "Datos de stock ambiguos: confirmar inventario real por SKU/variante antes de decidir.",
      event_intelligence_severity:
        "high",
      event_intelligence_advisory_only:
        true,
...
export function getRadarAdvisorEvent(
  ...
  if (
    ...
    alert.advisor_message =
      `Luna muestra ${quantityLabel} unidades a nivel producto; validar cantidad por variante.`
    alert.proposed_next_step =
      "Validar inventario por variante/SKU antes de listar o escalar."
  }
