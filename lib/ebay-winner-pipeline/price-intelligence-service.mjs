...
export async function getLatestPriceIntelligenceForCandidateKey({
  supabase,
  candidateKey,
}) {
  const key =
...
export async function getPriceIntelligenceForCandidate({
  ...
  const latestByCandidateKey =
    await getLatestPriceIntelligenceForCandidateKey({
      supabase,
      candidateKey,
    })

  if (latestByCandidateKey) {
    snapshots.push(
