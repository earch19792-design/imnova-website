...
function getLandedEvidencePayload(input) {
  ...
  const shippingStrategy =
    toCleanString(
      input.shipping_strategy ||
        input.shippingStrategy
    )

  const allowedShippingStrategy =
    [
      "free_shipping",
      "paid_shipping",
      "high_shipping",
      "unknown",
    ].includes(shippingStrategy)
      ? shippingStrategy
      : "unknown"

  const landedPriceSource =
    toCleanString(
      input.landed_price_source ||
        input.landedPriceSource
    )
  ...
  return {
    ...
    shipping_strategy:
      allowedShippingStrategy,
    landed_price_source:
      landedPriceSource === "manual_observed"
        ? "manual_observed"
        : "manual_observed",
  }
}
...
function buildSafeRawPayload(normalized, input) {
  const landedEvidence =
    getLandedEvidencePayload(input)
