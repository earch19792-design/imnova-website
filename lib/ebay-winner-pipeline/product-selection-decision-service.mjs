...
export const DEFAULT_PRODUCT_SELECTION_CONFIG = {
  // EBAY_US non-Store/Starter conservative fallback. A category-aware fee
  // snapshot should be passed through estimatedEbayFees whenever available.
  ebayFeePercent: 15.3,
  ebayFixedFee: 0.4,
...
export function calculateProductEconomics(candidate = {}, config = {}) {
  ...
  const appliedEbayFixedFee = totalRevenue <= 10
    ? Math.min(options.ebayFixedFee, 0.3)
    : Math.max(options.ebayFixedFee, 0.4)
  ...
  return {
    ...
    feeEstimate: {
      ...
      scheduleVersion: "EBAY_US_SELLING_FEES_2026_07_01_V1",
      categoryAwareSnapshotUsed: explicitEbayFees !== null,
      fallbackRatePercent: options.ebayFeePercent,
      appliedFixedOrderFee: explicitEbayFees === null ? appliedEbayFixedFee : null,
      exactFee: false,
    },
    totalCost,
    netProfit,
