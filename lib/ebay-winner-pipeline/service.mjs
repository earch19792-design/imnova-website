...
function getRadarProductInventoryContext(
  ...
  const rawInventoryContext =
    isObject(radarProduct.raw?.inventory_context)
      ? radarProduct.raw.inventory_context
      : {}
...
function buildRadarProductFromCandidate(
  ...
  return {
    ...
    inventory_quantity:
      ...
      normalizedPayload.inventory_context?.inventory_quantity ??
...
function getPriceIntelligenceMarketConfidence(priceIntelligence) {
  if (!priceIntelligence) {
    return null
  }
  ...
  const confidence =
    firstValue(
      priceIntelligence.market_confidence,
      priceIntelligence.confidence,
      priceIntelligence.confidence_label
    )
...
function getComplianceRiskLevel(compliance, codePatterns = []) {
  const findings =
...
function getImageAuthorizationStatus(candidate, radarProduct) {
  const value =
    firstValue(
      candidate.images_authorized,
      candidate.authorized_images,
      radarProduct.images_authorized,
      radarProduct.authorized_images
    )
...
function getStockStatus(candidate, radarProduct, stockAvailable) {
  const status =
    firstValue(
      candidate.inventory_status,
      candidate.inventory_context?.inventory_status,
      radarProduct.inventory_status,
      radarProduct.raw?.inventory_context?.inventory_status
    )
...
export function mapPipelineResultToProductSelectionCandidate({
  radarProduct = {},
  result = {},
  ...
  const profitScenario =
    isObject(result.profitScenario)
      ? result.profitScenario
      : {}
  const compliance =
    isObject(result.compliance)
      ? result.compliance
      : {}
  const source =
    getNestedObject(candidate, "source")
  const inventoryContext =
    getNestedObject(candidate, "inventory_context")
  const rawInventoryContext =
    getNestedObject(
      getNestedObject(radarProduct, "raw"),
      "inventory_context"
    )
  const assumptions =
    getNestedObject(profitScenario, "assumptions")
  ...
  return {
    ...
    supplierCost:
      toFiniteNumber(
        firstValue(
          profitScenario.luna_cost,
          profitScenario.supplier_unit_cost,
          assumptions.supplier_unit_cost,
          candidate.cost,
          radarProduct.price
        )
      ),
    supplierShippingCost:
      toFiniteNumber(
        firstValue(
          ...
          profitScenario.shipping_cost,
          assumptions.shipping_cost,
          assumptions.estimated_shipping_cost,
    ...
    estimatedEbayFees:
      toFiniteNumber(
        firstValue(
          profitScenario.estimated_ebay_fee,
          profitScenario.ebay_fee,
          assumptions.estimated_ebay_fee,
          candidate.estimated_ebay_fee
        )
      ),
    buyerShippingCharge:
      toFiniteNumber(
        firstValue(
          ...
          candidate.buyer_shipping_charge,
          radarProduct.buyer_shipping_charge
        )
      ),
    stockAvailable,
    stockStatus:
    ...
    shippingTimeDays:
      toFiniteNumber(
        firstValue(
          candidate.shipping_time_days,
          candidate.shippingTimeDays,
          radarProduct.shipping_time_days,
          radarProduct.shippingTimeDays
        )
      ),
    weight:
      toFiniteNumber(
    ...
    fragile:
      firstValue(
        candidate.fragile,
        radarProduct.fragile
      ) === true,
    returnRisk:
      firstValue(
        candidate.return_risk,
        candidate.returnRisk,
        radarProduct.return_risk,
        radarProduct.returnRisk,
        "low"
      ),
    brandRisk:
    ...
    competitorCount:
      toFiniteNumber(
        firstValue(
          priceIntelligence?.active_listing_count,
          priceIntelligence?.active_count,
          priceIntelligence?.competitor_count,
          priceIntelligence?.competitorCount
        )
      ),
    soldCompsMedianPrice:
      toFiniteNumber(
        firstValue(
          priceIntelligence?.sold_median_price,
          priceIntelligence?.soldCompsMedianPrice,
          priceIntelligence?.sold_avg_price,
          priceIntelligence?.active_avg_price
        )
      ),
    marketConfidence:
      getPriceIntelligenceMarketConfidence(
        priceIntelligence
      ),
  }
}

export function buildPipelineProductSelectionDecision({
  radarProduct = {},
  result = {},
  priceIntelligence = null,
  config = {},
} = {}) {
  const productSelectionCandidate =
