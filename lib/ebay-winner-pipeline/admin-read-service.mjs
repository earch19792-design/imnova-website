...
function getPipelineReanalysisAdvisorFromCandidate(
  candidate
) {
  const normalizedPayload =
    candidate?.normalized_payload &&
    typeof candidate.normalized_payload === "object" &&
    !Array.isArray(candidate.normalized_payload)
      ? candidate.normalized_payload
      : {}

  const sourcePayload =
    candidate?.source_payload &&
    typeof candidate.source_payload === "object" &&
    !Array.isArray(candidate.source_payload)
      ? candidate.source_payload
      : {}

  return normalizedPayload.pipeline_reanalysis_advisor ||
    sourcePayload.pipeline_reanalysis_advisor ||
    null
}

export function getProductSelectionAdvisorFromCandidate(
  candidate
) {
  const normalizedPayload =
    candidate?.normalized_payload &&
  ...
  return normalizedPayload.product_selection_advisor ||
    sourcePayload.product_selection_advisor ||
    null
}
...
function getSafeStringArray(value) {
...
function getCandidateInventoryFields({
  normalized_payload,
  source_payload,
}) {
  const normalizedPayload =
  ...
  const normalizedInventoryContext =
    getObjectPayload(
      normalizedPayload.inventory_context
    )

  const sourceInventoryContext =
    getObjectPayload(
      sourcePayload.inventory_context
    )
  ...
  const sourceRawInventoryContext =
    getObjectPayload(
      sourceRaw.inventory_context
    )

  return {
    inventory_quantity:
      toFiniteNumber(
        normalizedPayload.inventory_quantity ??
        normalizedInventoryContext.inventory_quantity ??
        normalizedPayload.stock ??
        sourcePayload.inventory_quantity ??
        sourceInventoryContext.inventory_quantity ??
        sourceRawInventoryContext.inventory_quantity
      ),
    inventory_scope:
      normalizedPayload.inventory_scope ||
      normalizedInventoryContext.inventory_scope ||
      sourcePayload.inventory_scope ||
      sourceInventoryContext.inventory_scope ||
      sourceRawInventoryContext.inventory_scope ||
      null,
    inventory_confidence:
      normalizedPayload.inventory_confidence ||
      normalizedInventoryContext.inventory_confidence ||
      sourcePayload.inventory_confidence ||
      sourceInventoryContext.inventory_confidence ||
      sourceRawInventoryContext.inventory_confidence ||
      null,
    inventory_source:
      normalizedPayload.inventory_source ||
      normalizedInventoryContext.inventory_source ||
      sourcePayload.inventory_source ||
      sourceInventoryContext.inventory_source ||
      sourceRawInventoryContext.inventory_source ||
      null,
  }
}

function getCandidateImageFields(
  candidate
) {
  const normalizedPayload =
    getObjectPayload(
  ...
  return {
    ...
    image_urls:
      imageUrls,
    images_authorized:
      normalizedPayload.images_authorized === true ||
      sourcePayload.images_authorized === true,
  }
}

function getSafeAdminCandidate(
  candidate
) {
  const {
    normalized_payload,
  ...
  return {
    ...
    ...getCandidateInventoryFields({
      normalized_payload,
      source_payload,
    }),
    ...getCandidateImageFields({
      normalized_payload,
      source_payload,
    }),
  }
      ...
      candidates.map(candidate => {
        ...
        return {
          ...
          pipelineReanalysisAdvisor:
            getPipelineReanalysisAdvisorFromCandidate(
              candidate
            ),
          productSelectionAdvisor:
            getProductSelectionAdvisorFromCandidate(
