...
const DEFAULT_EBAY_FEE_RULE = {
  ...
  fixed_order_fee:
    0.4,
  percentage_tiers: [
    { up_to: 7500, percent: 13.6 },
    { above: 7500, percent: 2.35 },
  ],
  notes:
    "Fee eBay usado como default. Confirmar categoría final antes de publicar.",
}

const EBAY_CATEGORY_FEE_RULES = [
  {
    category_group:
      "books_music_movies",
    category_match:
    ...
    final_value_fee_percent:
      15.3,
    percentage_tiers: [
      { up_to: 7500, percent: 15.3 },
      { above: 7500, percent: 2.35 },
    ],
    notes:
      "Regla V0 para libros, musica y peliculas.",
  },
  {
    category_group:
      "wallets_jewelry",
    category_match:
      "women's bags & handbags|jewelry & watches",
    final_value_fee_percent:
      15,
    percentage_tiers: [],
    notes:
      "Regla V0 para carteras y joyeria.",
  },
  {
    category_group:
      "guitars_basses",
    category_match:
      "guitar|guitars|bass|basses|bajo|guitarra",
    final_value_fee_percent:
      6.7,
    percentage_tiers: [
      { up_to: 7500, percent: 6.7 },
      { above: 7500, percent: 2.35 },
    ],
    notes:
      "Regla V0 para guitarras y bajos.",
  },
  {
    category_group:
      "heavy_machinery",
    ...
    final_value_fee_percent:
      3,
    percentage_tiers: [
      { up_to: 15000, percent: 3 },
      { above: 15000, percent: 0.5 },
    ],
    notes:
      "Regla V0 para maquinaria pesada.",
  },
  {
    category_group:
      "nfts",
  ...
  },
  {
    category_group:
      "sneakers_150_plus",
    category_match:
      "sneaker|sneakers|athletic shoes|tenis",
    final_value_fee_percent:
      8,
    percentage_tiers: [],
    thresholds: [
    ...
    waive_fixed_order_fee: true,
    notes:
      "Regla V0 para sneakers con total comprador igual o mayor a $150.",
  },
]
...
function getCategoryText(candidate) {
  return [
...
function hasConfirmedCategory(candidate) {
  const categoryId = candidate?.suggested_category_id || candidate?.category_id
  const categoryName = candidate?.suggested_category_name || candidate?.category_name
  return Boolean(/^\d+$/.test(String(categoryId || "")) && toStringValue(categoryName))
}

function resolveSupplierModel(candidate = {}) {
  const explicitModel =
    candidate.supplier_model ||
    candidate.supplierModel ||
    candidate.supplier_strategy?.supplier_model

  if (explicitModel) {
    return String(explicitModel)
  }
  ...
  if (
    sourceText.includes("lunaportex") ||
    sourceText.includes("luna portex") ||
    sourceText.includes("luna_portex")
  ) {
    return "luna_as_supplier"
  }

  return "unknown_supplier_model"
}

function getDefaultOperatingCost({
  candidate,
  supplierModel,
  field,
  defaultCost,
  ...
  if (explicitCost !== null) {
    return {
      ...
      note:
        "Costo operativo tomado del candidato.",
    }
  }

  if (supplierModel === "luna_as_supplier") {
    return {
      cost:
        0,
      source:
      ...
      note:
        "Luna Portex es el proveedor directo; no se suma fulfillment/empaque por defecto.",
    }
  }
  ...
  return {
    ...
    note:
      "Costo operativo default. Confirmar si IMNOVA o un fulfillment externo lo asume.",
  }
}

function feeTermMatches(categoryText, term) {
  const escaped = term.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
  return new RegExp(`(^|[^a-z0-9])${escaped}([^a-z0-9]|$)`, "i").test(categoryText)
}

function ruleMatches(rule, categoryText, totalRevenue, listingPrice) {
  if (!categoryText) {
    return false
  }
  ...
  const terms =
    rule.category_match
      .split("|")
      .map(term => normalizeFeeText(term.trim()))
      .filter(Boolean)

  const matchesText = terms.some(term => feeTermMatches(categoryText, term))
  ...
  return thresholds.every(threshold => {
    const minTotalRevenue =
      toNumber(threshold.min_total_revenue)
    const minListingPrice =
      toNumber(threshold.min_listing_price)

    return (minTotalRevenue === null || totalRevenue >= minTotalRevenue) &&
      (minListingPrice === null || listingPrice >= minListingPrice)
  })
}

export function resolveEbayFeeRule(candidate = {}, totalRevenue = 0, config = {}) {
  const configuredStoreTier = toStringValue(config.ebayStoreTier).toUpperCase()
  const storeProfileConfirmed = ["NO_STORE", "STARTER"].includes(configuredStoreTier)
  const configuredPercent =
    toNumber(config.ebayFeePercent) ?? DEFAULT_EBAY_FEE_RULE.final_value_fee_percent
  const configuredFixed =
    toNumber(config.ebayFixedOrderFee) ?? DEFAULT_EBAY_FEE_RULE.fixed_order_fee
  const defaultRule = {
    ...DEFAULT_EBAY_FEE_RULE,
    final_value_fee_percent: configuredPercent,
    fixed_order_fee: totalRevenue <= 10
      ? Math.min(configuredFixed, 0.3)
      : Math.max(configuredFixed, 0.4),
    percentage_tiers: [
      { up_to: 7500, percent: configuredPercent },
      { above: 7500, percent: 2.35 },
    ],
  }

  const categoryConfirmed =
    hasConfirmedCategory(candidate)

  const categoryText =
    getCategoryText(candidate)

  const categoryRule =
    categoryConfirmed
      ? EBAY_CATEGORY_FEE_RULES.find(rule =>
          ruleMatches(
            rule,
            categoryText,
            totalRevenue,
            toNumber(candidate?.estimated_sale_price) ?? totalRevenue
          )
        )
      : null

  if (!categoryConfirmed || !categoryRule) {
    return {
      ...defaultRule,
      fee_source:
        "default_most_categories",
      confidence:
        storeProfileConfirmed && categoryConfirmed ? "high" : "medium",
      category_confirmed:
        categoryConfirmed,
      fee_schedule_version: "EBAY_US_SELLING_FEES_2026_07_01_V1",
      store_profile_status: storeProfileConfirmed
        ? configuredStoreTier
        : "UNVERIFIED_CONSERVATIVE_NO_STORE_OR_STARTER",
  ...
  return {
    ...
    ...categoryRule,
    insertion_fee:
      defaultRule.insertion_fee,
    insertion_fee_after_free_allowance:
      defaultRule.insertion_fee_after_free_allowance,
    free_listing_allowance:
      defaultRule.free_listing_allowance,
    fixed_order_fee:
      categoryRule.waive_fixed_order_fee
        ? 0
        : totalRevenue <= 10
          ? Math.min(defaultRule.fixed_order_fee, 0.3)
          : Math.max(defaultRule.fixed_order_fee, 0.4),
    fee_source:
      "category_rule",
    confidence:
      storeProfileConfirmed ? "high" : "medium",
    category_confirmed:
      true,
    fee_schedule_version: "EBAY_US_SELLING_FEES_2026_07_01_V1",
    store_profile_status: storeProfileConfirmed
...
export function calculateEbayFee(totalRevenue, feeRule) {
  const tiers = Array.isArray(feeRule.percentage_tiers) ? feeRule.percentage_tiers : []
  let variableFee = 0
  if (tiers.length) {
    let previousLimit = 0
    for (const tier of tiers) {
      const upTo = toNumber(tier.up_to)
      const above = toNumber(tier.above)
      const percent = toNumber(tier.percent) ?? 0
      if (upTo !== null) {
        const portion = Math.max(0, Math.min(totalRevenue, upTo) - previousLimit)
        variableFee += portion * (percent / 100)
        previousLimit = upTo
      } else if (above !== null && totalRevenue > above) {
        variableFee += (totalRevenue - above) * (percent / 100)
      }
    }
  } else {
    variableFee = totalRevenue * (feeRule.final_value_fee_percent / 100)
  }
  return roundMoney(variableFee + feeRule.fixed_order_fee)
}
...
export function getPipelineReanalysisAdvisor({
  ...
  const hasReanalysisSignal =
    hasStockRecoverySignal ||
    hasMarginRecoverySignal
  ...
  if (
    ...
    return {
      ...
      reason:
        "Producto previamente bloqueado, pero cambio la condicion que podia bloquearlo. No se desbloquea automaticamente: debe reanalizarse y solo avanza si ahora pasa stock, margen, riesgo y estrategia comercial.",
      required_human_approval:
        true,
      priority:
        "high",
      proposed_next_step:
        "Reprocesar en eBay Pipeline con inventario y Price Intelligence actual. Si pasa validaciones, el candidato puede salir de BLOCKED y continuar hacia paquete/listing.",
      unlock_policy:
        "paused_until_blocker_changes",
      unlock_conditions:
        [
          "Stock confirmado para la variante o cantidad suficiente para el pack recomendado.",
          "Precio, costo, fees y envio sostienen margen minimo.",
          "Riesgo de compliance, imagenes y datos operativos sin bloqueos criticos.",
          "La estrategia comercial resultante soporta listing individual o paquete rentable.",
        ],
      success_path:
        "Si el reproceso aprueba las validaciones, Pipeline actualiza el candidato y lo deja continuar hacia listing package, draft y publicacion con aprobacion humana.",
      recovery_signal_type:
        hasStockRecoverySignal && hasMarginRecoverySignal
          ? "stock_and_margin"
          : hasStockRecoverySignal
            ? "stock"
            : "margin",
...
export function calculateProfitScenario(candidate, config = {}) {
  ...
  const explicitEstimatedSalePrice =
    toNumber(candidate.estimated_sale_price)

  const estimatedSalePrice =
    roundMoney(
      explicitEstimatedSalePrice ||
      targetPriceBase
    )

  const salePriceBasis =
    explicitEstimatedSalePrice !== null
      ? (
        toNumber(candidate.current_listing_price) !== null
          ? "current_listing_price"
          : "recommended_or_user_sale_price"
      )
  ...
  const buyerHandlingCharge = roundMoney(
    toNumber(candidate.buyer_handling_charge) || 0
  )
  ...
  const estimatedSalesTaxForFee = roundMoney(
    Math.max(0, toNumber(candidate.estimated_sales_tax) || 0)
  )
  const ebayFeeBasisTotal = roundMoney(totalRevenue + estimatedSalesTaxForFee)
  ...
  const supplierModel =
    resolveSupplierModel(candidate)

  const fulfillmentAssumption =
    getDefaultOperatingCost({
      candidate,
      supplierModel,
      field:
        "fulfillment_cost",
  ...
  const fulfillmentCost =
    roundMoney(
      fulfillmentAssumption.cost
    )

  const packagingAssumption =
    getDefaultOperatingCost({
      candidate,
      supplierModel,
      field:
  ...
  return {
    ...
    buyer_shipping_charge:
      buyerShippingCharge,
    buyer_handling_charge:
      buyerHandlingCharge,
    total_revenue:
      totalRevenue,
    ebay_fee_basis_total:
      ebayFeeBasisTotal,
    estimated_sales_tax_for_fee:
      estimatedSalesTaxForFee,
    luna_cost:
      roundMoney(cost),
    supplier_model:
      supplierModel,
    ...
    fulfillment_cost_source:
      fulfillmentAssumption.source,
    packaging_cost:
      packagingCost,
    packaging_cost_source:
      packagingAssumption.source,
    operating_cost_note:
      supplierModel === "luna_as_supplier"
        ? "Luna Portex es el proveedor directo; fulfillment/empaque no se suman por defecto salvo costo explícito."
        : "Fulfillment/empaque son supuestos operativos editables.",
    ...
    assumptions: {
      ...
      ebay_fee_note:
        ebayFeeRule.notes,
      ebay_fee_schedule_version:
        ebayFeeRule.fee_schedule_version,
      ebay_store_profile_status:
        ebayFeeRule.store_profile_status,
      ebay_fee_exact_claimed:
        false,
      ebay_fee_basis_includes_sales_tax_estimate:
        estimatedSalesTaxForFee > 0,
      paymentFeePercent:
        options.paymentFeePercent,
      advertisingPercent:
        options.advertisingPercent,
...
export function buildHumanExplanation(candidate, profitScenario, compliance, score, validation = null) {
  ...
  if (
    ...
    const missingFields =
      validation?.missingFields?.length > 0
        ? ` Faltan: ${validation.missingFields.join(", ")}.`
        : ""

    return `Producto necesita datos antes de avanzar.${missingFields} Profit estimado $${profitScenario.net_profit.toFixed(2)}, margen ${profitScenario.net_margin_percent.toFixed(0)}%, score ${score.winner_score}.`
  }
