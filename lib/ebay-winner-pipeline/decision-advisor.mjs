...
function buildMultipackProfitAdvisorV0({
      ...
      return {
        ...
        recommendation:
          missingInputs.length > 0
            ? "Validar cantidad y peso antes de publicar pack."
            : scenario.pass_10_percent_margin
              ? "Pack viable para revisar contra comparables eBay."
              : "El pack todavia no alcanza margen suficiente.",
        seller_note:
          "El pack puede diluir el envio fijo; validar comparables multipack antes de listar.",
      }
    })
  ...
  const recommendedStrategy =
    bestScenario && missingInputs.length === 0
...
function buildStrategicSummary({
  ...
  if (
    ...
    if (multipackAdvisor?.is_multipack_candidate) {
      return {
        commercial_status:
