...
function buildMultipackProfitAdvisorV0({
      ...
      const scenario =
        calculateCostScenario({
          ...
          ebayFeePercent:
            toNumber(costBreakdown?.ebay_fee_percent) || 0,
          ebayFixedOrderFee:
            toNumber(costBreakdown?.ebay_fixed_fee) ?? 0.4,
          paymentFeePercent:
            toNumber(costBreakdown?.payment_fee_percent) || 0,
          promotionPercent:
            toNumber(costBreakdown?.promotion_percent) || 0,
  ...
  const conservativePackHypotheses =
    packHypotheses
      .filter(
        scenario =>
          scenario.stock_sufficient &&
          ...
          toNumber(
            scenario.pack_quantity
          ) <= 6
      )
      .sort(
        (a, b) =>
          (toNumber(b.pack_quantity) || 0) -
          (toNumber(a.pack_quantity) || 0)
      )

  const bestHypothesis =
    (
      unitMarginHealthy &&
      conservativePackHypotheses.length > 0
        ? conservativePackHypotheses
        : packHypotheses.sort(
            (a, b) =>
              (toNumber(b.net_margin_percent) || -999) -
              (toNumber(a.net_margin_percent) || -999)
  ...
  const summary =
    bestHypothesis
      ? !hasConfirmedStock
        ...
        : unitMarginHealthy
        ? "Unidad organica primero. El pack es una hipotesis secundaria para revisar con peso, dimensiones y comparables."
