...
test("fixture Luna sanitizado extrae specs, packing y secciones promocionales", async () => {
  const extraction = await extractProductCaseEvidence({
    sourceUrl: INTERACTIVE_ACCEPTANCE_URL,
    capturedAt: CAPTURED_AT,
  ...
  assert.equal(present.length, 23)
  assert.deepEqual(
  ...
  assert.deepEqual(
    byField("marketing_claim").map((entry) => entry.normalizedValue),
    marketingSections,
  )
  assert.equal(
  ...
  assert.equal(
    byField("title").some((entry) =>
      marketingSections.includes(entry.normalizedValue)
    ),
    false,
  )
