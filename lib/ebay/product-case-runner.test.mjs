...
import {
  ...
  validateGeneralProductComparableMarketEvidence,
...
function manualMarketMetrics(overrides = {}) {
  return {
    totalSold: {
      originalLabel: "Total sold",
      rawValue: "42",
      currency: "",
    },
    sellThroughRate: {
      originalLabel: "Sell-through rate",
      rawValue: "18.5%",
      currency: "",
    },
    averageSoldPrice: {
      originalLabel: "Average sold price",
      rawValue: "$24.50",
      currency: "USD",
    },
    minimumSoldPrice: {
      originalLabel: "Minimum sold price",
      rawValue: "$18.00",
      currency: "USD",
    },
    maximumSoldPrice: {
      originalLabel: "Maximum sold price",
      rawValue: "$31.99",
      currency: "USD",
    },
    activeListingCount: {
      originalLabel: "Active listings",
      rawValue: "67",
      currency: "",
    },
...
function generalMarketSessionInput(document, overrides = {}) {
  return {
    document,
    researcher: "Ernesto Antonio Rodriguez Chavarria",
    marketplace: "EBAY_US",
    sourceType: "EBAY_PRODUCT_RESEARCH_MANUAL",
    query: "men electric foil shaver single pack",
    researchPeriodDays: 90,
    conditionFilter: "New",
    buyingFormatFilter: "All",
    categoryFilter: "Shavers",
    priceFilter: "USD 10-50",
    itemLocationFilter: "United States",
    capturedAt: "2026-07-28T17:00:00.000Z",
    sourceReference:
      "https://www.ebay.com/sh/research?marketplace=EBAY-US",
    rawVisibleResearchText:
      "42 sold | 18.5% sell-through | average sold price $24.50",
    humanNotes:
      "Investigación manual de comparables generales; no identidad exacta.",
    manuallyReadConfirmed: true,
    sellOneLikeThisNotUsedConfirmed: true,
    generalComparisonOnlyConfirmed: true,
    metrics: manualMarketMetrics(),
    ...overrides,
  }
}

function generalComparableInput(document, session, index, overrides = {}) {
  const suffix = String(index).padStart(2, "0")
  return {
    document,
    comparableId: `general-comparable-${suffix}`,
    researchSessionId: session.sessionId,
    sourceType: session.sourceType,
    ebayItemId: `1234567890${suffix}`,
    listingUrl: `https://www.ebay.com/itm/1234567890${suffix}`,
    title: `Men electric foil shaver comparable ${index}`,
    listingState: "SOLD",
    price: "24.50",
    currency: "USD",
    shippingPrice: "0",
    condition: "New",
    category: "Shavers",
    soldDate: `2026-07-${String(10 + index).padStart(2, "0")}`,
    observedAttributes: "Foil head\nSingle shaver",
    differencesFromSupplierProduct:
      "Brand and model are not attributed to the Luna offer",
    humanDecision: "INCLUDE_AS_GENERAL_COMPARABLE",
    humanReason:
      "Comparable general por tipo de producto y cantidad; no es match exacto.",
    exactMatchConfirmed: false,
    updatedAt: `2026-07-28T17:${String(index).padStart(2, "0")}:00.000Z`,
    ...overrides,
  }
}

async function electricRazorLimitedMarketCase() {
  const identityCase = await electricRazorCatalogLimitationCase()
  const catalog = await saveSupplierCatalogLimitationRecord(
    supplierCatalogLimitationInput(identityCase.document),
  )
  return { ...identityCase, document: catalog.updatedDocument }
}

async function electricRazorCompletedLimitedMarketCase() {
  const limited = await electricRazorLimitedMarketCase()
  const sessionSaved = await saveGeneralMarketResearchSession(
    generalMarketSessionInput(limited.document),
  )
  let document = sessionSaved.updatedDocument
  for (const index of [1, 2, 3]) {
    const saved = await saveGeneralMarketComparable(
      generalComparableInput(document, sessionSaved.session, index),
    )
    document = saved.updatedDocument
  }
  return { ...limited, session: sessionSaved.session, document }
}
...
test("corrección humana refresca el audit legacy antes del export V2 sin mutar el histórico", async (t) => {
  for (const action of ["REPLACE", "DELETE"]) {
    await t.test(action, async () => {
      ...
      assert.equal(envelope.version, "PRODUCT_CASE_WORKSPACE_EXPORT_V4")
      assert.deepEqual(
...
test("catálogo Luna agotado habilita sólo Fase 5 limitada sin completar identidad", async () => {
  ...
  const limitedResearch = await saveGeneralMarketResearchSession(
    generalMarketSessionInput(saved.updatedDocument),
  )
  workspace.document = limitedResearch.updatedDocument
  ...
  assert.equal(limitedMarketPhase.output.generalMarketPhaseStatus,
    "IN_PROGRESS")
...
test("export/import V4 preserva declaración y rechaza tampering; V2 no la activa", async () => {
  const identityCase = await electricRazorCatalogLimitationCase()
  const saved = await saveSupplierCatalogLimitationRecord(
    supplierCatalogLimitationInput(identityCase.document),
  )
  ...
  assert.equal(envelope.version, "PRODUCT_CASE_WORKSPACE_EXPORT_V4")
  assert.equal(envelope.outputContractVersion, "PRODUCT_CASE_OUTPUT_CONTRACT_V4")
  const imported = await importProductCaseWorkspaceExport(
    JSON.stringify(envelope),
  )
  assert.deepEqual(
...
test("sesión manual de Product Research conserva raw, MISSING y cero efectos", async () => {
  const limited = await electricRazorLimitedMarketCase()
  assert.equal(
    deriveGeneralComparableMarketPhaseStatus(limited.document),
    "NOT_STARTED",
  )
  const saved = await saveGeneralMarketResearchSession(
    generalMarketSessionInput(limited.document),
  )
  assert.equal(
    saved.session.contractVersion,
    GENERAL_PRODUCT_COMPARABLE_MARKET_EVIDENCE_CONTRACT_VERSION,
  )
  assert.equal(saved.session.marketplace, "EBAY_US")
  assert.equal(saved.session.sourceType, "EBAY_PRODUCT_RESEARCH_MANUAL")
  assert.equal(saved.session.metrics.totalSold.normalizedValue, 42)
  assert.equal(saved.session.metrics.sellThroughRate.normalizedValue, 18.5)
  assert.equal(saved.session.metrics.averageSoldPrice.normalizedValue, 24.5)
  assert.equal(saved.session.metrics.shippingIncludedInMetric, "UNKNOWN")
  assert.equal(
    saved.session.rawHumanInput.rawVisibleResearchText,
    "42 sold | 18.5% sell-through | average sold price $24.50",
  )
  assert.deepEqual(saved.session.identityContext, {
    ...
    identityReviewId:
      limited.document.identityReview.humanReview.reviewId,
    identityReviewContentHash:
      limited.document.identityReview.humanReview.contentHash,
    supplierCatalogLimitationReviewId:
      limited.document.supplierCatalogLimitation.activeAttestation.reviewId,
    supplierCatalogLimitationContentHash:
      limited.document.supplierCatalogLimitation.activeAttestation.contentHash,
    canonicalSupplierUrl:
      limited.document.supplierSourceCapture.supplierUrl,
    supplierSourceContentHash:
      limited.document.supplierSourceCapture.contentHash,
    parserVersion: limited.document.supplierSourceCapture.parserVersion,
    sourceContractVersion:
      limited.document.supplierSourceCapture.sourceContractVersion,
    productType: "Men's electric foil shaver",
    packQuantity: 1,
  })
  assert.deepEqual(
    validateGeneralProductComparableMarketEvidence(saved.updatedDocument),
    { valid: true, errors: [] },
  )
  assert.equal(
    deriveGeneralComparableMarketPhaseStatus(saved.updatedDocument),
    "IN_PROGRESS",
  )
  assert.equal(saved.updatedDocument.marketEvidence.soldExact, "MISSING")
  assert.equal(saved.updatedDocument.marketEvidence.activeExact, "MISSING")
  assert.equal(saved.updatedDocument.marketEvidence.marketCeiling, "MISSING")
  assert.equal(saved.updatedDocument.marketEvidence.soldExactCount, 0)
  assert.equal(saved.updatedDocument.identityReview.status, "PARTIAL")
  assert.deepEqual(saved.safety, {
    supabaseWrites: 0,
    ebayWrites: 0,
    openAiCalls: 0,
    whatsappCalls: 0,
  ...
  const allMissing = Object.fromEntries([
    ...
    originalLabel: key,
    rawValue: "MISSING",
    currency: key.includes("Price") ? "USD" : "",
  }]))
  const missing = await saveGeneralMarketResearchSession(
    generalMarketSessionInput(limited.document, {
      capturedAt: "2026-07-28T17:01:00.000Z",
      rawVisibleResearchText: "No visible metric values in this view.",
      metrics: {
        ...allMissing,
        shippingIncludedInMetric: "UNKNOWN",
      },
  ...
  for (const key of Object.keys(allMissing)) {
    assert.equal(missing.session.metrics[key].status, "MISSING", key)
    assert.equal(missing.session.metrics[key].normalizedValue, null, key)
  }
  assert.equal(
    JSON.stringify(missing.session.metrics).includes("1250"),
    false,
    "Luna supplier stock must never become a demand metric",
  )
})

test("Fase 5 rechaza confirmaciones, métricas, URLs y contenido sensible inválidos", async () => {
  const limited = await electricRazorLimitedMarketCase()
  const invalidSessions = [
    [{ manuallyReadConfirmed: false },
      /GENERAL_MARKET_HUMAN_CONFIRMATIONS_REQUIRED/],
    [{ sellOneLikeThisNotUsedConfirmed: false },
      /GENERAL_MARKET_HUMAN_CONFIRMATIONS_REQUIRED/],
    [{ generalComparisonOnlyConfirmed: false },
      /GENERAL_MARKET_HUMAN_CONFIRMATIONS_REQUIRED/],
    [{ researchPeriodDays: 7 }, /GENERAL_MARKET_RESEARCH_PERIOD_INVALID/],
    [{ sourceReference: "http://www.ebay.com/sh/research" },
      /GENERAL_MARKET_SOURCE_REFERENCE_INVALID/],
    [{ rawVisibleResearchText: "<html><body>copied page</body></html>" },
      /GENERAL_MARKET_FULL_HTML_FORBIDDEN/],
    [{ humanNotes: "Visible by buyer@example.com" },
      /GENERAL_MARKET_SENSITIVE_CONTENT_FORBIDDEN/],
    [{ metrics: manualMarketMetrics({
      totalSold: {
        originalLabel: "Total sold",
        rawValue: "1.5",
    ...
    }) }, /GENERAL_MARKET_METRIC_COUNT_INVALID/],
    [{ metrics: manualMarketMetrics({
      averageSoldPrice: {
        originalLabel: "Average sold price",
        rawValue: "-1",
    ...
    }) }, /GENERAL_MARKET_METRIC_VALUE_INVALID/],
  ]
  for (const [overrides, expected] of invalidSessions) {
    await assert.rejects(
      saveGeneralMarketResearchSession(
        generalMarketSessionInput(limited.document, overrides),
      ),
      expected,
    )
  }
  ...
  for (const [overrides, expected] of [
    [{ listingUrl: "http://www.ebay.com/itm/123" },
      /GENERAL_MARKET_LISTING_URL_INVALID/],
    [{ sourceType: "EBAY_ACTIVE_LISTING_MANUAL" },
      /GENERAL_MARKET_COMPARABLE_SESSION_SOURCE_TYPE_MISMATCH/],
    [{ price: "-1" }, /GENERAL_MARKET_COMPARABLE_REQUIRED_FIELD_INVALID/],
    [{ title: "Contact buyer@example.com about this listing" },
      /GENERAL_MARKET_SENSITIVE_CONTENT_FORBIDDEN/],
  ]) {
    await assert.rejects(
      saveGeneralMarketComparable(generalComparableInput(
        sessionSaved.updatedDocument,
  ...
  const missingShipping = await saveGeneralMarketComparable(
    generalComparableInput(
      sessionSaved.updatedDocument,
      sessionSaved.session,
      1,
      { shippingPrice: "" },
  ...
  assert.deepEqual(missingShipping.comparable.shippingPrice, {
    status: "MISSING",
    rawValue: "",
...
test("comparables generales cubren decisiones, suficiencia, edición y colisiones", async () => {
  const limited = await electricRazorLimitedMarketCase()
  const sessionSaved = await saveGeneralMarketResearchSession(
    generalMarketSessionInput(limited.document),
  )
  const session = sessionSaved.session
  const supplierFactsBefore = structuredClone(
    sessionSaved.updatedDocument.evidence,
  )
  let document = sessionSaved.updatedDocument
  const decisions = [
  ...
  for (const [offset, humanDecision] of decisions.entries()) {
    const index = offset + 1
    const saved = await saveGeneralMarketComparable(
      generalComparableInput(document, session, index, {
        humanDecision,
        humanReason: humanDecision === "EXCLUDE_NOT_COMPARABLE"
          ? "Excluido porque sus diferencias impiden una comparación útil."
          : humanDecision === "NEEDS_MORE_EVIDENCE"
            ? "Falta información visible para decidir si es comparable general."
            : "Incluido sólo como comparable general revisado humanamente.",
      }),
    )
    assert.equal(saved.comparable.exactMatchConfirmed, false)
    assert.match(saved.comparable.contentHash, /^sha256:[a-f0-9]{64}$/)
    assert.match(saved.comparable.evidenceId,
      /^pcr-general-comparable-[a-f0-9]{20}$/)
    assert.deepEqual(saved.safety, sessionSaved.safety)
    document = saved.updatedDocument
  }
  assert.deepEqual(document.evidence, supplierFactsBefore)
  assert.equal(
    deriveGeneralComparableMarketPhaseStatus(document),
    "NEEDS_MORE_EVIDENCE",
  )
  const research = document.marketEvidence.generalProductComparableResearch
  assert.equal(research.minimumIncludedComparables,
    GENERAL_PRODUCT_COMPARABLE_MINIMUM_INCLUDED)
  assert.equal(research.comparables.filter((entry) =>
    entry.humanDecision === "INCLUDE_AS_GENERAL_COMPARABLE"
  ).length, 3)
  assert.equal(research.comparables.filter((entry) =>
    entry.humanDecision === "EXCLUDE_NOT_COMPARABLE"
  ).length, 1)
  assert.equal(research.comparables.filter((entry) =>
    entry.humanDecision === "NEEDS_MORE_EVIDENCE"
  ).length, 1)
  ...
  await assert.rejects(
    saveGeneralMarketComparable(generalComparableInput(
      ...
      session,
      6,
      { comparableId: "general-comparable-01" },
    )),
    /GENERAL_MARKET_COMPARABLE_ID_COLLISION/,
  )
  await assert.rejects(
    saveGeneralMarketComparable(generalComparableInput(
      document,
      session,
      6,
      { ebayItemId: research.comparables[0].ebayItemId },
    )),
    /GENERAL_MARKET_EBAY_ITEM_ID_COLLISION/,
  )
  await assert.rejects(
    saveGeneralMarketComparable(generalComparableInput(
      document,
    ...
    /GENERAL_MARKET_COMPARABLE_EXACT_MATCH_FORBIDDEN/,
  )

  const pending = research.comparables.find((entry) =>
    entry.humanDecision === "NEEDS_MORE_EVIDENCE"
  )
  assert.ok(pending)
  const resolved = await saveGeneralMarketComparable(
    generalComparableInput(document, session, 5, {
      replaceComparableId: pending.comparableId,
      humanDecision: "EXCLUDE_NOT_COMPARABLE",
      humanReason:
        "Revisado de nuevo y excluido; no se trató como identidad exacta.",
      updatedAt: "2026-07-28T17:20:00.000Z",
    }),
  )
  document = resolved.updatedDocument
  assert.equal(resolved.comparable.comparableId, pending.comparableId)
  assert.notEqual(resolved.comparable.contentHash, pending.contentHash)
  assert.equal(
    deriveGeneralComparableMarketPhaseStatus(document),
  ...
  await assert.rejects(
    saveGeneralMarketComparable(generalComparableInput(
      ...
      {
        replaceComparableId: pending.comparableId,
        comparableId: "renamed-comparable",
        updatedAt: "2026-07-28T17:21:00.000Z",
      },
    )),
    /GENERAL_MARKET_COMPARABLE_ID_IMMUTABLE/,
  )

  const first = document.marketEvidence.generalProductComparableResearch
    .comparables.find((entry) => entry.comparableId === "general-comparable-01")
  assert.ok(first)
  const edited = await saveGeneralMarketComparable(
    generalComparableInput(document, session, 1, {
      replaceComparableId: first.comparableId,
      title: "Edited general foil shaver market evidence title",
      updatedAt: "2026-07-28T17:22:00.000Z",
    }),
  )
  assert.equal(edited.comparable.comparableId, first.comparableId)
  assert.equal(edited.comparable.createdAt, first.createdAt)
  assert.notEqual(edited.comparable.contentHash, first.contentHash)
  assert.equal(edited.comparable.rawHumanInput.title,
    "Edited general foil shaver market evidence title")
  document = edited.updatedDocument
  ...
  assert.equal(phase5.status, "COMPLETED")
  assert.equal(phase5.output.generalMarketPhaseStatus,
    "COMPLETED_WITH_LIMITATIONS")
  assert.ok(output.operationalPipeline.slice(5).every((entry) =>
    entry.status === "BLOCKED"
  ))
  ...
  const deletedComparable = await deleteGeneralMarketComparable({
    document,
    comparableId: "general-comparable-03",
  })
  assert.equal(deletedComparable.marketEvidence
    .generalProductComparableResearch.comparables.some((entry) =>
      entry.comparableId === "general-comparable-03"
    ), false)
  assert.equal(
    deriveGeneralComparableMarketPhaseStatus(
      deletedComparable,
  ...
  const editedSession = await saveGeneralMarketResearchSession(
    generalMarketSessionInput(document, {
      replaceSessionId: session.sessionId,
      capturedAt: "2026-07-28T17:30:00.000Z",
      query: "edited general men foil shaver query",
      priceFilter: "USD 15-45",
    }),
  )
  assert.equal(editedSession.session.sessionId, session.sessionId)
  assert.notEqual(editedSession.session.contentHash, session.contentHash)
  assert.ok(editedSession.updatedDocument.marketEvidence
    .generalProductComparableResearch.comparables.every((entry) =>
      entry.reviewRequiredAfterSessionChange === true
    ))
  assert.equal(
    deriveGeneralComparableMarketPhaseStatus(editedSession.updatedDocument),
    "NEEDS_MORE_EVIDENCE",
  )
  const deletedSession = await deleteGeneralMarketResearchSession({
    document: editedSession.updatedDocument,
    sessionId: session.sessionId,
  })
  assert.equal(deletedSession.marketEvidence
    .generalProductComparableResearch.sessions.length, 0)
  assert.equal(deletedSession.marketEvidence
    .generalProductComparableResearch.comparables.length, 0)
  assert.equal(
    deriveGeneralComparableMarketPhaseStatus(deletedSession),
    "NOT_STARTED",
  )
})

test("cambios de identidad, fuente o declaración invalidan Fase 5 atómicamente", async () => {
  const completed = await electricRazorCompletedLimitedMarketCase()
  assert.equal(
    deriveGeneralComparableMarketPhaseStatus(completed.document),
    "COMPLETED_WITH_LIMITATIONS",
  )

  const replacedCatalog = await saveSupplierCatalogLimitationRecord(
    supplierCatalogLimitationInput(completed.document, {
      reviewedAt: "2026-07-28T18:00:00.000Z",
      humanReason:
        "Nueva declaración humana después de volver a revisar el catálogo.",
      rawHumanInput: {
        humanReason:
          "Nueva declaración humana después de volver a revisar el catálogo.",
      },
  ...
  assert.equal(replacedCatalog.updatedDocument.marketEvidence
    .generalProductComparableResearch.sessions.length, 0)
  assert.equal(replacedCatalog.updatedDocument.marketEvidence
    .generalProductComparableResearch.comparables.length, 0)
  assert.equal(
    deriveGeneralComparableMarketPhaseStatus(
      replacedCatalog.updatedDocument,
    ),
    "NOT_STARTED",
  )

  const identityEdited = await saveHumanIdentityReviewRecord(
    electricRazorReviewInput(completed, {
      document: completed.document,
      reviewedAt: "2026-07-28T18:01:00.000Z",
      humanReason:
        "Revisión nueva del tipo general, sin confirmar identidad exacta.",
      rawHumanInput: {
        humanReason:
          "Revisión nueva del tipo general, sin confirmar identidad exacta.",
      },
  ...
  const sourceChanged = transitionProductCaseSupplierCapture({
    document: completed.document,
    replacement: null,
  })
  assert.equal(sourceChanged.supplierCatalogLimitation.activeAttestation, null)
  assert.equal(sourceChanged.marketEvidence
    .generalProductComparableResearch.sessions.length, 0)
  assert.equal(sourceChanged.marketEvidence
    .generalProductComparableResearch.comparables.length, 0)

  const deletedCatalog = deleteSupplierCatalogLimitationRecord({
    document: completed.document,
  })
  assert.equal(deletedCatalog.supplierCatalogLimitation.activeAttestation, null)
  assert.equal(deletedCatalog.marketEvidence
    .generalProductComparableResearch.sessions.length, 0)
  assert.equal(deletedCatalog.marketEvidence
    .generalProductComparableResearch.comparables.length, 0)
})

test("V4 roundtrip preserva Fase 5 y rechaza hashes, IDs, referencias y valores manipulados", async () => {
  const completed = await electricRazorCompletedLimitedMarketCase()
  const workspace = structuredClone(
    SANITIZED_DETERMINISTIC_COMPLETE_PRODUCT_CASE_FIXTURE.workspaceState,
  )
  workspace.document = completed.document
  workspace.economicsPolicy = null
  workspace.scenarioDraft = null
  workspace.imageApprovals = []
  workspace.imageObservations = structuredClone(
    completed.document.imageAnalysis.observations,
  )
  const envelope = createProductCaseWorkspaceExport({
    workspaceState: workspace,
    exportedAt: "2026-07-28T19:00:00.000Z",
  })
  assert.equal(envelope.version, "PRODUCT_CASE_WORKSPACE_EXPORT_V4")
  assert.equal(envelope.outputContractVersion, "PRODUCT_CASE_OUTPUT_CONTRACT_V4")
  const imported = await importProductCaseWorkspaceExport(
  ...
  const importedResearch = imported.workspaceState.document.marketEvidence
    .generalProductComparableResearch
  assert.deepEqual(
    importedResearch,
    completed.document.marketEvidence.generalProductComparableResearch,
  )
  assert.deepEqual(
    imported.workspaceState.document.supplierCatalogLimitation,
    completed.document.supplierCatalogLimitation,
  )
  assert.deepEqual(
    imported.workspaceState.document.identityReview.humanReview,
    completed.document.identityReview.humanReview,
  )
  assert.equal(importedResearch.sessions[0].rawHumanInput
    .rawVisibleResearchText,
  "42 sold | 18.5% sell-through | average sold price $24.50")
  assert.equal(importedResearch.comparables[0].exactMatchConfirmed, false)

  const canonicalOnce = createProductCaseWorkspaceExport({
    workspaceState: imported.workspaceState,
    exportedAt: envelope.exportedAt,
  })
  const importedTwice = await importProductCaseWorkspaceExport(
    JSON.stringify(canonicalOnce),
  )
  const canonicalTwice = createProductCaseWorkspaceExport({
    workspaceState: importedTwice.workspaceState,
    exportedAt: envelope.exportedAt,
  })
  assert.deepEqual(canonicalTwice, canonicalOnce)

  const invalidExportWorkspace = structuredClone(workspace)
  invalidExportWorkspace.document.marketEvidence
    .generalProductComparableResearch.contractVersion = "TAMPERED"
  assert.throws(
    () => createProductCaseWorkspaceExport({
      workspaceState: invalidExportWorkspace,
      exportedAt: "2026-07-28T19:00:01.000Z",
    }),
    /PRODUCT_CASE_IMPORT_GENERAL_MARKET_EVIDENCE_INVALID/,
  )

  for (const [label, mutate] of [
    ["session hash", (research) => {
      research.sessions[0].contentHash = `sha256:${"0".repeat(64)}`
    }],
    ["session query", (research) => {
      research.sessions[0].query = "tampered query"
    }],
    ["comparable hash", (research) => {
      research.comparables[0].contentHash = `sha256:${"1".repeat(64)}`
    }],
    ["evidenceId", (research) => {
      research.comparables[0].evidenceId = "pcr-general-comparable-tampered"
    }],
    ["session reference", (research) => {
      research.comparables[0].researchSessionId = "missing-session"
    }],
    ["price", (research) => {
      research.comparables[0].price = 999
    }],
    ["exact match", (research) => {
      research.comparables[0].exactMatchConfirmed = true
    }],
  ]) {
    const tampered = structuredClone(canonicalOnce)
    mutate(tampered.workspaceState.document.marketEvidence
      .generalProductComparableResearch)
    await assert.rejects(
      importProductCaseWorkspaceExport(JSON.stringify(tampered)),
      /GENERAL_MARKET|CRYPTOGRAPHIC_PROVENANCE|PRODUCT_CASE_IMPORT/,
      label,
    )
  }
})

test("import V3 preserva Fases 1-4 y exige nueva investigación manual", async () => {
  const limited = await electricRazorLimitedMarketCase()
  const workspace = structuredClone(
    SANITIZED_DETERMINISTIC_COMPLETE_PRODUCT_CASE_FIXTURE.workspaceState,
  )
  workspace.document = limited.document
  workspace.economicsPolicy = null
  workspace.scenarioDraft = null
  workspace.imageApprovals = []
  workspace.imageObservations = structuredClone(
    limited.document.imageAnalysis.observations,
  )
  const legacyV3 = createProductCaseWorkspaceExport({
    workspaceState: workspace,
    exportedAt: "2026-07-28T19:10:00.000Z",
  })
  legacyV3.version = PRODUCT_CASE_PRE_MARKET_EVIDENCE_WORKSPACE_EXPORT_VERSION
  legacyV3.outputContractVersion =
    PRODUCT_CASE_PRE_MARKET_EVIDENCE_OUTPUT_CONTRACT_VERSION
  await assert.rejects(
    importProductCaseWorkspaceExport(JSON.stringify(legacyV3)),
    /PRODUCT_CASE_IMPORT_PRE_MARKET_EVIDENCE_PROFILE_INVALID/,
  )
  delete legacyV3.workspaceState.document.marketEvidence
    .generalProductComparableResearch
  delete legacyV3.output.document.marketEvidence
    .generalProductComparableResearch
  const imported = await importProductCaseWorkspaceExport(
    JSON.stringify(legacyV3),
  )
  assert.equal(
    imported.sourceWorkspaceExportVersion,
    PRODUCT_CASE_PRE_MARKET_EVIDENCE_WORKSPACE_EXPORT_VERSION,
  )
  assert.deepEqual(
    imported.workspaceState.document.identityReview.humanReview,
    limited.document.identityReview.humanReview,
  )
  assert.deepEqual(
    imported.workspaceState.document.supplierCatalogLimitation,
    limited.document.supplierCatalogLimitation,
  )
  assert.equal(imported.workspaceState.document.marketEvidence
    .generalProductComparableResearch.sessions.length, 0)
  assert.equal(imported.workspaceState.document.marketEvidence
    .generalProductComparableResearch.comparables.length, 0)
  assert.ok(imported.importWarnings.includes(
    PRODUCT_CASE_PRE_MARKET_EVIDENCE_OUTPUT_WARNING,
  ))
  assert.equal(
    deriveGeneralComparableMarketPhaseStatus(
      imported.workspaceState.document,
    ),
    "NOT_STARTED",
  )
})

test("UI de catálogo agotado expone formulario canónico de Fase 5 y red local-only", () => {
  const page = read("app/admin/ebay/product-case-runner/page.tsx")
  const phase5Component = read(
    "app/admin/ebay/product-case-runner/phase-5-market-evidence.tsx",
  )
  const marketUi = `${page}\n${phase5Component}`
  ...
  assert.match(
    page,
    /anchorId: "phase-5-market-evidence",\s+focusId: "market-research-session-form-heading"/,
  )
  assert.match(page, /id="phase-5-market-evidence"[\s\S]*?tabIndex=\{-1\}/)
  for (const anchor of [
    "PHASE_5_MARKET_EVIDENCE",
    "MARKET_RESEARCH_SESSION_FORM",
    "MARKET_RESEARCH_SESSION_CARD",
    "GENERAL_COMPARABLE_FORM",
    "GENERAL_COMPARABLE_CARD",
    "MARKET_EVIDENCE_BLOCKERS",
    "MARKET_EVIDENCE_SUMMARY",
  ]) {
    assert.match(marketUi, new RegExp(anchor), anchor)
  }
  assert.match(
    marketUi,
    /GENERAL_PRODUCT_COMPARABLE_MARKET_EVIDENCE_CONTRACT_VERSION/,
  )
  assert.match(marketUi, /EBAY_PRODUCT_RESEARCH_MANUAL/)
  assert.match(marketUi, /EBAY_ACTIVE_LISTING_MANUAL/)
  assert.match(marketUi, /deriveGeneralComparableMarketPhaseStatus/)
  assert.match(marketUi, /\{phaseStatus\}/)
  assert.match(marketUi, /INCLUDE_AS_GENERAL_COMPARABLE/)
  assert.match(marketUi, /EXCLUDE_NOT_COMPARABLE/)
  assert.match(marketUi, /NEEDS_MORE_EVIDENCE/)
  assert.match(marketUi, /Volver al bloqueo/)
  assert.match(marketUi, /Editar/)
  assert.match(marketUi, /Eliminar/)
  assert.match(marketUi, /CAPTURE_GENERAL_PRODUCT_COMPARABLE_MARKET_EVIDENCE/)
  assert.match(marketUi, /external requests:\s*0 · mutating requests:\s*0/)
  assert.match(marketUi, /manual/i)
  assert.match(marketUi, /Sell One Like This/)
  assert.match(marketUi, /identidad exacta/i)
  assert.match(marketUi, /MISSING/)
  assert.match(marketUi, /shippingIncludedInMetric/)
  assert.match(marketUi, /rawVisibleResearchText/)
  assert.match(marketUi, /differencesFromSupplierProduct/)
  assert.match(marketUi, /observedAttributes/)
  assert.doesNotMatch(marketUi, /type=["']password["']/i)
  assert.doesNotMatch(marketUi, /name=["'](?:cookie|token|password)["']/i)
  assert.match(
    page,
