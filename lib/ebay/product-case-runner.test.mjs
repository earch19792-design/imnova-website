...
test("captura autenticada interactiva separa precios, stock, specs, claims y MISSING", async () => {
  ...
  const sourceCapture = createManualAuthenticatedSupplierSourceCapture({
  ...
  assert.equal(sourceCapture.sensitiveContentStored, false)
...
test("captura manual rechaza HTML completo, credenciales y datos de cuenta", () => {
...
test("la superficie nueva no contiene ejecución o persistencia externa", () => {
  ...
  assert.match(
    page,
    /sourceAccess\.status\s*===\s*"AUTHENTICATED_SOURCE_REQUIRED"\s*\?\s*createManualAuthenticatedSupplierSourceCapture/,
  )
