...
test("control failure remains generic while durable Trace stays fail closed", () => {
  ...
  assert.match(source, /shippingStatus === "PROVEN"/)
  assert.match(source, /Date\.parse\(shippingFreshUntil\) <= Date\.now\(\)/)
  assert.match(source, /const priceAuthorized = result\.PRICE_AUTHORIZED === true/)
  assert.match(source, /economicsAuthority\.status === "PROVEN"/)
  assert.match(source, /priceAuthorizationBlockers:/)
})
