...
test("product navigation teardown recovers the same bounded job on the cart document", async () => {
  const background = await readFile(new URL(
    "../../tools/browser-extensions/luna-shipping-capture/background.js",
    import.meta.url), "utf8")
  let connectListener = null
  ...
  const chrome = {
    ...
    scripting: { executeScript: async (value) => { injected.push(value) } },
    webNavigation: {
  ...
  navigationCommitted({ tabId: 11, frameId: 0,
    url: "https://pay.shopify.com/opaque-checkout" })
  navigationCompleted({ tabId: 11, frameId: 0,
    url: "https://pay.shopify.com/opaque-checkout" })
  await new Promise((resolve) => setTimeout(resolve, 0))
  assert.ok(progress.some((entry) =>
    entry.state === "CHECKOUT_NAVIGATION_OBSERVED" &&
    entry.checkoutNavigationHost === "pay.shopify.com" &&
    entry.checkoutNavigationOrigin === "https://pay.shopify.com"))
...
test("checkout distinguishes guest flow from explicit auth and captures a safe quote", async () => {
  ...
  selectors = (selector) => selector.includes('data-testid*="line-item"')
    ? [shopLine]
    : selector.includes('data-testid*="subtotal"') ? [shopSubtotal]
      : selector.includes('data-testid*="shipping"') ? [shopShipping]
        : selector.includes('data-testid*="total"') ? [shopTotal]
