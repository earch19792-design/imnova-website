...
test("fresh page and service-worker wake use the same bounded external handshake", async () => {
  ...
  for (let wake = 0; wake < 2; wake += 1) {
    ...
    assert.deepEqual(JSON.parse(JSON.stringify(pong)), {
      ...
      extensionVersion: "1.0.35", extensionBuildVersion: "1.0.35",
    ...
    assert.equal(diagnostic?.extensionVersion, "1.0.35")
...
test("Seller OS page proves the external bridge before opening the job port", async () => {
  ...
  assert.match(page, /EXPECTED_EXTENSION_VERSION = "1\.0\.35"/)
...
test("SHOP_APP_CHECKOUT_ALLOWLIST_REGRESSION_GUARD permits only an active bounded job", async () => {
  ...
  for (const state of ["BIND_REQUEST_ACCEPTED",
    "BIND_EXISTING_CHECKOUT_SEARCH_STARTED",
    "BIND_CHECKOUT_BOOTSTRAP_REQUIRED", "BIND_BOOTSTRAP_PRODUCT_OPENED",
    ...
    assert.ok(bootstrapTraceStates.includes(state), state)
  }
