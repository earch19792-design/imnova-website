...
test("bounded runtime trace accepts only safe allowlisted evidence", () => {
  ...
  const discoveryDiagnostics = normalizeLunaShippingRuntimeTraceEventV1(
    traceEvent({ state: "BIND_SHOP_APP_TAB_DISCOVERY_RESULT",
      ...
      shopAppHostTabCount: 2, shopAppProbedCount: 2,
...
test("Seller OS page proves the external bridge before opening the job port", async () => {
  ...
  assert.match(page, /EXPECTED_EXTENSION_VERSION = "1\.0\.26"/)
...
test("SHOP_APP_CHECKOUT_ALLOWLIST_REGRESSION_GUARD permits only an active bounded job", async () => {
  ...
  let shopTabs = [{ id: 11, active: false, windowId: 2 }]
  let validationMatches = true
  let bindingRejectReason = null
  let probeRejectReason = null
  ...
  const chrome = {
    ...
    tabs: {
      ...
      query(queryInfo, callback) {
        ...
        if (!shopTabQueryStalls) callback(queryInfo.url ? shopTabs : normalTabs)
      },
      sendMessage(_tabId, message, _options, callback) {
        tabMessages.push(message)
        if (message.type === "PROBE_LUNA_CANONICAL_DESTINATION") {
          if (probeRejectReason) {
            callback({ accepted: false, error: probeRejectReason })
            return
          }
          callback({ accepted: true, shipToAvailable: true,
  ...
  assert.ok(tabQueries.some((query) => query.windowType === "normal" &&
    !("active" in query) && !("currentWindow" in query) && !("url" in query)))
  assert.ok(tabQueries.some((query) => query.windowType === "normal" &&
    query.url === "https://shop.app/*" && !("active" in query) &&
    !("currentWindow" in query)))
  ...
  assert.equal(discoveryTrace?.tabsQueryTotalCount, 2)
  assert.equal(discoveryTrace?.shopAppHostTabCount, 1)
  assert.equal(discoveryTrace?.shopAppProbedCount, 1)
  ...
  shopTabs = [{ id: 11, active: false, windowId: 2 },
  ...
  shopTabs = [{ id: 11, active: false, windowId: 2 }]
  storedDestination = {}
  probeRejectReason = "BIND_SHIP_TO_NOT_FOUND"
  firstPortMessage({ type: "SELLER_OS_BIND_LUNA_CANONICAL_DESTINATION" })
  await new Promise((resolve) => setTimeout(resolve, 20))
