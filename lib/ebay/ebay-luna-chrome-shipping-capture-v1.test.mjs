...
test("bounded runtime trace accepts only safe allowlisted evidence", () => {
  ...
  const totalDiagnostics = normalizeLunaShippingRuntimeTraceEventV1(traceEvent({
    ...
    totalAmountCandidateFound: true,
    totalLabelAmountContainerFound: true,
  }), NOW)
  assert.equal(totalDiagnostics.totalLabelAmountContainerFound, true)
...
test("Seller OS page proves the external bridge before opening the job port", async () => {
  ...
  assert.match(page, /EXPECTED_EXTENSION_VERSION = "1\.0\.23"/)
...
test("SHOP_APP_CHECKOUT_ALLOWLIST_REGRESSION_GUARD permits only an active bounded job", async () => {
  ...
  const chrome = {
    ...
    tabs: {
      ...
      query(_queryInfo, callback) {
        if (!shopTabQueryStalls) callback(shopTabs)
      },
    ...
    storage: { local: {
      get(_key, callback) { callback(storedDestination) },
      set(value, callback) {
  ...
  assert.equal(progress.at(-1)?.error,
    "CANONICAL_DESTINATION_FINGERPRINT_PERSIST_FAILED")
  ...
  shopTabs = [{ id: 11, url: "https://shop.app/pay/one" },
    { id: 12, url: "https://shop.app/pay/two" }]
  firstPortMessage({ type: "SELLER_OS_BIND_LUNA_CANONICAL_DESTINATION" })
  await new Promise((resolve) => setTimeout(resolve, 20))
  assert.equal(progress.at(-1)?.error,
    "BIND_CHECKOUT_TAB_AMBIGUOUS")
  shopTabs = [{ id: 11, url: "https://shop.app/pay/opaque-checkout" }]
  storedDestination = {}
...
test("checkout distinguishes guest flow from explicit auth and captures a safe quote", async () => {
  ...
  const shopShipTo = visible({
    textContent: "Ship to Private Recipient 123 Private Street United States FL 33487",
    getAttribute: () => null,
  })
  ...
  selectors = (selector) => selector.startsWith("[data-shipping-address]")
    ? [otherShipTo] : shopSelectors(selector)
  ...
  const markers = context.shopPayMarkerSnapshot(job())
  ...
  // Real Shop Pay can expose Total as a semantic label whose currency and
  // amount are separate visible siblings instead of one Luna-style row.
  const splitTotalLabel = visible({ textContent: "Total" })
  const splitTotalCurrency = visible({ textContent: "USD" })
  const splitTotalAmount = visible({ textContent: "$17.95" })
  const splitTotalRow = visible({ textContent: "",
    children: [splitTotalLabel, splitTotalCurrency, splitTotalAmount],
    querySelectorAll: () => [splitTotalLabel, splitTotalCurrency,
      splitTotalAmount] })
  splitTotalLabel.parentElement = splitTotalRow
  splitTotalLabel.nextElementSibling = splitTotalCurrency
  splitTotalLabel.closest = () => splitTotalRow
  splitTotalCurrency.parentElement = splitTotalRow
  splitTotalCurrency.previousElementSibling = splitTotalLabel
  splitTotalCurrency.nextElementSibling = splitTotalAmount
  splitTotalAmount.parentElement = splitTotalRow
  splitTotalAmount.previousElementSibling = splitTotalCurrency
  selectors = (selector) => selector.startsWith("dt,dd,th")
    ? [shopSubtotal, shopShipping, splitTotalLabel, splitTotalCurrency,
      splitTotalAmount] : []
  const splitTotalResult = context.labeledMoneyResult("total")
  assert.equal(splitTotalResult.value, 17.95)
  assert.equal(splitTotalResult.labelFound, true)
  assert.equal(splitTotalResult.currencyFound, true)
  assert.equal(splitTotalResult.amountCandidateFound, true)
  assert.equal(splitTotalResult.labelAmountContainerFound, true)

  const separatedCurrency = visible({ textContent: "USD" })
  const separatedAmount = visible({ textContent: "$17.95" })
  const separatedCurrencyTotal = visible({ textContent: "Total",
    children: [separatedCurrency, separatedAmount],
    querySelectorAll: () => [separatedCurrency, separatedAmount] })
  separatedCurrency.parentElement = separatedCurrencyTotal
  separatedAmount.parentElement = separatedCurrencyTotal
  selectors = (selector) => selector.startsWith("dt,dd,th")
    ? [shopSubtotal, shopShipping, separatedCurrencyTotal] : []
  assert.equal(context.labeledMoneyResult("total").value, 17.95)
  ...
  selectors = (selector) => selector.includes('data-testid*="subtotal"')
    ? [shopSubtotal]
    : selector.includes('data-testid*="shipping"') ? [freeShipping]
      : selector.includes('data-testid*="total"') ? [freeTotal] : []
  ...
  assert.equal(context.shopPayQuoteFailure({
    shopPayMarkerSubtotal: true,
    shopPayMarkerShipping: false,
    shopPayMarkerTotal: true,
    shopPayMarkerShippingAmount: false,
  }, null, true), "SHOP_PAY_SHIPPING_NOT_FOUND")
  assert.equal(context.shopPayQuoteFailure({
    shopPayMarkerSubtotal: true,
    shopPayMarkerShipping: true,
    shopPayMarkerTotal: true,
    shopPayMarkerShippingAmount: false,
  }, null, true), "SHOP_PAY_MONEY_PARSE_FAILED")
...
test("live monitor reuses the existing admin transport and same-day event store", async () => {
  ...
  assert.match(page, /replayPendingBindAfterUpgrade/)
  assert.match(page, /CANONICAL_BIND_COMPLETED/)
  assert.match(page, /event\.extensionVersion !== EXPECTED_EXTENSION_VERSION/)
