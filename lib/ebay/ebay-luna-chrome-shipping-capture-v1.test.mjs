...
test("idle bridge reconnect restores durable binding and restarts production without refresh", async () => {
  ...
  assert.match(page, /if \(reconnecting\) return/)
  assert.doesNotMatch(page, /if \(!busy \|\| reconnecting\) return/)
  ...
  assert.match(page, /attachPort\(resumedPort, jobToResume\)/)
...
test("BFCache restore closes and deterministically recreates the page port", async () => {
  ...
  assert.match(pageShow, /attachPort\(resumedPort, pageLifecycleJobToResume\)/)
