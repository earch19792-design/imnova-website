...
const {
  ...
  frontierShippingRefreshRequiredV1,
  shippingFrontierTurnV1,
...
test("historical persisted frontier quotes become discoverable when expired, without changing history", () => {
  for (const shippingStatus of ["SHIPPING_DURABLY_PERSISTED", "SHIPPING_OBSERVED"]) {
    const f = {shippingStatus, shippingCaptureEvidence:{observedAt:new Date(NOW-7*86400000).toISOString()}, evaluatedAt:new Date(NOW).toISOString()}
    const before = structuredClone(f)
    assert.equal(frontierShippingRefreshRequiredV1(f,NOW),true)
    assert.deepEqual(f,before)
    assert.equal(frontierShippingRefreshRequiredV1({...f,shippingCaptureEvidence:{observedAt:new Date(NOW-1000).toISOString()}},NOW),false)
    assert.equal(frontierShippingRefreshRequiredV1({...f,shippingCaptureEvidence:{observedAt:new Date(NOW-21600000).toISOString()}},NOW),true)
    assert.equal(frontierShippingRefreshRequiredV1({...f,shippingCaptureEvidence:{}},NOW),true)
    assert.equal(frontierShippingRefreshRequiredV1({...f,shippingCaptureEvidence:{observedAt:new Date(NOW+1000).toISOString()}},NOW),true)
  }
})
...
function exactPreDispatchTrace(exactJob) {
  const captureSessionIdHash = `sha256:${createHash("sha256")
    .update(exactJob.captureSessionId).digest("hex")}`
  return {
    contractVersion: LUNA_SHIPPING_RUNTIME_TRACE_VERSION,
    traceId: `luna-shipping-trace-v1:${captureSessionIdHash}`,
    candidateId: exactJob.identity.candidateId,
    sequence: 1,
    timestamp: new Date().toISOString(),
    extensionVersion: "1.0.57",
...
test("bounded runtime trace accepts only safe allowlisted evidence", () => {
  ...
  assert.equal(normalizeLunaShippingRuntimeTraceEventV1(traceEvent({
    state: "EXACT_JOB_RESOLVED", event: "EXACT_JOB_RESOLVED",
  }), NOW).state, "EXACT_JOB_RESOLVED")
  assert.equal(normalizeLunaShippingRuntimeTraceEventV1(traceEvent({
...
test("job normalization allows only exact Luna product URLs and canonical facts", () => {
  ...
  const unicodeProductUrl =
    "https://lunaportex.com/products/✨new✨-exact-product🎁"
  assert.equal(normalizeLunaChromeShippingJobV1({ ...job(), identity: {
    ...job().identity, canonicalProductUrl: unicodeProductUrl,
  } }).identity.canonicalProductUrl,
  "https://www.lunaportex.com/products/%E2%9C%A8new%E2%9C%A8-exact-product%F0%9F%8E%81")
  assert.throws(() => normalizeLunaChromeShippingJobV1({ ...job(), identity: {
    ...job().identity,
    canonicalProductUrl: "https://lunaportex.com/products/exact%2Fescape",
  } }), /LUNA_SHIPPING_PRODUCT_URL_INVALID/)
})

test("shipping-only jobs preserve an unknown sale price without inventing economics", () => {
  const shippingOnlyJob = normalizeLunaChromeShippingJobV1({
    ...job(), salePriceUsd: null,
  })
  assert.equal(shippingOnlyJob.salePriceUsd, null)
  const result = certifyLunaChromeShippingVisibleCaptureV1({
    job: shippingOnlyJob, capture: capture(), now: NOW,
  })
  assert.equal(result.quote.status, "AVAILABLE")
  assert.equal(result.economics, null)
  assert.throws(() => normalizeLunaChromeShippingJobV1({
    ...job(), salePriceUsd: 0,
  }), /LUNA_SHIPPING_EXTENSION_JOB_INVALID/)
})
...
test("effective unpacked MV3 artifact has only fixed hosts and coherent build", async () => {
  ...
  assert.equal(manifest.version, "1.0.57")
  assert.equal(manifest.name, "Seller OS — Luna Shipping Capture")
  assert.deepEqual(manifest.permissions,
    ["scripting", "webNavigation", "storage", "alarms"])
  ...
  assert.match(background, /EXTENSION_BUILD_VERSION = "1\.0\.57"/)
  assert.match(background, /SHOP_APP_HOST_PATTERN = "https:\/\/shop\.app\/\*"/)
  assert.match(background, /CHECKOUT_HOSTS\.has\("shop\.app"\)/)
  assert.match(content, /extensionBuildVersion: "1\.0\.57"/)
  assert.match(content, /decodeURIComponent\(encodedHandle\)\.normalize\("NFKC"\)/)
  ...
  assert.match(background, /files: \['checkout-observation\.js'\]/)
  assert.match(background, /documentIds: \[frame\.documentId\]/)
...
test("fresh page, worker wake, and extension reload preserve one durable binding", async () => {
  ...
  for (let wake = 0; wake < 3; wake += 1) {
    ...
    assert.deepEqual(JSON.parse(JSON.stringify(pong)), {
      ...
      extensionId: LUNA_SHIPPING_EXTENSION_ID,
      extensionVersion: "1.0.57", extensionBuildVersion: "1.0.57",
    ...
    assert.equal(diagnostic?.extensionVersion, "1.0.57")
...
test("shop.app static content authority parses, ACKs once, and answers capability probe", async () => {
  ...
  assert.deepEqual(Object.keys(response).sort(), [
    ...
    "subtotalMarker", "totalMarker", "diagnosticVersion", "markersEvaluated",
    "requiredMarkersReady", "productIdentityStatus", "quantityIdentityStatus",
  ].sort())
...
test("Seller OS page proves the external bridge before opening the job port", async () => {
  const page = await readFile(new URL(
    "../../app/admin/ebay/luna-shipping-capture/luna-shipping-capture-control-plane.tsx", import.meta.url),
  "utf8")
  assert.match(page, /SELLER_OS_LUNA_SHIPPING_PING/)
  assert.match(page, /LUNA_SHIPPING_EXTENSION_READY/)
  assert.match(page, /EXPECTED_EXTENSION_VERSION = "1\.0\.57"/)
...
test("idle bridge reconnect restores durable binding and restarts production without refresh", async () => {
  ...
  assert.match(page, /const reconnectToken = \+\+reconnectGeneration/)
  assert.match(page, /canonicalBindingStatusRead = false/)
  assert.match(page, /setCanonicalBindingStatusReady\(false\)/)
  assert.match(page, /activeJobStatusReady = false/)
  assert.match(page, /discoveryRetryTimer = null/)
  assert.match(page, /wakeLunaShippingExtensionV1/)
  assert.match(page, /attachPort\(resumedPort, jobToResume, resolve\)/)
  assert.match(page, /const accepted = await new Promise<boolean>/)
  assert.match(page, /if \(jobToResume && !pendingExactPortDispatch\)/)
  assert.match(page, /PORT_HANDSHAKE_ACK/)
  assert.match(page, /SELLER_OS_GET_LUNA_CANONICAL_DESTINATION_STATUS/)
  ...
  assert.match(page,
    /const attemptProductionAcquisition = \(\) => \{[\s\S]*?INITIAL_AUTO_CLAIM_STARTED/)
  assert.match(page,
    /canonicalBindingStatusRead = true[\s\S]*?attemptProductionAcquisition\(\)/)
  assert.match(page,
...
test("BFCache restore closes and deterministically recreates the page port", async () => {
  ...
  assert.match(page,
    /acquireCurrentPort = async \(jobToResume\)[\s\S]*?wakeLunaShippingExtensionV1\(runtime, pingExtensionOnce, wait\)/)
  assert.match(page,
    /acquireCurrentPort = async \(jobToResume\)[\s\S]*?runtime\.connect\(EXTENSION_ID/)
  assert.match(page, /RESUME_ACTIVE_LUNA_SHIPPING_JOB/)
  assert.match(page, /acquireCurrentPort = async \(jobToResume\)[\s\S]*?canonicalBindingStatusRead = false/)
  assert.match(page, /acquireCurrentPort = async \(jobToResume\)[\s\S]*?activeJobStatusReady = false/)
  assert.match(page, /PORT_HANDSHAKE_ACK/)
  assert.match(page, /SELLER_OS_GET_LUNA_CANONICAL_DESTINATION_STATUS/)
  assert.match(page, /SELLER_OS_GET_ACTIVE_LUNA_SHIPPING_JOB_STATUS/)
  ...
  assert.match(page,
    ...
    "presentation and exact-target auto-claim guards remain intact")
  ...
  assert.match(page, /const EXPECTED_EXTENSION_VERSION = "1\.0\.57"/)
})
...
test("canonical bind control remains available in exact LIVE target mode", async () => {
  const page = await readFile(new URL(
    "../../app/admin/ebay/luna-shipping-capture/luna-shipping-capture-control-plane.tsx", import.meta.url),
  "utf8")
  const bindHandler = page.match(
    /const bindCanonicalDestination = \(\) => \{[\s\S]*?bindDestinationRef\.current = bindCanonicalDestination/)?.[0] ?? ""
  const sharedBindMechanism = page.match(
    /recoverCanonicalDestination = \(automatic: boolean\) => \{[\s\S]*?const bindCanonicalDestination/)?.[0] ?? ""
  const bindPresentation = page.match(
    /\{showCanonicalBindControl \? <button[\s\S]*?Vincular perfil canónico US de Seller OS[\s\S]*?<\/p> : null\}/)?.[0] ?? ""
  assert.ok(bindHandler)
  assert.ok(sharedBindMechanism)
  assert.ok(bindPresentation)
  assert.doesNotMatch(bindHandler, /hasExactLiveTarget/,
    "exact-target mode must not reject explicit canonical binding")
  assert.match(sharedBindMechanism,
    /resolve_jobs[\s\S]*?purpose: "CANONICAL_BIND_BOOTSTRAP"/,
    "manual and autonomous recovery reuse the same canonical bootstrap")
  assert.match(page,
    /canonicalBindControlVisible\(connected,[\s\S]*?canonicalBindingStatusReady,[\s\S]*?canonicalDestinationBound\)/)
  assert.doesNotMatch(bindPresentation, /!liveTarget|hasExactLiveTarget/)

  const bindVisible = (connected, statusReady, bound) =>
    connected && statusReady && !bound
  assert.equal(bindVisible(true, true, false), true,
    "EXACT_TARGET_NO_BINDING exposes the bind control")
  assert.equal(bindVisible(true, true, false), true,
    "GLOBAL_NO_BINDING exposes the same bind control")
  assert.equal(bindVisible(true, true, true), false,
    "VALID_BINDING does not require the control")

  const shippingEnabled = (connected, statusReady, bound, mismatch, busy) =>
    connected && statusReady && bound && !mismatch && !busy
  assert.equal(shippingEnabled(true, true, false, false, false), false,
    "shipping remains blocked until canonical binding is ready")
  assert.match(page, /if \(hasExactLiveTarget \|\| busy \|\| !extensionReady/,
    "global auto-claim isolation remains intact in exact-target mode")
})
...
test("production worker safely restores a missing canonical profile before claiming", async () => {
  const page = await readFile(new URL(
    "../../app/admin/ebay/luna-shipping-capture/luna-shipping-capture-control-plane.tsx", import.meta.url),
  "utf8")
  const initialClaim = page.match(
    /const attemptProductionAcquisition = \(\) => \{[\s\S]*?\n    \}/)?.[0] ?? ""
  ...
  assert.match(initialClaim,
    /recoverCanonicalDestination\?\.\(true\)/,
    "bridgeOnly runtime restores the existing canonical authority without a click")
  assert.match(initialClaim, /!canonicalDestinationMismatchProven/,
    "a proven mismatch is never overwritten by recovery")
  assert.doesNotMatch(initialClaim, /!canonicalBindingReady/)
  assert.match(initialClaim, /!activeJobStatusReady/)
  assert.match(initialClaim, /discoveryInFlight = true/)
  ...
  assert.match(noBindingStatus, /scheduleProductionAcquisition\(0\)/)
  assert.doesNotMatch(noBindingStatus, /attemptProductionAcquisition\(\)/,
    "extension status schedules the existing backoff-gated timer")
  assert.match(page, /Vincular perfil canónico US de Seller OS/)
  assert.match(page,
    /canonicalBindingRecoveryInFlight[\s\S]*?purpose: "CANONICAL_BIND_BOOTSTRAP"/)
  assert.match(page,
    /destinationMismatch[\s\S]*?canonicalDestinationMismatchProven = true/)
  assert.match(page,
    /canonicalDestinationBindingPresent = true[\s\S]*?attemptProductionAcquisition\(\)/,
    "durable binding readback resumes normal job acquisition")
})

test("production acquisition is leased, single-flight, persistent, and truthfully presented", async () => {
  const [page, route, server, dashboard, migration, background] =
    await Promise.all([
      readFile(new URL(
      ...
      readFile(new URL("./ebay-luna-chrome-shipping-capture-server-v1.ts",
        import.meta.url), "utf8"),
      readFile(new URL("../../app/admin/seller-os-operational-dashboard.tsx",
        import.meta.url), "utf8"),
      readFile(new URL(
  ...
  assert.match(page, /const DISCOVERY_RETRY_INTERVAL_MS = 30_000/)
  assert.match(page, /response\?\.unicodeProductHandleSupport !== true/)
  assert.match(page, /discoveryInFlight/)
  assert.match(page,
    /const scheduleProductionAcquisition[\s\S]*?window\.setTimeout/)
  assert.match(page,
    /DISCOVERY_RETRY_SCHEDULED[\s\S]*?scheduleProductionAcquisition\(\)/)
  assert.match(page,
    /message\.success !== true[\s\S]*?mode === "AUTO"[\s\S]*?scheduleProductionAcquisition\(\)/)
  assert.match(page, /eligiblePendingJobCount === 0[\s\S]*?IDLE_NO_PENDING_WORK/)
  assert.doesNotMatch(page,
    /connected && canonicalBindingStatusReady && canonicalDestinationBound[\s\S]{0,100}return "READY"/)
  assert.match(route, /acquireLunaChromeShippingJobsV1/)
  assert.match(route, /SELLER_OS_LUNA_SHIPPING_JOB_CLAIM_V1/)
  assert.match(server, /claim_seller_os_luna_shipping_job_v2/)
  assert.match(server, /eligiblePendingJobCount: eligible\.length/)
  assert.match(migration, /primary key \(account_key, candidate_id\)/i)
  assert.match(migration, /interval '15 minutes'/)
  assert.match(migration, /enable row level security/i)
  assert.match(migration,
    /revoke all on table public\.seller_os_luna_shipping_job_claims[\s\S]*?anon, authenticated/i)
  assert.match(dashboard,
    /status === "IDLE_NO_PENDING_WORK"[\s\S]*?sin trabajo pendiente/)
  assert.doesNotMatch(dashboard,
    /status === "READY"\) return "Chrome owner conectado · sin trabajo pendiente"/)
  assert.match(background, /decodeURIComponent\(encodedHandle\)/)
  assert.match(background, /unicodeProductHandleSupport: true/)
  assert.match(background, /snapshotDigest/)
})
...
test("exact LIVE target is isolated from the global queue and presentation", async () => {
  ...
  const oosCompletion = page.match(
    /setStatus\("PRODUCTION_JOB_COMPLETED"\)[\s\S]*?attemptProductionAcquisition\(\)/)?.[0] ?? ""
  ...
  assert.match(page, /const EXPECTED_EXTENSION_VERSION = "1\.0\.57"/)
  assert.match(page, /requireDurableDispatchAck: true/)
  assert.match(page, /preDispatchTrace/)
  assert.match(page, /LUNA_EXACT_DISPATCH_DURABLE_TRACE_REQUIRED/)
  ...
  assert.match(page, /TRACE_DURABLE=\$\{traceDurable\}/)
  assert.match(page, /TRACE_CANDIDATE_MATCH=\$\{exactTraceCandidateMatch\}/)
  assert.match(page, /DISPATCH_ACK=\$\{exactDispatchAcknowledged\}/)
  for (const stage of ["CLICK_HANDLER_REACHED", "EXACT_JOB_RESOLVED",
    "TRACE_CREATE_REQUESTED", "TRACE_CREATE_ACCEPTED",
    "DISPATCH_POSTED_TO_PORT", "EXTENSION_MESSAGE_RECEIVED",
    "EXTENSION_ACK_EMITTED", "PAGE_ACK_RECEIVED"]) {
    assert.match(page, new RegExp(`${stage}=\\$\\{`),
      `${stage} must remain visible for the real-browser canary`)
  }
  assert.match(page,
    /event\.candidateId === exactLiveCandidateId[\s\S]*?setExactTraceCandidateMatch\(true\)/)
  assert.match(page,
    /message\.durableDispatchAcknowledged !== true[\s\S]*?setExactDispatchAcknowledged\(true\)/)
  assert.match(page,
    /setLiveCaptureAttempts\([\s\S]*?type: PAGE_DISPATCH_COMMITTED/,
    "the page records attempt and ACK before permitting Luna navigation")
  const liveHandler = page.match(
    /const beginLiveCapture = \(\) => \{[\s\S]*?liveTriggerRef\.current = beginLiveCapture/)?.[0] ?? ""
  assert.ok(liveHandler.indexOf('markExactDispatchStage("exactJobResolved")') <
    liveHandler.indexOf('markExactDispatchStage("traceCreateRequested")'))
  assert.ok(liveHandler.indexOf("persistRuntimeTraceSnapshot([event])") <
    liveHandler.indexOf("sendCurrent(event)"),
  "durable trace persistence must precede the real browser port dispatch")
  assert.match(liveHandler,
    /persistRuntimeTraceSnapshot\(\[event\]\)[\s\S]*?exactDispatchDurable\.add\(event\.traceId\)[\s\S]*?traceCreateAccepted[\s\S]*?sendCurrent\(event\)/)
  assert.match(page,
    /mode === "LIVE" && \(!preDispatchTrace[\s\S]*?LUNA_EXACT_DISPATCH_DURABLE_TRACE_REQUIRED[\s\S]*?return/,
  "failed trace creation must leave DISPATCH_POSTED_TO_PORT=false")
  assert.match(page, /IGNORED_OUT_OF_SCOPE_SET_BY=/)
...
test("exact LIVE delivery waits for a current handshaken port and never counts a stale post", async () => {
  ...
  const exactPortDispatch = page.match(
    /const dispatchPendingExactPortJob = \(\) => \{[\s\S]*?\n    \}/)?.[0] ?? ""
  const sendCurrent = page.match(
    /const sendCurrent = \([\s\S]*?\n    \}/)?.[0] ?? ""
  const receiptHandler = page.match(
    /if \(message\?\.type === EXTENSION_DISPATCH_RECEIVED\) \{[\s\S]*?\n          return\n        \}/)?.[0] ?? ""

  assert.match(exactPortDispatch,
    /\["PORT_GENERATION_MATCH", readyPortGeneration === generation, "true"\]/,
    "STALE_PORT must not be dispatchable")
  assert.match(exactPortDispatch, /pending\.postedGeneration === generation/)
  assert.match(exactPortDispatch, /portGeneration: generation/)
  assert.match(exactPortDispatch, /postMessageThrown: true/)
  assert.match(exactPortDispatch, /reconnectCurrentPort\?\.\(\)/)
  assert.doesNotMatch(sendCurrent,
    /markExactDispatchStage\("dispatchPostedToPort"\)/,
    "a local postMessage call is not a delivery acknowledgement")
  assert.match(receiptHandler,
    /markExactDispatchStage\("dispatchPostedToPort"\)/)
  assert.match(receiptHandler, /markExactDispatchStage\("extensionMessageReceived"\)/)
  assert.match(page, /sourcePort !== port \|\| sourceGeneration !== currentPortGeneration/)
  assert.match(page, /pendingExactPortDispatch\.postedGeneration = null/)
  assert.match(page, /pendingExactPortDispatch = null[\s\S]*?PAGE_DISPATCH_COMMITTED/)
  assert.match(page, /resultIdentityMismatchActualId:/)
  assert.match(page, /resultIdentityMismatchExpectedId:/)
  assert.match(page, /PORT_INSTANCE_CURRENT=/)
  assert.match(page, /BACKGROUND_ONMESSAGE_REACHED=/)
  assert.match(background, /if \(sellerPort !== port\) return/)
  assert.match(background, /message\?\.type === PORT_HANDSHAKE/)
  assert.match(background, /acceptedPortGeneration = message\.portGeneration/)
  assert.match(background,
    /message\.portGeneration !== acceptedPortGeneration/)
  assert.match(background,
    /sameJob\(activeJob, exact\)[\s\S]*?activeRuntimeTrace\?\.traceId !== exactPreDispatchTrace\.traceId[\s\S]*?postExactDispatchReceived/,
    "reconnect must reuse the same trace and exact job")
})

test("pre-dispatch guards distinguish local post from background receipt and expose local failure", async () => {
  const page = await readFile(new URL(
    "../../app/admin/ebay/luna-shipping-capture/luna-shipping-capture-control-plane.tsx", import.meta.url),
  "utf8")
  const exactPortDispatch = page.match(
  ...
  assert.match(exactPortDispatch, /PENDING_EXACT_DISPATCH/)
  assert.match(exactPortDispatch, /CURRENT_PORT_PRESENT/)
  assert.match(exactPortDispatch, /PORT_INSTANCE_CURRENT/)
  assert.match(exactPortDispatch, /PORT_GENERATION_MATCH/)
  assert.match(exactPortDispatch, /TRACE_DURABLE/)
  assert.match(exactPortDispatch, /TRACE_CANDIDATE_MATCH/)
  assert.match(exactPortDispatch, /EXACT_CANDIDATE_MATCH/)
  assert.match(exactPortDispatch,
    /firstFailedGuard: failedGuard\?\.\[0\] \?\? "NONE"/)
  assert.match(exactPortDispatch,
    /currentPort\.postMessage[\s\S]*?postMessageCalled: true[\s\S]*?markExactDispatchStage\("dispatchPostedToPort"\)/,
    "local post must be observable before any background receipt")
  assert.match(page, /FAIL_STATE_SET_BY=/)
  assert.match(page, /CURRENT_BLOCKER=\$\{currentBlocker\}/)
  assert.match(page, /traceBlocker !== "NONE"[\s\S]*?error \|\| "NONE"/,
    "a local failure must not render CURRENT_BLOCKER=NONE")
})

test("exact LIVE explicitly acquires a current port and classifies real identity dimensions", async () => {
  const page = await readFile(new URL(
    "../../app/admin/ebay/luna-shipping-capture/luna-shipping-capture-control-plane.tsx", import.meta.url),
  "utf8")
  const acquisition = page.match(
    /acquireCurrentPort = async \(jobToResume\) => \{[\s\S]*?\n      \}/)?.[0] ?? ""
  const resultHandler = page.match(
    /if \(!active \|\| message\?\.type !== "LUNA_SHIPPING_JOB_RESULT"\) return[\s\S]*?setStatus\("RESULT_POSTED"\)/)?.[0] ?? ""

  assert.match(acquisition, /const stalePort = port[\s\S]*?port = null/,
    "stale port must be invalidated without waiting for its disconnect callback")
  assert.match(acquisition, /stalePort\?\.disconnect\(\)/)
  assert.match(acquisition,
    /const accepted = await new Promise<boolean>[\s\S]*?attachPort\(resumedPort, jobToResume, resolve\)/,
    "port acquisition must await the exact generation handshake")
  assert.match(acquisition,
    /!accepted \|\| port !== resumedPort \|\|[\s\S]*?readyPortGeneration !== currentPortGeneration/)
  assert.match(page,
    /pendingExactPortDispatch\.postedGeneration = null[\s\S]*?acquireCurrentPort/,
    "the same pending exact trace/job must be reused after reacquisition")
  assert.match(page,
    /setLiveCaptureAttempts\(\(current\) => current \+ 1\)/)
  assert.ok(page.indexOf("setLiveCaptureAttempts((current) => current + 1)") >
    page.indexOf("if (message?.type === JOB_DISPATCH_ACK)"),
  "attempts must remain zero until extension ACK")

  assert.ok(resultHandler.indexOf("if (message.success !== true)") <
    resultHandler.indexOf("const exactIdentityMatches"),
  "same-candidate error envelopes must not become false identity mismatches")
  assert.match(resultHandler, /identityMismatchDimension: job \? "CANDIDATE" : "JOB"/)
  assert.match(resultHandler, /identityMismatchDimension: "JOB"/)
  assert.match(page, /identityMismatchDimension: !candidateMatches[\s\S]*?"CANDIDATE" : !traceMatches \? "TRACE" : "GENERATION"/)
  assert.match(page, /identityMismatchDimension: "GENERATION"/)
  assert.match(page, /IDENTITY_MISMATCH_DIMENSION=/)
  assert.match(page, /acceptExactScope\(\)[\s\S]*?markExactDispatchStage\("extensionMessageReceived"\)|markExactDispatchStage\("extensionMessageReceived"\)[\s\S]*?acceptExactScope\(\)/)
})
...
test("page canary job passes the extension validator and missing fields fail closed", async () => {
  ...
  const pageCanaryJob = { ...job(), ...session, salePriceUsd: null }
...
test("exact LIVE dispatch is durably acknowledged before navigation and recovers the same job", async () => {
  ...
  const chrome = {
    ...
    tabs: {
      ...
      update: async (_id, { url }) => { updatedTabs.push(url); return {} },
      onRemoved: { addListener() {} },
    },
  }
  runInNewContext(background, { chrome, crypto: webcrypto, URL, TextDecoder,
    TextEncoder, Uint8Array, atob, btoa, setTimeout, clearTimeout })
  const item5810Identity = {
    ...job().identity,
    candidateId: `sha256:${"5".repeat(64)}`,
    canonicalProductUrl:
      "https://www.lunaportex.com/products/language-translator-device-portable-translator-device-with-138-languages-4-1-touch-screen-smart-voice-photo-translator-real-time-offline-online-translation-for-business-learning-travel-black",
    lunaProductId: "9220805755104",
    lunaVariantId: "48809607659744",
    supplierSku: "ITEM5810",
  }
  const session = issueLunaShippingCaptureSessionV1({
    secret: "test-only-secret-not-a-production-value".repeat(2),
    candidateId: item5810Identity.candidateId,
    snapshotDigest: `sha256:${"c".repeat(64)}`,
    now: NOW,
  })
  const exactJob = { ...job(), ...session, identity: item5810Identity,
    productName: "Z6 AI Translator 138 Languages" }
  const exactProductUrl = new URL(exactJob.identity.canonicalProductUrl)
  assert.equal(exactProductUrl.hostname, "www.lunaportex.com")
  assert.equal(exactProductUrl.pathname.startsWith("/products/"), true)
  assert.equal(exactProductUrl.pathname.slice("/products/".length).length > 180,
    true, "the exact ITEM5810 regression must cover the former extension cap")
  assert.equal(normalizeLunaChromeShippingJobV1(exactJob)
    .identity.canonicalProductUrl, exactJob.identity.canonicalProductUrl)
  const unrelatedSession = issueLunaShippingCaptureSessionV1({
    secret: "test-only-secret-not-a-production-value".repeat(2),
    candidateId: job().identity.candidateId,
    snapshotDigest: `sha256:${"d".repeat(64)}`,
    now: NOW,
  })
  const unrelatedJob = { ...job(), ...unrelatedSession }
  const connect = () => {
    const posted = []
    let onMessage = null
  ...
  const first = connect()
  first.onMessage({ type: "START_SHIPPING_JOB", job: unrelatedJob })
  await waitForTestCondition(() => createdTabs.length === 1)
  first.onMessage({ type: "SELLER_OS_LUNA_SHIPPING_PORT_HANDSHAKE_V1",
    portGeneration: 1 })
  assert.equal(first.posted.findLast((entry) =>
    entry.type === "LUNA_SHIPPING_PORT_HANDSHAKE_ACK_V1")?.portCurrent, true)
  first.onMessage({ type: "START_SHIPPING_JOB", job: exactJob,
    requireDurableDispatchAck: true, portGeneration: 1 })
  await waitForTestCondition(() => first.posted.some((entry) =>
    entry.error === "LUNA_EXACT_DISPATCH_PRETRACE_INVALID"))
  assert.equal(updatedTabs.length, 0,
    "trace creation failure must not dispatch or navigate the exact job")
  const preDispatchTrace = exactPreDispatchTrace(exactJob)
  first.onMessage({ type: "START_SHIPPING_JOB", job: exactJob,
    requireDurableDispatchAck: true,
    preDispatchTrace, portGeneration: 1 })
  await waitForTestCondition(() => first.posted.some((entry) =>
    entry.type === "LUNA_SHIPPING_EXTENSION_DISPATCH_RECEIVED" &&
    entry.candidateId === exactJob.identity.candidateId))
  await waitForTestCondition(() => first.posted.some((entry) =>
    entry.event?.state === "JOB_DISPATCHED" &&
    entry.event?.candidateId === exactJob.identity.candidateId))
  assert.equal(updatedTabs.length, 0,
    "exact job must not navigate before durable Seller OS acknowledgement")
  const dispatch = first.posted.find((entry) =>
    entry.event?.state === "JOB_DISPATCHED" &&
    entry.event?.candidateId === exactJob.identity.candidateId)?.event
  assert.equal(dispatch?.candidateId, exactJob.identity.candidateId)
  const exactStates = first.posted.filter((entry) =>
    entry.event?.candidateId === exactJob.identity.candidateId)
    .map((entry) => entry.event.state)
  assertOrderedSubsequence(exactStates,
    ["EXACT_JOB_RESOLVED", "BRIDGE_CONNECTED", "JOB_DISPATCHED"])
  assert.equal(dispatch?.traceId, preDispatchTrace.traceId)
  assert.equal(dispatch?.sequence, 3)
  ...
  assert.equal(updatedTabs.length, 0,
    "extension ACK must reach the page before exact navigation")
  const acknowledgement = first.posted.find((entry) =>
    entry.type === "LUNA_SHIPPING_JOB_DISPATCH_ACK")
  assert.equal(acknowledgement?.durableDispatchAcknowledged, true)
  assert.equal(dispatch?.candidateId === exactJob.identity.candidateId, true,
    "TRACE_CANDIDATE_MATCH=true")
  assert.equal(acknowledgement?.durableDispatchAcknowledged, true,
    "DISPATCH_ACK=true")
  first.onMessage({
    type: "SELLER_OS_LUNA_SHIPPING_PAGE_DISPATCH_COMMITTED",
    candidateId: exactJob.identity.candidateId,
    traceId: dispatch.traceId,
  })
  await waitForTestCondition(() => updatedTabs.length === 1)

  first.disconnect()
  const reconnected = connect()
  reconnected.onMessage({ type: "SELLER_OS_LUNA_SHIPPING_PORT_HANDSHAKE_V1",
    portGeneration: 2 })
  assert.equal(reconnected.posted.findLast((entry) =>
    entry.type === "LUNA_SHIPPING_PORT_HANDSHAKE_ACK_V1")
    ?.portGeneration, 2)
  ...
  assert.equal(recovered?.dispatchMessageReceived, true)
  assert.equal(recovered?.dispatchAckEmitted, true)
  assert.equal(recovered?.traceEvents?.some((entry) =>
    entry.state === "JOB_DISPATCHED"), true)
  assert.equal(recovered?.traceEvents?.every((entry) =>
    entry.candidateId === exactJob.identity.candidateId), true,
  "reconnect must preserve the exact ITEM5810 trace scope")
  reconnected.onMessage({ type: "START_SHIPPING_JOB", job: exactJob,
    requireDurableDispatchAck: true, preDispatchTrace,
    portGeneration: 2 })
  await waitForTestCondition(() => reconnected.posted.some((entry) =>
    entry.type === "LUNA_SHIPPING_EXTENSION_DISPATCH_RECEIVED" &&
    entry.portGeneration === 2 && entry.traceId === preDispatchTrace.traceId))
  const retriedReceipt = reconnected.posted.findLast((entry) =>
    entry.type === "LUNA_SHIPPING_EXTENSION_DISPATCH_RECEIVED")
  assert.equal(retriedReceipt?.backgroundOnMessageReached, true)
  assert.equal(retriedReceipt?.messageTypeAccepted, true)
  assert.equal(retriedReceipt?.messageCandidateId,
    exactJob.identity.candidateId)
  assert.equal(retriedReceipt?.backgroundActiveJobCandidateId,
    exactJob.identity.candidateId)
  assert.equal(createdTabs.length, 1)
  assert.equal(updatedTabs.length, 1,
    "the same durable trace retry must not create another job or navigation")
...
test("product page proves exact identity and dispatches its visible Add to Cart control", async () => {
  ...
  const unicodeJob = { ...job(), identity: { ...job().identity,
    canonicalProductUrl:
      "https://www.lunaportex.com/products/%E2%9C%A8new%E2%9C%A8-exact-product%F0%9F%8E%81",
  } }
  context.location.pathname =
    "/products/%E2%9C%A8new%E2%9C%A8-exact-product%F0%9F%8E%81"
  const unicodeExact = await context.exactProduct(unicodeJob)
  assert.equal(String(unicodeExact.product.id),
    unicodeJob.identity.lunaProductId)
  assert.equal(context.exactProductUrl(unicodeJob.identity.canonicalProductUrl)
...
test("capture handoff reuses append-only profitability facts and ACKs only after readback", async () => {
  ...
  assert.doesNotMatch(page,
    /candidateIds: \[CANARY_ID\][\s\S]{0,80}CANONICAL_BIND_BOOTSTRAP/)
  assert.match(server,
...
test("operator-bound US destination proof survives durable shipping readback exactly", async () => {
  ...
  const candidateId = deriveCurrentCommercialCandidateIdentityV1({
    accountKey: "seller-os-test", productId: lunaProductId,
    variantId: lunaVariantId, supplierSku,
  ...
  const decoyFamilyId = `market-family-v1:sha256:${"a".repeat(64)}`
  const decoySource = {
    ...source,
    calculatedAt: new Date(NOW - 120_000).toISOString(),
  ...
  function resultQuery(result) {
    const query = {
      select() { return query }, eq() { return query }, in() { return query }, contains() { return query },
  ...
  const supabase = {
    from(table) {
      ...
      if (table === "ebay_current_listing_packages_v1") {
        return resultQuery({ data: [{ package_data: {
          currentPublicationFactoryV1: {
            version: "SELLER_OS_CURRENT_PUBLICATION_FACTORY_V1",
            authorityPolicy: "CURRENT_ONLY", reuseLegacyPreparation: false,
            productId: lunaProductId, variantId: lunaVariantId,
            supplierSku,
          },
        } }], error: null })
      }
    ...
    async rpc(name, params) {
      if (name === "get_seller_os_latest_profitability_frontiers_v1") {
        return params.p_family_ids
          ...
          : { data: { frontiers: [decoySource, source] }, error: null }
      }
  ...
  source.frontier.shippingStatus = "SHIPPING_DURABLY_PERSISTED"
  source.frontier.shippingCaptureEvidence = {observedAt:new Date(NOW-7*86400000).toISOString()}
  const autoJobs = await resolveLunaChromeShippingJobsV1({supabase,accountKey:"seller-os-test",sessionSecret,now:NOW})
  assert.ok(autoJobs.some(job=>job.identity.candidateId===candidateId), "stale persisted quote is discovered without explicit candidate IDs")
  source.frontier.shippingCaptureEvidence.observedAt = new Date(NOW-1000).toISOString()
  const freshJobs = await resolveLunaChromeShippingJobsV1({supabase,accountKey:"seller-os-test",sessionSecret,now:NOW})
  assert.equal(freshJobs.some(job=>job.identity.candidateId===candidateId),false)
  const capturePost = {
    candidateId,
    lunaProductId,
    lunaVariantId,
  ...
  assert.equal(result.durableReadbackMatch, true)
  assert.equal(persistedFrontier.familyId, familyId)
...
test("product-page OOS terminal reuses durable candidate authority and auto-nexts", async () => {
  ...
  assert.match(page, /attemptProductionAcquisition\(\)/)
...
test("bounded class alternation prevents LIVE jobs from indefinitely starving preparation",()=>{
 let last="economic-shipping-refresh-v1:live",frontiers=3,live=20
 const served=[]
 for(let turn=0;turn<6;turn++){
  if(shippingFrontierTurnV1(frontiers,last)){served.push('frontier');frontiers--;last=null}
  else {served.push('live');live--;last='economic-shipping-refresh-v1:live'}
 }
 assert.deepEqual(served,['frontier','live','frontier','live','frontier','live'])
 assert.equal(frontiers,0);assert.equal(live,17)
 assert.equal(shippingFrontierTurnV1(0,last),false)
 assert.equal(shippingFrontierTurnV1(1,null),false)
})
