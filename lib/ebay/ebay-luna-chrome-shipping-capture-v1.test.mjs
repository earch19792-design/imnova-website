...
test("idle bridge reconnect restores durable binding and restarts production without refresh", async () => {
  ...
  assert.match(disconnectHandler, /canonicalBindingReady = false/)
  ...
  assert.match(page,
    /const bound = message\.canonicalDestinationBound === true[\s\S]*?canonicalBindingReady = bound/)
  assert.match(page,
    /const startInitialProductionClaim = \(\) => \{[\s\S]*?INITIAL_AUTO_CLAIM_STARTED/)
  assert.match(page,
    /canonicalBindingReady = bound[\s\S]*?startInitialProductionClaim\(\)/)
  assert.match(page,
    /setStatus\("CANONICAL_BINDING_REQUIRED"\)[\s\S]*?setError\("CANONICAL_BINDING_REQUIRED"\)/)
