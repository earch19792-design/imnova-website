...
test("fresh page, worker wake, and extension reload preserve one durable binding", async () => {
  ...
  const manifest = JSON.parse(manifestRaw)
  const controlUrl =
    "https://imnova-website-z1qh-canonical-preview.vercel.app/admin/ebay/luna-shipping-capture"
  const oldControlUrl =
  ...
  for (let wake = 0; wake < 3; wake += 1) {
    ...
    assert.deepEqual(JSON.parse(JSON.stringify(pong)), {
      ...
      extensionId: LUNA_SHIPPING_EXTENSION_ID,
      extensionVersion: "1.0.42", extensionBuildVersion: "1.0.42",
    ...
    assert.equal(diagnostic?.extensionVersion, "1.0.42")
...
test("Seller OS page proves the external bridge before opening the job port", async () => {
  ...
  assert.match(page, /EXPECTED_EXTENSION_VERSION = "1\.0\.42"/)
  ...
  assert.match(finalCanaryGate,
    /return connected && canonicalBindingStatusReady &&[\s\S]*?!canonicalDestinationMismatch && !canaryRunInProgress/)
  assert.doesNotMatch(finalCanaryGate, /canonicalDestinationMatch/)
  assert.doesNotMatch(finalCanaryGate, /canonicalDestinationBound/)
  assert.match(page,
    /disabled=\{!canStartFinalCanary\}[\s\S]*?Ejecutar canary final/)
  assert.match(page,
    /if \(busy \|\| !extensionReady \|\| !canonicalBindingStatusRead\) return/)
  const canStart = (connected, bindingStatusReady, mismatch, running) =>
    connected && bindingStatusReady && !mismatch && !running
  assert.equal(canStart(true, true, false, false), true)
  assert.equal(canStart(true, false, false, false), false)
  assert.equal(canStart(false, true, false, false), false)
  assert.equal(canStart(true, true, false, true), false)
  assert.equal(canStart(true, true, true, false), false)
  ...
  assert.match(page,
    /SELLER_OS_EXTENSION_ORIGIN[\s\S]*imnova-website-z1qh-canonical-preview/)
...
test("idle bridge reconnect restores durable binding and restarts production without refresh", async () => {
  ...
  assert.match(page,
    /setStatus\("CANONICAL_AUTO_BIND_PENDING"\)[\s\S]*?setError\(""\)/)
  assert.doesNotMatch(page, /CANONICAL_BINDING_REQUIRED/)
  assert.doesNotMatch(disconnectHandler,
    /SELLER_OS_BIND_LUNA_CANONICAL_DESTINATION/)
})

test("canary UI enables from a successful no-binding storage classification and fails closed otherwise", async () => {
  const page = await readFile(new URL(
    "../../app/admin/ebay/luna-shipping-capture/page.tsx", import.meta.url),
  "utf8")
  ...
  assert.match(diagnosticHandler, /"LEGACY_BINDING_VALID"/)
  assert.match(diagnosticHandler,
  ...
  assert.match(page,
    /finalCanaryStartEnabled\(connected,[\s\S]*?canonicalDestinationMismatch, running\)/)
  assert.match(page, /Usar destino actual como benchmark canónico/)
  assert.doesNotMatch(page, /\.click\(\)/)

  const enabled = (connected, statusRead, mismatch, busy) =>
    connected && statusRead && !mismatch && !busy
  assert.equal(enabled(true, true, false, false), true,
    "NO_BINDING_PRESENT with READ_OK enables first-run auto-bind canary")
  assert.equal(enabled(true, true, false, false), true,
    "an existing valid binding remains eligible")
  assert.equal(enabled(true, true, true, false), false,
    "a proven existing mismatch stays fail-closed")
  assert.equal(enabled(true, false, false, false), false,
    "storage read failure stays disabled")
  assert.equal(enabled(false, true, false, false), false,
    "extension unavailable stays disabled")
  assert.equal(enabled(true, true, false, true), false,
...
test("production page worker claims its first eligible job without canary or capture bootstrap", async () => {
  ...
  assert.doesNotMatch(initialClaim, /!canonicalDestinationBound/)
  ...
  assert.match(noBindingStatus, /CANONICAL_AUTO_BIND_PENDING/)
  assert.match(noBindingStatus, /startInitialProductionClaim\(\)/)
  assert.doesNotMatch(noBindingStatus, /CANONICAL_BINDING_REQUIRED/)
})
...
test("page canary job passes the extension validator and missing fields fail closed", async () => {
  ...
  connectListener({
    name: "SELLER_OS_LUNA_SHIPPING_CAPTURE_V1",
    sender: { url: "https://imnova-website-z1qh-canonical-preview.vercel.app/admin/ebay/luna-shipping-capture" },
    disconnect() {},
    postMessage(value) { posted.push(value) },
...
test("SHOP_APP_CHECKOUT_ALLOWLIST_REGRESSION_GUARD rearms AUTO_NEXT checkout observation", async () => {
  ...
  const chrome = {
    ...
    storage: { local: {
      get(_key, callback) {
        if (storageReadFails || pendingReadbackFailure) {
  ...
  const triggerCheckoutNavigation = (checkoutJob, sender) => {
    let response = null
    runtimeMessageListener({ type: "LUNA_CHECKOUT_NAVIGATION_TRIGGERED",
      captureSessionId: checkoutJob.captureSessionId }, sender,
  ...
  const statusResponseStart = progress.length
  firstPortMessage({
    type: "SELLER_OS_GET_LUNA_CANONICAL_DESTINATION_STATUS",
  })
  await waitForTestCondition(() => progress.slice(statusResponseStart)
    .some((entry) => {
      if (entry.type !== "LUNA_CANONICAL_DESTINATION_STATUS") return false
      statusResponse = entry
      return true
    }))
  assert.equal(statusResponse?.canonicalDestinationBound, true)
  storageReadFails = true
  statusResponse = null
  const failedStatusResponseStart = progress.length
  firstPortMessage({
    type: "SELLER_OS_GET_LUNA_CANONICAL_DESTINATION_STATUS",
  })
  await waitForTestCondition(() => progress.slice(failedStatusResponseStart)
    .some((entry) => {
      if (entry.type !== "LUNA_CANONICAL_DESTINATION_STATUS") return false
      statusResponse = entry
      return true
    }))
  assert.equal(statusResponse?.error, "BINDING_STORAGE_READ_FAILED")
  assert.equal(statusResponse?.canonicalDestinationBound, undefined)
  ...
  connectListener(port(1))
  statusResponse = null
  const restartedStatusResponseStart = progress.length
  firstPortMessage({
    type: "SELLER_OS_GET_LUNA_CANONICAL_DESTINATION_STATUS",
  })
  await waitForTestCondition(() => progress.slice(restartedStatusResponseStart)
    .some((entry) => {
      if (entry.type !== "LUNA_CANONICAL_DESTINATION_STATUS") return false
      statusResponse = entry
      return true
  ...
  assert.equal(statusResponse?.canonicalDestinationBound, true)
  assert.equal(statusResponse?.canonicalDestinationMatch, false)

  validationMatches = false
  const mismatchResponseStart = progress.length
  firstPortMessage({ type: "SELLER_OS_BIND_LUNA_CANONICAL_DESTINATION" })
  await waitForTestCondition(() => progress.slice(mismatchResponseStart)
    .some((entry) => entry.type ===
      "LUNA_CANONICAL_DESTINATION_BINDING_RESULT"))
  const mismatch = progress.slice(mismatchResponseStart).findLast((entry) =>
    entry.type === "LUNA_CANONICAL_DESTINATION_BINDING_RESULT")
  ...
  triggerCheckoutNavigation(activeJob, bootstrapCartSender)
  navigationCommitted({ tabId: 11, frameId: 0,
  ...
  const runHandoffProductionJob = async (sequence, productTabId,
    ...
    const navigationStart = createdTabs.length + updatedTabs.length
    firstPortMessage({ type: "START_SHIPPING_JOB", job: productionJob,
      productionAutoClaim: true })
    await waitForTestCondition(() =>
      createdTabs.length + updatedTabs.length > navigationStart)
    ...
    navigationCompleted({ tabId: productTabId, frameId: 0,
      url: "https://www.lunaportex.com/cart" })
    assert.ok(!progress.slice(progressStart).some((entry) =>
      entry.type === "LUNA_SHIPPING_JOB_RESULT"))
    triggerCheckoutNavigation(productionJob, cartSender)
    navigationCompleted({ tabId: productTabId, frameId: 0,
      url: "https://www.lunaportex.com/cart" })
    assert.ok(!progress.slice(progressStart).some((entry) =>
      entry.type === "LUNA_SHIPPING_JOB_RESULT"))
    ...
    let ack = null
    runtimeMessageListener({ type: "SHOP_APP_CHECKOUT_BOOTSTRAP_ACK",
      extensionBuildVersion: "1.0.42" }, checkoutSender,
    (value) => { ack = value })
  ...
  const schemaMismatchResultStart = progress.length
  firstPortMessage({ type: "SELLER_OS_BIND_LUNA_CANONICAL_DESTINATION" })
  await waitForTestCondition(() => progress.slice(schemaMismatchResultStart)
    .some((entry) => entry.type ===
      "LUNA_CANONICAL_DESTINATION_BINDING_RESULT"))
  assert.equal(progress.slice(schemaMismatchResultStart).findLast((entry) =>
    entry.type === "LUNA_CANONICAL_DESTINATION_BINDING_RESULT")?.error,
  "BIND_BOOTSTRAP_JOB_INVALID")
  ...
  const ambiguousResultStart = progress.length
  firstPortMessage({ type: "SELLER_OS_BIND_LUNA_CANONICAL_DESTINATION" })
  await waitForTestCondition(() => progress.slice(ambiguousResultStart)
    .some((entry) => entry.type ===
      "LUNA_CANONICAL_DESTINATION_BINDING_RESULT"))
  assert.equal(progress.slice(ambiguousResultStart).findLast((entry) =>
    entry.type === "LUNA_CANONICAL_DESTINATION_BINDING_RESULT")?.error,
  ...
  const shipToResultStart = progress.length
  firstPortMessage({ type: "SELLER_OS_BIND_LUNA_CANONICAL_DESTINATION" })
  await waitForTestCondition(() => progress.slice(shipToResultStart)
    .some((entry) => entry.type ===
      "LUNA_CANONICAL_DESTINATION_BINDING_RESULT"))
  assert.equal(progress.slice(shipToResultStart).findLast((entry) =>
    entry.type === "LUNA_CANONICAL_DESTINATION_BINDING_RESULT")?.error,
  "BIND_SHIP_TO_NOT_FOUND")
  bindingRejectReason = null
  progress.length = 0
  const activeJobNavigationStart = createdTabs.length + updatedTabs.length
  firstPortMessage({ type: "START_SHIPPING_JOB", job: activeJob })
  await waitForTestCondition(() =>
    createdTabs.length + updatedTabs.length > activeJobNavigationStart)
  ...
  (value) => { checkoutPhase = value })
  assert.equal(checkoutPhase?.accepted, true)
  triggerCheckoutNavigation(activeJob,
    { tab: { id: 11 }, url: "https://www.lunaportex.com/cart" })
  navigationCommitted({ tabId: 11, frameId: 0,
    url: "https://shop.app/opaque-checkout?authorization=must-not-escape" })
  ...
  const autoBindRequest = (fingerprint = `sha256:${"b".repeat(64)}`,
    overrides = {}) => new Promise((resolve) => {
    runtimeMessageListener({
      type: "AUTO_BIND_LUNA_CANONICAL_DESTINATION_V1",
      contractVersion: "LUNA_CANONICAL_DESTINATION_AUTO_BIND_V1",
      captureSessionId: activeJob.captureSessionId,
      fingerprint,
      fingerprintVersion: "LUNA_SHOP_PAY_DESTINATION_SHA256_V1",
      countryClass: "US", destinationUnambiguous: true,
      acceptedUsDestination: true,
      safeMarkerBooleans: { shipTo: true, shipping: true, subtotal: true,
        total: true, payNow: true }, ...overrides,
    }, checkoutSender, resolve)
  })

  // The runtime state machine writes only the first verified destination,
  // requires durable readback, and treats a later binding as immutable.
  storageWriteFails = true
  let autoBindResponse = await autoBindRequest()
  assert.equal(autoBindResponse?.error, "BIND_STORAGE_WRITE_FAILED")
  assert.equal(storedDestination.sellerOsLunaCanonicalDestinationBindingV1,
    undefined)
  storageWriteFails = false
  failReadbackAfterWrite = true
  autoBindResponse = await autoBindRequest()
  assert.equal(autoBindResponse?.error, "BIND_STORAGE_READBACK_FAILED")
  failReadbackAfterWrite = false
  storedDestination = {}
  autoBindResponse = await autoBindRequest()
  assert.equal(autoBindResponse?.success, true)
  assert.equal(autoBindResponse?.canonicalDestinationMatch, true)
  assert.equal(autoBindResponse?.operation, "AUTO_BIND_CANONICAL_DESTINATION")
  const autoBoundEnvelope = { ...storedDestination
    .sellerOsLunaCanonicalDestinationBindingV1 }
  assert.deepEqual(Object.keys(autoBoundEnvelope).sort(), [
    "boundAt", "canonicalDestinationFingerprint", "countryClass",
    "fingerprintVersion",
  ])
  const writesAfterFirstAutoBind = storageWriteCount
  autoBindResponse = await autoBindRequest()
  assert.equal(autoBindResponse?.success, true)
  assert.equal(autoBindResponse?.operation,
    "VALIDATE_EXISTING_CANONICAL_DESTINATION")
  assert.equal(storageWriteCount, writesAfterFirstAutoBind)
  const mismatchAutoBind = await autoBindRequest(`sha256:${"c".repeat(64)}`)
  assert.equal(mismatchAutoBind?.error,
    "CANONICAL_US_SHIPPING_PROFILE_MISMATCH")
  assert.equal(storageWriteCount, writesAfterFirstAutoBind)
  assert.equal(JSON.stringify(storedDestination
    .sellerOsLunaCanonicalDestinationBindingV1),
  JSON.stringify(autoBoundEnvelope))
  const unverifiableAutoBind = await autoBindRequest(undefined, {
    destinationUnambiguous: false,
  })
  assert.equal(unverifiableAutoBind?.error,
    "CANONICAL_DESTINATION_AUTO_BIND_UNVERIFIABLE")
  assert.equal(storageWriteCount, writesAfterFirstAutoBind)
  storedDestination = {}
  ...
  const checkoutBindingResultStart = progress.length
  secondPortMessage({ type: "SELLER_OS_BIND_LUNA_CANONICAL_DESTINATION" })
  await waitForTestCondition(() => progress.slice(checkoutBindingResultStart)
    .some((entry) => entry.type ===
      "LUNA_CANONICAL_DESTINATION_BINDING_RESULT"))
  const bindingResult = progress.slice(checkoutBindingResultStart).findLast((entry) =>
    entry.type === "LUNA_CANONICAL_DESTINATION_BINDING_RESULT")
  ...
  await waitForTestCondition(() => storedBindingRead !== null)
  ...
  const secondJobNavigationStart = createdTabs.length + updatedTabs.length
  secondPortMessage({ type: "START_SHIPPING_JOB", job: activeJob })
  await waitForTestCondition(() =>
    createdTabs.length + updatedTabs.length > secondJobNavigationStart)
  ...
  const unobservedNavigationStart = createdTabs.length + updatedTabs.length
  secondPortMessage({ type: "START_SHIPPING_JOB", job: unobservedJob,
    productionAutoClaim: true })
  await waitForTestCondition(() =>
    createdTabs.length + updatedTabs.length > unobservedNavigationStart)
  ...
  triggerCheckoutNavigation(unobservedJob,
    { tab: { id: 33 }, frameId: 0,
      url: "https://www.lunaportex.com/cart" })
  await waitForTestCondition(() => progress.some((entry) =>
    entry.type === "LUNA_SHIPPING_JOB_RESULT" && entry.error ===
      "CHECKOUT_NAVIGATION_NOT_OBSERVED"), 25_000)
  ...
  const notTriggeredJob = sequentialJob(5)
  const notTriggeredNavigationStart = createdTabs.length + updatedTabs.length
  secondPortMessage({ type: "START_SHIPPING_JOB", job: notTriggeredJob,
    productionAutoClaim: true })
  await waitForTestCondition(() =>
    createdTabs.length + updatedTabs.length > notTriggeredNavigationStart)
  const notTriggeredProductSender = { tab: { id: 33 }, frameId: 0,
    url: notTriggeredJob.identity.canonicalProductUrl }
  let notTriggeredCart = null
  runtimeMessageListener({ type: "SET_ACTIVE_LUNA_SHIPPING_JOB_PHASE",
    phase: "AWAITING_CART_CONFIRMATION",
    captureSessionId: notTriggeredJob.captureSessionId,
    originalCartSnapshot: [] }, notTriggeredProductSender,
  (value) => { notTriggeredCart = value })
  assert.equal(notTriggeredCart?.accepted, true)
  let notTriggeredCheckout = null
  runtimeMessageListener({ type: "SET_ACTIVE_LUNA_SHIPPING_JOB_PHASE",
    phase: "AWAITING_CHECKOUT_SHIPPING",
    captureSessionId: notTriggeredJob.captureSessionId, cartSubtotalUsd: 6 },
  { tab: { id: 33 }, frameId: 0,
    url: "https://www.lunaportex.com/cart" },
  (value) => { notTriggeredCheckout = value })
  assert.equal(notTriggeredCheckout?.accepted, true)
  await waitForTestCondition(() => progress.some((entry) =>
    entry.type === "LUNA_SHIPPING_JOB_RESULT" &&
    entry.capture?.candidateId === notTriggeredJob.identity.candidateId &&
    entry.error === "CHECKOUT_NAVIGATION_NOT_TRIGGERED"))
  assert.ok(progress.some((entry) =>
    entry.type === "LUNA_SHIPPING_JOB_PROGRESS" &&
    entry.state === "CHECKOUT_NAVIGATION_TIMEOUT"))
  assert.ok(!progress.some((entry) => entry.type ===
    "LUNA_SHIPPING_JOB_RESULT" && entry.capture?.candidateId ===
      notTriggeredJob.identity.candidateId && entry.error ===
      "REAL_CHECKOUT_HOST_NOT_ALLOWLISTED"))
  ...
  const restartedStatusStart = progress.length
  restartedPortMessage({
    type: "SELLER_OS_GET_LUNA_CANONICAL_DESTINATION_STATUS",
  })
  await waitForTestCondition(() => progress.slice(restartedStatusStart)
    .some((entry) => entry.type === "LUNA_CANONICAL_DESTINATION_STATUS"))
  const restartedStatus = progress.slice(restartedStatusStart)
    .findLast((entry) => entry.type === "LUNA_CANONICAL_DESTINATION_STATUS")
  assert.equal(restartedStatus?.type,
    "LUNA_CANONICAL_DESTINATION_STATUS")
  assert.equal(restartedStatus?.canonicalDestinationBound, true)
  assert.equal(restartedStatus?.canonicalDestinationMatch, false)
...
test("checkout distinguishes guest flow from explicit auth and captures a safe quote", async () => {
  ...
  // A first production checkout may establish exactly one durable US binding
  // without dispatching the operator-only bind control or any purchase action.
