...
test("Seller OS page proves the external bridge before opening the job port", async () => {
  ...
  assert.match(page, /EXPECTED_EXTENSION_VERSION = "1\.0\.17"/)
...
test("SHOP_APP_CHECKOUT_ALLOWLIST_REGRESSION_GUARD permits only an active bounded job", async () => {
  ...
  const chrome = {
    ...
    tabs: {
      ...
      update: async () => ({}),
      query(_queryInfo, callback) { callback(shopTabs) },
    ...
    storage: { local: {
      ...
      set(value, callback) { storedDestination = { ...value }; callback() },
    } },
  ...
  assert.equal(progress.findLast((entry) =>
    entry.type === "LUNA_CANONICAL_DESTINATION_BINDING_RESULT")?.success, true)
  shopTabs = []
  firstPortMessage({ type: "SELLER_OS_BIND_LUNA_CANONICAL_DESTINATION" })
  await new Promise((resolve) => setTimeout(resolve, 0))
  assert.equal(progress.at(-1)?.error,
    "CANONICAL_BINDING_CHECKOUT_TAB_NOT_FOUND")
  shopTabs = [{ id: 11, url: "https://shop.app/pay/one" },
    { id: 12, url: "https://shop.app/pay/two" }]
  firstPortMessage({ type: "SELLER_OS_BIND_LUNA_CANONICAL_DESTINATION" })
  await new Promise((resolve) => setTimeout(resolve, 0))
  assert.equal(progress.at(-1)?.error,
    "CANONICAL_BINDING_CHECKOUT_TAB_AMBIGUOUS")
  ...
  assert.deepEqual(Object.keys(storedBinding).sort(), [
    "canonicalDestinationFingerprint", "countryClass", "fingerprintVersion",
  ])
