...
test("extension is origin-limited and transfers structured visible rows without network scraping", () => {
  ...
  assert.match(content, /Product Research · v1\.2\.0/)
  assert.match(content, /Capturar y continuar/)
  ...
  assert.doesNotMatch(content, /\bfetch\s*\(|document\.cookie|localStorage|outerHTML|innerHTML|toDataURL|toBlob|captureVisibleTab/)
  assert.doesNotMatch(content, /\.src\b|getAttribute\(["']src/)
  const archive = readFileSync(
    "public/seller-os-tools/ebay-product-research-capture-extension-v1.2.0.zip",
  )
...
test("visual observation migration is append-only and explicitly excludes reconstructive content", () => {
  ...
  assert.match(receiver, /Patrones visuales observados/)
  assert.match(receiver, /No prueban causalidad/)
})
