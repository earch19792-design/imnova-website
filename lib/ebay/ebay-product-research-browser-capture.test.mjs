...
test("v1.2.38 preserves the v1.2.13 tabular row extraction contract byte-for-byte", () => {
...
test("v1.2.38 accepts query-bound zero-sold and selector-independent switched-to-active notices", () => {
  const content = readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/content.js", "utf8",
  )
  const migration = readFileSync(
...
test("v1.2.38 recognizes current eBay switched-to-active notice variants without accepting an active grid alone", () => {
  const content = readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/content.js", "utf8",
  )
...
test("v1.2.38 keeps Product Research readiness independent of benign URL representation", () => {
  const content = readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/content.js", "utf8",
  )
...
test("v1.2.38 preserves one authenticated receiver and one chronological CTA", () => {
  const receiver = readFileSync(
    "app/admin/ebay/mobile-review/product-research-capture/page.tsx", "utf8",
  )
  const content = readFileSync(
