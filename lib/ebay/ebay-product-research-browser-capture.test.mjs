...
test("v1.2.9 preserves the complete v1.2.8 tabular extraction contract byte-for-byte", () => {
...
test("every operator surface names the current Product Research extension", () => {
  ...
  assert.match(surfaces, /v1\.2\.9/)
  assert.doesNotMatch(surfaces, /v1\.2\.[0-7]/)
})
...
test("Seller OS downloads the same current guided extension validated by the suite", () => {
  ...
  assert.match(
    sellerOsUi,
    /ebay-product-research-capture-extension-v1\.2\.9\.zip/,
  )
  assert.match(sellerOsUi, /extensión asistida v1\.2\.9/)
  assert.doesNotMatch(
    sellerOsUi,
    /ebay-product-research-capture-extension-v1\.2\.[0-8]\.zip/,
  )
})
...
test("v1.2.9 preserves one authenticated receiver and one chronological CTA", () => {
  const receiver = readFileSync(
    "app/admin/ebay/mobile-review/product-research-capture/page.tsx", "utf8",
  )
  const content = readFileSync(
