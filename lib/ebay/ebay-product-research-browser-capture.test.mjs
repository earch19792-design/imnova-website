...
test("extension is origin-limited and transfers structured visible rows without network scraping", () => {
  ...
  assert.deepEqual(manifest.host_permissions, [
    "https://www.ebay.com/sh/research*", "https://i.ebayimg.com/*",
  ])
  ...
  assert.match(content, /Product Research · v1\.2\.10/)
  ...
  const archive = readFileSync(
    "public/seller-os-tools/ebay-product-research-capture-extension-v1.2.10.zip",
  )
  ...
  assert.deepEqual([...archiveEntries.keys()].sort(), ["README.md", "background.js", "content.js", "manifest.json"])
...
test("v1.2.10 preserves the v1.2.9 and v1.2.8 tabular extraction contracts byte-for-byte", () => {
  const current = readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/content.js", "utf8",
  )
  const previousArchive = readFileSync(
    "public/seller-os-tools/ebay-product-research-capture-extension-v1.2.9.zip",
  )
  const previous = storedZipEntries(previousArchive).get("content.js").toString("utf8")
  const v128 = storedZipEntries(readFileSync(
    "public/seller-os-tools/ebay-product-research-capture-extension-v1.2.8.zip",
  )).get("content.js").toString("utf8")
  const tabularContract = (value) => value.slice(
    value.indexOf("const REQUIRED_FIELDS"),
    value.indexOf("function visibleInViewport"),
  )
  assert.ok(tabularContract(current).length > 20_000)
  assert.equal(tabularContract(current), tabularContract(previous))
  assert.equal(tabularContract(current), tabularContract(v128))
})
...
test("every operator surface names the current Product Research extension", () => {
  ...
  assert.match(surfaces, /v1\.2\.10/)
  assert.doesNotMatch(surfaces, /v1\.2\.[0-8](?!\d)/)
})
...
test("Seller OS downloads the same current guided extension validated by the suite", () => {
  ...
  assert.match(
    sellerOsUi,
    /ebay-product-research-capture-extension-v1\.2\.10\.zip/,
  )
  assert.match(sellerOsUi, /extensión asistida v1\.2\.10/)
  assert.doesNotMatch(
    sellerOsUi,
...
test("v1.2.10 preserves one authenticated receiver and one chronological CTA", () => {
  const receiver = readFileSync(
    "app/admin/ebay/mobile-review/product-research-capture/page.tsx", "utf8",
  )
  const content = readFileSync(
