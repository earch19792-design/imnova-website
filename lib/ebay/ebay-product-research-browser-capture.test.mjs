...
test("extension is origin-limited and transfers structured visible rows without network scraping", () => {
  ...
  assert.match(content, /Product Research · v1\.2\.25/)
  ...
  const archive = readFileSync(
    "public/seller-os-tools/ebay-product-research-capture-extension-v1.2.25.zip",
  )
...
test("v1.2.25 preserves the v1.2.13 tabular row extraction contract byte-for-byte", () => {
...
test("v1.2.25 accepts query-bound zero-sold and selector-independent switched-to-active notices", () => {
  const content = readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/content.js", "utf8",
  )
  const migration = readFileSync(
...
test("v1.2.25 recognizes current eBay switched-to-active notice variants without accepting an active grid alone", () => {
  const content = readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/content.js", "utf8",
  )
...
test("v1.2.25 keeps Product Research readiness independent of benign URL representation", () => {
  const content = readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/content.js", "utf8",
  )
...
test("Seller OS downloads the same current guided extension validated by the suite", () => {
  ...
  assert.match(
    sellerOsUi,
    /ebay-product-research-capture-extension-v1\.2\.25\.zip/,
  )
  assert.match(sellerOsUi, /extensión asistida v1\.2\.25/)
  assert.doesNotMatch(
    sellerOsUi,
...
test("v1.2.25 preserves one authenticated receiver and one chronological CTA", () => {
  const receiver = readFileSync(
    "app/admin/ebay/mobile-review/product-research-capture/page.tsx", "utf8",
  )
  const content = readFileSync(
