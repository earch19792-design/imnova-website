...
test("extension is origin-limited and transfers structured visible rows without network scraping", () => {
  ...
  assert.match(content, /Product Research · v1\.2\.11/)
  ...
  const archive = readFileSync(
    "public/seller-os-tools/ebay-product-research-capture-extension-v1.2.11.zip",
  )
...
test("v1.2.11 preserves the v1.2.10 tabular extraction contract byte-for-byte", () => {
...
test("v1.2.11 accepts only a query-bound official zero-sold message", () => {
  const content = readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/content.js", "utf8",
  )
  const migration = readFileSync(
...
test("every operator surface names the current Product Research extension", () => {
  ...
  assert.match(surfaces, /v1\.2\.11/)
  assert.doesNotMatch(surfaces,
    /extension-v1\.2\.10\.zip|extensión asistida v1\.2\.10/)
})
...
test("Seller OS downloads the same current guided extension validated by the suite", () => {
  ...
  assert.match(
    sellerOsUi,
    /ebay-product-research-capture-extension-v1\.2\.11\.zip/,
  )
  assert.match(sellerOsUi, /extensión asistida v1\.2\.11/)
  assert.doesNotMatch(
    sellerOsUi,
...
test("v1.2.11 preserves one authenticated receiver and one chronological CTA", () => {
  const receiver = readFileSync(
    "app/admin/ebay/mobile-review/product-research-capture/page.tsx", "utf8",
  )
  const content = readFileSync(
