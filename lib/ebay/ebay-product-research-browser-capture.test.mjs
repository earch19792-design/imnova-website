...
test("extension is origin-limited and transfers structured visible rows without network scraping", () => {
  ...
  assert.match(content, /Product Research · v1\.2\.3/)
  assert.match(content, /Capturar y continuar/)
  ...
  assert.match(content, /setSearchInputValue\(input, query\)[\s\S]*PRODUCT_RESEARCH_SAFE_SUBMIT_NOT_FOUND/)
  assert.match(content, /PRODUCT_RESEARCH_SAFE_SUBMIT_NOT_FOUND[\s\S]*return[\s\S]*form\.requestSubmit\(\)/)
  ...
  const archive = readFileSync(
    "public/seller-os-tools/ebay-product-research-capture-extension-v1.2.3.zip",
  )
