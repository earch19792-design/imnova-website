...
test("extension is origin-limited and transfers structured visible rows without network scraping", () => {
  ...
  assert.match(content, /Product Research · v1\.2\.8/)
...
test("Seller OS downloads the same current guided extension validated by the suite", () => {
  ...
  assert.match(
    sellerOsUi,
    /ebay-product-research-capture-extension-v1\.2\.8\.zip/,
  )
  assert.match(sellerOsUi, /extensión asistida v1\.2\.8/)
  assert.doesNotMatch(
    sellerOsUi,
    /ebay-product-research-capture-extension-v1\.2\.[0-7]\.zip/,
  )
})
...
test("v1.2.8 reuses one authenticated receiver and exposes one chronological CTA", () => {
  const receiver = readFileSync(
    "app/admin/ebay/mobile-review/product-research-capture/page.tsx", "utf8",
  )
  const content = readFileSync(
