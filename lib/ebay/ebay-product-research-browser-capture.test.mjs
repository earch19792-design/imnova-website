...
test("extension is origin-limited and transfers structured visible rows without network scraping", () => {
  ...
  assert.match(content, /Product Research · v1\.2\.5/)
  assert.match(content, /Capturar y continuar/)
  ...
  const archive = readFileSync(
    "public/seller-os-tools/ebay-product-research-capture-extension-v1.2.5.zip",
  )
...
test("every operator surface names the current Product Research extension", () => {
  ...
  assert.match(surfaces, /v1\.2\.5/)
  assert.doesNotMatch(surfaces, /v1\.2\.[0-4]/)
})
...
test("Seller OS downloads the same current guided extension validated by the suite", () => {
  ...
  assert.match(
    sellerOsUi,
    /ebay-product-research-capture-extension-v1\.2\.5\.zip/,
  )
  assert.match(sellerOsUi, /extensión asistida v1\.2\.5/)
  assert.doesNotMatch(
    sellerOsUi,
    /ebay-product-research-capture-extension-v1\.2\.[0-4]\.zip/,
  )
})
...
test("receiver validates eBay origin and route resumes the same Loop 1 run without Discovery or writes", () => {
  ...
  assert.match(receiver, /VOLVER A SELLER OS/)
...
test("a table from the wrong tab becomes navigation-only and never reaches import", () => {
  ...
  assert.match(receiver, /nextQuery: payload\.result\.queryPlan\?\.nextQuery\?\.searchQuery \?\? null/)
...
test("the prepared query UI separates product family, exact query, open and copy actions", () => {
  ...
  assert.match(panel, /queryKey\(task\?\.action_schema\?\.query\) === queryKey\(nextQuery\)/)
