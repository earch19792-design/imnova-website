...
function endedItemCaptureFixture({
  itemId = "278219209833",
  body = "This listing was ended by the seller because the item is no longer available.",
  links = [],
  banner = "ENDED",
  soldPrice = "US $25.95",
  displayedPrice = "US $25.95",
} = {}) {
  let listener = null
  const element = ({ href = "", innerText = "", scope = null,
    className = "", children = {} } = {}) => ({ href, innerText,
    textContent: innerText, hidden: false, className,
    getAttribute: (name) => name === "aria-hidden" ? null
      : name === "content" ? null : null,
    getBoundingClientRect: () => ({ width: 100, height: 20 }),
    closest: () => scope,
    querySelector: (selector) => children[selector] ?? null })
  const sellerLinks = links.map(({ href, text = "Seller", scoped = true }) =>
    element({ href, innerText: text, scope: scoped ? {} : null }))
  const bannerElement = element({ innerText: banner })
  const soldPriceElement = element({ innerText: soldPrice })
  const card = element({ innerText: `${banner} ${soldPrice}`,
    children: {
      ".x-item-condensed-card__banner": bannerElement,
      ".x-item-condensed-card__sold-price": soldPriceElement,
    } })
  const marker = element({ innerText: body })
  const price = element({ innerText: displayedPrice })
  const document = {
    body: { innerText: body },
    querySelectorAll: (selector) => {
      if (selector === "*") return []
      if (selector === ".x-item-condensed-card") return [card]
      if (selector.includes("a[href*='/usr/'")) return sellerLinks
      if (selector.includes("data-testid*='ended'")) return [marker]
      if (selector.includes(".x-price-primary")) return [price]
      return []
    },
  }
  const window = { location: { protocol: "https:", hostname: "www.ebay.com",
    pathname: `/itm/${itemId}`, href: `https://www.ebay.com/itm/${itemId}` },
  ...
  runInNewContext(readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/ended-item-content.js",
    "utf8"), { window, document, chrome, URL, Date, console })
  let response = null
  listener({ type: "IMNOVA_AUTOMATED_ENDED_ITEM_DETAIL_CAPTURE_V1",
    expectedItemId: itemId }, { id: "extension-id" },
  (value) => { response = structuredClone(value) })
  return response
}

test("ended-item capture binds seller identity to the exact eBay item", () => {
  const itemId = "278219209833"
  const captured = endedItemCaptureFixture({ itemId, links: [
    { href: `https://www.ebay.com/sch/agomarketing89/m.html?item=${itemId}` },
    { href: "https://www.ebay.com/str/carrysuperstore", scoped: false },
  ] })
  assert.equal(captured.success, true)
  assert.equal(captured.sellerUsername, "agomarketing89")
  assert.equal(captured.sellerIdentityStatus, "PROVEN")
  assert.equal(captured.sellerIdentityKey, "SELLER:agomarketing89")
  assert.equal(captured.realizedPriceStatus, "UNPROVEN")
})

test("active item detail can prove seller but never promotes its current price", () => {
  const itemId = "366617258035"
  const captured = endedItemCaptureFixture({ itemId,
    body: "4 available 1 sold US $21.95 or Best Offer Buy It Now",
    links: [{ href:
      `https://www.ebay.com/sch/novalradingllc/m.html?item=${itemId}` }] })
  assert.equal(captured.sellerUsername, "novalradingllc")
  assert.equal(captured.endedItemIdentityStatus, "NOT_PROVEN")
  assert.equal(captured.displayedSoldPriceAmount, null)
  assert.equal(captured.realizedTransactionPriceAmount, null)
  assert.equal(captured.realizedPriceStatus, "UNPROVEN")
})

test("ended-item capture accepts the exact condensed seller sid and rejects an ENDED price as realized", () => {
  const itemId = "298660359800"
  const captured = endedItemCaptureFixture({ itemId, links: [
    { href: "https://www.ebay.com/sch/i.html?sid=galactic.rummage" },
  ], banner: "ENDED", soldPrice: "US $37.99", displayedPrice: "US $37.99" })
  assert.equal(captured.sellerUsername, "galactic.rummage")
  assert.equal(captured.endedItemIdentityStatus, "PROVEN_SAME_ENDED_ITEM")
  assert.equal(captured.displayedSoldPriceAmount, 37.99)
  assert.equal(captured.realizedTransactionPriceAmount, null)
  assert.equal(captured.realizedPriceStatus, "UNPROVEN")
})

test("ended-item capture confirms only an explicit SOLD condensed price", () => {
  const itemId = "298660359801"
  const captured = endedItemCaptureFixture({ itemId, links: [
    { href: `https://www.ebay.com/fdbk/mweb_profile?username=exact-seller&item_id=${itemId}` },
  ], body: "This item sold.", banner: "SOLD", soldPrice: "US $41.50",
  displayedPrice: "US $41.50" })
  assert.equal(captured.sellerUsername, "exact-seller")
  assert.equal(captured.realizedTransactionPriceAmount, 41.5)
  assert.equal(captured.realizedPriceStatus, "REALIZED_PRICE_CONFIRMED")
  assert.equal(captured.marketplaceWrites, 0)
})
...
test("deduplication is stable and persistence keeps only bounded title evidence", () => {
  const result = parseProductResearchBrowserCapture({
  ...
  assert.match(stored, /"bounded_title_evidence":"Lysol Disinfecting Wipes Lemon 3 x 15 ct"/)
...
test("extension is origin-limited and transfers structured visible rows without network scraping", () => {
  ...
  const ended = readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/ended-item-content.js", "utf8",
  )
  ...
  assert.match(content, /Product Research · v1\.2\.28/)
  ...
  assert.equal(manifest.key.length > 300, true)
  ...
  assert.match(background, /IMNOVA_EBAY_NEAR_EXACT_SOLD_ENRICHMENT_V1/)
  assert.match(background, /PRODUCT_RESEARCH_ADMIN_BRIDGE_V2/)
  assert.match(background, /runtimeReachable: true/)
  assert.match(background, /serviceWorkerResponse: "PROBE_ACK"/)
  assert.match(background, /void injectAdminBridgeIntoExistingTabs\(\)/)
  for (const failure of ["CONTENT_SCRIPT_CONTEXT_INVALIDATED",
    "EXTENSION_RUNTIME_UNREACHABLE", "SERVICE_WORKER_MESSAGE_FAILED"]) {
    assert.match(readFileSync(
      "tools/browser-extensions/ebay-product-research-capture/admin-bridge.js",
      "utf8"), new RegExp(failure))
  }
  assert.match(background, /MAX_ENDED_ITEM_DETAIL_CAPTURES = 6/)
  assert.match(background, /runEndedItemDetailTask/)
  assert.match(background, /PROVEN_SAME_ENDED_ITEM/)
  assert.match(background, /tasks\.length > 6/)
  assert.match(background, /runNearExactSoldTask/)
  assert.match(background, /status: "FAILED"/)
  assert.match(background, /function enrichmentFailureClass/)
  for (const failureClass of ["TIMEOUT", "NAVIGATION_FAILED", "CHALLENGE",
    "PAGE_PARSE_FAILED", "REQUEST_FAILED"]) {
    assert.match(background, new RegExp(`return "${failureClass}"`))
  }
  assert.match(background, /failureClass: enrichmentFailureClass\(errorCode\)/)
  assert.doesNotMatch(background,
    /if \(!completed\.length\) throw new Error\("NEAR_EXACT_SOLD_ALL_TASKS_FAILED"\)/)
  assert.match(background, /completed\.flatMap/)
  assert.match(background, /failedTaskCount/)
  assert.match(background, /chrome\.tabs\.remove\(tab\.id\)/)
  assert.match(sold, /function sellerIdentity\(card, itemId\)/)
  assert.match(sold, /sellerIdentityKey: "SELLER_IDENTITY_UNKNOWN"/)
  assert.match(sold, /UNIQUE_PUBLIC_SOLD_RESULT_SELLER/)
  assert.match(sold, /sellerProfileUrl/)
  assert.match(sold, /soldConfirmationStatus: "SOLD_CONFIRMED"/)
  assert.match(sold, /displayedPriceStatus: "DISPLAYED_PRICE_CONFIRMED"/)
  assert.match(sold, /shipping\|delivery\|env\[ií\]o/)
  assert.match(sold, /fieldProvenance/)
  assert.match(sold, /PROVEN_ZERO_BY_SEARCH_FILTER/)
  assert.match(sold, /SOLD_SEARCH_FREE_SHIPPING_FILTER/)
  assert.match(sold, /freeShippingFilterRequired/)
  assert.match(background, /url\.searchParams\.set\("LH_FS", "1"\)/)
  assert.match(background, /waitForFreeShippingNavigation/)
  assert.match(background, /documentId: captureBinding\.documentId/)
  assert.match(background, /documentIds: \[captureBinding\.documentId\]/)
  assert.match(background, /verifyBoundCapture/)
  assert.match(background, /mergeFreeShippingEvidence/)
  assert.match(background, /navigationRetryCount/)
  assert.match(background, /freeShippingBranchStatus/)
  assert.match(background, /endedItemDetailOutcomes/)
  assert.match(ended, /ENDED_ITEM_STATUS_AND_SELLER_NOT_PROVEN/)
  assert.match(ended, /ENDED_ITEM_PUBLIC_DETAIL/)
  assert.match(ended, /REALIZED_PRICE_CONFIRMED/)
  assert.match(ended, /ENDED_ITEM_STABLE_SELLER_PROFILE_URL/)
  assert.match(ended, /SELLER_PROFILE:/)
  assert.match(ended, /sellerIdentityKey: "SELLER_IDENTITY_UNKNOWN"/)
  assert.match(ended, /vendedor\|vendeur/)
  assert.match(ended, /AMBIGUOUS_PUBLIC_SELLER_PROFILES/)
  assert.match(ended, /usernames\.size === 1/)
  assert.match(ended, /stores\.size === 1/)
  assert.match(ended, /PROVEN_SAME_ITEM_ID/)
  assert.match(ended, /SOLD_ITEM_PUBLIC_DETAIL_SAME_ITEM_ID/)
  assert.match(ended, /elementsAcrossOpenRoots/)
  assert.doesNotMatch(ended,
    /querySelectorAll\([\s\S]{0,180}seller=[\s\S]{0,80}\.filter\(visible\)/)
  ...
  assert.deepEqual([...archiveEntries.keys()].sort(), [
    "README.md", "admin-bridge.js", "background.js", "content.js",
    "ended-item-content.js", "manifest.json", "navigation-binding.js", "sold-content.js",
    "worker-control-recovery.js",
  ])
...
test("v1.2.40 preserves the v1.2.13 tabular row extraction contract byte-for-byte", () => {
...
test("v1.2.40 accepts query-bound zero-sold and selector-independent switched-to-active notices", () => {
  const content = readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/content.js", "utf8",
  )
  const migration = readFileSync(
...
test("v1.2.40 recognizes current eBay switched-to-active notice variants without accepting an active grid alone", () => {
  const content = readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/content.js", "utf8",
  )
...
test("v1.2.40 keeps Product Research readiness independent of benign URL representation", () => {
  const content = readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/content.js", "utf8",
  )
...
test("Seller OS downloads the same current guided extension validated by the suite", () => {
  ...
  assert.match(sellerOsUi, /EBAY_ONE_CLICK_RESEARCH_EXTENSION_ARTIFACT\.archivePath/)
  assert.match(sellerOsUi, /EBAY_ONE_CLICK_RESEARCH_EXTENSION_ARTIFACT\.version/)
  assert.match(sellerOsUi, /Artefacto esperado/)
  assert.doesNotMatch(
    sellerOsUi,
...
test("v1.2.40 preserves one authenticated receiver and one chronological CTA", () => {
  const receiver = readFileSync(
    "app/admin/ebay/mobile-review/product-research-capture/page.tsx", "utf8",
  )
  const content = readFileSync(
