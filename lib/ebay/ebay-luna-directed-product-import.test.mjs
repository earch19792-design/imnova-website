...
test("accepts an exact approved Luna handle up to the StockGuard bound", () => {
  const handle = `product-${"a".repeat(212)}`
  assert.equal([...handle].length, 220)
  assert.equal(parseDirectedLunaProductUrl(
    `https://lunaportex.com/products/${handle}`).handle, handle)
  assert.throws(() => parseDirectedLunaProductUrl(
    `https://lunaportex.com/products/${handle}a`),
...
test("reads the official public product without trusting redirects or malformed identity", async () => {
  ...
  assert.equal(product.sourceMode, "PUBLIC_READ_ONLY_PRODUCT_PAGE")
...
test("preserves only valid explicit public variant inventory quantity", async () => {
  const withQuantity = await fetchDirectedLunaProduct(lunaUrl, lunaFetch({
    ...sourcePayload,
    variants: [{ ...sourcePayload.variants[0], inventory_quantity: 0 }],
  }))
  assert.equal(withQuantity.variants[0].sourceInventoryQuantity, 0)
  assert.equal(withQuantity.variants[0].sourceInventoryQuantityExplicit, true)

  await assert.rejects(
    fetchDirectedLunaProduct(lunaUrl, lunaFetch({
      ...sourcePayload,
      variants: [{ ...sourcePayload.variants[0], inventory_quantity: "unknown" }],
    })),
    /LUNA_DIRECTED_IMPORT_VARIANT_REQUIRED/,
  )
})
