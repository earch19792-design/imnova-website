...
test("bounded visual generation preserves Product Truth and records budget without eBay writes", async () => {
  ...
  const asset = supabase.inserts.find((row) =>
    row.table === "ebay_listing_image_assets")?.payload
  assert.equal(asset.qa_result.productTruthPreserved, true)
  assert.equal(asset.transformation.sellerOsVisualVariant
    .sourceImageFullResolutionCertified, true)
})
