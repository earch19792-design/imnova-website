...
import {
  assessSellerOsVisualExperimentV1,
...
function metric(itemId, value, options = {}) {
  return {
    value, unit: null,
    availability: options.availability ?? "AVAILABLE",
    completeness: options.completeness ?? "COMPLETE",
    source: { system: "EBAY_ANALYTICS", operation: "TRAFFIC_REPORT",
      evidenceReference: options.reference ?? `analytics:${itemId}` },
    capturedAt: options.capturedAt ?? "2026-08-29T12:00:00.000Z",
    marketplace: { marketplaceId: "EBAY_US", accountAlias: "primary" },
    identity: { itemId, variationKey: null, sku: "ITEM5810" }, grain: "ITEM",
    reportingWindow: options.reportingWindow ?? {
      start: "2026-08-22T00:00:00.000Z", end: "2026-08-29T00:00:00.000Z",
      timeZone: "UTC" },
    freshness: { status: options.freshness ?? "FRESH",
      ageSeconds: options.freshness === "STALE" ? 86_401 : 0,
...
function experimentListing(itemId, analyticsAvailable) {
  const unavailable = (code) => metric(itemId, null, {
    availability: "UNAVAILABLE", completeness: "UNPROVEN",
    reportingWindow: null, capturedAt: null, limitationCode: code })
  const analytics = analyticsAvailable ? {
    impressions: metric(itemId, 200), ebay_views: metric(itemId, 20),
    ctr_reported: metric(itemId, 10), ctr_calculated: unavailable("NOT_NEEDED"),
    orders: metric(itemId, 2), units_sold: metric(itemId, 2),
  } : {
    impressions: unavailable("EBAY_MONITOR_ANALYTICS_429"),
    ebay_views: unavailable("EBAY_MONITOR_ANALYTICS_429"),
    ctr_reported: unavailable("EBAY_MONITOR_ANALYTICS_429"),
    ctr_calculated: unavailable("EBAY_MONITOR_ANALYTICS_429"),
    orders: unavailable("EBAY_MONITOR_ANALYTICS_429"),
    units_sold: unavailable("EBAY_MONITOR_ANALYTICS_429"),
  }
  return {
    ...listing(itemId, "https://i.ebayimg.com/images/g/one/s-l1600.png"),
    stock: { state: "IN_STOCK_SIGNAL", freshness: { status: "FRESH" },
      sourceContractStatus: "HEALTHY", supplierLinkageStatus: "CERTIFIED" },
    metrics: { ...analytics, net_profit: metric(itemId, 5.25),
      contribution: metric(itemId, 5.25) },
  }
}

function selectedVisual() {
  return { ebayItemId: "366582586826", observation: "Producto pequeño",
    objective: "Aumentar reconocimiento", hypothesis: "Puede mejorar CTR",
    productTruthFingerprint: `sha256:${"a".repeat(64)}`,
    selectedVariantId: "asset-a", variants: [{ assetId: "asset-a",
...
test("durable prepared experiment waits for compatible Analytics without converting null to zero", () => {
  const assessment = assessSellerOsVisualExperimentV1({
    monitor: monitor([experimentListing("366582586826", false)]),
    ebayItemId: "366582586826", visual: selectedVisual(),
    lifecycleStatus: "READY",
    experimentId: "48299f52-d0b8-47e3-8e64-23ffed0fc78e",
  })
  assert.equal(assessment.workflowState, "READY_WAITING_FOR_BASELINE")
  assert.equal(assessment.baseline.status, "UNPROVEN")
  assert.equal(assessment.baseline.impressions, null)
  assert.equal(assessment.baseline.nullIsZero, false)
  assert.equal(assessment.experimentStartBlockedByAnalytics, true)
  assert.equal(assessment.ebayWriteAllowed, false)
  assert.equal(assessment.ebayWriteExecuted, false)
  assert.equal(assessment.allNonAnalyticsGuardsPass, true)
})

test("stale compatible Analytics is exposed as LAST_KNOWN_GOOD but never activates the experiment", () => {
  const row = experimentListing("366582586826", true)
  for (const key of ["impressions", "ebay_views", "ctr_reported"]) {
    row.metrics[key].freshness = {
      status: "STALE", ageSeconds: 86_401, maximumAgeSeconds: 86_400,
    }
  }
  const inputMonitor = monitor([row])
  inputMonitor.backend.trafficScopes = { currentLivePortfolio: {
  ...
  assert.equal(assessment.baseline.status, "LAST_KNOWN_GOOD")
  assert.equal(assessment.baseline.freshness, "STALE")
  assert.equal(assessment.baseline.impressions.value, 200)
  assert.equal(assessment.experimentStartBlockedByAnalytics, true)
  assert.equal(assessment.ebayWriteAllowed, false)
})

test("unproven economics remains tri-state evidence, never a false business outcome", () => {
  const row = experimentListing("366582586826", true)
  row.metrics.net_profit = metric("366582586826", null, {
    availability: "INSUFFICIENT_EVIDENCE", completeness: "UNPROVEN",
    reportingWindow: null, capturedAt: null,
    limitationCode: "ECONOMICS_INPUT_PROVENANCE_INCOMPLETE" })
  row.metrics.contribution = row.metrics.net_profit
  row.metrics.supplier_cost = metric("366582586826", 44.2, {
    availability: "PARTIAL", completeness: "PARTIAL",
    limitationCode: "SUPPLIER_COST_CURRENCY_UNPROVEN" })
  row.metrics.shipping = metric("366582586826", null, {
    availability: "INSUFFICIENT_EVIDENCE", completeness: "UNPROVEN",
    limitationCode: "ECONOMICS_INPUT_PROVENANCE_INCOMPLETE" })
  row.metrics.fees = row.metrics.shipping
  row.metrics.promoted_fees = row.metrics.shipping
  const assessment = assessSellerOsVisualExperimentV1({
    monitor: monitor([row]), ebayItemId: "366582586826",
    visual: selectedVisual(), lifecycleStatus: "READY",
    experimentId: "48299f52-d0b8-47e3-8e64-23ffed0fc78e",
  })
  const economicsGuard = assessment.guards.find((guard) =>
    guard.code === "ECONOMICS_NON_NEGATIVE")
  assert.equal(economicsGuard.status, "UNPROVEN")
  assert.equal(economicsGuard.passed, null)
  assert.equal(assessment.economics.nonNegative, null)
  assert.equal(assessment.economics.unprovenIsNegativeBusinessOutcome, false)
  assert.equal(assessment.economics.inputs.supplierCost.value, 44.2)
  assert.equal(assessment.economics.inputs.supplierCost.status, "UNPROVEN")
  assert.equal(assessment.ebayWriteAllowed, false)
})

test("a durable proven-rate pre-sale model satisfies economics without being relabeled as realized fee", () => {
  const row = experimentListing("366582586826", true)
  row.metrics.net_profit = metric("366582586826", null, {
    availability: "INSUFFICIENT_EVIDENCE", completeness: "UNPROVEN",
    reportingWindow: null, capturedAt: null,
  ...
  const assessment = assessSellerOsVisualExperimentV1({
    ...
    preSaleEconomicsEvidence: {
      ...
      economicsEvidenceClass: "PROVEN_RATE_PRE_SALE_MODEL",
  ...
  const economicsGuard = assessment.guards.find((guard) =>
    guard.code === "ECONOMICS_NON_NEGATIVE")
  assert.equal(economicsGuard.status, "PASSED")
  assert.equal(assessment.economics.status, "AVAILABLE")
  assert.equal(assessment.economics.profitUsd, 5.57)
  assert.equal(assessment.economics.evidenceClass,
    "PROVEN_RATE_PRE_SALE_MODEL")
  assert.equal(assessment.economics.feeEvidenceClass,
    "PROVEN_RATE_PRE_SALE_FEE_MODEL")
  assert.equal(assessment.economics.preSaleModel.realizedFee, null)
})

test("compatible item-grain baseline advances readiness while preserving one-variable isolation", () => {
  const assessment = assessSellerOsVisualExperimentV1({
    monitor: monitor([experimentListing("366582586826", true)]),
    ebayItemId: "366582586826", visual: selectedVisual(),
    lifecycleStatus: "READY",
    experimentId: "48299f52-d0b8-47e3-8e64-23ffed0fc78e",
  })
  assert.equal(assessment.workflowState, "READY_TO_APPLY")
  assert.equal(assessment.baseline.status, "AVAILABLE")
  assert.equal(assessment.baseline.impressions.value, 200)
  assert.equal(assessment.ebayWriteAllowed, true)
  assert.equal(assessment.variable, "HERO_IMAGE")
  assert.equal(assessment.policy.doNotTouchVariableWhileActive, "HERO_IMAGE")
  assert.equal(assessment.policy.onePrimaryVariable, true)
  assert.equal(assessment.policy.causalClaimAllowed, false)
})

test("experiment UI explains evidence, hypothesis and blocks apply while baseline is absent", () => {
  const page = readFileSync("app/admin/ebay/listing-optimization/page.tsx", "utf8")
  const route = readFileSync("app/api/admin/ebay/strategic-review/route.ts", "utf8")
  assert.match(page, /Experimento de imagen principal|imagen hero/)
  assert.match(page, /Esperando baseline de Analytics/)
  assert.match(page, /Las métricas ausentes no se convierten en cero/)
  assert.match(page, /Aplicar variante/)
  assert.match(page, /GANÓ, PERDIÓ o SIN EVIDENCIA SUFICIENTE/)
  assert.match(route, /actorId: access\.auth\.userId/)
  assert.doesNotMatch(route, /ReviseFixedPriceItem/)
})
