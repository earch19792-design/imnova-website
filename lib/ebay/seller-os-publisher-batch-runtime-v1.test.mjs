...
test("one owner click binds the exact current material cohort and runtime operates children", () => {
  ...
  const batchRuntime = route.slice(
    ...
    route.indexOf("async function authorizeAndRunSellerOsPublisherBatchV1"),
  )
...
test("exact batch lineage resumes through semantic readback without a new Offer", () => {
  ...
  assert.match(route, /EXACT_DURABLE_BATCH_APPROVAL_CONTINUATION/)
  assert.match(route,
    /batchContinuationAuthority && material\.exact/)
  assert.match(route, /newHumanApprovalCreated: false/)
  assert.match(route, /packageMutationAllowed: false/)
  assert.match(route, /EBAY_AUTHORIZED_PUBLICATION_OPPORTUNITY_INVALID/)
  assert.match(route, /EBAY_AUTHORIZED_PUBLICATION_APPROVED_IMAGES_REQUIRED/)
  const handoff = read(
    "lib/ebay/ebay-quick-pick-canonical-publish-handoff-v1.ts",
  )
  ...
  assert.match(route, /"EBAY_DRAFT_ONLY_ACTIVE_APPROVAL_EXISTS"/)
  assert.match(route, /"QUICK_PICK_PUBLISH_BATCH_AUTHORITY_AMBIGUOUS"/)
  assert.match(route,
    /rearm_seller_os_publisher_batch_exact_authority_child_v2/)
  const v2 = read(
    "supabase/migrations/20260905185000_rearm_exact_batch_authority_v2.sql",
  )
  assert.match(v2, /attempt_count < 6/)
  assert.match(v2, /marketplace_write_count = 0/)
  assert.match(v2, /EXACT_BATCH_APPROVAL_LINEAGE_RESUME_SAFE/)
  assert.doesNotMatch(v2, /update public\.ebay_listing_packages/)
})

test("final publication SQL accepts only exact immutable batch authority", () => {
  const migration = read(
    "supabase/migrations/20260905184500_align_final_publication_with_exact_batch_authority_v1.sql",
  )
  assert.match(migration,
    /SELLER_OS_PUBLISHER_BATCH_AUTHORIZATION_V1/)
  assert.match(migration, /v_batch_child\.package_digest =/)
  assert.match(migration, /authorizedImagesDigest/)
  assert.match(migration, /v_batch_authorized and v_package\.status = 'ready_for_review'/)
  assert.match(migration,
    /if not v_batch_authorized then[\s\S]*ebay_same_day_pilot_candidates/)
  assert.match(migration,
    /jsonb_array_length\(v_images\) < 1/)
  assert.match(migration,
    /not v_batch_authorized and jsonb_array_length\(v_images\) <> 6/)
  assert.match(migration,
    /if v_batch_authorized then[\s\S]*return next v_publication/)
  assert.doesNotMatch(migration, /ITEM3438|FL-LONG-SPOONS|ITEM5313/)
})
