...
function monitor(target = listing()) {
  return {
    ...
    liveCertification: {
      ...
      oauth: { status: "AVAILABLE", tokenReceived: true, scopes: [{
        scope: "https://api.ebay.com/oauth/api_scope",
        classifications: ["READ_REQUIRED", "READ_AVAILABLE",
          "WRITE_CAPABLE_BUT_NOT_USED"],
        evidenceOperation:
          "TRADING_GET_USER+TRADING_GET_MY_EBAY_SELLING+TRADING_GET_ITEM_MARKETPLACE",
      }] },
...
test("Q Inventory degradation does not block a proven Trading write grant", () => {
  const input = monitor()
  input.liveCertification.oauth.status = "PARTIAL"
  assert.equal(preflight(input).executionEligible, true)
})

test("R an unproven Trading base grant remains fail closed", () => {
  const input = monitor()
  input.liveCertification.oauth.scopes = []
  assert.ok(preflight(input).blockerCodes.includes(
