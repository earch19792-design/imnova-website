...
test("the reusable visual continuation remains available but is not rerun by the full-Luna residual path", async () => {
  ...
  assert.match(source, /conditionOnlyReconciliation/)
  assert.match(source, /conditionRepairSearchRepeated: false/)
  assert.match(source, /conditionRepairVisionRepeated: false/)
  ...
  assert.doesNotMatch(route, /continueLunaQuickPickVisualTopSellerEnrichmentV1/)
  assert.match(route, /FULL_LUNA_PAGE_PRIMARY_EVIDENCE/)
  assert.match(route, /visualIdentityTopSellerEnrichment[\s\S]+rerunCount: 0/)
