...
function safePromotionEligibility(overrides = {}) {
  return {
    evidenceLevel: "E4",
...
test("clasifica cero visibilidad sólo después de edad y ventana oficial completas", () => {
  const diagnostic = diagnosePostPublicationListing(input({
    confirmedUnitsSold: 2,
    promotionEligibility: safePromotionEligibility(),
  }))
  ...
  assert.equal(diagnostic?.promotionRecommendation.status, "NOT_RECOMMENDED")
  assert.equal(diagnostic?.promotionRecommendation.recommendedRatePercent, 0)
  assert.equal(diagnostic?.promotionRecommendation.applyFromSellerOs, false)
  assert.equal(diagnostic?.evidence.commercialPolicy.capability, "blocked")
...
test("Commercial Monitor crea tarea accionable in-app y la envía por WhatsApp sin aplicarla solo", () => {
  ...
  assert.match(panel,
    /REVISAR Y AUTORIZAR PROMOCIÓN \$\{promotion\.recommendedRatePercent/)
})
...
test("ACTIVE_ONLY y cero ventas mantienen promoción bloqueada aunque exista visibilidad cero", () => {
  const diagnostic = diagnosePostPublicationListing(input({
    promotionEligibility: safePromotionEligibility({
      evidenceLevel: "E1",
  ...
  assert.equal(diagnostic?.promotionRecommendation.recommendedRatePercent, 0)
  assert.equal(diagnostic?.promotionRecommendation.applyFromSellerOs, false)
  assert.match(diagnostic?.promotionRecommendation.reason ?? "",
    /cero ventas confirmadas no autorizan promoción/i)
})

test("actividad estimada y ventas ausentes mantienen promoción en cero", () => {
  for (const promotionEligibility of [
    safePromotionEligibility({
      evidenceLevel: "E2",
      salesClassification: "SOLD_ESTIMATED",
      confirmedSalesSource: null,
      confirmedUnitsSold: 20,
    }),
    safePromotionEligibility({
      evidenceLevel: "E4",
      salesClassification: "INSUFFICIENT_EVIDENCE",
      confirmedSalesSource: null,
      confirmedUnitsSold: Number.NaN,
    }),
  ]) {
    const diagnostic = diagnosePostPublicationListing(input({
