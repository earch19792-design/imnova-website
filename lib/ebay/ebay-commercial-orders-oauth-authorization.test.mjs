...
test("declara el callback dedicado como canónico y mantiene legacy bloqueado", () => {
  assert.equal(
    EBAY_COMMERCIAL_ORDERS_CANONICAL_CALLBACK_PATH,
...
test("callback hace intercambio server-side, GetUser y no llama Orders ni APIs de escritura", () => {
  ...
  assert.match(service, /feature\/centralize-ebay-mobile-center/)
  ...
  assert.match(runbook, /commercial-orders-oauth\/callback/)
