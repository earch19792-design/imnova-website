...
test("commercial orders readonly solicita sólo base y fulfillment readonly", () => {
  const state = createEbayCommercialOAuthState()
  const consentText = buildEbayCommercialOrdersConsentUrl({
    clientId: "client-id-public",
    runame: "registered-runame",
    state,
    scopeProfile: "COMMERCIAL_ORDERS_READONLY",
  })
  const consent = new URL(consentText)
  assert.equal(
    consent.searchParams.get("scope"),
    EBAY_COMMERCIAL_ORDERS_READONLY_SCOPES.join(" "),
  )
  assert.deepEqual(EBAY_COMMERCIAL_ORDERS_READONLY_SCOPES, [
    "https://api.ebay.com/oauth/api_scope",
    "https://api.ebay.com/oauth/api_scope/sell.fulfillment.readonly",
  ])
  assert.doesNotMatch(consent.searchParams.get("scope") ?? "", /commerce\.message/)
  assert.doesNotMatch(consentText, /scope=[^&]+\+https%3A|%252F/)
