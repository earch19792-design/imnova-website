...
test("PASS_OPTIONAL_CORPORATE_NAME_TOKEN_DOES_NOT_CONSTRAIN_MODEL_RETRIEVAL", () => {
  const result = buildProductResearchCommercialQueryStrategyV1({
    productName: "Arrow Storage Products Anchor Kit #AK100",
    sourceField: "identity_result.queryPlan",
    sourceAuthority: "SELLER_OS_STRUCTURED_PRODUCT_IDENTITY_V1_3",
    structuredIdentity: {
      version: "SELLER_OS_STRUCTURED_PRODUCT_IDENTITY_V1_3",
      productFamily: "ARROW_STORAGE_PRODUCTS_ANCHOR_KIT_AK100",
      canonicalFamilyPhrase: "Arrow Storage Products Anchor Kit Ak100",
      identitySufficient: true,
      model: "AK100",
      formFactors: ["arrow storage products anchor kit ak100"],
      queryPlan: {
        exactStrong: "ak100 arrow storage products anchor kit",
        nearExactFamily: "arrow storage products anchor kit ak100",
        functionalFamily: "arrow storage products anchor kit ak100",
        broadFallback: "arrow storage products anchor kit ak100",
        progression: ["ak100 arrow storage products anchor kit",
          "arrow storage products anchor kit ak100"],
      },
    },
  })
  const queries = result.queries.map((entry) => entry.query)
  assert.deepEqual(queries, [
    "ak100 arrow storage anchor kit",
    "arrow ak100 anchor kit",
    "arrow storage ak100",
    "ak100 anchor kit",
  ])
  assert.ok(queries.every((query) => /\bak100\b/.test(query)))
  assert.ok(queries.every((query) =>
    /\banchor kit\b/.test(query) || /\barrow storage\b/.test(query)))
  assert.equal(queries.includes("anchor kit"), false)
  assert.ok(queries.every((query) => !/\bproducts\b/.test(query)))
})
...
test("PASS_PET_TRAINING_DOORBELL_NOUN_EQUIVALENCE_IS_CONTEXT_BOUND", () => {
  const entity = extractProductResearchEntityV1({
    productName: "2-Pack Pet Training Doorbells for Dogs & Cats",
  })
  assert.equal(entity.productNoun, "doorbells")
  assert.equal(entity.productFamilyCandidate, "pet training doorbells")
  assert.deepEqual(entity.definingFamilyQualifiers, ["pet", "training"])
  assert.deepEqual(entity.featureQualifiers, ["dogs", "cats"])
  assert.deepEqual(entity.countOrSetQualifiers, ["2", "pack"])

  for (const title of [
    "2 Pack Pet Training Doorbell for Dogs Cats",
    "2 Pack Pet Training Doorbells for Dogs Cats",
    "2 Pack Pet Training Door Bell for Dogs Cats",
    "2 Pack Pet Training Door Bells for Dogs Cats",
    "2Pack Dog Training Bells for Door Potty Training Dogs Cats Door Bell Tell Bell",
  ]) {
    const result = classifyCommercialComparableV1({
      entity, title, detectedCount: 2,
    })
    assert.notEqual(result.classification, "FALSE_POSITIVE", title)
    assert.notEqual(result.classification, "ADJACENT_BUT_NOT_COMPARABLE", title)
    assert.equal(result.reasons.includes("PRODUCT_NOUN_ABSENT"), false, title)
  }
})

test("PASS_PET_TRAINING_BELL_EQUIVALENCE_REJECTS_OTHER_BELL_FORMS", () => {
  const entity = extractProductResearchEntityV1({
    productName: "2-Pack Pet Training Doorbells for Dogs & Cats",
  })
  for (const [title, detectedCount] of [
    ["2 Pack Hanging Strap Bells for Dog Potty Training Door", 2],
    ["2 Pack Sleigh Bells for Pet Training Door", 2],
    ["2 Pack Bicycle Bells for Pet Training", 2],
    ["2 Pack Wireless Electronic Doorbells for Dogs", 2],
    ["2 Pack Pet Training Wireless Electronic Doorbells for Dogs Cats", 2],
    ["2 Pack Counter Service Bells for Pet Training", 2],
    ["1 Pack Dog Training Bell for Door Potty Training", 1],
    ["1 Pack Pet Training Doorbell for Dogs Cats", 1],
    ["2 Pack Ultrasonic Bark Collars for Dog Training", 2],
    ["2 Pack Decorative Bells for Dog Training Door", 2],
  ]) {
    const result = classifyCommercialComparableV1({
      entity, title, detectedCount,
    })
    assert.ok(["FALSE_POSITIVE", "ADJACENT_BUT_NOT_COMPARABLE"]
      .includes(result.classification), title)
  }
})
