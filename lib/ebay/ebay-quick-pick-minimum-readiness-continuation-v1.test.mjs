...
test("systemic route does not rerun exhausted research and exposes owner UI", async () => {
  const [route, page, capture] = await Promise.all([
  ...
  assert.match(route, /continueLunaQuickPickMinimumReadinessV1/)
  assert.match(route, /automaticResolutionContractCurrent[\s\S]*automaticResolutionExhausted/)
  assert.match(route, /OWNER_FACT_CAPTURE/)
  assert.match(route, /auth\.accessRole !== SELLER_OS_ACCESS_ROLES\.owner/)
  assert.match(route,
    /explicitCandidateScope[\s\S]*requestedKeys[\s\S]*\? keys : receiptKeys/)
  assert.match(route,
    /receiptCards = explicitCandidateScope \? \[\]/)
