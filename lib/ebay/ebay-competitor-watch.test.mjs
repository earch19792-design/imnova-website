...
test("dos vendedores activos permiten competir con margen controlado de 10% y sin promoción", () => {
...
test("la aceptación puede bajar al piso cuando el precio actual sí deja espacio", () => {
  ...
  assert.equal(recommendation?.proposedItemPrice, 18)
  assert.equal(recommendation?.controlledRiskTenPercent, false)
  assert.equal(recommendation?.proposedPassesProfitGate, true)
...
test("recomienda precio sólo con venta exacta reciente, pack comparable y costo Luna fresco", () => {
  ...
  assert.equal(result.priceRecommendation?.promotionRecommendation.recommendedRatePercent, 2)
