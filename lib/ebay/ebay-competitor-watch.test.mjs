...
test("una recomendación activa nueva alerta aunque el patrón de precio ya existía", () => {
  ...
  assert.equal(result.activeMarketPriceRecommendation?.action,
    "HOLD_PENDING_CONFIRMED_SALES")
  assert.equal(result.activeMarketPriceRecommendation?.proposedItemPrice, null)
  assert.equal(result.activeMarketPriceRecommendation?.promotionEligible, false)
  assert.deepEqual(result.activeMarketPriceRecommendation?.blockerCodes, [
...
test("dos vendedores activos sólo permiten observar y mantienen promoción en cero", () => {
  ...
  assert.equal(recommendation?.proposedItemPrice, null)
  assert.equal(recommendation?.priceChangeEligible, false)
  assert.equal(recommendation?.promotionEligible, false)
  assert.equal(recommendation?.observationOnly, true)
  assert.equal(recommendation?.activeMarketNotConfirmedSale, true)
})

test("cien vendedores activos tampoco habilitan una bajada", () => {
  const recommendation = buildActiveMarketPriceRecommendation({
    medianLandedPrice: 18,
  ...
  assert.equal(recommendation?.promotionRecommendation.applyFromSellerOs, false)
...
test("recomienda precio sólo con venta exacta reciente, pack comparable y costo Luna fresco", () => {
  ...
  assert.equal(result.priceRecommendation?.promotionRecommendation.recommendedRatePercent, 0)
  assert.equal(result.priceRecommendation?.promotionRecommendation.automaticPromotionAllowed, false)
  assert.equal(result.priceRecommendation?.capability, "preview_only")
