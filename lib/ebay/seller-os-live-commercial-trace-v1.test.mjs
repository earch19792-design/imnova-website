...
test("default 6.99 estimate cannot replace missing exact qty1 shipping", () => {
  const report = buildEbaySellerKeywordDemandValidation({
    candidate: { productName: "HD 1080P Webcam AWCS06F" },
    comparables: [
      { itemId: "1", title: "HD 1080P Webcam AWCS06F", price: 49.99,
  ...
  const decision = buildCommercialDecisionV1({ supplierCost: 17.17,
    shipping: null, claimConflictCount: 0,
    complianceStatus: "PASS",
    market: buildCommercialMarketProjectionV1(report) })
  assert.equal(decision.finalDecision, "HOLD_SHIPPING_UNPROVEN")
  assert.equal(decision.economics, null)
  assert.equal(decision.minimumMarginSafePrice, null)
  assert.equal(decision.recommendedPrice, null)
})
...
test("trace storage and UI preserve the hard safety boundary", async () => {
  ...
  assert.match(service, /readCommercialTraceShippingReceiptV1/)
  assert.match(service, /allowCrossTraceReuse: true/)
