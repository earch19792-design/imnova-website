...
const mayelCommercialSource = readFileSync(
  "lib/ebay/ebay-mayel-commercial-intelligence-v1.ts", "utf8")
const mayelCommercialCompiled = ts.transpileModule(mayelCommercialSource,
  { compilerOptions: { module: ts.ModuleKind.ES2022,
    target: ts.ScriptTarget.ES2022 } }).outputText
const mayelCommercialModule = await import(
  `data:text/javascript;base64,${Buffer.from(mayelCommercialCompiled).toString("base64")}`)
globalThis.__buildMayelCommercialIntelligenceV1 =
  mayelCommercialModule.buildMayelCommercialIntelligenceV1

const mayelOptimizationSource = readFileSync(
  "lib/ebay/ebay-mayel-full-listing-commercial-optimization-v1.ts", "utf8")
const mayelOptimizationCompiled = ts.transpileModule(mayelOptimizationSource,
  { compilerOptions: { module: ts.ModuleKind.ES2022,
    target: ts.ScriptTarget.ES2022 } }).outputText
const mayelOptimizationModule = await import(
  `data:text/javascript;base64,${Buffer.from(mayelOptimizationCompiled).toString("base64")}`)
globalThis.__buildMayelFullListingOptimizationStateV1 =
  mayelOptimizationModule.buildMayelFullListingOptimizationStateV1
...
implementation = implementation
  ...
  .replace(/import \{[\s\S]*?\} from "\.\/ebay-mayel-commercial-intelligence-v1"\n/,
    "const buildMayelCommercialIntelligenceV1 = globalThis.__buildMayelCommercialIntelligenceV1\n")
  .replace(/import \{\n  buildMayelFullListingOptimizationStateV1,\n\} from "\.\/ebay-mayel-full-listing-commercial-optimization-v1"\n/,
    "const buildMayelFullListingOptimizationStateV1 = globalThis.__buildMayelFullListingOptimizationStateV1\n")
...
test("the API render collection has exact server parity without a client join", () => {
  ...
  const safeMutationCanary = Object.freeze({ ebayItemId: fixture.itemId,
    currentValue: "Window Privacy Film One Way",
    proposedValue: "Window Privacy Film One Way Black",
  ...
  assert.equal(result.taskListings[0].safeMutationCanary,
    safeMutationCanary)
  assert.equal(result.taskListings.filter((listing) =>
    listing.safeMutationCanary).length, 1)
})
...
test("the iPad surface is role-bounded, touch-first, and never renders false zero", () => {
  ...
  assert.match(page, /data-commercial-feed-blocks-visual="false"/)
  assert.match(page, /\["VISUAL", "Visual"\]/)
  assert.match(page, /\["MERCADO", "Mercado"\]/)
  assert.match(page, /\["RENTABILIDAD", "Rentabilidad"\]/)
  assert.match(page, /\["RECOMENDACIONES", "Recomendaciones eBay"\]/)
  assert.match(page, /Revalidar mercado/)
  assert.match(page, /setView\("VISUAL"\)/)
  assert.match(page, /view === "VISUAL" && <div/)
  assert.match(page, /dashboard && view !== "VISUAL"/)
  assert.doesNotMatch(page,
    /dashboard &&[^\n]*view === "VISUAL"[^\n]*MayelVisualWorkstation/)
  assert.match(page,
    /La aplicación y el readback en eBay se validan por separado/)
  assert.match(page, /value === null \|\| value === undefined \|\| value === ""/)
  assert.deepEqual([...page.matchAll(/label: "([^"]+)"/g)]
    .slice(0, 8).map((match) => match[1]), ["Inicio", "Mis tareas",
      "Listings LIVE", "Mejoras sugeridas", "Estación visual", "Resultados",
      "Historial", "Ayuda"])
  assert.doesNotMatch(page,
  ...
  assert.match(route, /READ_EBAY_PROMOTION_RECOMMENDATION/)
  assert.match(route, /X-Seller-OS-Mayel-Recommendation": "READ_ONLY"/)
  assert.match(route, /promotionWrites: 0, sendOffers: 0, buyerMessages: 0/)
