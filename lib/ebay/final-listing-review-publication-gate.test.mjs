...
test("package saves preserve only the server-owned Luna final-review binding", () => {
  const current = lunaSupplierEvidence().listingPackage.package_data
  const foreignReadiness = {
    version: "LUNA_SUPPLIER_IMAGE_AUTO_READY_V1",
    imageReady: true,
    validCompliantImageCount: 99,
  }
  const next = reconcileFinalListingReviewPackageEvidence({
    currentPackageData: current,
    nextPackageData: {
      title: "Window Privacy Film",
      supplierImageReadiness: foreignReadiness,
    },
  })
  assert.equal(next.title, "Window Privacy Film")
  assert.equal(
    next.supplierImageReadiness,
    current.supplierImageReadiness,
  )
  assert.notEqual(next.supplierImageReadiness, foreignReadiness)

  const withoutDurableAuthority = reconcileFinalListingReviewPackageEvidence({
    currentPackageData: {},
    nextPackageData: { supplierImageReadiness: foreignReadiness },
  })
  assert.equal("supplierImageReadiness" in withoutDurableAuthority, false)
})

test("legacy review absence reconciles to the exact four durable Luna assets", async () => {
  const durable = lunaSupplierEvidence()
  const tables = []
  class Query {
  ...
  const supabase = {
    from(table) {
      tables.push(table)
      if (table === "ebay_reference_guided_final_listing_review_previews") {
        return new Query(table, { data: null, error: null })
      }
      if (table === "ebay_listing_packages") {
        return new Query(table, {
          data: durable.listingPackage,
          error: null,
        })
      }
      if (table === "ebay_listing_image_assets") {
        return new Query(table, { data: durable.assets, error: null })
      }
      throw new Error(`UNEXPECTED_TABLE_${table}`)
    },
  }
  const gate = await loadFinalListingReviewPublicationGate({
    supabase,
    listingPackageId: "3a394c94-108b-4ca0-b373-5e589dc4a652",
    actorId: actor,
  })
  assert.equal(gate.allowed, true)
  assert.equal(gate.reason, null)
