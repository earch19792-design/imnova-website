...
test("bounded history is not unavailable when the exact complete current job is in the prefix", () => {
  const prefix = (name, rows) => ({ status: "PARTIAL", rows, truncated: true,
    limitationCode: `${name}_RESULT_LIMIT_REACHED` })
  const input = { decisions: { status: "AVAILABLE", rows: [decision([backpack])] },
    jobs: prefix("SELLER_OS_LUNA_STOCK_CHECK_JOBS_V1", [job()]),
    observations: prefix("SELLER_OS_LUNA_STOCK_OBSERVATIONS_V1", [observation(backpack, "OBSERVED_IN_STOCK", true, null)]) }
  const r = project(input)
  assert.equal(r.stock.state, "IN_STOCK_SIGNAL")
  assert.equal(r.stock.freshness.status, "FRESH")
  assert.equal(r.stock.quantity.value, null)
  for (const mutate of [i => i.observations.rows = [], i => i.jobs.status = "ERROR",
    i => i.observations.limitationCode = "SOURCE_TIMEOUT",
    i => i.observations.rows[0].luna_variant_id = "other",
    i => i.observations.rows[0].component_identity_id = "legacy-component",
    i => i.observations.rows[0].observed_at = "2026-08-20T00:00:00Z"]) {
    const changed = structuredClone(input); mutate(changed)
    assert.notEqual(project(changed).stock?.state, "IN_STOCK_SIGNAL")
  }
})
