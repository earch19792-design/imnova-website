...
} = await import("./ebay-publication-oauth-start-diagnostic.ts")
...
test("physical start diagnostic is bounded, metadata-only and classifies guards", () => {
  assert.equal(ebayPublicationOAuthStartFailedGuard(
    "EBAY_SELLER_OAUTH_REAUTH_ADMIN_USER_REQUIRED",
  ), "ADMIN_AUTH")
  assert.equal(ebayPublicationOAuthStartFailedGuard(
    "EBAY_PUBLICATION_OAUTH_WRITE_GATES_MUST_REMAIN_OFF",
  ), "FEATURE_BOUNDARY")
  assert.equal(ebayPublicationOAuthStartFailedGuard(
    "EBAY_SELLER_OAUTH_REAUTH_RUNTIME_CREDENTIAL_MISMATCH",
  ), "CLIENT_CONFIG")
  assert.equal(safeEbayPublicationOAuthHandoffPersistenceCode({
    code: "23505",
    message: "duplicate key violates unique constraint " +
      "ebay_publication_oauth_one_active_uidx",
  }), "EBAY_PUBLICATION_OAUTH_ACTIVE_HANDOFF_EXISTS")
  assert.equal(safeEbayPublicationOAuthHandoffPersistenceCode({
    code: "42501",
  }), "EBAY_PUBLICATION_OAUTH_HANDOFF_PERMISSION_DENIED")
  const diagnostic = createEbayPublicationOAuthStartSafeDiagnostic({
    START_HTTP_STATUS: 403,
    EXACT_FAILURE_CODE:
      "EBAY_SELLER_OAUTH_REAUTH_RUNTIME_CREDENTIAL_MISMATCH",
    FAILED_GUARD_NAME: "CLIENT_CONFIG",
    ADMIN_SESSION_PRESENT: true,
    ADMIN_SESSION_VALID: true,
    OWNER_AUTHORITY_MATCH: true,
    REQUEST_HOST_CLASS: "SELLER_OS_DEDICATED_PREPROD",
    HOST_MATCH: true,
    DEPLOYMENT_ENVIRONMENT: "SELLER_OS_DEDICATED_PREPROD",
    ENVIRONMENT_MATCH: true,
    PRODUCTION_OAUTH_ENABLED: false,
    EBAY_PRODUCTION_CLIENT_CONFIG_PRESENT: true,
    RUNAME_CONFIG_PRESENT: true,
    CALLBACK_CONFIG_PRESENT: true,
    STATE_SECRET_PRESENT: true,
    COOKIE_CAN_BE_ISSUED: false,
    STATE_CREATED: false,
    STATE_COOKIE_SET: false,
    SET_COOKIE_HEADER_PRESENT: false,
    REDIRECT_HOST: null,
    ADMIN_AUTH_PASS: true,
    HOST_PASS: true,
    ENVIRONMENT_PASS: true,
    CONFIG_PASS: true,
    STATE_SECRET_PASS: true,
    COOKIE_SERIALIZED: false,
    REDIRECT_BUILT: false,
  })
  assert.deepEqual(Object.keys(diagnostic).sort(), [
    "ADMIN_SESSION_PRESENT", "ADMIN_SESSION_VALID",
    "CALLBACK_CONFIG_PRESENT", "COOKIE_CAN_BE_ISSUED",
    "DEPLOYMENT_ENVIRONMENT", "EBAY_PRODUCTION_CLIENT_CONFIG_PRESENT",
    "ENVIRONMENT_MATCH", "EXACT_FAILURE_CODE",
    "EXPECTED_DEPLOYMENT_ENVIRONMENT", "EXPECTED_HOST_CLASS",
    "FAILED_GUARD_NAME", "FAILURE_STAGE", "HOST_MATCH",
    "INTERNAL_SAFE_FAILURE_CODE", "OWNER_AUTHORITY_MATCH",
    "PRODUCTION_OAUTH_ENABLED", "PUBLISH_TARGET", "REDIRECT_HOST",
    "REQUEST_HOST_CLASS", "REQUEST_REACHED_NODE_HANDLER",
    "RUNAME_CONFIG_PRESENT", "SET_COOKIE_HEADER_PRESENT",
    "START_HTTP_STATUS", "STATE_COOKIE_SET", "STATE_CREATED",
    "STATE_SECRET_PRESENT", "ADMIN_AUTH_PASS", "HOST_PASS",
    "ENVIRONMENT_PASS", "CONFIG_PASS", "STATE_SECRET_PASS",
    "COOKIE_SERIALIZED", "REDIRECT_BUILT",
  ].sort())
  assert.equal(diagnostic.FAILURE_STAGE, "EBAY_CLIENT_CONFIG")
  assert.doesNotMatch(JSON.stringify(diagnostic),
    /token|cookieValue|stateValue|authCode|clientSecret|refreshToken/i)
})

test("publication OAuth remains environment-bound, browser-state protected, ciphertext-only and zero-write", async () => {
  const [service, startRoute, startDiagnostic, statusRoute, callback,
    staleCallback, dashboard, adminSessionRoute, adminSessionCookieContract,
    migration, boundaries, middleware] =
    await Promise.all([
      ...
      readFile(
        "lib/ebay/ebay-publication-oauth-start-diagnostic.ts",
        "utf8",
      ),
      ...
      readFile("lib/admin-session-cookie-contract.ts", "utf8"),
      readFile(
  ...
  assert.match(service,
    /\.in\("status", \["pending", "claimed"\]\)[\s\S]*\.lte\("expires_at", now\)/)
  assert.match(service,
    /EBAY_PUBLICATION_OAUTH_STALE_HANDOFF_EXPIRY_FAILED/)
  assert.match(service,
    /safeEbayPublicationOAuthHandoffPersistenceCode\(error\)/)
  assert.match(service, /onBoundaryMarker\?\.\("STATE_CREATED"\)/)
  assert.match(service, /onBoundaryMarker\?\.\("COOKIE_SERIALIZED"\)/)
  assert.match(service, /onBoundaryMarker\?\.\("REDIRECT_BUILT"\)/)
  ...
  assert.match(dashboard,
    /<form method="post"[\s\S]*action="\/api\/admin\/ebay\/publication-oauth\/start"/)
  assert.doesNotMatch(dashboard, /window\.location\.assign\(authorizationUrl\)/)
  assert.match(dashboard, /OAuth no iniciado · \$\{safeFailureCode\}/)
  assert.match(startRoute,
    /NextResponse\.redirect\(prepared\.authorizationUrl,[\s\S]*status: 303/)
  assert.match(startRoute,
    /SELLER_OS_PUBLICATION_OAUTH_START_SESSION_COOKIE/)
  assert.match(adminSessionRoute,
    /SELLER_OS_PUBLICATION_OAUTH_START_SESSION_COOKIE/)
  assert.match(adminSessionCookieContract,
    /"\/api\/admin\/ebay\/publication-oauth\/start"/)
  assert.match(adminSessionCookieContract, /httpOnly: true/)
  assert.match(adminSessionCookieContract, /sameSite: "strict"/)
  assert.match(startRoute,
    /EBAY_PUBLICATION_OAUTH_START_DIAGNOSTIC_EVENT/)
  assert.match(startDiagnostic, /REQUEST_REACHED_NODE_HANDLER/)
  assert.match(startDiagnostic, /EXACT_FAILURE_CODE/)
  assert.match(startDiagnostic, /FAILED_GUARD_NAME/)
  assert.match(startDiagnostic, /ADMIN_SESSION_PRESENT/)
  assert.match(startDiagnostic, /COOKIE_CAN_BE_ISSUED/)
  assert.doesNotMatch(startRoute,
    /console\.(?:info|log|warn|error)\([^\n]*(?:clientSecret|refreshToken|accessToken|authorizationUrl)/)
