...
test("owner-only extension is separate, optional, exact-host and contains no browser automation", async () => {
  ...
  assert.match(contract, /BUILD_VERSION = "1\.0\.3"/)
  assert.match(content, /VERSION = "1\.0\.3"/)
  ...
  assert.match(background, /diagnoseSessionCookieApplicability/)
  ...
  assert.match(popup, /MISSING_COOKIE_IDENTITY_COUNT/)
  assert.match(popup, /result\.cookieSetCandidateCount !== 1/)
  assert.match(popup,
    /LUNA_OWNER_EXTENSION_ENCRYPTED_HANDOFF_DELIVERED_SET_1/)
...
test("extension WebCrypto envelope is accepted by the existing one-time server contract", async () => {
  ...
  const envelope = await extension.encryptSessionPayload(challenge, {
    cookieHeader: "customer_account_session=owner-extension-fixture",
    capturedAt: new Date(capturedAt).toISOString(),
    validatedAt: new Date(capturedAt).toISOString(),
    expiresAt: new Date(capturedAt + 3_600_000).toISOString(),
  })
  const result = await withEnvironment(dedicatedPreprod(), () =>
    domain.consumeSellerOsLunaOwnerHandoffV1({
...
test("owner extension diagnoses exact-url parent-domain and exact-path cookies without exposing identities", async () => {
  const extension = await import(new URL(
    "../../tools/browser-extensions/luna-owner-session-handoff/contract.mjs",
    import.meta.url,
  ))
  ...
  const cookies = [{
    ...
    name: "non_contract_exact_path", value: "safe-test-fixture-b",
    domain: "www.lunaportex.com", path: "/account", secure: true,
    expirationDate: expiry,
  }]
  const result = extension.diagnoseSessionCookieApplicability(cookies)
  assert.deepEqual(result, {
    browserApplicableCookieCount: 2,
    capturedCookieCount: 1,
    missingCookieIdentityCount: 1,
    extraCookieIdentityCount: 0,
    domainClass: "PARENT_AND_EXACT_HOST",
    pathClass: "ROOT_AND_EXACT_PATH",
  })
  const serialized = JSON.stringify(result)
  assert.doesNotMatch(serialized,
    /customer_account_session|non_contract_exact_path|safe-test-fixture/)
})
