...
test("existing protected reads retain the browser worker while the automatic cron uses public exact reads", async () => {
  const [serverRead, cron] = await Promise.all([
  ...
  assert.match(serverRead, /fetchLunaAuthenticatedBrowserProductV1/)
  assert.doesNotMatch(cron, /fetchLunaAuthenticatedBrowserProductV1/)
  assert.match(cron, /fetchPublicLunaProductForActiveListingMonitor/)
  assert.doesNotMatch(cron, /fetchLunaAuthenticatedDirectedProductV1/)
})
