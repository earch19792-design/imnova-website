...
test("Quick Pick remains a shared-factory feeder with bounded AI and no publish path", async () => {
  ...
  assert.match(page, /continueAutomaticResolution/)
  assert.match(page, /action: "CONTINUE_FULL_LUNA_EVIDENCE"/)
  assert.match(page, /fullLunaBrandEvidenceReviewPending/)
  assert.match(page, /Continuar resolución automática/)
  assert.match(page, /Resolución automática completada/)
  ...
  assert.ok(route.indexOf('body.action === "CONTINUE_FULL_LUNA_EVIDENCE"')
    < route.indexOf("if (!auth.userId)"))
