...
test('injected native fetch is called without the dependency object as receiver',async()=>{
 const result=await transport.submitQualityUploadV1({...base,request:async function(){
  assert.equal(this,undefined)
  return Response.json({success:true,status:{reportFreshness:'STALE'}})
 }})
...
test('successful import reports exact attempt ID and truthful stages; ledger failure is not generic 500',async()=>{
 ...
 assert.equal(p.uploadTrace.UPLOAD_ATTEMPT_ROW_CREATED,true)
 ...
 assert.equal(bad.uploadTrace.stages.UPLOAD_ATTEMPT_LEDGER.REACHED,true)
 assert.equal(bad.uploadTrace.UPLOAD_ATTEMPT_ROW_CREATED,false)
})
