...
test("commercial readiness supersedes stale resolving lifecycle presentation", () => {
  const projected = projectQuickPickOwnerCardV1(card({
    ...
    runtimeMaterialization: { materialPackageCurrent: true,
      persistedCommercialEconomicsComplete: true,
      ownerActionPathAvailable: true },
    authorizationBinding: { imageCount: 1 } },
  }))
...
test("market-test ready preserves exact demand as unproven and presents continuation", () => {
  const projected = projectQuickPickOwnerCardV1(card({ state: "READY",
    ...
    listingReview: { finalListingPackageReady: true,
...
test("technical ready without a current commercial package is not visible ready", () => {
  const projected = projectQuickPickOwnerCardV1(card({
    state: "READY", marketTestReady: true,
    listingReview: { finalListingPackageReady: true,
      publishAuthorizationHandoff: { ownerPublicationDecisionReady: true },
  ...
  assert.equal(projected.state, "WAITING")
  assert.equal(projected.disposition, "PREPARING_OWNER_REVIEW_PACKAGE")
  assert.equal(projected.publisherActionability.authoritativeReady, false)
  assert.equal(projected.publisherActionability.failureClass,
    "READY_WITH_STALE_OR_INCOMPLETE_PACKAGE")
})
