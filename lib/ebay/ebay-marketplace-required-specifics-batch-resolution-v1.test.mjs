...
test("conflicting visual Brand evidence is persisted fail-closed without a synthetic brand", async () => {
  const product = candidate(44, {
    exactImageUrls: ["https://images.example/front.png",
  ...
  const resolver = createOpenAiRequiredSpecificsBatchResolverV1({
    ...
    output_text: JSON.stringify({ resolutions: [{
      ...
      evidenceReferences: [],
      evidenceEntailsValue: false,
      materialConflict: true,
      ownerInputRequired: true,
      brandEvidenceStatus: "CONFLICT",
      allExactProductImagesReviewed: true,
      explicitBrand: "Brand A; Brand B",
    }] }),
  }), { status: 200, headers: { "Content-Type": "application/json" } }))
  const output = await resolver({ stage: "VISION", marketplaceId: "EBAY_US",
    categoryId: product.categoryId, products: [product] })
  const resolution = output.candidates[0].resolutions[0]
  assert.equal(resolution.brandEvidenceStatus, "CONFLICT")
  assert.equal(resolution.allExactProductImagesReviewed, true)
  assert.equal(resolution.explicitBrand, null)
