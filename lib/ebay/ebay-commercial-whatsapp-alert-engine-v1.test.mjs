...
test("eight Meta template contracts exist and approval is never assumed", () => {
  ...
  assert.ok(WHATSAPP_TEMPLATE_DEFINITIONS_V1.every((row) =>
    ...
    row.examplePayload.classification === "NON_OPERATIONAL_TEMPLATE_EXAMPLE" &&
...
test("low-stock and stale evidence produce distinct truthful human previews", () => {
  const stale = renderCommercialWhatsAppAlertDryRunV1(base({
    family: "LOW_STOCK_OR_STALE_EVIDENCE",
    stock: { riskClass: "STALE_EVIDENCE", exactIdentity: true },
  }))
  const low = renderCommercialWhatsAppAlertDryRunV1(base({
    family: "LOW_STOCK_OR_STALE_EVIDENCE",
    stock: { riskClass: "LOW_STOCK_CONFIRMED", exactIdentity: true },
  }))
  assert.equal(stale.qualifies, true)
  assert.equal(low.qualifies, true)
  assert.match(stale.humanPreview.title, /vencida/i)
  assert.match(stale.humanPreview.problem, /no está confirmada/i)
  assert.match(low.humanPreview.title, /bajo confirmado/i)
  assert.doesNotMatch(low.humanPreview.problem, /vencida/i)
  assert.equal(stale.dispatchAllowed, false)
  assert.equal(low.dispatchAllowed, false)
})
