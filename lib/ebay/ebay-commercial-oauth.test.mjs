...
test("Message refresh reutiliza el scope-set exacto del grant canónico", () => {
  const source = readFileSync("lib/ebay/ebay-commercial-oauth.ts", "utf8")
  assert.match(source,
    /EBAY_COMMERCIAL_ORDERS_OAUTH_SCOPES\.join\(["'] ["']\)/)
  assert.doesNotMatch(source,
    /const MESSAGE_SCOPES = `\$\{BASE_SCOPE\} \$\{EBAY_COMMERCE_MESSAGE_SCOPE\}`/)
  assert.match(source, /scope: MESSAGE_SCOPES/)
})
