...
test("un WAITING_RETRY futuro se presenta programado sin convertirlo en pausa", () => {
  const monitor = deriveSameDayLiveMonitor({
    now: NOW,
    run: { status: "ACTIVE", stage: "RECONCILING_IDENTITY" },
    candidates: [{ ordinal: 1, machine_state: "RECONCILING_IDENTITY" }],
    jobs: [{
      ...
      last_error_code: "TRANSIENT_DEPENDENCY_FAILURE",
      available_at: "2026-07-18T18:05:00.000Z",
    }],
  })
  assert.equal(monitor.status, "QUEUED")
  assert.equal(monitor.businessLabel, "REINTENTO PROGRAMADO")
  assert.equal(monitor.nextAutomaticAttemptAt, "2026-07-18T18:05:00.000Z")
  assert.equal(monitor.workerHeartbeatAt, null)
  assert.equal(monitor.shouldAnimate, false)
})

test("un WAITING_RETRY vencido queda listo y uno sin fecha no inventa un instante", () => {
  const due = deriveSameDayLiveMonitor({
    now: NOW,
    run: { status: "ACTIVE", stage: "RECONCILING_IDENTITY" },
    candidates: [{ ordinal: 1, machine_state: "RECONCILING_IDENTITY" }],
    jobs: [{
      status: "WAITING_RETRY",
      last_error_code: "TRANSIENT_DEPENDENCY_FAILURE",
      available_at: "2026-07-18T17:59:00.000Z",
    }],
  })
  assert.equal(due.status, "QUEUED")
  assert.equal(due.businessLabel, "LISTO PARA REINTENTO")
  assert.equal(due.nextAutomaticAttemptAt, "2026-07-18T17:59:00.000Z")
  assert.equal(due.shouldAnimate, false)

  const invalid = deriveSameDayLiveMonitor({
    now: NOW,
    run: { status: "ACTIVE", stage: "RECONCILING_IDENTITY" },
    candidates: [{ ordinal: 1, machine_state: "RECONCILING_IDENTITY" }],
    jobs: [{ status: "WAITING_RETRY", available_at: "not-a-date" }],
  })
  assert.equal(invalid.status, "QUEUED")
  assert.equal(invalid.businessLabel, "EN COLA")
  assert.equal(invalid.nextAutomaticAttemptAt, null)
  assert.match(invalid.activityEvidence, /no se inventa una fecha/)
})
...
test("la animación sólo se habilita con lease y latido durable recientes", () => {
  ...
  assert.equal(monitor.workerHeartbeatAt, "2026-07-18T17:59:40.000Z")
  assert.match(monitor.activityEvidence, /Latido confirmado/)
})

test("un heartbeat fresco con un job LEASED antiguo no simula ejecución", () => {
  const monitor = deriveSameDayLiveMonitor({
    now: NOW,
    run: {
      status: "ACTIVE",
...
test("una fase automática sin job se presenta como recuperación requerida", () => {
  const monitor = deriveSameDayLiveMonitor({
    now: NOW,
    run: {
      status: "ACTIVE",
      stage: "CALCULATING_ECONOMICS",
      last_worker_heartbeat_at: "2026-07-18T17:59:40.000Z",
    },
    candidates: [{
      id: "candidate-orphan",
    ...
    jobs: [{ candidate_id: "candidate-orphan", status: "COMPLETED" }],
  })
  assert.equal(monitor.status, "RECOVERY_REQUIRED")
  assert.equal(monitor.recovery.required, true)
  assert.equal(monitor.shouldAnimate, false)
  assert.match(monitor.activityEvidence, /Sin job durable/)
  assert.match(monitor.nextHumanAction, /Reanudar ahora/)
})
...
test("WAITING_RETRY no fuerza PAUSED_EBAY sin la proyección quotaPaused", () => {
  const monitor = deriveSameDayLiveMonitor({
    now: NOW,
    run: { status: "ACTIVE", stage: "RECONCILING_IDENTITY" },
    candidates: [{ ordinal: 1, machine_state: "RECONCILING_IDENTITY" }],
    jobs: [{
      status: "WAITING_RETRY",
      last_error_code: "EBAY_READONLY_GET_429",
      rate_limit_resume_at: "2026-07-18T18:10:00.000Z",
      available_at: "2026-07-18T18:10:00.000Z",
    }],
  })
  assert.equal(monitor.status, "QUEUED")
  assert.equal(monitor.businessLabel, "REINTENTO PROGRAMADO")
...
test("timeline, lote y descarte traducido son información de negocio", () => {
  ...
  assert.equal(monitor.timeline.find((step) => step.id === "facts")?.visual, "facts")
  assert.equal(monitor.timeline.at(-1)?.id, "monitor")
...
test("un descarte económico explica mercado, piso, utilidad y aspectos obligatorios", () => {
  const explanation = explainSameDayRejectedCandidate({
    ...
    commercial_decision_summary: {
      ...
      evidence: { activeExactCount: 3, confirmedSoldExact: 0 },
    },
    economics_summary: {
...
test("cero comparables exactos no inventa una mediana eBay de cero dólares", () => {
  const explanation = explainSameDayRejectedCandidate({
    id: "candidate-no-market",
    ordinal: 3,
    product_title: "Food Scale with Nutritional Calculator",
    blockers: ["MARKET_PRICE_BELOW_MINIMUM_SAFE_PRICE"],
    commercial_decision_summary: {
      verdict: "NO_GO",
      economics: {
        activeMarketMedian: null,
        minimumSafePrice: 50.70,
        marketSupportsMinimumSafePrice: false,
      },
      evidence: { activeExactCount: 0, confirmedSoldExact: 0 },
    },
    economics_summary: { minimumOperatorPrice: 31.41 },
  })
  assert.match(explanation.headline, /no existe una mediana eBay exacta/i)
  assert.doesNotMatch(explanation.headline, /\$0\.00/)
  assert.doesNotMatch(explanation.details.join(" "), /por encima del mercado mediano/)
})

test("un bloqueo de aspectos prevalece sobre una decisión económica histórica", () => {
  const explanation = explainSameDayRejectedCandidate({
    id: "candidate-missing-brand",
    blockers: ["MISSING_BLOCKING"],
    commercial_decision_summary: {
      verdict: "NO_GO",
      economics: {
        activeMarketMedian: 0,
        minimumSafePrice: 37.58,
        marketSupportsMinimumSafePrice: false,
      },
      evidence: { activeExactCount: 0, confirmedSoldExact: 0 },
    },
    product_facts_summary: {
      resolvedRequirements: [{ status: "MISSING_BLOCKING", aspectName: "Brand" }],
    },
  })
  assert.match(explanation.headline, /Ficha pendiente/i)
  assert.equal(explanation.disposition, "CORRECTION_PENDING")
  assert.doesNotMatch(explanation.headline, /mediana eBay|\$0\.00/)
  assert.match(explanation.details.join(" "), /Brand/)
})
...
test("la UI futurista es veraz, responsive y respeta reduced motion", () => {
  ...
  assert.match(panel, /monitor\.nextAutomaticAttemptAt/)
  assert.match(panel, /monitor\.workerHeartbeatAt/)
  assert.match(panel, /1 · Estado del sistema/)
  assert.match(panel, /Checkpoint actual/)
  assert.match(panel, /motion-safe:animate-ping/)
  assert.match(panel, /motion-reduce:transition-none/)
  assert.match(panel, /Las etapas futuras permanecen bloqueadas/)
  assert.match(panel,
    /grid-cols-1[\s\S]{0,100}sm:grid-cols-2[\s\S]{0,100}lg:grid-cols-4/)
  assert.match(panel, /data-journey-phase-visual/)
  for (const visual of [
    "radar", "research", "identity", "economics", "facts", "creative",
    "publish", "monitor",
  ]) assert.match(panel, new RegExp(`visual === "${visual}"`))
  assert.match(panel, /aria-current=\{step\.status === "CURRENT" \? "step"/)
  assert.match(panel, /setSelectedPhaseId\(step\.id\)/)
  assert.match(panel, /Qué está haciendo ahora/)
  assert.match(panel, /Evaluación actual/)
  ...
  assert.match(panel, /monitor\.recovery\.required/)
  assert.match(panel, /Descarte anterior del lote:/)
  assert.match(panel, /Por qué se bloqueó este lote:/)
  assert.match(panel, /Productos pendientes de corregir antes de publicar/)
  assert.match(panel, /CORREGIR FICHA/)
  assert.match(panel, /AUTORIZAR EXCEPCIÓN Y PREPARAR LISTING EN SELLER OS/)
  assert.match(panel, /No activar Promoted Listings/)
  assert.match(panel, /Garantía al cliente de eBay sigue aplicando/)
  assert.match(panel, /Todavía no publica en eBay/)
})
