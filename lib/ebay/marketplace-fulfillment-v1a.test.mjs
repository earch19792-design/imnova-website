...
test("cancelled, refund y already fulfilled bloquean antes del adapter", () => {
  const service = readFileSync(new URL("../marketplace/fulfillment-v1a-service.ts", import.meta.url), "utf8")
  const guardDomain = readFileSync(new URL("../marketplace/fulfillment-v1a-domain.ts", import.meta.url), "utf8")
  assert.match(guardDomain, /ORDER_CANCELLED/)
  assert.match(guardDomain, /ORDER_REFUNDED/)
  assert.match(guardDomain, /ORDER_ALREADY_FULFILLED/)
  assert.match(service, /officialOrderGuard[\s\S]*if \(guard\.blocked\)/)
  assert.match(service, /const guard = await officialOrderGuard[\s\S]*adapterCalls \+= 1/)
})

test("guard oficial read-only bloquea cancelación, refund, fulfilled e identidad incorrecta sin PII", () => {
  const official = {
    orderId: identity.orderId,
    orderPaymentStatus: "PAID",
    orderFulfillmentStatus: "NOT_STARTED",
    cancelStatus: { cancelState: "NONE_REQUESTED" },
    buyer: { username: "private-buyer", email: "private@example.com" },
    lineItems: [{
      lineItemId: identity.lineItemId,
      legacyItemId: identity.listingId,
      sku: identity.marketplaceListingSku,
  ...
  const expected = [{
    ...
    marketplaceListingSku: identity.marketplaceListingSku,
    quantity: identity.quantity,
  }]
  const ready = normalizeEbayFulfillmentOrderGuard(official, identity.orderId, expected)
  assert.equal(ready.blocked, false)
  assert.equal(ready.buyerPiiReturned, false)
  assert.doesNotMatch(JSON.stringify(ready), /private-buyer|private@example/)
  assert.equal(normalizeEbayFulfillmentOrderGuard({ ...official, cancelStatus: { cancelState: "CANCEL_REQUESTED" } }, identity.orderId, expected).blockCode, "ORDER_CANCELLED")
  assert.equal(normalizeEbayFulfillmentOrderGuard({ ...official, orderPaymentStatus: "FULLY_REFUNDED" }, identity.orderId, expected).blockCode, "ORDER_REFUNDED")
  assert.equal(normalizeEbayFulfillmentOrderGuard({ ...official, orderFulfillmentStatus: "FULFILLED" }, identity.orderId, expected).blockCode, "ORDER_ALREADY_FULFILLED")
  assert.equal(normalizeEbayFulfillmentOrderGuard(official, identity.orderId, [{ ...expected[0], marketplaceListingSku: "WRONG" }]).blockCode, "ORDER_IDENTITY_MISMATCH")
})
...
test("guard de orden usa exclusivamente GET oficial antes de aprobar o simular", () => {
  const readers = readFileSync(new URL("./ebay-commercial-readers.ts", import.meta.url), "utf8")
  const section = readers.slice(readers.indexOf("export async function getEbayFulfillmentOrderGuard"), readers.indexOf("async function watcherRead"))
  assert.match(section, /method: "GET"/)
  assert.doesNotMatch(section, /method: "POST"|shipping_fulfillment/)
  assert.match(section, /verifyEbayCommercialOfficialAccount/)
  assert.match(section, /ebayWrites: 0/)
})
