...
test("a corrected source allocator gets one distinct recovery job", () => {
  ...
  const recovery = buildSameDayImageGenerationJobSpec({
    ...ids,
    accountKey: "imnova-ebay-us-primary:test",
    actorId: "66666666-6666-4666-8666-666666666666",
    listingPackageId: "77777777-7777-4777-8777-777777777777",
  ...
  assert.match(recovery.checkpoint.preGenerationHash, /^[0-9a-f]{64}$/)
  assert.match(
    recovery.idempotencyKey,
    new RegExp(
      `${recovery.checkpoint.preGenerationHash}:` +
        `${SAME_DAY_IMAGE_SOURCE_REUSE_RECOVERY_VERSION}$`,
    ),
  )
