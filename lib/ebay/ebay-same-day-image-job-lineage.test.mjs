...
test("a post-AI visual failure gets a distinct zero-spend deterministic fallback", () => {
  const recovery = buildSameDayImageGenerationJobSpec({
    ...ids,
    visualStrategyRecovery: true,
  ...
  assert.match(
    ...
    new RegExp(`${SAME_DAY_IMAGE_DETERMINISTIC_FALLBACK_RECOVERY_VERSION}$`),
  )
  assert.equal(
    recovery.checkpoint.deterministicFallbackRecoveryVersion,
    SAME_DAY_IMAGE_DETERMINISTIC_FALLBACK_RECOVERY_VERSION,
  )
  assert.equal(recovery.checkpoint.forceDeterministicImageFallback, true)
  assert.equal(recovery.checkpoint.ebayWrites, 0)
})
