...
test("Quick Pick route confines service-role execution to exact UPC resolution", async () => {
  const { readFileSync } = await import("node:fs")
  const route = readFileSync(
    "app/api/admin/ebay/luna-quick-pick/route.ts", "utf8")
  const resolverIndex = route.indexOf(
    'if (body.action === "RESOLVE_REQUIRED_UPC")')
  const humanGuardIndex = route.indexOf(
    'if (!auth.userId) return response', resolverIndex)
  const ownerReviewIndex = route.indexOf(
    'if (body.action === "OWNER_REVIEW")', resolverIndex)
  assert.ok(resolverIndex > 0)
  assert.ok(humanGuardIndex > resolverIndex)
  assert.ok(ownerReviewIndex > humanGuardIndex)
})
