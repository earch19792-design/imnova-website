...
test("same certified lane delegates only to active or exactly recovered batches", () => {
  ...
  assert.match(runtime,
    /eq\("status", "COMPLETED"\)[\s\S]*?gt\("expired_recovery_count", 0\)[\s\S]*?gte\("last_recovered_at", blockedAt\)/)
  assert.match(runtime,
    /get_seller_os_luna_shipping_recovery_readback_v1[\s\S]*?exact\.exactDurableReadbackMatch !== true[\s\S]*?exact\.exactResultCount !== 1/)
