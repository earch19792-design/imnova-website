...
test("candidate identities can only be claimed by ordered CURRENT runtime output", () => {
  ...
  assert.match(runtime, /autonomousGreenfieldCurrentPreparationReadyV1/)
...
test("CURRENT publication certification is automatic, exact and durable before commit", () => {
  assert.match(runtime, /continueLunaQuickPickRequiredSpecificsV1/)
  assert.match(runtime, /trigger: "DEPENDENCY_RECOVERY"/)
  assert.match(runtime,
    /exactShippingContinuation\?\.firstBlocker[\s\S]*exactFactoryOutcome\?\.reasonCode/)
  assert.match(runtime,
    /exactShippingContinuation\?\.candidateKey[\s\S]*exactFactoryOutcome\?\.candidateKey/)
  assert.match(runtime,
    /exactShippingContinuation = refreshedExactShippingContinuation/)
  assert.match(runtime,
    /AUTONOMOUS_STOCKING_EXACT_PREREQUISITE_REFRESH_INVALID/)
  assert.match(runtime,
    /durableCertification\.pass !== true[\s\S]*durableCertification\.durableReadbackPass !== true/)
  assert.match(runtime,
    /durableCertification\.currentPrepublicationEvidenceBound !== true/)
  assert.match(runtime, /artifacts\.packageId !== packageId/)
  assert.match(runtime, /CURRENT_PUBLICATION_CERTIFICATION_AUTOMATIC: true/)
  assert.match(runtime, /CERTIFICATION_PACKAGE_ID_MATCH:/)
  assert.match(runtime, /MANUAL_CERTIFICATION: false/)
  assert.match(runtime, /LEGACY_CERTIFICATION_REUSED_AS_AUTHORITY: false/)
  assert.match(runtime,
    /currentPublicationCertification:[\s\S]*durableReadbackPass: true/)
})

test("real CURRENT authority blockers are not masked by the continuation boundary", () => {
  const factory = readFileSync(
    "lib/ebay/ebay-smart-stocking-durable-factory-v1.ts",
    "utf8",
  )
  assert.match(factory,
    /const upstreamCurrentPathReady = \([\s\S]*proposedPlan\.listingReady === true/)
  assert.match(factory,
    /proposedPlan\.marketTestReady === true[\s\S]*MARKET_TEST_READY === "READY"/)
  assert.match(factory,
    /upstreamCurrentPathReady[\s\S]*proposedPlan\.blockers\.length === 0/)
  assert.match(factory,
    /&& upstreamCurrentReady \? \{[\s\S]*CURRENT_PREPUBLICATION_CONTINUATION_REQUIRED/)
...
test("same certified lane delegates only to active or exactly recovered batches", () => {
  ...
  assert.match(runtime,
    /automaticExpiredShippingClaimRecovery: recoveredBlocker ===[\s\S]*LUNA_SHIPPING_CLAIM_LEASE_EXPIRED_WITHOUT_DURABLE_RESULT/)
