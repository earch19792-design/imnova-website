...
test("verifica el seller autenticado y extrae sólo campos operativos seguros", () => {
  ...
  assert.equal(result.marketplaceSite, "US")
...
test("Item.Site is read as a direct child, never from Seller.Site", () => {
  const xml = getItemXml({ site: "Germany" })
    .replace("<Site>Germany</Site>", "")
    .replace("<Seller><UserID>official_seller</UserID></Seller>",
      "<Seller><UserID>official_seller</UserID><Site>US</Site></Seller><Site>Germany</Site>")
  const result = parseTradingManualListingResponses(getUserXml, xml, itemId)
  assert.equal(result.marketplaceSite, "Germany")
})
