...
import { currentPrepublicationProofV1 } from '../seller-os/publication-prevalidation-boundary-v1'
...
export async function certifyCurrentPrepublicationV1(input:{supabase:SupabaseClient;packageId:string;accountKey:string;actor:string}) {
 ...
 if(!currentPrepublicationProofV1(pub,proof))return {pass:false,preflight,durableReadbackPass:false,publicationWrites:0}
 ...
 const saved=await input.supabase.from('ebay_authorized_listing_publications').update({sanitized_result:next})
 ...
 const durableReadbackPass=!after.error && currentPrepublicationProofV1(actual,stored) && digest(stored)===digest(proof)
 ...
 return {pass:true,preflight,durableReadbackPass,currentPrepublicationEvidenceBound:true,publicationWrites:0,marketplaceWrites:0}
}
