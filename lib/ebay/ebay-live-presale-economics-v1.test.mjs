...
const {
  ...
  parseEbayPromotionRecommendationReadonlyV1,
...
test("promotion reader uses OAuth plus bounded official GETs only", async () => {
  const previous = {
    ...
    refreshToken: process.env.EBAY_MARKETING_READONLY_REFRESH_TOKEN,
  }
  process.env.EBAY_CLIENT_ID = "client-id"
  process.env.EBAY_CLIENT_SECRET = "client-secret"
  process.env.EBAY_MARKETING_READONLY_REFRESH_TOKEN = "refresh-token"
  const calls = []
...
test("official Recommendation API keeps AD rate separate from price discount", () => {
  const result = parseEbayPromotionRecommendationReadonlyV1({
    payload: { listingRecommendations: [{
      listingId: "366582586826",
      marketing: { ad: { promoteWithAd: "RECOMMENDED",
        bidPercentages: [{ basis: "TRENDING", value: "6.5" }] } },
    }] },
    ebayItemId: "366582586826",
    now: new Date("2026-08-30T12:00:00.000Z"),
  })
  assert.equal(result.status, "AVAILABLE")
  assert.equal(result.recommendationType, "AD_RATE")
  assert.equal(result.recommendedAdRatePercent, 6.5)
  assert.equal(result.adRateBasis, "TRENDING")
  assert.equal(result.recommendedDiscountPercent, null)
  assert.equal(result.priceDiscountRecommendation,
    "SEPARATE_CAPABILITY_NOT_RETURNED")
})

test("no exact recommendation is AVAILABLE with no recommendation, not an auth error", () => {
  const result = parseEbayPromotionRecommendationReadonlyV1({
    payload: { listingRecommendations: [] },
    ebayItemId: "366582586826",
  })
...
test("Recommendation reader uses only the dedicated readonly token and a read-only query POST", async () => {
  const previous = {
    clientId: process.env.EBAY_CLIENT_ID,
    clientSecret: process.env.EBAY_CLIENT_SECRET,
    refreshToken: process.env.EBAY_MARKETING_READONLY_REFRESH_TOKEN,
    generic: process.env.EBAY_SELLER_REFRESH_TOKEN,
  }
  process.env.EBAY_CLIENT_ID = "client-id"
  process.env.EBAY_CLIENT_SECRET = "client-secret"
  process.env.EBAY_MARKETING_READONLY_REFRESH_TOKEN = "marketing-token"
  ...
  const fakeFetch = async (input, init = {}) => {
    ...
    calls.push({ url, init })
    if (url.pathname === "/identity/v1/oauth2/token") {
      assert.match(String(init.body), /marketing-token/)
      assert.doesNotMatch(String(init.body), /generic-must-not-be-used/)
      assert.match(String(init.body), /sell.marketing.readonly/)
      return new Response(JSON.stringify({ access_token: "access-token" }), {
        status: 200, headers: { "Content-Type": "application/json" },
      })
    }
    assert.equal(url.pathname, "/sell/recommendation/v1/find")
    assert.equal(url.searchParams.get("filter"), "recommendationTypes:{AD}")
    assert.equal(init.method, "POST")
    assert.deepEqual(JSON.parse(init.body), {
      listingIds: ["366582586826"],
    })
    return new Response(JSON.stringify({ listingRecommendations: [] }), {
      status: 200, headers: { "Content-Type": "application/json" },
    })
  }
  try {
    const result = await readEbayPromotionRecommendationReadonlyV1(
      "366582586826", fakeFetch)
    assert.equal(result.recommendationAvailable, false)
    ...
    for (const [key, value] of Object.entries(previous)) {
      const name = key === "clientId" ? "EBAY_CLIENT_ID"
        : key === "clientSecret" ? "EBAY_CLIENT_SECRET"
          : key === "refreshToken" ? "EBAY_MARKETING_READONLY_REFRESH_TOKEN"
...
test("unknown buyer shipping or promotion returns one exact blocker and never zero", () => {
  ...
  assert.equal(promotion.promotion.state, "UNPROVEN")
  assert.equal(promotion.preSaleBaseFeeModelUsd, 10.19)
  assert.equal(promotion.realizedFee, null)
})
