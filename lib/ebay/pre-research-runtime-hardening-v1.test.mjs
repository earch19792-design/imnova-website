...
async function fixture() {
  ...
  await db.exec([
    ...
    functionSql(hardening, "prepare_seller_os_pre_research_batch_runner_canary_v1"),
    functionSql(hardening, "next_seller_os_pre_research_batch_plan_canary_v1"),
...
async function next(db, allowedBatchIds = [batch]) {
  await db.query(`select public.prepare_seller_os_pre_research_batch_runner_canary_v1(
    $1::text,$2::uuid[]) as result`, [account, allowedBatchIds])
  return (await db.query(`select public.next_seller_os_pre_research_batch_plan_canary_v1(
    $1::text,$2::uuid[]) as result`, [account, allowedBatchIds])).rows[0].result
}
...
test("one reused canonical plan reconciles every attached batch without duplicate capture", async () => {
  ...
  try {
    ...
    assert.equal((await next(db, [batch, reusedBatch])).planId, ids[0].plan)
    assert.equal((await claim(db, ids[0].plan)).claimed, true)
...
test("canary selector and recovery leave non-allowlisted authorized and historical batches untouched", async () => {
  const db = await fixture()
  const otherBatch = "30000000-0000-4000-8000-000000000002"
  const historicalBatch = "30000000-0000-4000-8000-000000000003"
  const otherPlan = "70000000-0000-4000-8000-000000000099"
  try {
    for (const [batchId, planId, state] of [
      [otherBatch, otherPlan, "AUTHORIZED"],
      [historicalBatch, ids[4].plan, "NEEDS_ATTENTION"],
    ]) {
      await db.query(`insert into public.seller_os_pre_research_batches_v1
        (batch_id,owner_authorization_id,marketplace_account_key,owner_user_id,
          command_client_id,batch_state,created_at)
        values ($1::uuid,$2::uuid,$3,$4::uuid,'chatgpt-control-client',
          $5,clock_timestamp()+interval '1 hour')`,
      [batchId, capability, account, owner, state])
      await db.query(`insert into public.seller_os_pre_research_batch_members_v1
        (member_id,batch_id,ordinal,plan_id,execution_state)
        values (gen_random_uuid(),$1::uuid,1,$2::uuid,'PENDING')`,
      [batchId, planId])
    }
    await db.query(`insert into public.marketplace_product_research_query_plans
      (id,marketplace_account_key,marketplace,source_context,status)
    ...
    await db.query(`insert into public.marketplace_product_research_query_tasks
      (id,plan_id,marketplace_account_key,marketplace,status)
      values (gen_random_uuid(),$1::uuid,$2,'EBAY_US','PENDING')`,
    [otherPlan, account])

    const off = (await db.query(`select
      public.prepare_seller_os_pre_research_batch_runner_v1($1::text) as prepared,
      public.next_seller_os_pre_research_batch_plan_v1($1::text) as next`,
    [account])).rows[0]
    assert.deepEqual(off.prepared, { healedBatches: 0, recoveredLeases: 0 })
    assert.equal(off.next.planId, null)
    assert.equal(await next(db, []), null)
    // A plan shared with a historical, non-allowlisted batch is not safe to
    // claim: its reconcile trigger would otherwise mutate that batch.
    assert.equal((await next(db, [batch])).planId, ids[0].plan)
    await db.query(`update public.seller_os_pre_research_batch_members_v1
      set execution_state='COMPLETED' where member_id=$1::uuid`,
    [ids[0].member])
    assert.equal((await next(db, [batch])).planId, ids[1].plan)
    assert.equal((await next(db, [batch, otherBatch])).planId, ids[1].plan)
    assert.equal((await next(db, [otherBatch])).planId, otherPlan)
    await db.query(`update public.seller_os_pre_research_batch_members_v1
      set execution_state='COMPLETED' where batch_id=$1::uuid
        and ordinal between 2 and 4`, [batch])
    assert.equal((await next(db, [batch])).planId, null) // Shared plan is quarantined.
    const other = (await db.query(`select batch_state,started_at,completed_at
      from public.seller_os_pre_research_batches_v1 where batch_id=$1::uuid`,
    [otherBatch])).rows[0]
    const historical = (await db.query(`select batch_state,started_at,
      completed_at from public.seller_os_pre_research_batches_v1
      where batch_id=$1::uuid`, [historicalBatch])).rows[0]
    assert.equal(other.batch_state, "AUTHORIZED")
    assert.equal(other.started_at, null)
    assert.equal(historical.batch_state, "NEEDS_ATTENTION")
    assert.equal(historical.started_at, null)
    assert.equal((await members(db))[0].execution_state, "COMPLETED")
    assert.equal((await db.query(`select count(*)::int as count from
      public.marketplace_product_research_query_plans
      where ebay_writes <> 0`)).rows[0].count, 0)
  } finally { await db.close() }
})
...
test("runner and migration preserve isolated explicit claims and no market writes", () => {
  ...
  assert.match(workerSelection,
    /if \(result\.planId === null &&[\s\S]*canaryGate[\s\S]*GET_NEXT_CURRENT_PACKAGE_KEYWORD_PLAN/)
  ...
  assert.ok(operator.indexOf("await assertPreResearchCanaryPlanClaimV1") <
    operator.indexOf("await claimMayelAutonomousResearchPlanV1"))
  ...
  assert.match(prepareRoute, /gate\.state !== "CANARY"/)
