...
const {
  ...
  getEbayOfficialOrdersLiveReadonly,
  hashEbayMonitorEvidenceIdentifier,
  previewEbayRegistryRepairRuntime,
  validateAccountTrafficDateMetadataV1,
...
const { getCommercialMonitorReadonly, supplyEvidence } = await import(
  "./commercial-monitor-readonly-service.ts"
)

const NOW = new Date("2026-08-08T16:00:00.000Z")
const { readCommercialMonitorReadonlySources } = await import("./commercial-monitor-readonly-repository.ts")
const { executeSellerOsAssistantToolV1 } = await import("./ebay-seller-os-assistant-gateway-v1.ts")
const { buildSellerOsOfficialOrdersReadV1 } = await import("./ebay-official-orders-read-v1.ts")
const { SELLER_OS_COMMERCIAL_CONTEXT_BUDGET_MS, createBudgetedReadonlyFetchV1 } =
  await import("./ebay-seller-os-read-budget-v1.ts")
...
test("canonical approved Luna identity hydrates supplier cost without a legacy registry mapping", () => {
  const evidence = supplyEvidence([{
    product_id: "legacy-internal-row-id",
    supplier_product_id: "9220805755104",
    source_key: "lunaportex",
    snapshot_id: "snapshot-item5810",
    supplier_variant_id: "48809607659744",
    sku: "ITEM5810",
    price: 44.2,
    available: true,
    inventory_quantity: null,
    captured_at: "2026-08-08T15:55:00.000Z",
    metadata: {},
  }], {
    source: "market_radar_sources",
    status: "AVAILABLE",
    rows: [{ key: "lunaportex", is_active: true,
      last_run_at: "2026-08-08T15:55:00.000Z",
      last_success_at: "2026-08-08T15:55:00.000Z" }],
    limitationCode: null,
    truncated: false,
  }, {
    source: "ebay_active_listing_sync_state",
    status: "AVAILABLE",
    rows: [{ account_key: ACCOUNT_KEY, latest_generation: 1,
      latest_started_at: "2026-08-08T15:55:00.000Z",
      latest_committed_generation: 1,
      latest_committed_at: "2026-08-08T15:55:00.000Z",
      targeted_luna_last_success_run_id: "targeted-item5810",
      targeted_luna_last_success_at: "2026-08-08T15:55:00.000Z",
      targeted_luna_last_error_at: null,
      targeted_luna_last_error_code: null }],
    limitationCode: null,
    truncated: false,
  }, NOW, {
    listingStatus: "ACTIVE",
    listingUpdatedAt: "2026-08-08T15:50:00.000Z",
    productId: null,
    supplierProductId: "9220805755104",
    supplierVariantId: "48809607659744",
    supplierSku: "ITEM5810",
  })

  assert.equal(evidence.length, 1)
  assert.equal(evidence[0].productId, "9220805755104")
  assert.equal(evidence[0].supplierVariantId, "48809607659744")
  assert.equal(evidence[0].sku, "ITEM5810")
  assert.equal(evidence[0].price, 44.2)
  assert.notEqual(evidence[0].parserHealth, "PARSER_ERROR")
})
...
function sellerListPage({
  ...
  return `<?xml version="1.0" encoding="utf-8"?>
    <GetSellerListResponse xmlns="urn:ebay:apis:eBLBaseComponents">
      <Ack>Success</Ack><PaginationResult>
        <TotalNumberOfPages>${totalPages}</TotalNumberOfPages>
        <TotalNumberOfEntries>${totalEntries}</TotalNumberOfEntries>
      </PaginationResult><HasMoreItems>${hasMore ?? page < totalPages}</HasMoreItems>
      <ItemArray><Item><ItemID>123456789012</ItemID>
        <Title>Live listing sanitized</Title><SKU>LIVE-SKU</SKU>${site ? `<Site>${site}</Site>` : ""}
        <ListingType>FixedPriceItem</ListingType><Quantity>4</Quantity>
        <SellingStatus><ListingStatus>Active</ListingStatus>
...
function fakeEbay(options = {}) {
  const calls = []
  const tradingRefreshTokens = []
  const fulfillmentRefreshTokens = []
  let accountTrafficRequestCount = 0
  const telemetry = {
    getItemInFlight: 0,
  ...
  const fetchImpl = async (url, init = {}) => {
    ...
    if (parsedUrl.pathname === "/identity/v1/oauth2/token") {
      ...
      if (scope === "https://api.ebay.com/oauth/api_scope") {
        tradingRefreshTokens.push(params.get("refresh_token"))
      }
    ...
    if (parsedUrl.pathname === "/ws/api.dll" &&
      ...
      const responseBody = options.getUserAckFailure
        ? `<GetUserResponse><Ack>Failure</Ack><Errors><ErrorCode>${
            options.getUserErrorCode ?? "931"
          }</ErrorCode><LongMessage>private-provider-message-test-only</LongMessage>
          </Errors></GetUserResponse>`
      ...
      return options.getUserTextFailure
        ...
        : xml(responseBody, options.getUserHttpStatus ?? 200)
    }
    if (parsedUrl.pathname === "/ws/api.dll" &&
    ...
    if (parsedUrl.pathname === "/ws/api.dll") {
      if (headers.get("X-EBAY-API-CALL-NAME") === "GetMyeBaySelling" &&
          options.sellerDiscoveryAckFailureErrorCode) {
        return xml(`<GetMyeBaySellingResponse><Ack>Failure</Ack><Errors>
          <ErrorCode>${options.sellerDiscoveryAckFailureErrorCode}</ErrorCode>
          <LongMessage>private-discovery-message-test-only</LongMessage>
        </Errors></GetMyeBaySellingResponse>`)
      }
      const page = Number(body.match(/<PageNumber>(\d+)<\/PageNumber>/)?.[1])
      if (headers.get("X-EBAY-API-CALL-NAME") === "GetSellerList") {
        if (options.sellerListAckFailureErrorCode) {
          return xml(`<GetSellerListResponse><Ack>Failure</Ack><Errors>
            <ErrorCode>${options.sellerListAckFailureErrorCode}</ErrorCode>
            <LongMessage>private-seller-list-message-test-only</LongMessage>
          </Errors></GetSellerListResponse>`)
        }
        return xml(sellerListPage({ page }))
      }
    ...
    if (parsedUrl.pathname === "/sell/analytics/v1/traffic_report") {
      const accountTrafficRequest = parsedUrl.searchParams.get("dimension") === "DAY"
      if (accountTrafficRequest) accountTrafficRequestCount += 1
      if (accountTrafficRequest && (options.accountTraffic429 ||
          (options.accountTraffic429Once && accountTrafficRequestCount === 1))) {
        return new Response(JSON.stringify({ errors: [{
          errorId: "rate-limit-test-only" }] }), {
          status: 429,
          headers: { "Content-Type": "application/json",
            "Retry-After": String(options.accountTrafficRetryAfter ?? 60) },
        })
      }
      const payload = analyticsPayload()
      if (accountTrafficRequest && options.accountTrafficAvailable) {
        payload.header.dimensionKeys = [{ key: "DAY" }]
        payload.records[0].dimensionValues = [{ value: "2026-08-07" }]
      }
      ...
      if (accountTrafficRequest && options.accountTrafficInvalidDatesOnce &&
          accountTrafficRequestCount === 1) {
        payload.lastUpdatedDate = "invalid-transient-metadata"
      }
    ...
    if (parsedUrl.pathname === "/sell/fulfillment/v1/order") {
      if (parsedUrl.searchParams.get("limit") === "1" &&
          options.fulfillmentIdentitySellerId) {
        return json({
          total: 1,
          orders: [{
            sellerId: options.fulfillmentIdentitySellerId,
            lineItems: [{
              listingMarketplaceId:
                options.fulfillmentIdentityMarketplaceId ?? "EBAY_US",
            }],
          }],
        })
      }
  ...
  return {
    calls,
    fetchImpl,
    tradingRefreshTokens,
    fulfillmentRefreshTokens,
    telemetry,
  }
}
...
test("preview Registry falla cerrado sin evidencia y conserva cero escrituras", async () => {
  ...
  assert.equal(result.REPAIR_ROW_CURRENT_STATUS_CLASS, "UNPROVEN")
  assert.equal(result.REPAIR_ROW_STATUS_RAW_VALUE_RECOGNIZED, "UNPROVEN")
  assert.equal(result.REPAIR_ROW_STATUS_REACTIVATABLE, "UNPROVEN")
  assert.equal(result.REPAIR_ROW_ACCOUNT_SCOPE_MATCH, "UNPROVEN")
  assert.equal(result.REPAIR_ROW_AUTHORITATIVE_ITEM_ID_STILL_LIVE,
    "UNPROVEN")
  assert.equal(result.REPAIR_ROW_ITEM_ID_UNIQUE_BOTH_SIDES, "UNPROVEN")
  assert.equal(result.REPAIR_ROW_COMPETING_RELATIONSHIP, "UNPROVEN")
  assert.equal(result.REGISTRY_LIFECYCLE_SUPPORTS_REACTIVATION, "UNPROVEN")
  assert.equal(result.REACTIVATION_CAS_SUPPORTED, "UNPROVEN")
  assert.equal(result.REPAIR_EXISTING_AUTOMATIC_COUNT, "UNPROVEN")
  assert.equal(result.HUMAN_REVIEW_REASON_REACTIVATION_NOT_ALLOWED_COUNT,
    "UNPROVEN")
  assert.equal(result.IDENTITY_UNPROVEN_COUNT, "UNPROVEN")
  assert.equal(result.AUTOMATIC_PRECONDITION_UNPROVEN_COUNT, "UNPROVEN")
  assert.equal(result.AUTOMATIC_TRANCHE_PRECONDITIONS_PASS, "UNPROVEN")
  assert.equal(result.HUMAN_REVIEW_WRITE_ALLOWED, "UNPROVEN")
  assert.equal(result.HUMAN_REVIEW_MUTATION_COUNT, "UNPROVEN")
...
test("preview Registry compara dos snapshots independientes del mismo request", async () => {
  ...
  const repairRow = activeRegistryRow({
    itemId: "700000000001",
    sku: "OLD-PRIVATE-SKU",
  })
  repairRow.source = "LEGACY_READONLY_SOURCE"
  repairRow.sync_key = "legacy-opaque-key"
  repairRow.listing_status = "ended"
  let lifecycleSnapshotReads = 0
  const lifecycle = await previewEbayRegistryRepairRuntime({
    startedAt: Date.parse(observedAt),
    readRegistryRepairEvidenceSnapshot: async () => {
      ...
      return structuredClone({
        ...
        observedAt,
        liveListings: [listing("700000000001", "PRIVATE-SKU-1")],
        registryRows: [repairRow],
        syncKeyLookupStatus: "AVAILABLE",
        existingRegistrySyncKeys: [],
      })
    },
  })
  assert.equal(lifecycleSnapshotReads, 2)
  assert.equal(lifecycle.REPAIR_EXISTING_COUNT, 1)
  assert.equal(lifecycle.REPAIR_PRECONDITION_STATUS, "PASS")
  assert.equal(lifecycle.REPAIR_ROW_CURRENT_STATUS_CLASS, "ENDED")
  assert.equal(lifecycle.REPAIR_ROW_STATUS_REACTIVATABLE, "YES")
  assert.equal(lifecycle.REACTIVATION_ALLOWED_FROM_ENDED, "YES")
  assert.equal(lifecycle.REACTIVATION_CAS_SUPPORTED, "YES")
  assert.equal(lifecycle.REPAIR_EXISTING_AUTOMATIC_COUNT, 1)
  assert.equal(lifecycle.HUMAN_REVIEW_REASON_REACTIVATION_NOT_ALLOWED_COUNT, 0)
  assert.deepEqual(lifecycle.REPAIR_FIELDS_TO_CHANGE,
    ["ebay_sku", "listing_status"])
  assert.equal(lifecycle.REGISTRY_MUTATIONS, 0)
  assert.equal(lifecycle.EBAY_WRITES, 0)
  assert.doesNotMatch(JSON.stringify(lifecycle),
    /700000000001|PRIVATE-SKU|OLD-PRIVATE-SKU|opaque-listing-anchor/)

  const reviewRow = structuredClone(repairRow)
  reviewRow.listing_status = "unknown"
  let reviewSnapshotReads = 0
  const review = await previewEbayRegistryRepairRuntime({
    startedAt: Date.parse(observedAt),
    readRegistryRepairEvidenceSnapshot: async () => {
      reviewSnapshotReads += 1
      return structuredClone({
        accountKey: ACCOUNT_KEY,
        observedAt,
        liveListings: [listing("700000000001", "PRIVATE-SKU-1")],
  ...
  assert.equal(reviewSnapshotReads, 2)
  assert.equal(review.REPAIR_EXISTING_AUTOMATIC_COUNT, 0)
  assert.equal(review.REPAIR_EXISTING_COUNT, 0)
  assert.equal(review.CREATE_NEW_COUNT, 0)
  assert.equal(review.MARK_STALE_COUNT, 0)
  assert.equal(review.HUMAN_REVIEW_COUNT, 1)
  assert.equal(review.LIVE_HUMAN_REVIEW_COUNT, 1)
  assert.equal(review.REGISTRY_HUMAN_REVIEW_COUNT, 1)
  assert.equal(review.HUMAN_REVIEW_REASON_REACTIVATION_NOT_ALLOWED_COUNT, 1)
  assert.equal(review.IDENTITY_UNPROVEN_COUNT, 0)
  assert.equal(review.AUTOMATIC_PRECONDITION_UNPROVEN_COUNT, 0)
  assert.equal(review.AUTOMATIC_TRANCHE_PRECONDITIONS_PASS, "YES")
  assert.equal(review.HUMAN_REVIEW_WRITE_ALLOWED, "NO")
  assert.equal(review.HUMAN_REVIEW_MUTATION_COUNT, 0)
  assert.equal(review.DRY_RUN_READY_FOR_APPROVAL, "YES")
  assert.equal(review.REGISTRY_MUTATIONS, 0)
  assert.equal(review.EBAY_WRITES, 0)
})
...
test("coordinador produce ledger sólo-read, scopes y DTO sanitizado", async () => {
  ...
  assert.deepEqual(fake.tradingRefreshTokens, [])
...
test("listing Trading live sin objeto Inventory conserva existencia y separa representación", async () => {
  ...
  assert.equal(monitor.listings[0].identity.listedQuantity, 3)
  assert.equal(monitor.listings[0].identity.listedPrice, 19.95)
  assert.equal(monitor.listings[0].identity.currency, "USD")
  assert.equal(monitor.listings[0].identity.source,
    "EBAY_TRADING_GET_MY_EBAY_SELLING")
  assert.deepEqual(monitor.listings[0].discovery.livePresence, {
    status: "LIVE_ACTIVE",
    source: "EBAY_TRADING_GET_MY_EBAY_SELLING",
...
test("GetUser conserva sólo ErrorCode numérico seguro al fallar", async () => {
  const result = await run(fakeEbay({
    getUserAckFailure: true,
    getUserErrorCode: "931",
  }))
  assert.equal(result.account.status, "BLOCKED")
  assert.deepEqual(result.orders.gapCodes, [
    "EBAY_MONITOR_ACCOUNT_IDENTITY_TRADING_ERROR_931",
    "NO_EVIDENCE_DOES_NOT_PROVE_ZERO",
  ])
  assert.doesNotMatch(JSON.stringify(result),
    /private-provider-message-test-only/i)
})

test("GetMyeBaySelling 518 usa GetSellerList bounded para current-live", async () => {
  const result = await run(fakeEbay({
    sellerDiscoveryAckFailureErrorCode: "518",
  }))
  assert.equal(result.account.status, "CERTIFIED")
  assert.equal(result.discovery.status, "AVAILABLE")
  assert.equal(result.discovery.listings.length, 1)
  assert.equal(result.orders.status, "CERTIFIED")
  ...
  assert.doesNotMatch(JSON.stringify(result),
    /private-discovery-message-test-only/i)
})

test("GetSellerList conserva sólo ErrorCode numérico seguro al fallar", async () => {
  const result = await run(fakeEbay({
    sellerDiscoveryAckFailureErrorCode: "518",
    sellerListAckFailureErrorCode: "518",
  }))
  assert.equal(result.account.status, "CERTIFIED")
  assert.deepEqual(result.orders.gapCodes, [
  ...
  assert.doesNotMatch(JSON.stringify(result),
    /private-seller-list-message-test-only/i)
})

test("Official Orders no depende de discovery seller-wide con Error 518", async () => {
  const fake = fakeEbay({
    getUserAckFailure: true,
    getUserErrorCode: "518",
    fulfillmentIdentitySellerId: USER_ID,
    sellerDiscoveryAckFailureErrorCode: "518",
    sellerListAckFailureErrorCode: "518",
  })
  const result = await getEbayOfficialOrdersLiveReadonly({
    accountKey: ACCOUNT_KEY,
    accountAlias: ACCOUNT_ALIAS,
    environment: environment(),
    fetchImpl: fake.fetchImpl,
  ...
  assert.equal(result.orders.status, "CERTIFIED")
  assert.equal(result.canonicalAccountBinding, "MATCHED")
  assert.equal(result.refreshCapability, true)
  assert.equal(result.accountIdentitySource,
    "EBAY_SELL_FULFILLMENT_GET_ORDERS_SELLER_ID")
  assert.equal(fake.calls.some((call) =>
    call.tradingCall === "GetMyeBaySelling" ||
    call.tradingCall === "GetSellerList"), false)
  assert.equal(result.safety.marketplaceWrites, 0)
  assert.equal(result.safety.buyerPiiReturned, false)
  assert.doesNotMatch(JSON.stringify(result), new RegExp(USER_ID, "i"))
})

test("GetUser 518 usa sellerId Fulfillment sin reautorizar ni exponer PII", async () => {
  const fake = fakeEbay({
    getUserAckFailure: true,
    getUserErrorCode: "518",
    fulfillmentIdentitySellerId: USER_ID,
  ...
  assert.equal(result.account.status, "CERTIFIED")
  assert.equal(result.account.bindingMatched, true)
  assert.equal(result.account.source,
    "EBAY_SELL_FULFILLMENT_GET_ORDERS_SELLER_ID")
  assert.equal(result.discovery.status, "AVAILABLE")
  assert.equal(result.orders.status, "CERTIFIED")
  assert.equal(result.safety.buyerPiiReturned, false)
  assert.equal(result.calls.filter((call) =>
    ...
    call.status === "FAILED").length, 4)
  assert.equal(result.calls.filter((call) =>
    call.operation === "FULFILLMENT_GET_ORDERS" &&
    call.status === "SUCCEEDED").length, 3)
  assert.doesNotMatch(JSON.stringify(result), new RegExp(USER_ID, "i"))
})

test("fallback Fulfillment 518 falla cerrado ante sellerId distinto", async () => {
  const result = await run(fakeEbay({
    getUserAckFailure: true,
    getUserErrorCode: "518",
    fulfillmentIdentitySellerId: "other-seller",
  }))
  assert.equal(result.account.status, "BLOCKED")
  assert.equal(result.account.bindingMatched, false)
  assert.deepEqual(result.orders.gapCodes, [
    "EBAY_MONITOR_ACCOUNT_IDENTITY_MISMATCH",
    "NO_EVIDENCE_DOES_NOT_PROVE_ZERO",
  ])
})
...
test("Listings sweep certifies every official item beyond the commercial monitor cap", async () => {
  const fake = fakeEbay({ sellerItemsWithoutSite: 33 })
  const result = await getEbayOfficialLiveListingSweepReadonly({
    accountKey: ACCOUNT_KEY,
    accountAlias: ACCOUNT_ALIAS,
    environment: environment(),
    fetchImpl: fake.fetchImpl,
  ...
  assert.equal(result.status, "CERTIFIED_COMPLETE")
  assert.equal(result.paginationComplete, true)
  assert.equal(result.totalEntries, 33)
  assert.equal(result.listings.length, 33)
  assert.equal(fake.calls.filter((call) => call.tradingCall === "GetItem").length, 33)
  assert.equal(fake.calls.some((call) => call.tradingCall === "ReviseItem"), false)
  assert.equal(fake.calls.some((call) => call.tradingCall === "AddItem"), false)
})

test("Listings sweep rejects a capped cohort without persisting partial coverage", async () => {
  const fake = fakeEbay({ sellerItemsWithoutSite: 81 })
  const result = await getEbayOfficialLiveListingSweepReadonly({
    accountKey: ACCOUNT_KEY,
    accountAlias: ACCOUNT_ALIAS,
    environment: environment(),
  ...
  assert.equal(result.failedOperation, "TRADING_GET_ITEM_MARKETPLACE")
...
test("Listings sweep retains ambiguous variation identity for registry review", async () => {
  const fake = fakeEbay({ repeatedItemWithoutVariationIdentity: true })
  const result = await getEbayOfficialLiveListingSweepReadonly({
    accountKey: ACCOUNT_KEY,
    accountAlias: ACCOUNT_ALIAS,
    environment: environment(),
  ...
  assert.equal(result.listings.every((listing) =>
    listing.identityAmbiguous === true), true)
  assert.equal(fake.calls.filter((call) => call.tradingCall === "GetItem").length, 1)
})
...
test("certificación parcial conserva Items US y limita Analytics al subconjunto", async () => {
  ...
  const accountTrafficCall = fake.calls.find((call) =>
    call.path === "/sell/analytics/v1/traffic_report" &&
    call.search.includes("dimension=DAY"))
  const listingTrafficCall = fake.calls.find((call) =>
    call.path === "/sell/analytics/v1/traffic_report" &&
    call.search.includes("dimension=LISTING"))
  assert.ok(accountTrafficCall)
  assert.ok(listingTrafficCall)
  assert.match(listingTrafficCall.search, /123456789012/)
  assert.doesNotMatch(listingTrafficCall.search, /123456789013/)
...
test("DTO final conserva live, error SELECT y excluye PII/secretos", async () => {
  ...
  assert.equal(monitor.backend.capabilities.registry.status, "UNAVAILABLE")
  assert.equal(monitor.backend.capabilities.registry.currentLiveCount, null)
  assert.equal(monitor.backend.capabilities.registry.matchedCount, null)
  assert.equal(monitor.backend.capabilities.registry.humanReviewCount, null)
  assert.equal(monitor.backend.capabilities.registry.coveragePercent, null)
  assert.deepEqual(monitor.backend.capabilities.registry.limitationCodes,
    ["COMMERCIAL_REGISTRY_READ_FAILED"])
  assert.equal(monitor.backend.operationalHealth.manualReview.authority,
    "DECISION_TAXONOMY_V2")
  assert.equal(monitor.backend.operationalHealth.manualReview.status, "PARTIAL")
  assert.equal(monitor.backend.operationalHealth.manualReview.value, null)
  assert.equal(monitor.backend.operationalHealth.manualReview.zeroIsAuthoritative,
    false)
  assert.equal(monitor.backend.operationalHealth.manualReview.falseZeroGuard.status,
    "PASS")
  assert.equal(monitor.backend.livePortfolioIntegrity.deterministicGuards.find(
    (guard) => guard.guardCode ===
      "OPERATIONAL_REVIEW_FALSE_ZERO_GUARD")?.status, "PASS")
  assert.equal(monitor.listings[0].discovery.livePresence.status, "LIVE_ACTIVE")
...
test("órdenes Item no se duplican sobre variaciones sin SKU", async () => {
  ...
  assert.doesNotMatch(JSON.stringify(monitor.listings), /order-sensitive-id/)
  assert.equal(monitor.backend.saleAlerts.alerts[0].orderId,
    "order-sensitive-id")
  assert.equal(monitor.backend.saleAlerts.alerts[0].lineItemId,
    "line-sensitive-id")
})
...
test("órdenes Item no se duplican sobre variaciones con SKU repetido", async () => {
  ...
  assert.doesNotMatch(JSON.stringify(monitor.listings),
    /shared-order-sensitive-id/)
  assert.equal(monitor.backend.saleAlerts.alerts[0].orderId,
    "shared-order-sensitive-id")
  assert.equal(monitor.backend.saleAlerts.alerts[0].lineItemId,
    "shared-line-sensitive-id")
})
...
test("link supplier almacenado no cruza variaciones con SKU compartido", async () => {
  ...
  assert.equal(blue.stock.state, "STOCK_UNKNOWN")
  assert.equal(blue.stock.supplierVariantId, null)
  assert.equal(blue.stock.limitationCode,
...
test("one monitor review acquires at most one canonical Account Traffic snapshot", async () => {
  const fake = fakeEbay({ accountTrafficAvailable: true })
  const result = await run(fake)
  const upstreamCalls = fake.calls.filter((call) =>
    call.path === "/sell/analytics/v1/traffic_report" &&
    call.search.includes("dimension=DAY"))

  assert.equal(upstreamCalls.length, 1)
  assert.equal(result.analytics.accountTraffic.status, "AVAILABLE")
  assert.equal(result.analytics.accountTraffic.scopeType,
    "ACCOUNT_TRAFFIC_SCOPE")
  assert.equal(result.analytics.accountTraffic.upstreamSnapshotAcquisitionCount,
    1)
  assert.equal(result.analytics.accountTraffic.metadataValidationStatus, "VALID")
  assert.match(result.analytics.accountTraffic.accountTrafficSnapshotId,
    /^account-traffic-snapshot:/)
  assert.match(result.analytics.accountTraffic.auditSpanId,
    /^account-traffic-audit:/)
  assert.equal(result.analytics.accountTraffic.retryCount, 0)
  assert.equal(result.analytics.accountTraffic.cacheHitCount, 0)
  assert.equal(result.analytics.accountTraffic.snapshotReuseStatus, "ACQUIRED")
  assert.equal(result.analytics.accountTraffic.snapshotReuseReasonCode,
    "CACHE_MISS_ACQUIRED")
})

test("matching account and reporting-window reads reuse one canonical Account Traffic snapshot", async () => {
  const fake = fakeEbay({ accountTrafficAvailable: true })
  const first = await run(fake)
  const subsequent = []
  for (let index = 0; index < 4; index += 1) {
    subsequent.push(await run(fake))
  }
  const upstreamCalls = fake.calls.filter((call) =>
    call.path === "/sell/analytics/v1/traffic_report" &&
    call.search.includes("dimension=DAY"))
  ...
  assert.equal(upstreamCalls.length, 1)
  assert.equal(first.analytics.accountTraffic.upstreamSnapshotAcquisitionCount,
    1)
  assert.equal(subsequent.every((result) =>
    result.analytics.accountTraffic.accountTrafficSnapshotId ===
      first.analytics.accountTraffic.accountTrafficSnapshotId), true)
  assert.equal(subsequent.every((result) =>
    result.analytics.accountTraffic.upstreamSnapshotAcquisitionCount === 0 &&
      result.analytics.accountTraffic.snapshotReuseStatus === "REUSED" &&
      result.analytics.accountTraffic.snapshotReuseReasonCode ===
        "FRESH_MATCHING_ACCOUNT_WINDOW"), true)
  assert.equal(subsequent.at(-1).analytics.accountTraffic.cacheHitCount, 4)
})

test("Account Traffic retries invalid date metadata once and exposes bounded telemetry", async () => {
  const fake = fakeEbay({ accountTrafficAvailable: true,
    accountTrafficInvalidDatesOnce: true })
  const result = await run(fake)
  const accountTraffic = result.analytics.accountTraffic
  const upstreamCalls = fake.calls.filter((call) =>
    call.path === "/sell/analytics/v1/traffic_report" &&
    call.search.includes("dimension=DAY"))

  assert.equal(upstreamCalls.length, 2)
  assert.equal(accountTraffic.status, "AVAILABLE")
  assert.equal(accountTraffic.metadataValidationStatus, "VALID")
  assert.equal(accountTraffic.metadataValidationReasonCode, null)
  assert.equal(accountTraffic.upstreamSnapshotAcquisitionCount, 2)
  assert.equal(accountTraffic.retryCount, 1)
  assert.equal(accountTraffic.retryPolicy,
    "ONE_RETRY_ON_DATE_METADATA_INVALID")
  assert.ok(accountTraffic.cumulativeAcquisitionCount >= 2)
})

test("Account Traffic invalid metadata fails closed with explicit telemetry and no false zero", async () => {
  const metadata = validateAccountTrafficDateMetadataV1({
    startDate: "2026-07-09",
    endDate: "2026-08-07",
    lastUpdatedDate: "invalid",
  })
  assert.equal(metadata.status, "INVALID")
  assert.equal(metadata.reasonCode,
    "ACCOUNT_TRAFFIC_LAST_UPDATED_DATE_INVALID")
  ...
  assert.equal(upstreamCalls.length, 2)
  assert.equal(accountTraffic.status, "UNAVAILABLE")
  assert.equal(accountTraffic.metadataValidationStatus, "INVALID")
  assert.equal(accountTraffic.metadataValidationReasonCode,
    "ACCOUNT_TRAFFIC_START_DATE_INVALID")
  assert.equal(accountTraffic.upstreamSnapshotAcquisitionCount, 2)
  assert.equal(accountTraffic.retryCount, 1)
  assert.equal(accountTraffic.impressions, null)
  assert.equal(accountTraffic.listingViews, null)
  assert.equal(accountTraffic.quantitySold, null)
  assert.equal(accountTraffic.ctr, null)
})

test("Account Traffic 429 fails closed without fabricating zero", async () => {
  const fake = fakeEbay({ accountTraffic429: true })
  const result = await run(fake)
  const accountTraffic = result.analytics.accountTraffic
  const upstreamCalls = fake.calls.filter((call) =>
    call.path === "/sell/analytics/v1/traffic_report" &&
  ...
  assert.equal(upstreamCalls.length, 1)
  assert.equal(accountTraffic.status, "UNAVAILABLE")
  assert.equal(accountTraffic.completeness, "UNPROVEN")
  assert.equal(accountTraffic.impressions, null)
  assert.equal(accountTraffic.listingViews, null)
  assert.equal(accountTraffic.quantitySold, null)
  assert.equal(accountTraffic.ctr, null)
  assert.equal(accountTraffic.upstreamSnapshotAcquisitionCount, 1)
  assert.equal(accountTraffic.metadataValidationStatus, "NOT_ATTEMPTED")
  assert.match(accountTraffic.auditSpanId, /^account-traffic-audit:/)
  assert.deepEqual(accountTraffic.gapCodes,
    ["EBAY_MONITOR_ACCOUNT_TRAFFIC_429"])
  const trafficCalls = fake.calls.filter((call) =>
    call.path === "/sell/analytics/v1/traffic_report")
  assert.equal(trafficCalls.length, 1,
    "a DAY-level 429 must stop the listing-level request in the same refresh")
})

test("Account Traffic respects Retry-After and the next 200 replaces bounded negative evidence", async () => {
  const fake = fakeEbay({ accountTraffic429Once: true,
    accountTrafficAvailable: true, accountTrafficRetryAfter: 60 })
  let clockMs = Date.parse(NOW)
  const read = () => run(fake, { clock: () => new Date(clockMs) })

  const first = await read()
  assert.equal(first.analytics.status, "UNAVAILABLE")
  assert.deepEqual(first.analytics.accountTraffic.gapCodes,
    ["EBAY_MONITOR_ACCOUNT_TRAFFIC_429"])
  assert.equal(fake.calls.filter((call) =>
    call.path === "/sell/analytics/v1/traffic_report").length, 1)

  clockMs += 30_000
  const duringBackoff = await read()
  assert.deepEqual(duringBackoff.analytics.accountTraffic.gapCodes,
  ...
  assert.equal(recovered.analytics.accountTraffic.status, "AVAILABLE")
  assert.equal(recovered.analytics.status, "CERTIFIED")
  assert.equal(fake.calls.filter((call) =>
    call.path === "/sell/analytics/v1/traffic_report").length, 3)
})

test("PASS_ONE_SLOW_EXTERNAL_DEPENDENCY_RETURNS_PARTIAL_NOT_TIMEOUT", async () => {
  const fake = fakeEbay()
  let stalled = 0
  const started = Date.now()
  const live = await run({ ...fake, fetchImpl: async (url, init) => {
    if (new Headers(init.headers).get("X-EBAY-API-CALL-NAME") === "GetMyeBaySelling") {
      stalled++
      return new Promise((_, reject) => {
        const timer = setTimeout(() => reject(new Error("TEST_TIMEOUT_NOT_BOUNDED")), 2000)
        init.signal.addEventListener("abort", () => { clearTimeout(timer); reject(init.signal.reason) }, { once: true })
      })
    }
    return fake.fetchImpl(url, init)
  } }, { readLimits: { budgetMs: 700, perCallTimeoutMs: 250, isolateIndependentReads: true } })
  assert(Date.now() - started < 1500)
  assert.equal(stalled, 1)
  assert.equal(live.discovery.status, "UNAVAILABLE")
  assert.equal(live.discovery.totalEntries, null)
  assert.equal(live.orders.status, "CERTIFIED")
  assert.notEqual(live.discovery.inventory.status, "UNAVAILABLE")
  assert(live.discovery.gapCodes.some(code => /TIMEOUT/.test(code)))
  const sources = await readCommercialMonitorReadonlySources(fakeSupabase(), ACCOUNT_KEY,
    { deadlineAt: Date.now() + 300 })
  const monitor = await getCommercialMonitorReadonly(fakeSupabase(),
    { accountKey: ACCOUNT_KEY, accountAlias: ACCOUNT_ALIAS }, live, NOW, sources)
  const context = executeSellerOsAssistantToolV1({ toolName: "seller_os_get_commercial_context",
    arguments: {}, monitor: { ...monitor, officialOrders: buildSellerOsOfficialOrdersReadV1({
      orders: live.orders, analytics: live.analytics }) } })
  assert.equal(context.currentLivePortfolio.activeListings, null)
  assert.equal(context.marketplaceWrites, 0)
  assert(context.sourceReaders.some(reader => reader.status !== "AVAILABLE"))
  assert(SELLER_OS_COMMERCIAL_CONTEXT_BUDGET_MS < 30000)
  console.log("PASS_COMMERCIAL_CONTEXT_WITHIN_MCP_BUDGET")
  console.log("PASS_UNAVAILABLE_NOT_ZERO")
  console.log("PASS_READ_ONLY_SAFETY")
})

test("PASS_INDEPENDENT_EVIDENCE_PRESERVED", async () => {
  const stock = { observation_id: "source-stock-receipt", observed_availability: true,
    observed_supplier_quantity: null, observed_at: NOW.toISOString() }
  const experiment = { experiment_id: "source-experiment", lifecycle_status: "RUNNING" }
  const event = { id: "source-sale-event", detected_at: NOW.toISOString() }
  const db = fakeSupabase({ seller_os_luna_stock_observations: [stock],
    ebay_listing_experiments_v1: [experiment], commercial_alert_events: [event] })
  const from = db.from
  db.from = table => {
    const query = from(table)
    if (["ebay_active_listings", "alert_delivery_outbox"].includes(table)) query.then = () => new Promise(() => {})
    return query
  }
  const started = Date.now()
  const sources = await readCommercialMonitorReadonlySources(db, ACCOUNT_KEY,
    { deadlineAt: Date.now() + 100 })
  assert(Date.now() - started < 500)
  assert.equal(sources.registry.status, "ERROR")
  assert.equal(sources.registry.limitationCode, "SELLER_OS_READ_BUDGET_EXHAUSTED")
  assert.deepEqual(sources.lunaStockObservations.rows, [stock])
  assert.deepEqual(sources.experiments.rows, [experiment])
  assert.equal(sources.orders.status, "AVAILABLE")
  assert.deepEqual(sources.saleEvents.rows, [event])
  assert.equal(sources.saleDeliveries.status, "ERROR")
  console.log("PASS_EXISTING_AUTHORITY_REUSE")
})

test("PASS_NO_UNBOUNDED_RETRIES", async () => {
  const fake = fakeEbay()
  const live = await run(fake, { readLimits: { budgetMs: 700, maximumCalls: 6,
    perCallTimeoutMs: 250, isolateIndependentReads: true } })
  assert(live.calls.length <= 6)
  const refreshes = fake.calls.filter(call => call.path === "/identity/v1/oauth2/token")
  assert(refreshes.length <= 3)
  let databaseCalls = 0
  const bounded = createBudgetedReadonlyFetchV1({ deadlineAt: Date.now() + 1000, timings: [],
    fetchImpl: async () => { databaseCalls++; return new Response("upstream unavailable", {status: 520}) } })
  await assert.rejects(bounded("https://database.example/rest/v1/evidence", {method: "GET"}),
    error => error.name === "AbortError")
  assert.equal(databaseCalls, 1)
  await assert.rejects(bounded("https://database.example/rest/v1/evidence", {method: "POST"}),
    /READ_ONLY_REQUIRED/)
  assert.equal(databaseCalls, 1)
  const expired = createBudgetedReadonlyFetchV1({ deadlineAt: Date.now() - 1, timings: [],
    fetchImpl: async () => { databaseCalls++; return new Response("{}") } })
  await assert.rejects(expired("https://database.example/rest/v1/evidence"), /BUDGET_EXHAUSTED/)
  assert.equal(databaseCalls, 1)
})

test("PASS_NO_UNBOUNDED_PAGINATION", async () => {
  const fake = fakeEbay({ sellerReportedTotal: 25000 })
  const live = await run(fake, { readLimits: { budgetMs: 700, maximumCalls: 8,
    isolateIndependentReads: true } })
  assert(live.calls.length <= 8)
  assert(live.discovery.pagesRead <= 125)
  assert(live.orders.pagesRead <= 10)
  assert(live.safety.marketplaceWrites === 0 && live.safety.databaseWrites === 0)
})
