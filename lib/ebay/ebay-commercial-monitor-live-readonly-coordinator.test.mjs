    ...
    if (parsedUrl.pathname === "/sell/inventory/v1/inventory_item") {
      ...
      if (options.inventoryTotalDrift) {
        ...
        return offset === 0
          ? json({
              total: 100,
              next: "https://api.ebay.com/sell/inventory/v1/inventory_item?limit=50&offset=50",
...
test("consumer Inventory protegido devuelve sólo metadata del sobre vacío", async () => {
  const fake = fakeEbay({ inventoryOmittedEmpty: true })
...
test("consumer Inventory distingue formato, auth, timeout, red y presupuesto", async () => {
  const cases = [
    [{ inventoryMalformedEmpty: true }, "RESPONSE_FORMAT_CHANGED", 3],
    [{ inventoryNonJsonContentType: true }, "RESPONSE_FORMAT_CHANGED", 3],
    [{ malformedJsonPath: "/sell/inventory/v1/inventory_item" },
      "RESPONSE_FORMAT_CHANGED", 3],
    [{ inventoryHttpStatus: 401 }, "HTTP_401", 3],
    [{ inventoryHttpStatus: 403 }, "HTTP_403", 3],
    ...
    [{ inventoryTimeout: true }, "TIMEOUT", 3],
    [{ inventoryNetworkFailure: true }, "NETWORK", 3],
  ]
