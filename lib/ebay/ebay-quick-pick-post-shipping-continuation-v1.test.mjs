...
test("an exact recovery scope does not expand into ineligible batch siblings", async () => {
  let batchScopeReads = 0
  const calls = []
  const result = await continueLunaQuickPickPostShippingRuntimeV1({
    supabase: {}, accountKey: "account-1", candidateKeys: [candidateA],
    scopeMode: "EXACT_REQUEST", taxonomyReader: async () => ({}),
    dependencies: {
      readBatchScope: async () => {
        batchScopeReads += 1
        return [candidateA, candidateB]
      },
      continueRequiredSpecifics: async (input) => {
        calls.push(input.candidateKeys)
        return { claimed: 1, productsEvaluated: 1 }
      },
      continueMinimumReadiness: async () => ({}),
    },
  })
  assert.equal(batchScopeReads, 0)
  assert.deepEqual(calls, [[candidateA]])
  assert.equal(result.scopeMode, "EXACT_REQUEST")
  assert.equal(result.scopedCandidateCount, 1)
})
