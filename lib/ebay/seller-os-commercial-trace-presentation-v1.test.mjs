...
test("technical codes are translated in the primary experience", () => {
  ...
  assert.match(humanComparableReasonV1({
    comparableClass: "FUNCTIONAL_COMPARABLE",
    pricingAuthorityClass: "BRANDED_CATEGORY_SIGNAL_ONLY",
  }), /no determina el precio del producto genérico/)
...
test("advisory target and historical shipping never become an authorized price", () => {
  const view = buildCommercialTracePresentationV1({ trace: { ...trace,
    result: { ...result, RECOMMENDED_PRICE: 25,
      SHIPPING_AUTHORITY: { status: "PROVEN", durableReceiptId:
        "33333333-3333-4333-8333-333333333333",
        freshUntil: "2026-09-23T23:00:00Z" },
      FEE_AUTHORITY: { status: "MISSING" },
      PRICE_AUTHORIZED: false, FINAL_AUTHORIZED_PRICE: null,
      PRICE_AUTHORIZATION_BLOCKERS: ["HOLD_FEE_AUTHORITY_UNPROVEN"] },
  }, events })
  assert.equal(view.dossier.pricing.recommendedPrice, 25)
  assert.equal(view.dossier.pricing.priceAuthorized, false)
  assert.equal(view.dossier.pricing.finalAuthorizedPrice, null)
  assert.deepEqual(view.dossier.pricing.authorizationBlockers,
    ["HOLD_FEE_AUTHORITY_UNPROVEN"])
  assert.equal(view.dossier.economics.shippingStatus, "PROVEN")
  assert.equal(view.dossier.economics.feeAuthorityStatus, "MISSING")
  assert.match(view.dossier.pricing.rationale, /no es un precio final autorizado/)
  const legacy = buildCommercialTracePresentationV1({ trace, events })
  assert.equal(legacy.dossier.pricing.priceAuthorized, false)
  assert.equal(legacy.dossier.pricing.finalAuthorizedPrice, null)
})

test("expired durable shipping remains visible but is not fresh authority", () => {
  const view = buildCommercialTracePresentationV1({ trace: { ...trace,
    result: { ...result, PRICE_AUTHORIZED: true,
      FINAL_AUTHORIZED_PRICE: 25,
      FEE_AUTHORITY: { status: "PROVEN" },
      ECONOMICS_AUTHORITY: { status: "PROVEN" },
      SHIPPING_AUTHORITY: { status: "PROVEN",
      durableReceiptId: "33333333-3333-4333-8333-333333333333",
      freshUntil: "2026-09-13T18:00:00Z" } },
  }, events })
  assert.equal(view.dossier.economics.shippingStatus, "STALE")
  assert.equal(view.dossier.pricing.priceAuthorized, false)
  assert.equal(view.dossier.economics.shippingReceiptId,
    "33333333-3333-4333-8333-333333333333")
  assert.equal(view.dossier.pricing.finalAuthorizedPrice, null)
})
...
test("UI keeps technical evidence collapsed and restores durable traces", async () => {
  ...
  const dashboard = await readFile(new URL(
    "../../app/admin/ebay-seller-os/page.tsx", import.meta.url), "utf8")
  ...
  assert.doesNotMatch(workspace, /Publicar producto/)
  assert.match(workspace, /0 publicaciones/)
  assert.match(dashboard, /CommercialTraceWorkspace/)
  assert.match(dashboard, /commercial-decision-loop/)
