...
test("workspace hydration uses a terminating GET preflight and never auto-prepares", () => {
  ...
  assert.match(page, /La aprobación humana anterior venció/)
  assert.match(page, /Preparar nueva autorización UNPUBLISHED/)
  assert.match(page, /prepareExactV3UnpublishedAuthorization/)
  assert.match(page, /unpublishedPreflightRetry/)
  assert.match(page, /AUTHORIZATION_PREFLIGHT_EXPIRED/)
  assert.match(page, /Preparar y autorizar sólo persiste el preflight/)
  assert.match(page, /Esta autorización no escribe todavía en eBay/)
  assert.match(page, /Reconciliar Inventory Item \+ Offer UNPUBLISHED/)
  assert.doesNotMatch(
    page,
...
test("legacy draft GET stays read-only and hydrates a completed final V3 review", () => {
  ...
  assert.match(contextLoader, /SAME_DAY_PUBLICATION_PACKAGE_NOT_READY/)
