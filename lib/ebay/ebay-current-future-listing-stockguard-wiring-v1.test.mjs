...
test("canonical Luna product linkage accepts Unicode handles and remains fail closed", () => {
  const unicodeUrl =
    "https://lunaportex.com/products/✨new✨-luxury-fashion-heart-shape-🎁"
  const accepted = evaluatePublishWithStockguardContractV1({
    sellerSku: "IMNOVA1", components: [component({
      canonicalLunaUrl: unicodeUrl,
    })], expectedComponentCount: 1, economicsReady: true,
    monitorEnrollmentIntentPrepared: true,
  })
  assert.equal(accepted.exactLunaLinkageReady, true)
  assert.equal(accepted.publishAllowed, true)

  for (const canonicalLunaUrl of [
    `${unicodeUrl}?variant=1`, `${unicodeUrl}#fragment`,
    `${unicodeUrl}/extra`, "https://competitor.example/products/exact-product",
    "http://lunaportex.com/products/exact-product",
    "https://lunaportex.com/products/has space",
  ]) {
    const rejected = evaluatePublishWithStockguardContractV1({
      sellerSku: "IMNOVA1", components: [component({ canonicalLunaUrl })],
      expectedComponentCount: 1, economicsReady: true,
      monitorEnrollmentIntentPrepared: true,
    })
    assert.equal(rejected.exactLunaLinkageReady, false, canonicalLunaUrl)
    assert.equal(rejected.publishAllowed, false, canonicalLunaUrl)
  }
})

test("CURRENT SQL linkage authority mirrors the Unicode-safe fail-closed scope", () => {
  const sql = readFileSync(new URL(
    "../../supabase/migrations/20260912235500_current_execution_unicode_luna_url_v1.sql",
    import.meta.url), "utf8")
  assert.match(sql, /read_current_publication_execution_contract_v1/)
  assert.match(sql, /\[\^\/\?#\[:space:\]\[:cntrl:\]\]\+\$/)
  assert.match(sql, /CURRENT_EXECUTION_CANONICAL_LINKAGE_UNPROVEN/)
  assert.match(sql, /SERVICE_ROLE_REQUIRED/)
  assert.match(sql, /CURRENT_PUBLICATION_DUPLICATE_IDENTITY_CONFLICT/)
  assert.doesNotMatch(sql, /\[A-Za-z0-9%\._~-\]\+/)
})
