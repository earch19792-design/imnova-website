...
test("server and UI use the V3 gate; prepare does not accept a browser revision id", () => {
  ...
  const finalGate = readFileSync("lib/ebay/final-listing-review-publication-gate.ts", "utf8")
  assert.match(route, /loadFinalListingReviewPublicationGate/g)
  assert.doesNotMatch(route, /loadVisualStrategyV3PublicationGate/g)
  assert.match(finalGate, /ebay_reference_guided_final_listing_review_previews/)
  assert.match(finalGate, /final_visual_set_locked/)
...
test("flag-false initial prepare is metadata-only and progressed attempts stay idempotent", () => {
  ...
  assert.match(route, /existingAttempt[\s\S]*composition_manifest_hash/)
  assert.match(route, /status: existingAttempt \? 200 : 202/)
  assert.match(route, /persistedManifestMatches/)
  assert.match(route, /initialPrepareInvariant/)
  assert.doesNotMatch(route, /REFERENCE_GUIDED_PREPARE_INVARIANT_FAILED/)
...
test("existing V3 attempt hides prepare and exposes the private human review", () => {
  const route = readFileSync("app/api/admin/ebay/images/route.ts", "utf8")
  const ui = readFileSync("app/admin/ebay/listing-workspace/page.tsx", "utf8")
  assert.match(ui, /!referenceGuidedAttemptId && <button/)
  assert.match(ui, /Revisión visual V3 · independiente de Luna/)
  assert.match(ui, /Contrato: 0 Portada principal \+ 1–6 secundarias/)
  assert.match(ui, /Canary rechazado · evidencia preservada/)
  assert.match(ui, /Secundaria 1 · variante MAIN/)
  assert.match(ui, /Secundaria 1 · variante determinista SIDE/)
  assert.match(ui, /Selección humana final registrada/)
  assert.match(ui, /referenceGuidedPositionOne\.output_preview_url/)
  assert.match(route, /createSignedUrl\(outputPath, 300\)/)
