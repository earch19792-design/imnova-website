...
test("relay allowlists only canonical Assistant Gateway tools and bounded arguments", async () => {
  ...
  assert.equal(SELLER_OS_ASSISTANT_TOOLS_V1.length, 14)
})
...
test("WhatsApp sale-alert status traverses the relay as a read without monitor or send", async () => {
  ...
  assert.match(result.TRACE_ID, /^[a-f0-9-]{36}$/)
  const { TRACE_ID, ...evidenceResult } = result
  assert.deepEqual(evidenceResult, expected)
...
test("cloud relay and complete MCP registry expose zero write-capable tools", () => {
  ...
  assert.equal(safety.registeredToolCount, 16)
  assert.equal(safety.assistantWriteTools, 0)
  assert.equal(safety.allToolsReadOnly, true)
  assert.deepEqual(safety.writeToolNames, [])
})

test("Preview traverses authenticated relay with exact Item ID and stable correlation, without writes", async () => {
  let reads = 0, seenTrace
  const executor = createSellerOsCloudReadRelayExecutorV1({
    environment: clientEnvironment(), now: () => NOW,
    fetcher: async (url, init) => handleSellerOsCloudReadRelayRequestV1(new Request(url, init), {
      environment: serverEnvironment(), now: () => NOW,
      previewCollector: async (itemId, traceId) => {
        reads++; assert.equal(itemId, "366643122092"); seenTrace = traceId
        return { status: "NEEDS_EVIDENCE", TRACE_ID: traceId,
          blockers: ["NO_DEFENSIBLE_PRIMARY_QUERY_CONCEPT"],
          safety: { persistenceUsed: false, marketplaceWrites: 0, providerCalls: 0 } }
      },
    }),
  })
  const result = await executor({ toolName: "seller_os_prepare_listing_optimization_preview", arguments: { itemId: "366643122092" } })
  assert.equal(reads, 1); assert.equal(result.TRACE_ID, seenTrace)
  assert.equal(result.status, "NEEDS_EVIDENCE"); assert.equal(result.safety.providerCalls, 0)
  await assert.rejects(executor({ toolName: "seller_os_prepare_listing_optimization_preview",
    arguments: { itemId: "366643122092", accountKey: "other" } }), /ARGUMENT_NOT_ALLOWLISTED/)
  assert.equal(reads, 1)
})

test("dedicated preprod relay requires every existing project/domain/runtime/staging signal", async () => {
  const dedicated = serverEnvironment({ VERCEL_ENV: "production", VERCEL_TARGET_ENV: "production",
    VERCEL: "1", VERCEL_PROJECT_ID: "prj_XvOpSg1jhmLLG1yOCFhAbiLEn222",
    VERCEL_PROJECT_PRODUCTION_URL: "imnova-seller-os-preprod.vercel.app",
    EBAY_PRO_RUNTIME: "staging", NEXT_PUBLIC_SUPABASE_URL: "https://vsfthqydfrdzulldbfbe.supabase.co" })
  let reads = 0
  const run = environment => handleSellerOsCloudReadRelayRequestV1(signedRequest(envelope({
    toolName: "seller_os_prepare_listing_optimization_preview", arguments: { itemId: "366643122092" },
  })), { environment, now: () => NOW, previewCollector: async () => {
    reads++; return { status: "NEEDS_EVIDENCE", safety: { marketplaceWrites: 0 } }
  } })
  assert.equal((await run(dedicated)).status, 200)
  assert.equal(reads, 1)
  for (const key of ["VERCEL_TARGET_ENV", "VERCEL", "VERCEL_PROJECT_ID", "VERCEL_PROJECT_PRODUCTION_URL",
    "EBAY_PRO_RUNTIME", "NEXT_PUBLIC_SUPABASE_URL"]) {
    assert.equal((await run({ ...dedicated, [key]: "wrong" })).status, 404, key)
  }
  assert.equal(reads, 1)
})

test("fee context relay requires exact item and rejects extra scope or write arguments", async () => {
  let calls = 0
  const options = { environment: serverEnvironment(), now: () => NOW,
    feeContextCollector: async (itemId) => { calls++; return { itemId, marketplaceWrites: 0 } },
    monitorLoader: async () => { throw Error("MONITOR_MUST_NOT_RUN") } }
  const base = { toolName: "seller_os_internal_read_fee_context" }
  for (const args of [{}, { itemId: "invalid" }, { itemId: "366650054490", limit: 20 },
    { itemId: "366650054490", account: "other" }, { itemId: "366650054490", write: true }]) {
    const response = await handleSellerOsCloudReadRelayRequestV1(signedRequest(envelope({ ...base, arguments: args })), options)
    assert.equal(response.status, 400)
  }
  assert.equal(calls, 0)
  const request = signedRequest(envelope({ ...base, arguments: { itemId: "366650054490" } }))
  const response = await handleSellerOsCloudReadRelayRequestV1(request, options)
  assert.equal(response.status, 200)
  assert.equal((await response.json()).result.itemId, "366650054490")
  assert.equal(calls, 1)
})

test('package fee context is exact, read-only, and cannot combine live and package identities',async()=>{
 let calls=0
 const options={environment:serverEnvironment(),now:()=>NOW,packageFeeContextCollector:async packageId=>{calls++;return {packageId,marketplaceWrites:0}},feeContextCollector:async()=>{throw Error('NO_LIVE_READ')}}
 const base={toolName:'seller_os_internal_read_fee_context'},packageId='24535b37-0335-4984-a36c-dbb73a7560da'
 for(const args of [{packageId:'bad'},{packageId,itemId:'366650054490'},{packageId,write:true},{packageId,account:'other'}])assert.equal((await handleSellerOsCloudReadRelayRequestV1(signedRequest(envelope({...base,arguments:args})),options)).status,400)
 assert.equal(calls,0)
 const response=await handleSellerOsCloudReadRelayRequestV1(signedRequest(envelope({...base,arguments:{packageId}})),options)
 assert.equal(response.status,200);assert.equal((await response.json()).result.packageId,packageId);assert.equal(calls,1)
})

test('publication preflight relay permits only exact read-only package scope',async()=>{
 let calls=0;const packageId='24535b37-0335-4984-a36c-dbb73a7560da',toolName='seller_os_internal_read_publication_preflight'
 const options={environment:serverEnvironment(),now:()=>NOW,publicationPreflightCollector:async id=>{calls++;return {packageId:id,marketplaceWrites:0}}}
 for(const args of [{},{packageId:'bad'},{packageId,itemId:'366650054490'},{packageId,write:true},{packageId,publish:true},{packageId,limit:1}])
  assert.equal((await handleSellerOsCloudReadRelayRequestV1(signedRequest(envelope({toolName,arguments:args})),options)).status,400)
 assert.equal(calls,0)
 const response=await handleSellerOsCloudReadRelayRequestV1(signedRequest(envelope({toolName,arguments:{packageId}})),options)
 assert.equal(response.status,200);assert.equal((await response.json()).result.packageId,packageId);assert.equal(calls,1)
})
