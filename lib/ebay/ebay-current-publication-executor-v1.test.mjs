...
test('Successful one-shot publish requires full active readback before durable confirmation',async()=>{
 ...
 f.deps.collection=async()=>({safe:true,status:f.p.listing_id?'PUBLISHED':'UNPUBLISHED',listingId:f.p.listing_id??null})
 f.deps.offer=async()=>({safe:true,offerId:'offer',listingId:id})
 f.deps.register=async()=>({registration:{id:'registration'},verification:{status:'verified',connectorListingStatus:'active',connectorListingId:'active',connectorEbaySku:'sku',
 ...
 const r=await execute(f.input,f.deps);assert.equal(r.pass,true);assert.equal(r.PUBLICATION_WRITE_COUNT??r.publicationWrites,1);assert.equal(r.OFFICIAL_READBACK_PASS,true)
