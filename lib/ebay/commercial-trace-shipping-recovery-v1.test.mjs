...
const receiptAuthority = readFileSync(
  "lib/ebay/commercial-trace-shipping-receipt-authority-v1.ts", "utf8")
...
test("receipt is fail-closed on exact Trace, identity, truth and freshness bindings", () => {
  ...
  assert.match(server, /certifyCommercialTraceShippingReceiptV1/)
  assert.match(receiptAuthority,
    /LIVE_LISTING_SHIPPING_MAXIMUM_AGE_SECONDS \* 1_000/)
  assert.match(receiptAuthority, /observed \+ maximumAgeMs <= now/)
  assert.match(server, /marketplaceWrites: 0/)
  assert.match(server, /noRawDestinationAddress: true/)
})

test("shipping dispatch does not depend circularly on a proven sale price", () => {
  assert.match(server, /const salePriceUsd = money\(priceRange\.median\)/)
  assert.doesNotMatch(server,
    /truth\.supplierSku !== supplierSku \|\| salePriceUsd === null/)
  assert.match(server, /salePriceUsd, supplierCostUsd, productName/)
})

test("Trace decision loop and certification use the canonical Product Truth gate", () => {
  assert.match(trace,
    /productTruthSufficient: productTruth\.gate\.traceProductTruthSufficient/)
  assert.match(trace,
    /market\.samplingSufficientBeforeDemandUnproven &&\s*productTruth\.gate\.traceProductTruthSufficient/)
})
