...
const safeActiveListingTitleFailureMessage = (code) =>
  code === "EBAY_ACTIVE_TITLE_REVISION_REJECTED_21919474"
    ? "eBay rechazó esta vía porque el listing se administra con Inventory. No se aplicó el cambio."
...
function exactLineage() {
  return {
    ...
    activeListings: [{ id: activeId, ebay_item_id: itemId, ebay_sku: sku,
      ...
      last_ebay_sync_at: "2026-08-28T19:39:14.265Z" }],
...
test("an exact owner authorization enables only the bound operator", () => {
  ...
  const authorization = { id: authorizationId, actor_user_id: operatorId,
    ...
    source_observed_at: "2026-08-28T19:39:14.265Z",
  ...
  const rejected = select({ authorizations: [{ ...authorization,
    phase: "terminal_failure",
    last_error_code: "EBAY_ACTIVE_TITLE_REVISION_REJECTED_21919474",
  }] })
  assert.equal(rejected.candidate.ownerApprovalStatus, "WRITE_FAILED")
  assert.equal(rejected.candidate.applyAvailable, false)
  assert.equal(rejected.candidate.failureCode,
    "EBAY_ACTIVE_TITLE_REVISION_REJECTED_21919474")
  assert.match(rejected.candidate.failureMessage, /Inventory/)
})
...
test("owner approval digest uses durable listing observation across requests", () => {
  const first = select()
  const laterRequest = select({ commercialExceptions: [canonicalSignal({
    lastObservationTime: "2026-09-03T00:31:53.000Z",
  })] })
  assert.equal(first.candidate.observedAt, "2026-08-28T19:39:14.265Z")
  assert.equal(laterRequest.candidate.observedAt, first.candidate.observedAt)
  assert.equal(laterRequest.candidate.authorizationDigest,
    first.candidate.authorizationDigest)
  const lineage = exactLineage()
  lineage.activeListings[0].last_ebay_sync_at = null
  assert.equal(select({ ...lineage }), null)
})
...
test("storage and route preserve owner authority, one write, and official readback", () => {
  ...
  assert.match(page, /ownerApprovalStatus === "WRITE_FAILED"/)
  assert.match(route, /result\.safeFailureMessage/)
