...
test("temporary route/page are isolated and never import mutable OAuth or writers", () => {
  ...
  assert.ok(route.indexOf("previewEbayRegistryRepairRuntime({") <
    route.indexOf("getSupabaseAdminClient()"))
  assert.match(route,
    /previewEbayRegistryRepairRuntime\(\{\s*startedAt: requestStartedAt,\s*fetchImpl,\s*reviewedEvidenceFingerprint: typeof reviewedEvidenceFingerprint === "string"\s*\?\s*reviewedEvidenceFingerprint\s*:\s*null,\s*\}\)/)
  assert.match(route,
