...
test("Analytics probe distinguishes endpoint availability from authoritative scope failure", async () => {
  const cases = [
    {
      status: 429,
      payload: { errors: [{ message: PRIVATE_DESCRIPTION }] },
      code: "ANALYTICS_ENDPOINT_RATE_LIMITED",
      notCode: "ANALYTICS_SCOPE_UNAVAILABLE",
    },
    {
      status: 503,
      payload: { errors: [{ message: PRIVATE_DESCRIPTION }] },
      code: "ANALYTICS_ENDPOINT_SERVER_UNAVAILABLE",
      notCode: "ANALYTICS_SCOPE_UNAVAILABLE",
    },
    {
      status: 403,
      payload: { errors: [{ message: "access denied" }] },
      code: "ANALYTICS_ENDPOINT_ACCESS_DENIED",
      notCode: "ANALYTICS_SCOPE_UNAVAILABLE",
    },
    {
      status: 403,
      payload: { error: "insufficient_scope" },
      code: "ANALYTICS_SCOPE_UNAVAILABLE",
    },
  ]
  for (const testCase of cases) {
    ...
    assert.match(thrown?.message ?? "", new RegExp(testCase.code))
    if (testCase.notCode) {
      assert.doesNotMatch(thrown?.message ?? "", new RegExp(testCase.notCode))
    }
    ...
    assert.doesNotMatch(thrown?.message ?? "", new RegExp(PRIVATE_DESCRIPTION))
  }
})

test("installed-token certification uses corrected Analytics endpoint classification", async () => {
  const previous = process.env.EBAY_SELLER_REFRESH_TOKEN
  process.env.EBAY_SELLER_REFRESH_TOKEN = PRIVATE_REFRESH
  try {
    for (const testCase of [
      { status: 429, code: "ANALYTICS_ENDPOINT_RATE_LIMITED" },
      { status: 502, code: "ANALYTICS_ENDPOINT_SERVER_UNAVAILABLE" },
    ]) {
      const fake = successfulCandidateFetch({
...
test("authoritative Analytics scope failure prevents OAuth candidate handoff", async () => {
  const ledger = new AtomicMemoryLedger()
  const stateHash = hashEbaySellerOAuthReauthState(STATE)
  ledger.rows.set(stateHash, {
    status: "PENDING",
  ...
  const fake = successfulCandidateFetch({
    analyticsResponse: () => new Response(
...
test("temporary route/page isolate generic OAuth and purpose-bind encrypted Commercial handoff", () => {
  ...
  assert.equal((route.match(/ebaySellerOAuthReauthCookieOptions\(0\)/g) ?? []).length, 3)
