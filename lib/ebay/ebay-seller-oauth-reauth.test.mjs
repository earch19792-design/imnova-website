...
test("temporary route/page isolate generic OAuth and purpose-bind encrypted Commercial handoff", () => {
  ...
  assert.match(route,
    /function clearOAuthStateCookies[\s\S]*EBAY_SELLER_OAUTH_REAUTH_COOKIE[\s\S]*EBAY_MARKETING_READONLY_OAUTH_COOKIE[\s\S]*ebaySellerOAuthReauthCookieOptions\(0\)/)
  assert.equal((route.match(/clearOAuthStateCookies\(response\)/g) ?? []).length, 4)
