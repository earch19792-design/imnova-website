...
test("temporary route/page are isolated and never import mutable OAuth or writers", () => {
  ...
  const runtime = [route, page, core, domain, ledger].join("\n")
  ...
  assert.ok(callbackCredentialGate < callbackRoute.indexOf("request.cookies.getAll"))
  assert.ok(callbackCredentialGate < callbackRoute.indexOf("getSupabaseAdminClient()"))
  assert.ok(callbackCredentialGate <
    callbackRoute.indexOf("claimAndVerifyEbaySellerOAuthReauth({"))
