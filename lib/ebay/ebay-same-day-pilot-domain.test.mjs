...
test("automation is durable, one-click, auto-resuming and has no eBay-write path", async () => {
  ...
  assert.match(home, /productResearchGuidance\?\.nextQuery\?\.searchQuery/)
  assert.match(home, /canonicalNextQuery \|\| String\(schema\.query/)
  assert.match(home, /extensión aplica sola la consulta/)
  assert.match(home, /“Aplicar y buscar” queda sólo como fallback/)
  ...
  assert.match(bootstrapRepair, /state\.candidates\.find/)
