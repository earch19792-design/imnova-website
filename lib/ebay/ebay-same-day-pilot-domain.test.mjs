...
test("automation is durable, one-click, auto-resuming and has no eBay-write path", async () => {
  const [migration, hardening, stateHardening, leasePreflight, claimSerialization, atomicGates, candidateCycles, service, home, capture, cron, workflow, route, handoff] = await Promise.all([
  ...
  assert.match(home, /Las posteriores permanecen ordenadas/)
  ...
  assert.match(home, /guidedQuery \|\| durableTaskQuery/)
  ...
  assert.match(home, /openTasks\.length \? "ESPERANDO TU CONFIRMACIÓN"[\s\S]*quotaPaused \? "PAUSADO POR EBAY"/)
  ...
  assert.match(reconciliationWorker, /decisionObservationIdSet\.has\(text\(result\.observationId\)\)/)
  ...
  assert.match(bootstrapRepair, /if \(activeState\.tasks\.some\(\(task\) => task\.status === "OPEN"\)\) return serialized\.superseded > 0/)
...
test("human gates are serialized globally across candidates and quota promotion is evidence-bound", async () => {
  const service = await readFile("lib/ebay/ebay-same-day-pilot-service.ts", "utf8")
  ...
  assert.match(serializer, /openTasks\.slice\(1\)/)
  ...
  assert.match(createTask, /before\.openTask\.candidate_id !== input\.candidateId/)
  assert.match(createTask, /before\.openTask\.gate_type !== input\.gateType/)
  assert.match(createTask, /after\.openTask\.id === text\(data\)/)
  ...
  assert.match(normalPromotion, /if \(serialized\.openTask\) return false/)
  ...
  assert.match(quotaPromotion, /if \(serialized\.openTask\) return false/)
