...
test("automation is durable, one-click, auto-resuming and has no eBay-write path", async () => {
  ...
  assert.match(service, /controlledExploratoryLunaConfirmationNeedsRefresh/)
  assert.match(service, /routeStaleControlledLunaConfirmationToGate/)
  ...
  assert.match(service, /waitingFor: "FRESH_LUNA_CONFIRMATION"/)
  assert.match(service, /LUNA_CONFIRMATION_STALE/)
  assert.match(service, /STALE_CONTROLLED_LUNA_CONFIRMATION_RECOVERED/)
  assert.match(service,
    /PRODUCT_FACT_CONTROLLED_EXPLORATORY_TARGET_INVALID/)
  assert.match(service, /STALE_LUNA_CONFIRMATION_RECOVERED/)
  assert.match(service, /recoveredWithoutRejection: true/)
  assert.match(service,
