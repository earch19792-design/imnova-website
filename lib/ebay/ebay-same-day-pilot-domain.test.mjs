...
test("the next cycle excludes attempts from existing active listings by supplier SKU and product ID", () => {
  const selected = selectSameDayQueue([
    candidate(101, {
      candidateKey: "attempt-sku-match",
      supplierVariantId: "variant-101",
      supplierSku: "ITEM-5126",
      marketRadarProductId: "product-5126",
      productTitle: "Test Product",
      brand: "Acme",
      mpn: "M-1",
    }),
    candidate(102, {
      candidateKey: "candidate-ok",
      supplierVariantId: "variant-102",
      supplierSku: "ITEM-9999",
      marketRadarProductId: "product-9999",
      productTitle: "Second Product",
      brand: "Acme",
      mpn: "M-2",
    }),
  ], new Date(), {
    supplierSkus: ["ITEM-5126"],
    marketRadarProductIds: ["product-5126"],
  })
  assert.deepEqual(selected.map((row) => row.supplierSku), ["ITEM-9999"])
})
...
test("publication in-flight states are not reactivated as bootstrap candidates", async () => {
  const service = await readFile("lib/ebay/ebay-same-day-pilot-service.ts", "utf8")
  assert.match(service, /const SAME_DAY_NON_RESTARTABLE_MACHINE_STATES = \[/)
  assert.match(service, /WAITING_ITEM_ID/)
  assert.match(service, /VERIFYING_PUBLISHED_LISTING/)
  assert.match(service, /REGISTERING_COMMERCIAL_MONITOR/)
  assert.match(service, /isNonRestartableMachineState\(machineState\)/)
  assert.match(service,
    /const active = activeState\.candidates\.find\([\s\S]{0,240}isNonRestartableMachineState\(machineState\)/)
  assert.match(service,
    /const active = rows\.find\(\(row\) =>\s*!\s*isNonRestartableMachineState\(row\.machine_state\)\)/)
})
