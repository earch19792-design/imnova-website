...
test("delegation enablement uses global authority predicates, not one listing's execution state", () => {
  const result = contract.buildMayelFullVisualDelegationPredicatesV1({
    ownerAuthenticated: true,
    workspaceReady: true,
    accountIdentityProven: true,
    delegationScopeValid: true,
    sellerOsExecutorReady: true,
    managementModelProven: false,
    currentOfficialImageSetProven: false,
    assetManifestValid: false,
    officialReadbackCapabilityReady: false,
  })
  assert.equal(result.buttonEnabled, true)
  assert.equal(result.firstBlockingPredicate, null)
  assert.equal(result.predicates.find((entry) =>
    entry.code === "MANAGEMENT_MODEL_PROVEN").requiredForDelegation, false)
})
