...
test("API, UI and migration preserve owner-only durable delegation with zero marketplace writes", () => {
  ...
  assert.doesNotMatch(ui,
    /La decisión final pertenece al owner/)
  assert.match(ui,
  ...
  assert.match(route, /identityFailureClass: delegation\.identityFailureClass/)
  assert.match(server, /readCanonicalEbayAccountIdentityAuthorityV1/)
  assert.match(server, /get_ebay_account_policy_readonly_refresh_token_v1/)
  assert.match(server, /EBAY_ACCOUNT_POLICY_OAUTH_VAULT_READ_FAILED/)
  assert.match(server, /FRESH_VERIFIED_EBAY_ACCOUNT_POLICY_PROFILE_V1/)
  assert.match(server, /EBAY_COMMERCIAL_ACCOUNT_IDENTITY_UNAVAILABLE/)
  assert.match(server, /if \(!liveReadUnavailable \|\| !durableVerifiedAt\) throw error/)
