...
test("legacy partial aggregate cannot reject complete preserved-frame evidence", () => {
  const preserved = happyPath({
    qa: {
      ...
      outputWidth: 1600,
      outputHeight: 1600,
      outputEdgeWhiteRatio: 1,
      exactSourceHashRecorded: true,
      generativeChangesMade: false,
      fullAuthorizedFramePreserved: true,
      sourceCenterChromaticRatio: .25,
      sourceVisualProfile: { productToneRisk: "STANDARD" },
    },
  })
  assert.equal(preserved.passed, true)
  assert.equal(preserved.outputQualityPass, true)
  assert.equal(preserved.materialProductEquivalencePass, true)
  const qa = automaticLunaImageQaResultV1({
    qa: { automaticStatus: "PARTIAL" },
    rights: rights(),
    automatic: preserved,
  })
  assert.equal(qa.automaticStatus, "PASSED")
})

test("one genuinely low-quality image is excluded and zero remains blocking", () => {
...
test("automatic image materialization recomputes package preparation", async () => {
  const { listingPackagePreparationReadinessV1 } = await import(
    "./luna-supplier-image-auto-runtime-v1.ts"
  )
  const base = {
    title: "Window Privacy Film 23.6 in x 9.84 ft",
    categoryId: "175757",
    description: "Verified product-truth description.",
    pricing: { targetPrice: 24.99 },
  }
  assert.equal(listingPackagePreparationReadinessV1({
    ...base, imageUrls: [],
  }), 80)
  assert.equal(listingPackagePreparationReadinessV1({
    ...base, imageUrls: ["https://example.com/durable.jpg"],
  }), 100)
})
