...
test("hashed durable sold evidence without an eBay item ID is not mislabeled primary", () => {
  const base = classified("durable-hash", null, 100, "Wireless Microphone")
  const result = resolveTopSellerMarketplaceFieldMappingV1({
    classifiedCandidates: [{ ...base, candidate: { ...base.candidate,
      itemId: null, candidateReference: `sha256:${"a".repeat(64)}` } }],
    residualSpecificNames: ["Type"],
    exactEvidence: [{ sourceField: "TITLE",
      text: "wireless clip-on microphone" }], existingProductTruth: {},
  })
  assert.equal(result.primaryReference, null)
  assert.equal(result.topReferenceSet.length, 1)
})
