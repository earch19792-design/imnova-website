...
test('official unavailable offer is exact 25713; arbitrary errors stay unproven',()=>{
 ...
 const physical=offer(25713);physical.body.errors[0].category='Request'
 assert.equal(classifyEbayListingManagementModelEvidenceV1({...base,offers:physical}).managementModel,'TRADING_MANAGED')
...
test('full-gallery readiness uses the slot contract, without weakening the historical one-to-two canary',async()=>{
 const {slotManifestMatchesGalleryV1}=await import('./mayel-gallery-slot-policy-v1.ts')
 const m=rebase().manifest;assert.equal(slotManifestMatchesGalleryV1(m,current),true)
 assert.equal(slotManifestMatchesGalleryV1({...m,proposedOrderedImages:m.proposedOrderedImages.slice(0,2)},current),false)
 const reordered=[...m.proposedOrderedImages];[reordered[1],reordered[2]]=[reordered[2],reordered[1]]
 assert.equal(slotManifestMatchesGalleryV1({...m,proposedOrderedImages:reordered},current),false)
})
test('a verified own sync is not gallery drift; a later external change still requires rebase',async()=>{
 const {galleryMatchesSyncReceiptV1}=await import('./mayel-gallery-slot-policy-v1.ts')
 const receipt={state:'SYNCED',official_readback:true,intent:{requestedChanges:{taskId}},binding:{executionManifestDigest:'new'},execution_receipt:{manifestDigest:'new',officialDigest:'after'}}
 const input={taskId,manifestDigest:'new',currentGalleryDigest:'after',receipts:[receipt]}
 assert.equal(galleryMatchesSyncReceiptV1(input),true)
 assert.equal(galleryMatchesSyncReceiptV1({...input,currentGalleryDigest:'external-change'}),false)
 assert.equal(galleryMatchesSyncReceiptV1({...input,manifestDigest:'next-generation'}),false)
 assert.equal(galleryMatchesSyncReceiptV1({...input,receipts:[{...receipt,official_readback:false}]}),false)
})
