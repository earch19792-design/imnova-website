...
test("gates técnicos, consentimiento referenciado, Preview, staging, rama e identidad bloquean el adapter real", () => {
  ...
  const compliance = readFileSync(new URL("./ebay-fulfillment-policy-compliance.ts", import.meta.url), "utf8")
  for (const flag of [
    ...
    "EBAY_FULFILLMENT_WRITTEN_CONSENT_ENABLED",
    "EBAY_FULFILLMENT_WRITTEN_CONSENT_REFERENCE_HASH",
  ]) assert.match(`${oauth}\n${compliance}`, new RegExp(flag))
  ...
  assert.match(oauth, /EBAY_FULFILLMENT_WRITTEN_CONSENT_REQUIRED/)
...
test("autorización humana exige todos los gates de escritura OFF y readiness no ejecuta Orders ni writes", () => {
  const authorization = readFileSync(new URL("./ebay-fulfillment-tracking-oauth-authorization.ts", import.meta.url), "utf8")
  const oauth = readFileSync(new URL("./ebay-fulfillment-tracking-oauth.ts", import.meta.url), "utf8")
...
test("Seller Command Center muestra conexión, estados, botón y resumen sanitizado", () => {
  ...
  assert.match(panel, /MANUAL_SELLER_HUB_TRACKING_REQUIRED/)
  assert.match(panel, /no almacena documentos ni PII/)
...
test("no cron V1B, no PII/secretos y worker real sólo se selecciona si executable", () => {
  ...
  const readiness = service.slice(
    service.indexOf("export function getMarketplaceFulfillmentV1BReadiness"),
    service.indexOf("export async function approveMarketplaceFulfillmentTrackingReal"),
  )
  ...
  assert.match(service, /writtenConsentReferenceHash/)
  assert.match(service, /writtenConsentReferenceBound: true/)
  assert.match(service, /writtenConsentReferenceExposed: false/)
  assert.doesNotMatch(readiness, /writtenConsentReferenceHash/)
