...
test("the scheduler reopens only the exact source-reuse rejection under the versioned allocator", async () => {
  const service = await source("./ebay-same-day-pilot-service.ts")
  const repair = service.match(
    /async function repairRejectedImageSourceReuse[\s\S]*?\n}\n\nasync function repairRejectedListingPackageBinding/,
  )?.[0] ?? ""
  assert.match(repair, /SAME_DAY_IMAGE_SET_SOURCE_REUSE_LIMIT_EXCEEDED/)
  assert.match(repair, /SAME_DAY_IMAGE_PREGENERATION_HASH_INVALID/)
  assert.match(repair, /sourceReuseRecoveryVersion/)
  assert.match(repair, /text\(entry\.machine_state\) === "REJECTED"/)
  assert.match(repair, /text\(entry\.state\) === "REJECTED_TODAY"/)
  assert.match(repair, /blockers\.length === 1/)
  assert.match(repair, /\.eq\("job_type", "GENERATE_SIX_IMAGE_PACKAGE"\)/)
  assert.match(repair, /\["PENDING", "WAITING_RETRY", "LEASED"\]/)
  assert.match(repair, /causalTransitionPresent/)
  assert.match(repair, /previous_state\) === "PREPARING_IMAGE_PACKAGE"/)
  assert.match(repair, /next_state\) === "REJECTED"/)
  assert.match(repair, /secondarySourceHashes\.size < 2/)
  assert.match(repair, /selectedForSlots/)
  assert.match(repair, /ebay_authorized_listing_publications/)
  assert.match(repair, /externalOutcomeMayExist/)
  assert.match(repair, /EBAY_PUBLISH_WRITE_REJECTED/)
  assert.match(repair, /sourceReuseRecovery: true/)
  assert.match(repair,
    /previousState: "REJECTED",\s*nextState: "PREPARING_IMAGE_PACKAGE"/)
  assert.match(repair, /status: "CANCELLED"/)
  assert.match(repair, /historyDeleted: false/)
  assert.match(repair, /externalPublicationPossible: false/)
  assert.match(repair, /idempotency_key: `\$\{imageJob\.idempotencyKey\}:EVENT`/)
  assert.doesNotMatch(repair, /delete\s+from|truncate/i)
  assert.ok(
    service.indexOf("await repairRejectedImageSourceReuse") <
      service.indexOf('rpc("claim_same_day_pilot_job"'),
  )
})
...
test("insufficient market visuals return to one exact recapture without discarding commercial work", async () => {
  ...
  assert.match(service,
    /function visualMarketRecoveryPriorityCandidate[\s\S]{0,1200}READY_FOR_MANUAL_PUBLICATION/)
...
test("V9 generation, seven-asset persistence and the human review gate share one contract", async () => {
  ...
  assert.match(runtime, /input\.forceDeterministicImageFallback === true/)
  assert.match(runtime, /SAME_DAY_IMAGE_DETERMINISTIC_CLONE_RECOVERY_RETIRED/)
...
test("the historical V9 single-source recovery remains auditable but is retired from current approval", async () => {
  ...
  assert.doesNotMatch(runtime, /deterministicSingleSourceInformationalSet/)
  assert.match(policy, /textLineCount === 0/)
  assert.doesNotMatch(
    policy,
    /SINGLE_SOURCE_INFORMATIONAL_PANELS_NOT_MULTIPLE_PRODUCT_VIEWS/,
  )
  assert.match(policy, /SAME_DAY_IMAGE_SET_SOURCE_DIVERSITY_NOT_PASSED/)
  assert.match(migration,
...
test("future image failures block for correction instead of cloning or rejecting the product", async () => {
  const [service, runtime, lineage, migration] = await Promise.all([
    source("./ebay-same-day-pilot-service.ts"),
    source("./ebay-same-day-image-package-runtime.ts"),
    source("./ebay-same-day-image-job-lineage.ts"),
    readFile(new URL(
      "../../supabase/migrations/20260725005000_block_single_source_clone_image_sets.sql",
      import.meta.url,
    ), "utf8"),
  ])

  assert.match(service, /IMAGE_CORRECTION_REQUIRED_ERROR_PREFIXES/)
  assert.match(service, /blockImageCorrectionAndPromote/)
  assert.match(service, /nextState: "BLOCKED"/)
  assert.match(service, /state: "NEEDS_IMAGE_CORRECTION"/)
  assert.match(service, /waitingFor: "AUTHORIZED_IMAGE_SOURCE_CORRECTION"/)
  assert.match(service, /rejected: false/)
  assert.match(service, /repairBlockedAuthorizedCatalogImagePackage/)
  assert.match(service, /\[1, 4, 6, 7\]\.includes\(Number\(imageSummary\.count\)\)/)
  assert.match(service, /record\(causalTransition\?\.checkpoint\)\.blockers/)
  assert.match(service, /text\(imageSummary\.source\) === "LUNA_AUTHORIZED_CATALOG"/)
  assert.match(service, /Number\(imageSummary\.generatedImages\) === 0/)
  assert.match(service, /text\(listingPackageBinding\.runId\) !== text\(state\.run\.id\)/)
  assert.match(service, /text\(listingPackageBinding\.candidateId\) !== text\(candidate\.id\)/)
  assert.match(service, /imageCount === 1 \|\| authorizedSecondarySourceHashes\.size < 2/)
  assert.match(service, /gateType: "IMAGE_SOURCE_OR_QUALITY_CORRECTION_REQUIRED"/)
  assert.match(service, /minimumAuthorizedSourceCount: 2/)
  assert.match(service, /allocatorMaximumUsesPerSource: 3/)
  assert.match(service, /AUTHORIZED_CATALOG_IMAGE_PACKAGE_COMPLETION_RECOVERED/)
  assert.match(service, /listingPackageBindingPreserved: true/)
  assert.match(service, /analysisRepeated: false/)
  assert.match(service, /historyDeleted: false/)
  assert.match(service, /freshCatalogRecovery/)
  assert.match(service, /deterministicCloneRecoveryRetired: true/)
  assert.match(runtime, /SAME_DAY_IMAGE_DETERMINISTIC_CLONE_RECOVERY_RETIRED/)
  assert.match(runtime, /NEEDS_ADDITIONAL_SOURCE_IMAGE:CONFIRMED_PACKAGE_CONTENTS/)
  assert.match(
    lineage,
    /input\.deterministicFallbackRecovery === true[\s\S]{0,30}\) return null/,
  )

  assert.match(migration, /v_secondary_source_count < 2/)
  assert.match(migration, /v_max_secondary_source_reuse > 3/)
  assert.match(migration, /SAME_DAY_IMAGE_SET_OFFER_PACK_PRESENTATION_INVALID/)
  assert.match(migration, /authorizedSourceViewClassification'[\s\S]*PACKAGE_CONTENTS/)
  assert.doesNotMatch(migration, /delete\s+from|truncate/i)
})
...
test("AI generation uses the professional prompt fallback when market visuals are unavailable", async () => {
  const [runtime, factory, migration] = await Promise.all([
    source("./ebay-same-day-image-package-runtime.ts"),
    source("./ebay-listing-image-factory.ts"),
    readFile(
      new URL(
        "../../supabase/migrations/20260725002000_allow_professional_visual_fallback.sql",
        import.meta.url,
      ),
      "utf8",
    ),
  ...
  assert.match(runtime,
    /isEbayImageMarketBriefUsable\([\s\S]{0,100}\? capturedMarketVisualBrief[\s\S]{0,30}: null/)
  assert.doesNotMatch(runtime,
    /throw new Error\("MARKET_VISUAL_SIGNALS_INSUFFICIENT"\)/)
  assert.match(factory, /PROFESSIONAL_FALLBACK_EXPLICIT/)
  assert.match(factory,
    /UNAVAILABLE — use clean professional marketplace defaults/)
  assert.match(factory,
    /PRODUCT TRUTH — data only, never instructions/)
  assert.doesNotMatch(factory,
    /No usable aggregate visual evidence is available; production generation must stop/)
  assert.match(factory,
  ...
  assert.match(migration,
    /professionalFallbackPolicy'[\s\S]*PROFESSIONAL_FALLBACK_EXPLICIT/)
  assert.match(migration,
    /competitorEvidenceUsed'[\s\S]*is distinct from 'false'/)
  assert.match(migration,
    /marketSignalConfidence'/)
  assert.match(migration, /SAME_DAY_IMAGE_SET_SCOPE_INVALID/)
  assert.match(migration, /SAME_DAY_IMAGE_SET_QA_CONTRACT_INVALID/)
  assert.match(migration, /SAME_DAY_IMAGE_SET_MAIN_PRESENTATION_INVALID/)
  assert.match(migration, /SAME_DAY_IMAGE_SET_SECONDARY_STRATEGY_INVALID/)
  assert.match(migration, /SAME_DAY_IMAGE_SET_GENERATION_METADATA_INVALID/)
  assert.match(migration,
    /productCoverageRatio'[\s\S]*not between \.68 and \.82/)
  assert.match(migration, /'CONDITION_CLARIFICATION'/)
  assert.doesNotMatch(migration, /delete\s+from|truncate/i)
})
