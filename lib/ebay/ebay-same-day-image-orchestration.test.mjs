...
test("insufficient market visuals return to one exact recapture without discarding commercial work", async () => {
  ...
  assert.match(recovery,
    /for \(const candidate of candidates\)[\s\S]{0,2200}continue[\s\S]{0,2200}return 1/)
  ...
  assert.match(service,
    /priorityVisualRecovery[\s\S]{0,350}Number\(candidate\.ordinal\) > Number\(priorityVisualRecovery\.ordinal\)/)
