...
test("real Registry sync-key collision fails CREATE absence closed", () => {
  ...
  row.sync_key =
    `EBAY_SELL_INVENTORY_READONLY:${ACCOUNT_KEY}:${live.itemId}:${live.sku}`
  const result = build(
...
test("unknown repair lifecycle is a precondition, not identity ambiguity", () => {
  const fixture = certifiedFixture()
  ...
  assert.equal(result.REPAIR_PRECONDITION_STATUS, "UNPROVEN")
  assert.equal(result.LIFECYCLE_UNPROVEN_ACTION, "REPAIR_EXISTING")
  assert.equal(result.LIFECYCLE_UNPROVEN_STAGE,
    "EXISTING_REGISTRY_ROW_ACTIVE_GUARD")
  assert.equal(result.LIFECYCLE_REQUIRED_SIGNAL, "LISTING_STATUS_ACTIVE")
  assert.equal(result.LIFECYCLE_SIGNAL_AVAILABLE, "YES")
  assert.equal(result.LIFECYCLE_FAILURE_CAUSE, "REGISTRY_ROW_NOT_ACTIVE")
  assert.equal(result.OTHER_SUBTYPE_LIFECYCLE_REQUIREMENT_COUNT, 1)
  assert.equal(result.UNPROVEN_REASON_OTHER, 0)
  assert.equal(result.FINAL_IDENTITY_UNPROVEN_COUNT, 0)
  assert.equal(result.FINAL_PRECONDITION_UNPROVEN_COUNT, 1)
  assert.equal(result.AMBIGUITY_CLASS, "REVIEWABLE_ONLY")
...
test("exact protected paused draft and ended rows may reactivate by Item ID CAS", () => {
...
test("reactivation fails closed when the scoped sync key suffix is absent", () => {
  const fixture = certifiedFixture()
  fixture.registryRows[0].listing_status = "paused"
  fixture.registryRows[0].source = "EBAY_SELL_INVENTORY_READONLY"
  fixture.registryRows[0].sync_key =
    `EBAY_SELL_INVENTORY_READONLY:${ACCOUNT_KEY}:`
  const result = build(fixture)
  assert.equal(result.RAW_REPAIR_EXISTING_COUNT, 1)
  assert.equal(result.REPAIR_PRECONDITION_STATUS, "UNPROVEN")
  assert.equal(result.REACTIVATION_CAS_SUPPORTED, "NO")
