...
const { resolveRadarCanonicalMarketplaceReadinessV1 } = await import(
...
function supabase(profileOverrides = {}) {
  const profile = {
    account_key: ACCOUNT_KEY,
  ...
  return { from(table) {
    assert.equal(table, "ebay_account_policy_profiles")
    return query
  } }
}
...
function taxonomy() {
  ...
  const aspects = [aspect("Brand"), aspect("Style"), aspect("Type")]
  return {
    status: "AVAILABLE",
    categoryTreeId: "0",
...
test("legacy package aspects never fill required specifics without exact Product Truth", async () => {
  const result = await resolveRadarCanonicalMarketplaceReadinessV1({
    supabase: supabase(),
    accountKey: ACCOUNT_KEY,
    opportunity: opportunity(),
  ...
  assert.equal(result.evidence.requiredItemSpecificsSatisfied, 0)
  assert.deepEqual(result.evidence.unsupportedRequiredSpecifics,
    ["Brand", "Style", "Type"])
