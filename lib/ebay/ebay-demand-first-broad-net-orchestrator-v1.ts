...
import { getEbayListingIdentityByLegacyItemId,
  runEbaySellerKeywordDemandValidation } from
...
import { buildRadarRevenueFactoryCandidateBatchV1,
  collectRadarRevenueFactoryCandidateBatchV1 } from
...
export type SellerOsDemandFirstFamilyCandidateV1 = Readonly<{
  status: "QUALIFIED" | "REFRESH_EXISTING_FAMILY" | "DUPLICATE" | "UNQUALIFIED"
  reason: string
...
export type SellerOsDemandFirstFamilyEvaluationV1 = Readonly<{
  ...
  existingFamilyRefreshCandidates: number
...
type ExistingFamilyRefreshAuthorityV1 = Readonly<{
  familyId: string
...
function safeStringArray(value: unknown, maximum: number) {
  return Object.freeze((Array.isArray(value) ? value : []).flatMap((entry) => {
    const item = text(entry, 160)
    return item ? [item] : []
  }).slice(0, maximum))
}

function existingFamilyRefreshAuthorityV1(value: unknown) {
  const row = record(value)
  const familyId = text(row.family_id ?? row.familyId, 120)
  const opportunityCaseId = text(
    row.opportunity_case_id ?? row.opportunityCaseId, 120)
  const familyName = text(row.family_name ?? row.familyName, 160)
  const observation = record(row.current_observation ?? row.currentObservation ??
    rows(row.observationSeries)[0])
  const observationId = text(observation.observation_id ??
    observation.observationId, 120)
  const evidenceObservedAt = instant(observation.evidence_observed_at ??
    observation.evidenceObservedAt)
  const maximumAgeSeconds = integer(observation.maximum_age_seconds ??
    observation.maximumAgeSeconds)
  const demandEvidenceDigest = text(observation.demand_evidence_digest ??
    observation.demandEvidenceDigest, 80)
  const enrollments = rows(row.monitor_enrollments ?? row.monitorEnrollments)
  const enrollment = enrollments.find((entry) => text(entry.last_observation_id ??
    entry.lastObservationId, 120) === observationId) ??
    record(row.current_enrollment ?? row.currentEnrollment ?? enrollments[0])
  const lastObservationId = text(enrollment.last_observation_id ??
    enrollment.lastObservationId, 120)
  ...
  const nextEligibleReviewAt = instant(enrollment.next_eligible_review_at ??
    enrollment.nextEligibleReviewAt)
  let familyIdentity: SellerOsMarketFamilyIdentityV1
  try {
    familyIdentity = normalizeSellerOsMarketFamilyIdentityV1(
      record(row.family_identity ?? row.familyIdentity) as
        SellerOsMarketFamilyIdentityV1)
  } catch {
    return null
  }
  ...
  return Object.freeze({ familyId, opportunityCaseId, familyName,
    familyIdentity,
    familyQuerySet: safeStringArray(row.current_family_query_set ??
      row.currentFamilyQuerySet, 16),
...
function isExistingFamilyRefreshDueV1(input: Readonly<{
  authority: ExistingFamilyRefreshAuthorityV1
  evaluatedAt: string
}>) {
  const now = Date.parse(input.evaluatedAt)
  const stale = Date.parse(input.authority.evidenceObservedAt) +
    input.authority.maximumAgeSeconds * 1000 < now
  const explicitlyDue = input.authority.nextEligibleReviewAt !== null &&
    Date.parse(input.authority.nextEligibleReviewAt) <= now
  return input.authority.monitorStatus === "ENROLLED" &&
    input.authority.lastObservationId === input.authority.observationId &&
    (explicitlyDue || (input.authority.nextReviewCondition ===
      "TIME_WINDOW_ELAPSED" && stale))
}

function isExistingFamilyRefreshEnrollmentRecoveryV1(
  authority: ExistingFamilyRefreshAuthorityV1) {
  return authority.monitorStatus === "ENROLLED" &&
    authority.lastObservationId !== authority.observationId
}

function exactQueryLineageMatchV1(input: Readonly<{
  clusterQueries: readonly string[]
  familyQuerySet: readonly string[]
}>) {
  const existing = new Set(input.familyQuerySet.map(normalizedFamilyClusterKey))
  return input.clusterQueries.some((query) =>
    existing.has(normalizedFamilyClusterKey(query)))
}

function candidateRank(left: SellerOsDemandFirstFamilyCandidateV1,
  right: SellerOsDemandFirstFamilyCandidateV1) {
  const priority = { REFRESH_EXISTING_FAMILY: 0, QUALIFIED: 1,
    DUPLICATE: 2, UNQUALIFIED: 3 } as const
...
export function evaluateSellerOsDemandFirstFamilyCandidatesV1(input: Readonly<{
  ...
  const evaluatedAt = instant(input.evaluatedAt) ?? new Date().toISOString()
  const existingRows = rows(input.existingCases)
  const refreshAuthorities = existingRows.flatMap((row) => {
    const authority = existingFamilyRefreshAuthorityV1(row)
    return authority ? [authority] : []
  })
  const exactExisting = new Map<string, ExistingFamilyRefreshAuthorityV1>(
    refreshAuthorities.map((authority) =>
    [authority.familyId, authority] as const))
  const semanticExisting = new Map<string, ExistingFamilyRefreshAuthorityV1>(
    refreshAuthorities.flatMap((authority) => {
    const identity = authority.familyIdentity
    return [[`${identity.category.toLowerCase()}\n${identity.productFunction.toLowerCase()}`,
      authority] as const]
  }))
  const reviewedExisting = new Map<string, ExistingFamilyRefreshAuthorityV1>(
    refreshAuthorities.flatMap((authority) => {
    const key = reviewedCommercialConceptKey({ familyName: authority.familyName,
      categoryId: authority.familyIdentity.category.match(
        /^ebay-us-category:(\d+)$/)?.[1] ?? null })
    return key ? [[key, authority] as const] : []
  }))
  const exactExistingIds = new Set(existingRows
    .flatMap((row) => text(row.family_id ?? row.familyId, 120) ?
      [text(row.family_id ?? row.familyId, 120) as string] : []))
  const semanticExistingKeys = new Set(existingRows.flatMap((row) => {
  ...
  const reviewedExistingKeys = new Set(existingRows.flatMap((row) => {
    const identity = record(row.family_identity ?? row.familyIdentity)
  ...
  for (const cluster of clusters) {
    ...
    const matchedRefreshAuthorities = [...new Map([
      exactExisting.get(familyId), semanticExisting.get(semantic),
      reviewedConcept === null ? undefined : reviewedExisting.get(reviewedConcept),
      ...refreshAuthorities.filter((authority) => exactQueryLineageMatchV1({
        clusterQueries: cluster.queries,
        familyQuerySet: authority.familyQuerySet,
      })),
    ].flatMap((authority) => authority ? [[authority.familyId, authority] as const] : []))
      .values()]
    const duplicateExists = exactExistingIds.has(familyId) ||
      semanticExistingKeys.has(semantic) ||
      (reviewedConcept !== null && reviewedExistingKeys.has(reviewedConcept)) ||
      matchedRefreshAuthorities.length > 0
    const refreshAuthority = matchedRefreshAuthorities.length === 1 && (
      isExistingFamilyRefreshDueV1({ authority: matchedRefreshAuthorities[0],
        evaluatedAt }) || isExistingFamilyRefreshEnrollmentRecoveryV1(
        matchedRefreshAuthorities[0])) ? matchedRefreshAuthorities[0] : null
    if (duplicateExists && refreshAuthority === null) {
      duplicatesSuppressed += 1
      candidates.push(Object.freeze({ status: "DUPLICATE" as const,
        reason: matchedRefreshAuthorities.length > 1
          ? "CANONICAL_FAMILY_REFRESH_IDENTITY_AMBIGUOUS"
    ...
    if (refreshAuthority && !enrollmentRecovery && (
      ...
      duplicatesSuppressed += 1
      candidates.push(Object.freeze({ status: "DUPLICATE" as const,
        reason: "EXISTING_FAMILY_REFRESH_NEW_SOLD_EVIDENCE_REQUIRED",
    ...
    const persistedIdentity = refreshAuthority?.familyIdentity ?? familyIdentity
    const persistedFamilyName = refreshAuthority?.familyName ?? canonicalFamilyName
    const persistedAttributeProfile = refreshAuthority ? {
      ...refreshAuthority.familyIdentity.structuredDefinition,
      "category id": cluster.categoryId,
      "product family": persistedFamilyName,
    } : familyIdentity.structuredDefinition
    const familyDefinition: SellerOsMarketFamilyDefinitionV1 = {
      identity: persistedIdentity, familyName: persistedFamilyName,
      familyQuerySet: unique([...(refreshAuthority?.familyQuerySet ?? []),
        ...cluster.queries, ...cluster.generatedQueries,
        ...aggregateResearch.generatedQueries.map((row) => row.query)]).slice(0, 16),
      keyProductAttributes: unique([...(refreshAuthority?.keyProductAttributes ?? []),
        "category id", "product family"]).slice(0, 32),
    ...
    const observation = buildSellerOsFamilyMarketObservationV1({
      ...
      demandEvidenceReferences: references, demandEvidenceDigest,
      ...
      attributeProfile: persistedAttributeProfile,
    ...
    const legacyCategorySuccessorResolved = refreshAuthority !== null &&
      !/^\d+$/.test(refreshAuthority.familyIdentity.structuredDefinition[
        "category id"] ?? "")
    candidates.push(Object.freeze({ status: refreshAuthority
      ? "REFRESH_EXISTING_FAMILY" as const : "QUALIFIED" as const,
      reason: refreshAuthority
        ? "EXISTING_FAMILY_NEW_OFFICIAL_SOLD_EVIDENCE_QUALIFIED"
  ...
  const qualifiedAfterCap = candidatesAfterCap.filter((candidate) =>
    candidate.status === "QUALIFIED").length
  const refreshAfterCap = candidatesAfterCap.filter((candidate) =>
    candidate.status === "REFRESH_EXISTING_FAMILY").length
  ...
  return Object.freeze({ rawClustersFormed: rawClusters.length,
    ...
    qualifiedFamilies: qualifiedAfterCap,
    existingFamilyRefreshCandidates: refreshAfterCap, duplicatesSuppressed,
...
function buildSellerOsDemandFirstBroadNetReplayFunnelV1(input: Readonly<{
  ...
  const rejectionReasonCounts = Object.fromEntries([
    ...
    (candidate) => !["QUALIFIED", "REFRESH_EXISTING_FAMILY"].includes(
      candidate.status)).reduce((counts, candidate) =>
  ...
  return Object.freeze({
    ...
    topCandidateClusters: Object.freeze(input.evaluation.candidatesAfterCap.map(
      (candidate) => Object.freeze({
        ...
        rejectionReason: ["QUALIFIED", "REFRESH_EXISTING_FAMILY"].includes(
          candidate.status) ? null : candidate.reason,
      }))),
  })
}

function existingFamilyCasesFromRadarV1(radarPayload: unknown) {
  const radar = record(radarPayload)
  return Object.freeze(rows(radar.families).map((family) => ({
    family_id: family.familyId,
    family_name: family.familyName,
    opportunity_case_id: family.opportunityCaseId,
    family_identity: family.familyIdentity ?? null,
    current_family_query_set: family.currentFamilyQuerySet ?? [],
    current_key_product_attributes: family.currentKeyProductAttributes ?? [],
    current_enrollment_enrolled_at: family.currentMonitorEnrolledAt ?? null,
    observationSeries: family.observationSeries ?? [],
    monitorEnrollments: family.monitorEnrollments ?? [],
  })))
}
...
export async function runSellerOsDemandFirstBroadNetServerReplayV1(
  ...
  const existingCases = existingFamilyCasesFromRadarV1(existingRadar)
...
export type SellerOsOnDemandFamilyDemandDiscoveryResultV1 = Readonly<{
  ...
  marketTestRadarFamily: Readonly<JsonRecord> | null
}>

type OnDemandKeywordDemandReaderV1 = typeof runEbaySellerKeywordDemandValidation

function onDemandDiscoveryResultV1(input: Partial<
  SellerOsOnDemandFamilyDemandDiscoveryResultV1> &
  Pick<SellerOsOnDemandFamilyDemandDiscoveryResultV1, "status">,
): SellerOsOnDemandFamilyDemandDiscoveryResultV1 {
  return Object.freeze({ status: input.status,
    ...
    marketTestRadarFamily: input.marketTestRadarFamily ?? null })
}

function buildOnDemandUnprovenMarketTestFamilyV1(input: Readonly<{
  productTitle: string
  productType: string | null
  report: Awaited<ReturnType<OnDemandKeywordDemandReaderV1>>
  activeEvidence: readonly MarketEvidenceV1[]
  lunaCatalogRow: JsonRecord
}>) {
...
/**
 * Bounded Quick Pick entry into the existing demand-first family authority.
 * The eBay reader and every persistence writer are the same authorities used by
 * Market Research and the Night Radar broad-net; this function only binds one
 * exact Luna identity to a newly proven family without running the global batch.
 */
export async function discoverAndPersistSellerOsOnDemandFamilyDemandV1(
  input: Readonly<{
    supabase: ReadWriteClient
    accountKey: string
    lunaCatalogRow: unknown
    demandReader?: OnDemandKeywordDemandReaderV1
  }>,
): Promise<SellerOsOnDemandFamilyDemandDiscoveryResultV1> {
  ...
  const supplierSku = text(row.sku, 120)
  const productTitle = text(row.title, 350)
  const variantTitle = text(row.variant_title, 200)
  const productType = text(row.product_type, 160)
  const supplierCost = money(row.price)
  if (!lunaProductId || !lunaVariantId || !supplierSku || !productTitle ||
      supplierCost === null || !/^\d{1,30}$/.test(lunaProductId) ||
  ...
  let report: Awaited<ReturnType<OnDemandKeywordDemandReaderV1>>
  try {
    report = await (input.demandReader ?? runEbaySellerKeywordDemandValidation)({
      productName: productTitle, productTitle, variantTitle,
      supplierSku, productType,
      gtin: text(row.barcode, 120),
    })
  } catch {
    return onDemandDiscoveryResultV1({ status: "DEMAND_DISCOVERY_UNAVAILABLE",
      reasonCode: "ON_DEMAND_EBAY_DEMAND_READ_UNAVAILABLE" })
  }
  const allEvidence = marketEvidenceFromKeywordDemandReportV1(report)
  const soldEvidence = allEvidence.filter((evidence) =>
  ...
  if (report.insightsAvailability !== "AVAILABLE" &&
    ...
    if (marketTestRadarFamily) return onDemandDiscoveryResultV1({
      ...
      demandNegativeEvidencePresent: false, marketTestRadarFamily,
    })
    return onDemandDiscoveryResultV1({ status: "DEMAND_DISCOVERY_UNAVAILABLE",
      reasonCode: `ON_DEMAND_MARKETPLACE_INSIGHTS_${report.insightsAvailability}` })
  }
  if (report.demandValidationBasis !== "VERIFIED_HISTORICAL_MULTI_SELLER" ||
    ...
    return onDemandDiscoveryResultV1({ status: "DEMAND_NOT_PROVEN",
      soldComparableCount: soldEvidence.length,
      reasonCode: "ON_DEMAND_OFFICIAL_SOLD_FAMILY_EVIDENCE_INSUFFICIENT" })
  }
  const request = normalizeMarketResearchRequestV1({ marketplace: "EBAY_US",
    seedType: "SEED_PRODUCT_TITLE", seedValue: productTitle,
    requestedWindowDays: 30, researchIntent: "FAMILY_DISCOVERY", queryBudget: 2,
    seedIdentity: { categoryId: null, categoryName: null, brand: null,
      gtin: text(row.barcode, 120), mpn: null, model: null, packCount: null,
      size: null, color: null } })
  const research = buildMarketOpportunityResearchV1({ request,
    evidence: soldEvidence, observedAt: report.evidenceAsOf,
    activeMarketStatus: "UNAVAILABLE", soldHistoryStatus: "AVAILABLE",
    paginationCoverage: "BOUNDED_ON_DEMAND_MARKETPLACE_INSIGHTS",
    sourceLimitations: ["EXACT_PRODUCT_DEMAND_NOT_CLAIMED",
      "MARKETPLACE_INSIGHTS_LIMITED_RELEASE"] })
  const categoryGroups = new Map<string, MarketEvidenceV1[]>()
  for (const evidence of soldEvidence) {
    const categoryId = evidence.categoryId as string
    categoryGroups.set(categoryId,
      [...(categoryGroups.get(categoryId) ?? []), evidence])
  }
  const eligibleCategories = [...categoryGroups].filter(([, evidence]) =>
    evidence.length >= 2 && new Set(evidence.flatMap((entry) =>
      entry.sellerReferenceHash ? [entry.sellerReferenceHash] : [])).size >= 2)
  if (eligibleCategories.length !== 1) {
    return onDemandDiscoveryResultV1({ status: "DEMAND_NOT_PROVEN",
      soldComparableCount: soldEvidence.length,
      reasonCode: eligibleCategories.length > 1
        ? "ON_DEMAND_DEMAND_FAMILY_AMBIGUOUS"
        : "ON_DEMAND_OFFICIAL_SOLD_FAMILY_EVIDENCE_INSUFFICIENT" })
  }
  const [categoryId, categoryEvidence] = eligibleCategories[0]
  const family = research.productFamilies.find((candidate) =>
    candidate.category.categoryId === categoryId &&
    candidate.soldEvidenceCount >= 2)
  const familyName = text(family?.canonicalLabel, 160)
  const evidenceObservedAt = instant(report.evidenceAsOf)
  const prices = categoryEvidence.flatMap((entry) =>
    entry.price === null ? [] : [entry.price])
  if (!familyName || familyName === "Unproven product family" ||
      !evidenceObservedAt || prices.length < 2) {
    return onDemandDiscoveryResultV1({ status: "DEMAND_NOT_PROVEN",
      soldComparableCount: categoryEvidence.length,
      reasonCode: "ON_DEMAND_CANONICAL_FAMILY_UNPROVEN" })
  }
  const references = unique(categoryEvidence.map((entry) => entry.evidenceId))
  const soldQuantity = categoryEvidence.reduce((sum, entry) =>
    sum + (entry.confirmedSoldQuantity ?? 0), 0)
  const familyDemandStatus = references.length >= 5 && soldQuantity >= 10
    ? "FAMILY_DEMAND_PROVEN" as const : "FAMILY_DEMAND_SUPPORTED" as const
  const demandKeywordFamilyTypes = new Set<DemandKeywordFamilyInputV1["familyType"]>([
    "CORE", "FORM_FACTOR", "FEATURE", "USE_CASE", "BENEFIT", "PACK_FORMAT",
    "AUDIENCE", "ATTRIBUTE",
  ...
  try {
    demandKeywordDna = buildSellerOsDemandKeywordDnaV1({
      keywordFamilies: research.keywordFamilies.flatMap((keywordFamily) =>
        demandKeywordFamilyTypes.has(
          ...
          familyType: keywordFamily.familyType as
            DemandKeywordFamilyInputV1["familyType"] }] : []),
      soldTitles: categoryEvidence.flatMap((entry) => entry.title ? [{
        title: entry.title,
        soldQuantityObserved: entry.confirmedSoldQuantity ?? 0,
        evidenceReference: entry.evidenceId,
      }] : []), familyDemandStatus, evidenceObservedAt,
      maximumAgeSeconds: MAXIMUM_AGE_SECONDS,
    })
  } catch {
    return onDemandDiscoveryResultV1({ status: "DEMAND_NOT_PROVEN",
      soldComparableCount: categoryEvidence.length,
      reasonCode: "ON_DEMAND_DEMAND_KEYWORD_DNA_UNAVAILABLE" })
  }
  const familyIdentity = normalizeSellerOsMarketFamilyIdentityV1({
    productFunction: familyName,
    buyerUseCase: text(report.searchQuery, 160) ?? familyName,
    category: `ebay-us-category:${categoryId}`,
    structuredDefinition: { "category id": categoryId,
      "product family": familyName },
  })
  const familyDefinition: SellerOsMarketFamilyDefinitionV1 = {
    identity: familyIdentity, familyName,
    familyQuerySet: unique([report.searchQuery,
      research.keywordSpine.terms.join(" "),
      ...research.generatedQueries.map((entry) => entry.query)]
      .flatMap((entry) => text(entry, 160) ? [text(entry, 160) as string] : []))
      .slice(0, 16),
    keyProductAttributes: unique(["category id", "product family",
      ...(productType ? ["supplier product type"] : [])]),
    keyBuyerIntentTerms: unique(research.keywordSpine.terms).slice(0, 8),
    demandKeywordDna,
    adapterContract: SELLER_OS_DEMAND_FIRST_BROAD_NET_ORCHESTRATOR_VERSION,
    adapterVersion: "1",
  }
  const windowEnd = new Date(evidenceObservedAt)
  const windowStart = new Date(windowEnd.getTime() -
    Math.max(1, report.soldRecencyDays) * 86_400_000)
  const observation = buildSellerOsFamilyMarketObservationV1({
    familyDefinition, observationWindowStart: windowStart.toISOString(),
    observationWindowEnd: windowEnd.toISOString(), familyDemandStatus,
    demandEvidenceClass: "OFFICIAL_SOLD_EVIDENCE", sourceStatus: "AVAILABLE",
    aggregationSemantics: "CUMULATIVE_SNAPSHOT",
    demandEvidenceReferences: references,
    demandEvidenceDigest: digest(references),
    soldComparableCount: references.length,
    soldQuantityEvidence: { quantity: soldQuantity,
      authorityClass: "OFFICIAL_EXTERNAL_FACT", evidenceReferences: references },
    activeComparableCount: null,
    sellerDiversity: new Set(categoryEvidence.flatMap((entry) =>
      entry.sellerReferenceHash ? [entry.sellerReferenceHash] : [])).size,
    priceBand: { currency: "USD", minimum: Math.min(...prices),
      maximum: Math.max(...prices) }, priceMedian: median(prices),
    priceDistributionEvidence: references, competitionState: "UNPROVEN",
    buyerIntentTerms: unique(research.keywordSpine.terms).slice(0, 8),
    keywordState: "AVAILABLE", demandKeywordDna,
    attributeProfile: { ...familyIdentity.structuredDefinition,
      ...(productType ? { "supplier product type": productType } : {}) },
    opportunityTypes: ["DEMAND_FIRST_TEST_LAUNCH"], evidenceObservedAt,
    sourceUpdatedAt: evidenceObservedAt, maximumAgeSeconds: MAXIMUM_AGE_SECONDS,
    sourceAdapter: "SELLER_OS_EBAY_MARKET_RESEARCH_GATEWAY_V1",
    sourceContractVersion: report.validationVersion,
    limitations: ["EXACT_PRODUCT_DEMAND_NOT_CLAIMED",
      "MARKETPLACE_INSIGHTS_LIMITED_RELEASE"],
  })
  const prospectiveRadar = { status: "AVAILABLE", families: [{
    familyId: observation.familyId, familyName,
    opportunityCaseId: observation.opportunityCaseId,
    observationSeries: [{ ...observation, fresh: true,
      soldQuantity, priceCurrency: "USD",
      priceBandMinimum: Math.min(...prices), priceBandMaximum: Math.max(...prices),
  ...
  const exactMatches = prospectiveMatch.candidates.filter((candidate) =>
    candidate.lunaProductId === lunaProductId &&
    candidate.lunaVariantId === lunaVariantId &&
    candidate.supplierSku === supplierSku)
  if (exactMatches.length !== 1) {
    return onDemandDiscoveryResultV1({ status: "DEMAND_NOT_PROVEN",
      soldComparableCount: categoryEvidence.length,
      reasonCode: prospectiveMatch.ambiguousFamilyAssignments > 0
        ? "ON_DEMAND_DEMAND_FAMILY_AMBIGUOUS"
        : "ON_DEMAND_FAMILY_TO_EXACT_LUNA_BINDING_UNPROVEN" })
  }
  try {
    const caseResult = await checkedRpc(input.supabase,
      "put_seller_os_market_opportunity_case_v1", {
        ...
        p_demand_keyword_dna: familyDefinition.demandKeywordDna,
      })
    if (text(caseResult.familyId, 120) !== observation.familyId ||
        text(caseResult.opportunityCaseId, 120) !== observation.opportunityCaseId ||
        text(caseResult.familyDefinitionVersionId, 120) !==
          observation.familyDefinitionVersionId) {
      throw new Error("ON_DEMAND_DEMAND_CASE_READBACK_MISMATCH")
    }
    const observationResult = await checkedRpc(input.supabase,
      "put_seller_os_family_market_observation_v1", {
        ...
        p_aggregation_semantics: "CUMULATIVE_SNAPSHOT",
        p_demand_evidence_references: observation.demandEvidenceReferences,
        p_sold_comparable_count: observation.soldComparableCount,
        p_sold_quantity: observation.soldQuantityEvidence?.quantity ?? null,
        p_active_comparable_count: null,
        p_seller_diversity: observation.sellerDiversity,
        p_price_currency: observation.priceBand?.currency ?? null,
        p_price_band_minimum: observation.priceBand?.minimum ?? null,
        p_price_band_maximum: observation.priceBand?.maximum ?? null,
        p_price_median: observation.priceMedian,
        p_price_distribution_evidence: observation.priceDistributionEvidence,
        p_competition_state: "UNPROVEN",
        p_buyer_intent_terms: observation.buyerIntentTerms,
        p_keyword_state: "AVAILABLE",
        p_demand_keyword_dna: observation.demandKeywordDna,
        p_attribute_profile: observation.attributeProfile,
        ...
        p_source_contract_version: report.validationVersion,
    ...
    if (observationId !== observation.observationId) {
      throw new Error("ON_DEMAND_DEMAND_OBSERVATION_READBACK_MISMATCH")
    }
    const enrollment = buildSellerOsOpportunityMonitorEnrollmentV1({
      familyIdentity, monitorPolicyVersion: MONITOR_POLICY,
      enrolledAt: evidenceObservedAt, status: "ENROLLED",
      nextReviewCondition: "TIME_WINDOW_ELAPSED", nextEligibleReviewAt: null,
      lastObservationId: observationId, lastEvaluatedAt: evidenceObservedAt,
    })
    const enrollmentResult = await checkedRpc(input.supabase,
      "put_seller_os_opportunity_monitor_enrollment_v1", {
        ...
        p_next_eligible_review_at: null,
        p_last_observation_id: observationId,
        p_last_evaluated_at: evidenceObservedAt,
      })
    if (text(enrollmentResult.lastObservationId, 120) !== observationId) {
      throw new Error("ON_DEMAND_DEMAND_ENROLLMENT_READBACK_MISMATCH")
    }
    const readback = await checkedRpc(input.supabase,
      "get_seller_os_family_market_radar_v1", {
        p_family_id: observation.familyId, p_limit: 1,
      })
    ...
    if (readback.status !== "AVAILABLE" ||
        text(readbackFamily?.familyId, 120) !== observation.familyId ||
        text(readbackObservation?.observationId, 120) !== observationId) {
      throw new Error("ON_DEMAND_DEMAND_RADAR_READBACK_MISMATCH")
    }
  } catch {
    return onDemandDiscoveryResultV1({ status: "DEMAND_DISCOVERY_UNAVAILABLE",
      soldComparableCount: categoryEvidence.length,
      familyId: observation.familyId, familyName,
      reasonCode: "ON_DEMAND_DEMAND_PERSISTENCE_UNAVAILABLE" })
  }
  return onDemandDiscoveryResultV1({ status: familyDemandStatus,
    soldComparableCount: categoryEvidence.length,
    familyBindingCreatedOrReused: true,
    familyId: observation.familyId, familyName })
}
...
export async function runSellerOsDemandFirstBroadNetCanaryV1(input: Readonly<{
  ...
  const candidates = buildSellerOsDemandFirstFamilyCandidatesV1({
    ...
    existingCases: existingFamilyCasesFromRadarV1(existingRadar),
  })
  ...
  const persistenceCandidates = candidates.filter((item) =>
    item.status === "QUALIFIED" || item.status === "REFRESH_EXISTING_FAMILY")
    .slice(0, Math.min(persistenceLimit,
      input.maximumFamiliesToPersist ?? persistenceLimit))
  let persisted = 0
  let newFamiliesPersisted = 0
  let existingFamiliesRefreshed = 0
  ...
  const refreshedFamilyNames: string[] = []
  for (const candidate of persistenceCandidates) {
    ...
    opportunityCasesCreated += candidate.status === "QUALIFIED" &&
      caseResult.outcome !== "IDEMPOTENT_SUCCESS" ? 1 : 0
    const observationResult = await checkedRpc(input.supabase,
      "put_seller_os_family_market_observation_v1", {
    ...
    if (candidate.status === "REFRESH_EXISTING_FAMILY") {
      existingFamiliesRefreshed += 1
      refreshedFamilyNames.push(definition.familyName)
    } else {
      newFamiliesPersisted += 1
    }
  }
  const productFitNames = persistenceCandidates.flatMap((candidate) =>
  ...
  return Object.freeze({
    ...
    browseItemLookupsAttempted: categoryResolution.browseItemLookupsAttempted,
    browseItemLookupsSucceeded: categoryResolution.browseItemLookupsSucceeded,
    existingFamilyRefreshCandidateCount: candidates.filter((item) =>
      item.status === "REFRESH_EXISTING_FAMILY").length,
    existingFamilyRefreshedCount: existingFamiliesRefreshed,
    legacyCategorySuccessorResolvedCount: persistenceCandidates.filter((item) =>
      item.status === "REFRESH_EXISTING_FAMILY" &&
      item.legacyCategorySuccessorResolved).length,
    refreshedFamilyNames: Object.freeze(refreshedFamilyNames),
    newFamilyCandidates: candidates.filter((item) =>
      item.status === "QUALIFIED").length,
    duplicatesSuppressed: candidates.filter((item) => item.status === "DUPLICATE").length,
    newFamiliesPersisted, duplicateFamiliesCreated: 0 as const,
    persistedFamilyCount: persisted, opportunityCasesCreated,
