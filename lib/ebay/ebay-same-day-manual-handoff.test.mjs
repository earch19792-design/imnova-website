...
test("builds an original facts-only Seller Hub package with operator price and zero writes", () => {
  ...
  assert.equal(result.package.title, "Example 3-Pack Example Product")
  assert.match(result.package.description, /3 packs × 1 unit each/)
...
test("accepts an authoritative value outside an incomplete eBay suggestion sample", () => {
  const base = input()
  const result = buildVerifiedManualSellerHubHandoff(input({
    factsSummary: {
      ...base.factsSummary,
      resolvedRequirements: [{
        aspectName: "Brand", required: true, status: "SATISFIED_VERIFIED",
        mappedFactKey: "brand", selectedValue: "Example",
        allowedValues: ["Other", "Unbranded"], selectionOnly: false,
...
test("still blocks a value outside a complete selection-only eBay list", () => {
  const base = input()
  const result = buildVerifiedManualSellerHubHandoff(input({
    factsSummary: {
      ...base.factsSummary,
      resolvedRequirements: [{
        aspectName: "Brand", required: true, status: "SATISFIED_VERIFIED",
        mappedFactKey: "brand", selectedValue: "Example",
        allowedValues: ["Other", "Unbranded"], selectionOnly: true,
        allowedValuesComplete: true,
      }],
    },
  }))
  assert.equal(result.ready, false)
  assert.ok(result.blockers.includes("ASPECT_VALUE_NOT_ALLOWED_BRAND"))
})
...
test("recommended aspects and unconfirmed shipping dimensions become visible warnings instead of false hard gates", () => {
  ...
  assert.ok(result.warnings.includes("SHIPPING_CONFIRMATION_REQUIRED_IN_SELLER_OS"))
  assert.ok(result.warnings.includes("OPTIONAL_ASPECT_MISSING_MATERIAL"))
})

test("uses verified package quantity in US listing copy and optional unit specifics", () => {
  const base = input()
  const result = buildVerifiedManualSellerHubHandoff(input({
    factsSummary: {
      ...base.factsSummary,
      resolvedRequirements: [
        ...base.factsSummary.resolvedRequirements,
        { aspectName: "Unit Quantity", required: false,
          status: "MISSING_OPTIONAL", mappedFactKey: null,
          selectedValue: null, allowedValues: [], selectionOnly: false,
          allowedValuesComplete: false },
        { aspectName: "Unit Type", required: false,
          status: "MISSING_OPTIONAL", mappedFactKey: null,
          selectedValue: null, allowedValues: ["Unit", "oz", "lb"],
          selectionOnly: true, allowedValuesComplete: true },
      ],
    },
  }))
  assert.equal(result.ready, true)
  assert.deepEqual(result.package.itemSpecifics["Unit Quantity"], ["3"])
  assert.deepEqual(result.package.itemSpecifics["Unit Type"], ["Unit"])
  assert.match(result.package.description, /Product details/)
  assert.match(
    result.package.description,
    /Offer quantity: 3 packs × 1 unit each \(3 total units\)/,
  )
  assert.match(result.package.description, /approved photos and item specifics/)
  assert.doesNotMatch(result.package.description, /Marca|Condición|Contenido/)
  assert.ok(!result.warnings.includes("OPTIONAL_ASPECT_MISSING_UNIT_QUANTITY"))
  assert.ok(!result.warnings.includes("OPTIONAL_ASPECT_MISSING_UNIT_TYPE"))
})

test("does not invent a Unit Type outside a complete eBay selection list", () => {
  const base = input()
  const result = buildVerifiedManualSellerHubHandoff(input({
    factsSummary: {
      ...base.factsSummary,
      resolvedRequirements: [
        ...
        { aspectName: "Unit Type", required: false,
          status: "MISSING_OPTIONAL", mappedFactKey: null,
          selectedValue: null, allowedValues: ["oz", "lb"],
          selectionOnly: true, allowedValuesComplete: true },
      ],
    },
  }))
  assert.equal(result.ready, true)
  assert.equal(result.package.itemSpecifics["Unit Type"], undefined)
  assert.ok(result.warnings.includes("OPTIONAL_ASPECT_MISSING_UNIT_TYPE"))
})
