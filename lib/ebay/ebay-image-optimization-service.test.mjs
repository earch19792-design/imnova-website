...
async function whiteBridgeOnWhiteFixture() {
  return sharp({
    create: { width: 900, height: 900, channels: 3, background: "#ffffff" },
  }).composite([{
    input: Buffer.from(
      '<svg xmlns="http://www.w3.org/2000/svg" width="900" height="900">' +
      '<path d="M430 0h40v360H430zM300 340h300v240H300z" fill="#fcfcfc"/>' +
      '<path d="M100 280h250v360H100zM550 280h250v360H550z" fill="#d8d2c5" stroke="#666" stroke-width="9"/>' +
      '<path d="M300 280h130v60H300zM470 280h130v60H470zM300 580h300v60H300z" fill="#777"/>' +
      '<rect x="135" y="380" width="175" height="150" rx="45" fill="#252525"/>' +
      '<rect x="590" y="380" width="175" height="150" rx="45" fill="#252525"/>' +
      '</svg>',
    ),
  }]).jpeg({ quality: 95 }).toBuffer()
}
...
test("fails closed when a near-white product bridge would be erased from its authorized source", async () => {
  const source = await whiteBridgeOnWhiteFixture()
  const main = await optimizeAuthorizedEbayMainImage(source)
  assert.equal(main.transformation.backgroundMethod,
    "AUTHORIZED_SOURCE_FRAMED_CONTAIN")
  assert.equal(main.qa.fullAuthorizedFramePreserved, true)
  assert.equal(main.qa.sourceVisualProfile.productToneRisk,
    "LIGHT_NEUTRAL_AMBIGUITY")
  assert.ok(main.qa.sourceAmbiguousConnectedLightRatio >= .01)
  assert.ok(main.qa.sourceAmbiguousInteriorLightRatio >= .0005)
  await assert.rejects(
    prepareAuthorizedEbaySecondaryForeground(source),
    /AUTHORIZED_FOREGROUND_LIGHT_NEUTRAL_AMBIGUITY/,
  )
})

test("does not trust a fully opaque alpha channel as white-product separation evidence", async () => {
  const jpeg = await whiteBridgeOnWhiteFixture()
  const opaqueAlphaSource = await sharp(jpeg).ensureAlpha(1).png().toBuffer()
  const main = await optimizeAuthorizedEbayMainImage(opaqueAlphaSource)
  assert.equal(main.transformation.backgroundMethod,
    "AUTHORIZED_SOURCE_FRAMED_CONTAIN")
  assert.equal(main.qa.fullAuthorizedFramePreserved, true)
  await assert.rejects(
    prepareAuthorizedEbaySecondaryForeground(opaqueAlphaSource),
    /AUTHORIZED_FOREGROUND_LIGHT_NEUTRAL_AMBIGUITY/,
  )
})
