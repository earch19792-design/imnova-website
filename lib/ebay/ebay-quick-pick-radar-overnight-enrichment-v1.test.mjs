...
test("cron and Dashboard reuse the existing route, resolver and owner runtime", async () => {
  ...
  "utf8")
  const vercel = await readFile(new URL("../../vercel.json", import.meta.url),
    "utf8")
  ...
  assert.match(cron, /materializeRadarRevenueFactoryCandidateBatchV1/)
  assert.equal((vercel.match(/market-radar-luna-sync/g) ?? []).length, 1)
