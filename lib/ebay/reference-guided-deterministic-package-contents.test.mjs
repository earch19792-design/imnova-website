...
test("private Phase A preview is exposed only through a temporary signed URL", () => {
  ...
  assert.match(page, /HUMAN_CONFIRMED_SINGLE_COMPLETE_UNIT_SIDE_VIEW/)
  assert.match(page, /packageContentsStatus/)
})

test("position 2 human approval is atomic, append-only, and provider-free", () => {
  const migration = readFileSync(new URL(
    "../../supabase/migrations/20260722031000_approve_successor_phase_a_position_2.sql",
    import.meta.url), "utf8")
  assert.match(migration, /HUMAN_CONFIRMED_SINGLE_COMPLETE_UNIT_SIDE_VIEW/)
  assert.match(migration, /preview_sha256,\s*decision, reason/)
  assert.match(migration, /'APPROVED', p_reason/)
  assert.match(migration, /set status = 'PASSED'/)
  assert.match(migration, /position between 3 and 6/)
  assert.match(migration, /provider_calls <> 2/)
  assert.doesNotMatch(migration,
    /reserve_ebay_reference_guided_provider_call|provider_calls\s*=|lease_owner\s*=/i)
})
