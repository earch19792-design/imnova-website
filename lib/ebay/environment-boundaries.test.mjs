...
test("dedicated preprod accepts explicit branch attestation only when Vercel omits native git ref", () => {
  const attested = getEbayDraftWriteEnvironmentBoundary({
    ...dedicatedPreprod,
    draftTarget: "PRODUCTION",
    draftMasterEnabled: true,
    draftProductionEnabled: true,
    ...
    deploymentAttestedGitRef:
      "feature/seller-os-dedicated-preprod-boundary-v1",
  })
  const nativeMismatch = getEbayDraftWriteEnvironmentBoundary({
    ...dedicatedPreprod,
    draftTarget: "PRODUCTION",
    draftMasterEnabled: true,
    draftProductionEnabled: true,
  ...
  assert.equal(attested.branchAuthority,
    "SELLER_OS_DEPLOYMENT_ATTESTED_GIT_REF")
  assert.equal(attested.productionDedicatedPreprodBound, true)
  assert.equal(attested.writeAllowed, true)
  assert.equal(nativeMismatch.branchAuthority, "NONE")
  assert.equal(nativeMismatch.productionDedicatedPreprodBound, false)
  assert.equal(nativeMismatch.writeAllowed, false)
})
...
test("only the browser OAuth start and callback defer Edge classification to their Node guards", () => {
  assert.equal(isEbayOAuthNodeGuardedCeremonyPath(
    "/api/admin/ebay/publication-oauth/start"), true)
  assert.equal(isEbayOAuthNodeGuardedCeremonyPath(
    "/api/admin/ebay/monitor/seller-oauth-reauth"), true)
  assert.equal(isEbayOAuthNodeGuardedCeremonyPath(
    "/api/admin/ebay/publication-oauth/status"), false)
  assert.equal(isEbayOAuthNodeGuardedCeremonyPath(
    "/api/admin/ebay/publication-oauth/start/extra"), false)
  assert.equal(isEbayOAuthNodeGuardedCeremonyPath(
    "/api/admin/ebay/draft-only"), false)
})
