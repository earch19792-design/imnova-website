...
test("Seller OS organiza la navegación canónica por intención del operador", () => {
  assert.deepEqual(
    ...
    ["monitor", "quick-pick", "opportunities", "listings", "experiments", "inventory", "orders",
  ...
  assert.deepEqual(SELLER_OS_MOBILE_NAVIGATION.map((item) => item.id),
    ["monitor", "quick-pick", "opportunities", "listings", "experiments"])
  assert.deepEqual(SELLER_OS_PRIMARY_NAVIGATION,
    SELLER_OS_MOBILE_NAVIGATION)
...
test("Inicio compacto demotes legacy launch workflow without deleting its consumers", () => {
  const home = read("app/admin/page.tsx")
  const today = read("app/admin/today-launch-panel.tsx")
  assert.match(home, /SellerOsOperationalDashboard/)
