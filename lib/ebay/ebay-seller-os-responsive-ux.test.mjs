...
test("Seller OS organiza la navegación canónica por intención del operador", () => {
  assert.deepEqual(
    ...
    ["dashboard", "opportunities", "listings", "stockguard", "orders",
      "analytics", "settings"],
  )
  ...
  assert.deepEqual(SELLER_OS_MOBILE_NAVIGATION.map((item) => item.id),
    ["dashboard", "opportunities", "listings", "stockguard"])
  assert.equal(new Set(SELLER_OS_NAVIGATION.map((item) => item.href)).size, 7)
  ...
  assert.doesNotMatch(desktop, /NavigationGroup label="Sistema"/)
