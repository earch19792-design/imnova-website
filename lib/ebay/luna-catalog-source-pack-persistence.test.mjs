...
test("reuses byte-identical content-addressed objects on a retry", async () => {
  ...
  assert.match(
    inserted[0].source_pack_version,
    /^LUNA_CATALOG_ORIGINAL_SOURCE_RESOLVER_V2_[0-9a-f]{16}$/,
  )
...
test("reconciles a concurrent immutable manifest insert without deleting its object", async () => {
  const native = Buffer.from("authorized concurrent catalog bytes")
  const pack = sourcePack(native)
  const inserted = []
  const removed = []
  let lookups = 0
  const racedPackId = "00000000-0000-4000-8000-000000000099"
  const supabase = {
    from() {
      return {
        select() { return this },
        ...
        async maybeSingle() {
          ...
          return lookups === 1
            ...
            : {
                data: {
                  id: racedPackId,
                  source_pack_hash: "resolved-by-query-filter",
                },
                error: null,
              }
        },
    ...
    storage: {
      from() {
        return {
          async upload() {
            return { data: { path: "created" }, error: null }
          },
          async download() {
  ...
  assert.equal(result.packId, racedPackId)
  assert.equal(inserted.length, 1)
