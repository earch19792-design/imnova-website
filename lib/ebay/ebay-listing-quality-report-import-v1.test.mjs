...
test("an unrecognized official-style sheet preserves safe sheet and header diagnostics", () => {
  const content = workbookWithSheets([{ name: "Category detail",
    headers: ["Item ID", "Custom label", "Daily impressions per listing",
      "Click-through rate", "Google Shopping rejections"],
    rows: [["123456789012", "SKU-1", "12", "1.5%", "None"]] }])
  assert.throws(() => parseEbayListingQualityReportV1({ format: "XLSX",
    fileName: "listing-quality-report.xlsx", content }), (error) => {
    assert.ok(error instanceof QualityReportValidationError)
    assert.equal(error.reason, "NO_VALID_SHEET")
    assert.deepEqual(error.diagnosis.worksheetNames, ["Category detail"])
    assert.ok(error.diagnosis.observedHeaderNames.includes("Item ID"))
    assert.ok(error.diagnosis.observedHeaderNames.includes(
      "Google Shopping rejections"))
    assert.equal(error.diagnosis.candidateSheetCount, 0)
    return true
  })
})
