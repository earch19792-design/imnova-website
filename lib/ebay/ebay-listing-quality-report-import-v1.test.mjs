...
function workbookWithSparseOfficialHeader() {
  ...
  const worksheet = `<?xml version="1.0"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData>
    <row r="1">${cell("A1", "eBay Listing Quality Report")}</row>
    <row r="3">${cell("A3", "Item title")}${cell("C3", "Item Id")}${cell("D3", "Number of photos")}</row>
    <row r="4">${cell("A4", "Sanitized product")}${cell("C4", "123456789012")}${cell("D4", "1")}</row>
    </sheetData></worksheet>`
  return Buffer.from(zipSync({
    "[Content_Types].xml": strToU8("<Types xmlns=\"http://schemas.openxmlformats.org/package/2006/content-types\"/>", true),
    "xl/workbook.xml": strToU8(`<?xml version="1.0"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Future category" sheetId="1" r:id="rId1"/></sheets></workbook>`, true),
    "xl/_rels/workbook.xml.rels": strToU8(`<?xml version="1.0"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/></Relationships>`, true),
    "xl/worksheets/sheet1.xml": strToU8(worksheet, true),
  })).toString("base64")
}
...
test("official XLSX rows with blank header columns are materialized densely", () => {
  const snapshot = parseEbayListingQualityReportV1({ format: "XLSX",
    fileName: "listing-quality-report.xlsx",
    content: workbookWithSparseOfficialHeader() })
  assert.equal(snapshot.workbook.recognizedSheetCount, 1)
  assert.equal(snapshot.rows[0].itemId, "123456789012")
  assert.equal(snapshot.rows[0].numberOfPhotos, 1)
  assert.equal(snapshot.rows[0].unknownFields["Column 2"], undefined)
  assert.equal(snapshot.workbook.pipeline.fileBufferCreated, true)
  assert.equal(snapshot.workbook.pipeline.xlsxLibraryInvoked, true)
  assert.equal(snapshot.workbook.pipeline.workbookOpenPass, true)
  assert.equal(snapshot.workbook.pipeline.workbookSheetEnumerationReached, true)
  assert.equal(snapshot.workbook.pipeline.schemaDiscoveryReached, true)
  assert.equal(snapshot.workbook.pipeline.rowParseReached, true)
  assert.equal(snapshot.workbook.pipeline.failedStage, "COMPLETE")
})
