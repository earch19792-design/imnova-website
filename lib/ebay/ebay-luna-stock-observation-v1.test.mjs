...
test("W0 workflow and P2 activation invariants remain explicit", () => {
  ...
  assert.equal(P2_I02_PREBUILD_LIVE_ACTIVATION_LOCKED, true)
...
test("resource is allowlisted through search/fetch and reuses the existing Vercel cron scheduler", async () => {
  ...
  assert.match(vercel, /ebay-active-listing-luna-monitor/)
  assert.match(vercel, /\*\/5 \* \* \* \*/)
  assert.match(route, /AUTO_CERTIFIED_OOS_END_LISTING_V1/)
  assert.doesNotMatch(route, /!activation\.productionSchedulerEnabled/)
