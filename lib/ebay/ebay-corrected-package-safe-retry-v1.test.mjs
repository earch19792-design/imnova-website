...
test("exact corrected UPC history may be excluded from the generic ledger collision guard", () => {
  const input = fixture()
  assert.equal(correctedPackageRetryLedgerExclusionApprovalIdV1({
    current: input.current,
    historical: input.historical,
    historicalSelfLineageExact: true,
  }), input.historical.approvalId)
})

test("ledger exclusion remains fail closed without exact lineage or corrected UPC", () => {
  const input = fixture()
  assert.equal(correctedPackageRetryLedgerExclusionApprovalIdV1({
    current: input.current,
    historical: input.historical,
    historicalSelfLineageExact: false,
  }), null)
  assert.equal(correctedPackageRetryLedgerExclusionApprovalIdV1({
    current: { ...input.current, upcs: [] },
    historical: input.historical,
    historicalSelfLineageExact: true,
  }), null)
})

test("GET and POST share the exact corrected-retry collision lineage", () => {
  const input = fixture()
  const currentSelfLineage = {
    exact: false,
    excludeApprovalId: null,
    reasonCode: "SELF_LINEAGE_LEDGER_NOT_FOUND",
  }
  const historicalSelfLineage = {
    exact: true,
    excludeApprovalId: input.historical.approvalId,
    reasonCode: "EXACT_OFFER_SELF_LINEAGE",
  }
  assert.equal(resolveCorrectedPackageRetryCollisionSelfLineageV1({
    current: input.current,
    historical: input.historical,
    currentSelfLineage,
...
test("an unrelated historical lineage never disables the SKU collision guard", () => {
  const input = fixture()
  const currentSelfLineage = {
    exact: false,
    excludeApprovalId: null,
  ...
  const unrelatedSelfLineage = {
    exact: true,
    excludeApprovalId: "unrelated-approval",
    reasonCode: "EXACT_OFFER_SELF_LINEAGE",
  }
  assert.equal(resolveCorrectedPackageRetryCollisionSelfLineageV1({
    current: input.current,
    historical: input.historical,
    currentSelfLineage,
    historicalSelfLineage: unrelatedSelfLineage,
  }), currentSelfLineage)
})

test("exact current self-lineage always remains the collision authority", () => {
  const input = fixture()
  const currentSelfLineage = {
    exact: true,
    excludeApprovalId: "current-approval",
    reasonCode: "EXACT_OFFER_SELF_LINEAGE",
  }
  const historicalSelfLineage = {
    exact: true,
