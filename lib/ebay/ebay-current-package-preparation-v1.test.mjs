...
test('CURRENT accepted payload requires exact durable receipt and never borrows another revision or Offer',()=>{
 ...
 for(const mutate of [x=>x.snapshot.content.title='different',x=>x.snapshot.generation='other',x=>x.preview.inventoryItemPayload.product.title='different']){
  const changed=structuredClone(r);mutate(changed);assert.equal(accepted(p,changed,now),false)
 }
