...
import {
  ...
  remoteLiveOperatorDisplayNameFromUser,
...
source = source
  ...
  .replace(/import \{\n  REMOTE_LIVE_OPERATOR_INTERNAL_EMAIL,[\s\S]*?\} from "\.\/remote-live-operator-identity"\n/,
    ...
    const normalizeRemoteLiveOperatorDisplayName = (value) => {
      if (typeof value !== "string") return null
...
test("username login maps to one internal account and retains username semantics", () => {
  ...
  assert.equal(sellerOsPasswordLoginIdentity("owner@example.com")?.email,
    "owner@example.com")
  assert.equal(normalizeRemoteLiveOperatorDisplayName("  Mayel  "), "Mayel")
  assert.equal(normalizeRemoteLiveOperatorDisplayName("   "), null)
  assert.equal(remoteLiveOperatorDisplayNameFromUser({ app_metadata: {
    operator_display_name: "Mayel",
  } }), "Mayel")
  assert.equal(remoteLiveOperatorDisplayNameFromUser({ user_metadata: {
    operator_display_name: "Nombre no confiable",
  } }), null)
})
...
test("one successful setup closes the singleton slot and stores authority in app_metadata", async () => {
  ...
  try {
    ...
    assert.equal(client.users[0].app_metadata.operator_username,
      "asistente.uno")
    assert.equal(client.users[0].app_metadata.operator_display_name, "Mayel")
    assert.equal(created.displayName, "Mayel")
...
test("the first-use surface has no public signup or browser-side service key", () => {
  ...
  assert.match(login, /setupDisplayName/)
  assert.match(login, /Tu nombre/)
  assert.match(route, /displayName: input\.displayName/)
