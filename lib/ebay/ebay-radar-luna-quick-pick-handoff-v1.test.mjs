...
test("null inventory is unknown rather than zero and does not discard an available candidate", () => {
  const signals = bridge.projectQualifiedRadarSignalsV1({ status: "AVAILABLE",
    families: [family({ digit: "1", name: "Microcurrent facial devices" })] })
  const result = bridge.buildBoundedRadarLunaShortlistsV1({ signals,
    catalogRows: [catalog({ inventory_quantity: null })] })
  const selected = result.byFamily.get(signals[0].familyId)
  assert.equal(selected.length, 1)
  assert.equal(selected[0].available, true)
  assert.equal(selected[0].identityClass, "STRONG")
})
...
test("shared identity matcher fails closed on conflicts and does not call family exact", () => {
  ...
  const singleKeywordFalsePositive = matcher.resolveSharedProductIdentityMatchV1({
    targetPhrases: ["9001e series battery switch", "switch", "blue"],
    candidateTitle: "Blue professional inverted striping paint spray",
  })
  assert.notEqual(singleKeywordFalsePositive.classification, "STRONG")
  const genericTwoWordFalsePositive = matcher.resolveSharedProductIdentityMatchV1({
    targetPhrases: ["9001e series battery switch", "battery switch"],
    candidateTitle: "High Current Heavy Duty Battery Disconnect Switch",
  })
  assert.notEqual(genericTwoWordFalsePositive.classification, "STRONG")
