...
test("interactive Mayel pages never mount the Research navigation fallback", () => {
  assert.doesNotMatch(ownerShell, /ProductResearchAutonomousAcquisitionV1/)
  assert.match(extensionWorker, /browserWorkerControl=1/)
  assert.match(extensionWorker, /chrome\.tabs\.create\([\s\S]*active: false/)
  assert.match(extensionWorker, /WORKER_CONTROL_ALARM/)
})
...
const acquisitionMigration = readFileSync(
  ...
  "utf8")
const downstreamResumeMigration = readFileSync(
  "supabase/migrations/20260906071527_mayel_product_research_downstream_resume_v1.sql",
  "utf8")
...
const evidenceQualityPrepareMigration = readFileSync(
  "supabase/migrations/20260919110000_pre_research_evidence_quality_v2_prepare.sql",
  "utf8")
const evidenceQualityActivateMigration = readFileSync(
...
test("authenticated browser worker acquires plans autonomously with one durable lease", () => {
  ...
  assert.match(service, /resumeMayelMarketRevalidationDownstreamV1/)
  assert.match(service, /repeatedBrowserCapture: false/)
  assert.match(service, /sameTimestamp/)
  assert.match(downstreamResumeMigration,
    /claim_next_live_listing_product_research_v2/)
  assert.match(downstreamResumeMigration,
    /plan\.status in \('ACTIVE', 'COMPLETED'\)/)
  assert.match(downstreamResumeMigration, /task\.status = 'PROCESSED'/)
  assert.match(fairnessMigration, /plan\.status = 'ACTIVE'/)
  assert.match(fairnessMigration, /task\.status = 'PENDING'/)
  assert.doesNotMatch(runner, /RESUME_MARKET_REVALIDATION_DOWNSTREAM/)
  assert.match(runner,
    /if \(autonomous\)[\s\S]*window\.location\.replace\(safeReturnPath\(\)\)/)
...
test("Luna Pre-Research disposition uses all settled deduplicated comparable evidence", () => {
  assert.match(service,
    /select\("id,status,capture_batch_id,quality_status,quality_metrics,commercial_evidence_entities"\)/)
  assert.match(service, /summarizeProductResearchComparableEvidenceV1\(planEvidenceRows\)/)
  assert.match(service, /rawObservedSoldQuantity/)
  assert.match(service, /acceptedComparableSoldQuantity/)
  assert.match(service, /acceptedComparableCount/)
  assert.doesNotMatch(service,
    /confirmedSoldQuantity > 0 \? "PRE_RESEARCH_MEDIUM"/)
})

test("durable RPC rejects HIGH or MEDIUM without accepted comparables", () => {
  assert.match(evidenceQualityActivateMigration,
    /LUNA_PRE_RESEARCH_RESULT_V2_2026_09_19/)
  assert.match(evidenceQualityActivateMigration,
    /p_result in \('PRE_RESEARCH_HIGH','PRE_RESEARCH_MEDIUM'\)[\s\S]*v_accepted_count <= 0/)
  assert.match(evidenceQualityActivateMigration,
    /v_accepted_count = 0[\s\S]*p_result <> 'INSUFFICIENT_MARKET_EVIDENCE'/)
  assert.match(evidenceQualityActivateMigration, /set search_path = ''/)
  assert.match(evidenceQualityActivateMigration,
    /grant execute[\s\S]*to service_role/)
  assert.match(evidenceQualityPrepareMigration,
    /derive_luna_pre_research_evidence_v2/)
})
