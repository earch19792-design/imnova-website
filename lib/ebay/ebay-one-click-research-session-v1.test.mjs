...
function createAdminBridgeHarness({
    ...
    runtime: {
      ...
      sendMessage: (command) => {
        ...
        return Promise.resolve({ success: true, ready: true,
          ...
          extensionVersion: "1.2.23", persistentCredential: false,
  ...
  ), { chrome, document, MutationObserver: HarnessMutationObserver, Promise, window })
  return {
    posted,
...
test("direct and same-document navigation reuse one route-aware admin bridge authority", async () => {
  ...
  assert.equal(bridgeAck.payload.extensionVersion, "1.2.23")
  ...
  )).payload.extensionVersion, "1.2.23")
  ...
  assert.equal(canonicalWorkerAck?.extensionVersion, "1.2.23")
...
test("an invalidated extension context fails closed and a hard-refreshed document boots a new bridge", async () => {
  ...
  assert.equal(response.success, true)
  assert.equal(response.payload.extensionVersion, "1.2.23")
})
