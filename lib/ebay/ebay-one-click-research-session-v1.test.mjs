    ...
    runtime: {
      ...
      sendMessage: (command) => {
        ...
        return Promise.resolve({ success: true, ready: true,
          ...
          extensionVersion: "1.2.24", persistentCredential: false,
...
function createBackgroundProbeHarness() {
  const listeners = []
  ...
  return {
    ...
    tabQueries,
    injections,
    installUpdate: async ({ reason = "update", tabs = [] } = {}) => {
      existingTabs = tabs
...
test("direct and same-document navigation reuse one route-aware admin bridge authority", async () => {
  ...
  assert.equal(bridgeAck.payload.extensionVersion, "1.2.24")
  ...
  )).payload.extensionVersion, "1.2.24")
  ...
  assert.equal(canonicalWorkerAck?.extensionVersion, "1.2.24")
...
test("an unpacked v1.2.24 update injects the guarded bridge into existing canonical tabs once", async () => {
  const manifest = JSON.parse(readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/manifest.json", "utf8",
  ))
  assert.equal(manifest.version, "1.2.24")
  ...
  const worker = createBackgroundProbeHarness()
  assert.equal(worker.installedListeners.length, 1)
  await worker.installUpdate({ tabs: [
  ...
  await worker.installUpdate({ reason: "chrome_update", tabs: [
    { id: 44, url: `${canonicalAdminOrigin}/admin/ebay/mobile-review` },
  ] })
  assert.equal(worker.tabQueries.length, 1)
  assert.equal(worker.injections.length, 2)
  ...
  bridge.injectBridge()
  assert.equal(bridge.listenerCount(), 1)
  assert.equal((await bridge.probe(
...
test("an invalidated extension context fails closed and a hard-refreshed document boots a new bridge", async () => {
  ...
  assert.equal(response.success, true)
  assert.equal(response.payload.extensionVersion, "1.2.24")
})
