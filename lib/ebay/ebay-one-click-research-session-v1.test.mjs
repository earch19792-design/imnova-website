...
function createAdminBridgeHarness({
  ...
  const window = {
    location: { origin: canonicalAdminOrigin, pathname },
    ...
    runtime: {
      ...
      sendMessage: (command) => {
        ...
        return Promise.resolve({ success: true, ready: true,
          ...
          extensionVersion: "1.2.25", persistentCredential: false,
  ...
  return {
    ...
    probe: (pathname) => {
      ...
      for (const listener of listeners) listener({
        ...
        frameId: 0,
        tab: { url: `${canonicalAdminOrigin}${pathname}` },
...
test("direct and same-document navigation reuse one route-aware admin bridge authority", async () => {
  ...
  const adminScopeMatch = `${canonicalAdminOrigin}/admin/ebay/*`
  assert.equal(manifest.version, "1.2.25")
  assert.ok(manifest.host_permissions.includes(adminScopeMatch))
  assert.deepEqual(adminContentScript.matches, [adminScopeMatch])
  assert.ok(!manifest.host_permissions.some((entry) =>
    /\*\.vercel\.app|<all_urls>/.test(entry)))

  const canonicalBridge = createAdminBridgeHarness({ pathname: canonicalPath })
  assert.equal(canonicalBridge.listenerCount(), 1)
  const bridgeAck = await canonicalBridge.probe(
    "77777777-7777-4777-8777-777777777777",
  )
  assert.equal(bridgeAck.success, true)
  assert.equal(bridgeAck.payload.extensionVersion, "1.2.25")
  ...
  )).payload.extensionVersion, "1.2.25")

  const spaBridge = createAdminBridgeHarness({ pathname: "/admin/ebay/monitor" })
  ...
  assert.equal(spaBridge.listenerCount(), 0)
  spaBridge.navigate("/admin/ebay/mobile-review")
  assert.equal(spaBridge.listenerCount(), 1)
  spaBridge.navigate("/admin/ebay/mobile-review")
  assert.equal(spaBridge.listenerCount(), 1)
  ...
  assert.equal(canonicalWorkerAck?.extensionVersion, "1.2.25")
...
test("v1.2.25 worker boot recovers existing tabs and remains idempotent across restarts", async () => {
  const manifest = JSON.parse(readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/manifest.json", "utf8",
  ))
  ...
  assert.deepEqual(worker.tabQueries, [
    { url: `${canonicalAdminOrigin}/admin/ebay/*` },
  ])
...
test("an invalidated extension context fails closed and a hard-refreshed document boots a new bridge", async () => {
  ...
  assert.equal(response.success, true)
  assert.equal(response.payload.extensionVersion, "1.2.25")
})
