...
test("the runtime accepts only proven capture-compatible artifacts and names restart support", () => {
  ...
  assert.deepEqual(EBAY_ONE_CLICK_RESEARCH_CAPTURE_COMPATIBILITY.map(
    ...
    { version: "1.2.27", browserRestartRecoverySupported: true },
  ])
  ...
  assert.throws(() => attestEbayOneClickResearchExtensionArtifact({
    extensionVersion: "1.2.27", manifestOriginMatch: false,
  }), /ONE_CLICK_RESEARCH_EXTENSION_ARTIFACT_MISMATCH/)
})
    ...
    runtime: {
      ...
      sendMessage: (command) => {
        ...
        return Promise.resolve({ success: true, ready: true,
          ...
          extensionVersion: "1.2.27", persistentCredential: false,
...
test("direct load, menu entry, SPA re-entry, refresh, and repetition reuse one bridge", async () => {
  ...
  for (const match of adminScopeMatches) {
  ...
  for (const [index, origin] of authorizedAdminOrigins.entries()) {
    ...
    assert.equal(bridgeAck.payload.extensionVersion, "1.2.27")
    ...
    )).payload.extensionVersion, "1.2.27")
  ...
  assert.equal(canonicalWorkerAck?.extensionVersion, "1.2.27")
...
test("v1.2.27 worker boot recovers existing tabs and remains idempotent across restarts", async () => {
  const manifest = JSON.parse(readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/manifest.json", "utf8",
  ))
...
test("an invalidated extension context fails closed and a hard-refreshed document boots a new bridge", async () => {
  ...
  assert.equal(response.success, true)
  assert.equal(response.payload.extensionVersion, "1.2.27")
})
