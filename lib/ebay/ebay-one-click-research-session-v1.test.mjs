...
test("the exact canonical opportunities route reuses the mobile-review bridge authority", async () => {
  ...
  const canonicalMatch = `${canonicalAdminOrigin}${canonicalPath}*`
  const fallbackMatch = `${canonicalAdminOrigin}/admin/ebay/mobile-review*`
  ...
  assert.ok(manifest.host_permissions.includes(canonicalMatch))
  assert.ok(manifest.host_permissions.includes(fallbackMatch))
  assert.ok(adminContentScript.matches.includes(canonicalMatch))
  assert.ok(adminContentScript.matches.includes(fallbackMatch))
  ...
  assert.equal(bridgeAck.payload.extensionVersion, "1.2.22")
  ...
  assert.equal(canonicalWorkerAck?.extensionVersion, "1.2.22")
...
test("the same extension owns a bounded page-authorized Product Research and Sold bridge", () => {
  ...
  assert.ok(manifest.host_permissions.includes(
    "https://imnova-website-z1qh-canonical-preview.vercel.app/admin/ebay/opportunity-queue/research*",
  ))
