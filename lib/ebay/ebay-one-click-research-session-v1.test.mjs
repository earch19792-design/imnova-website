...
test("the runtime accepts only proven capture-compatible artifacts and names restart support", () => {
  assert.deepEqual(EBAY_ONE_CLICK_RESEARCH_EXTENSION_ARTIFACT, {
    ...
    artifactSha256: "4b9f4f0fa632bbde95108dfab7ebc566b5d1cf2ce060bb50862ff3bf09325c8c",
    sourceTreeSha256: "a8ea89b80c677e392d6dcd3a925274551fdfb00d80f3b13ca7e1ffb16fc490cf",
  ...
  assert.deepEqual(attestEbayOneClickResearchExtensionArtifact({
    extensionVersion: "1.2.38", extensionId: "ajplldjfkdgigcibbplcffhafendcnei",
...
test("v1.2.38 worker boot recovers existing tabs and remains idempotent across restarts", async () => {
  const manifest = JSON.parse(readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/manifest.json", "utf8",
  ))
