...
test("the runtime artifact marker binds the exact v1.2.26 build and manifest origin", () => {
  ...
  assert.throws(() => attestEbayOneClickResearchExtensionArtifact({
    extensionVersion: "1.2.26", manifestOriginMatch: false,
  }), /ONE_CLICK_RESEARCH_EXTENSION_ARTIFACT_MISMATCH/)
})
    ...
    runtime: {
      ...
      sendMessage: (command) => {
        ...
        return Promise.resolve({ success: true, ready: true,
          ...
          extensionVersion: "1.2.26", persistentCredential: false,
...
test("direct load, menu entry, SPA re-entry, refresh, and repetition reuse one bridge", async () => {
  ...
  for (const [index, origin] of authorizedAdminOrigins.entries()) {
    ...
    assert.equal(bridgeAck.payload.extensionVersion, "1.2.26")
    ...
    )).payload.extensionVersion, "1.2.26")
  ...
  assert.equal(canonicalWorkerAck?.extensionVersion, "1.2.26")
...
test("v1.2.26 worker boot recovers existing tabs and remains idempotent across restarts", async () => {
  const manifest = JSON.parse(readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/manifest.json", "utf8",
  ))
  ...
  assert.doesNotMatch(background, /runtime\.onInstalled/)
...
test("an invalidated extension context fails closed and a hard-refreshed document boots a new bridge", async () => {
  ...
  assert.equal(response.success, true)
  assert.equal(response.payload.extensionVersion, "1.2.26")
})
