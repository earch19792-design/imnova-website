...
import {
  ...
  EBAY_ONE_CLICK_RESEARCH_CAPTURE_COMPATIBILITY,
  EBAY_ONE_CLICK_RESEARCH_EXTENSION_ARTIFACT,
  EBAY_ONE_CLICK_RESEARCH_BRIDGE_VERSION,
  EBAY_COMMERCIAL_TRACE_BRIDGE_RECONNECT_BOUNDS,
  ...
  probeEbayCommercialTraceBridgeWithReconnectV1,
  validateEbayCommercialTraceBridgeProofV1,
...
import { adaptMainSearchSoldCaptureForCanonicalImport,
  adaptMainSearchSoldCaptureForCommercialTrace } from
...
const dedicatedPreprodOrigin = "https://imnova-seller-os-preprod.vercel.app"
const dedicatedPreprodAlias = "https://imnova-ebay-mobile-preprod.vercel.app"
const authorizedAdminOrigins = [
  canonicalAdminOrigin, dedicatedPreprodOrigin, dedicatedPreprodAlias,
]

test("commercial trace accepts only fresh query-bound public SOLD rows", () => {
  const query = "5 in 1 microcurrent facial device"
  const captured = adaptMainSearchSoldCaptureForCommercialTrace({
    now,
    requestedQueries: [query],
    rows: [{ itemId: "123456789012", title:
      "5 in 1 Microcurrent Facial Device Face Massager", soldAt:
      "2026-08-25T12:00:00.000Z", capturedAt: "2026-08-26T11:59:00.000Z",
      queryOrResearchIdentity: query, displayedSoldPriceAmount: 24.99,
      bestOfferStatus: "UNKNOWN", shippingStatus: "OBSERVED",
      visibleShippingAmount: 4.99, sellerUsername: "market-seller",
      sellerProfileUrl: "https://www.ebay.com/usr/market-seller",
      sellerIdentityStatus: "PROVEN", sellerIdentitySource: "SELLER_PROFILE_LINK" }],
  })
  assert.equal(captured.acceptedRowCount, 1)
  assert.equal(captured.rows[0].confirmedSoldQuantity, 1)
  assert.equal(captured.rows[0].realizedPriceStatus, "UNPROVEN")
  assert.equal(captured.rows[0].soldConfirmationStatus, "SOLD_CONFIRMED")
  assert.equal(captured.rows[0].displayedPriceStatus,
    "DISPLAYED_PRICE_CONFIRMED")
  assert.equal(captured.rows[0].sellerIdentityStatus, "PROVEN")
  assert.equal(captured.rows[0].sellerIdentityKey, "SELLER:market-seller")
  assert.equal(captured.rows[0].pricingEligibility,
    "DISPLAYED_PRICE_NOT_REALIZED")
  assert.equal(captured.rows[0].soldFieldProvenance.sellerIdentity.source,
    "SOLD_SEARCH_ROW")
  assert.equal(captured.rows[0].soldFieldProvenance.shipping.status, "OBSERVED")
  assert.equal(captured.rows[0].soldFieldProvenance.realizedPrice.status,
    "UNPROVEN")
  const mismatch = adaptMainSearchSoldCaptureForCommercialTrace({ now,
    requestedQueries: [query], rows: [{ ...captured.rows[0],
      queryOrResearchIdentity: "owner supplied benchmark" }] })
  assert.equal(mismatch.acceptedRowCount, 0)
  assert.match(mismatch.rejectionReasons[0], /QUERY_MISMATCH/)
})

test("commercial trace rejects localized seller labels and preserves unknown shipping", () => {
  const query = "microcurrent facial device"
  const captured = adaptMainSearchSoldCaptureForCommercialTrace({ now,
    requestedQueries: [query], rows: [{ itemId: "123456789013", title:
      "Microcurrent Facial Device", soldAt: "2026-08-25T12:00:00.000Z",
      capturedAt: "2026-08-26T11:59:00.000Z",
      queryOrResearchIdentity: query, displayedSoldPriceAmount: 19.99,
      bestOfferStatus: "UNKNOWN", shippingStatus: "UNAVAILABLE",
      visibleShippingAmount: null, sellerUsername: "Vendedor eBay" }] })
  assert.equal(captured.acceptedRowCount, 1)
  assert.equal(captured.rows[0].sellerUsername, null)
  assert.equal(captured.rows[0].sellerIdentityStatus, "UNKNOWN")
  assert.equal(captured.rows[0].sellerIdentityKey,
    "SELLER_IDENTITY_UNKNOWN")
  assert.equal(captured.rows[0].visibleShippingAmount, null)
  assert.equal(captured.rows[0].shippingStatus, "UNAVAILABLE")
})

test("proven Free Shipping search branch yields zero shipping with field provenance", () => {
  const query = "generic facial device"
  const navigationUrl = "https://www.ebay.com/sch/i.html?_nkw=generic+facial+device&LH_Sold=1&LH_Complete=1&LH_FS=1"
  const captured = adaptMainSearchSoldCaptureForCommercialTrace({ now,
    requestedQueries: [query], rows: [{ itemId: "123456789014", title:
      "Generic Facial Device Massager", soldAt:
      "2026-08-25T12:00:00.000Z", capturedAt: "2026-08-26T11:59:00.000Z",
      queryOrResearchIdentity: query, displayedSoldPriceAmount: 21.5,
      bestOfferStatus: "UNKNOWN", shippingStatus: "OBSERVED",
      visibleShippingAmount: 0,
      shippingEvidence: "PROVEN_ZERO_BY_SEARCH_FILTER",
      searchBranch: "FREE_SHIPPING_ONLY", freeShippingFilterProven: true,
      navigationEvidence: { requestedUrl: navigationUrl,
        pendingUrl: navigationUrl, effectiveUrl: navigationUrl,
        documentId: "document-free-shipping-0001",
        navigationStartedAt: "2026-08-26T11:58:58.000Z",
        navigationCompletedAt: "2026-08-26T11:58:59.000Z",
        filterProofStatus: "FREE_SHIPPING_FILTER_PROVEN" },
      fieldProvenance: { shipping: {
        source: "SOLD_SEARCH_FREE_SHIPPING_FILTER", status: "OBSERVED",
        evidenceItemId: "123456789014" } } }] })
  assert.equal(captured.acceptedRowCount, 1)
  assert.equal(captured.rows[0].visibleShippingAmount, 0)
  assert.equal(captured.rows[0].shippingEvidence,
    "PROVEN_ZERO_BY_SEARCH_FILTER")
  assert.equal(captured.rows[0].displayedDeliveredPrice, 21.5)
  assert.equal(captured.rows[0].realizedPriceStatus, "UNPROVEN")
})

test("Free Shipping zero is rejected without an effective filter attestation", () => {
  const query = "generic facial device"
  const captured = adaptMainSearchSoldCaptureForCommercialTrace({ now,
    requestedQueries: [query], rows: [{ itemId: "123456789015", title:
      "Generic Facial Device", soldAt: "2026-08-25T12:00:00.000Z",
      capturedAt: "2026-08-26T11:59:00.000Z",
      queryOrResearchIdentity: query, displayedSoldPriceAmount: 20,
      bestOfferStatus: "UNKNOWN", shippingStatus: "OBSERVED",
      visibleShippingAmount: 0,
      shippingEvidence: "PROVEN_ZERO_BY_SEARCH_FILTER",
      searchBranch: "ALL_SOLD", freeShippingFilterProven: false }] })
  assert.equal(captured.acceptedRowCount, 0)
})

test("ended-item detail can prove fields but never erase explicit Best Offer", () => {
  const query = "microcurrent facial device"
  const captured = adaptMainSearchSoldCaptureForCommercialTrace({ now,
    requestedQueries: [query], rows: [{ itemId: "137402385677", title:
      "Microcurrent Facial Device", soldAt: "2026-08-25T12:00:00.000Z",
      capturedAt: "2026-08-26T11:59:00.000Z",
      queryOrResearchIdentity: query, displayedSoldPriceAmount: 23.99,
      bestOfferStatus: "EXPLICIT_PRESENT", shippingStatus: "OBSERVED",
      visibleShippingAmount: 4.99, sellerUsername: "ended-seller",
      sellerProfileUrl: "https://www.ebay.com/usr/ended-seller",
      ...
      fieldProvenance: {
        sellerIdentity: { source: "ENDED_ITEM_PUBLIC_DETAIL",
          status: "CONFIRMED", evidenceItemId: "137402385677" },
        shipping: { source: "ENDED_ITEM_PUBLIC_DETAIL",
          status: "OBSERVED", evidenceItemId: "137402385677" },
  ...
  assert.equal(captured.rows[0].realizedPriceStatus, "UNPROVEN")
  assert.equal(captured.rows[0].bestOfferStatus, "EXPLICIT_PRESENT")
  assert.equal(captured.rows[0].pricingEligibility,
    "DISPLAYED_PRICE_NOT_REALIZED")
  assert.equal(captured.rows[0].soldFieldProvenance.shipping.source,
    "ENDED_ITEM_PUBLIC_DETAIL")
})

test("commercial trace can close a proven zero-result SOLD query", () => {
  const captured = adaptMainSearchSoldCaptureForCommercialTrace({ now,
    requestedQueries: ["generic facial device"], rows: [] })
  assert.equal(captured.acceptedRowCount, 0)
  assert.equal(captured.soldFilterRequired, true)
})

test("the runtime accepts only proven capture-compatible artifacts and names restart support", () => {
  assert.deepEqual(EBAY_ONE_CLICK_RESEARCH_EXTENSION_ARTIFACT, {
    version: "1.2.40",
    buildId: "9a4d67ffe9729b737340c21ee2b450d692c63ca68d6bb79666cd132f79b39e43",
    artifactSha256: "9a4d67ffe9729b737340c21ee2b450d692c63ca68d6bb79666cd132f79b39e43",
    sourceTreeSha256: "5f0ccde772d32a9327f4e9533cf45fdb72fc235ef6638d3038b493015b0fce03",
    extensionId: "llngdlffmjbnbmbffkfbknkkddjoknka",
    expectedCapabilities: [
      "PRODUCT_RESEARCH", "PRODUCT_RESEARCH_BROWSER_CAPTURE",
      "NEAR_EXACT_SOLD_ENRICHMENT", "ENDED_ITEM_PUBLIC_DETAIL",
      "FREE_SHIPPING_FILTER_PROVENANCE", "DOCUMENT_BOUND_CAPTURE",
      "BROWSER_WORKER_CONTROL", "NO_COOKIE_ACCESS", "NO_MARKETPLACE_WRITES",
    ],
    archivePath:
      "/seller-os-tools/ebay-product-research-capture-extension-v1.2.40.zip",
  })
  assert.deepEqual(attestEbayOneClickResearchExtensionArtifact({
    extensionVersion: "1.2.40", extensionId: "llngdlffmjbnbmbffkfbknkkddjoknka",
    manifestOriginMatch: true,
  }), {
    extensionVersion: "1.2.40",
    buildId: "9a4d67ffe9729b737340c21ee2b450d692c63ca68d6bb79666cd132f79b39e43",
    artifactSha256: "9a4d67ffe9729b737340c21ee2b450d692c63ca68d6bb79666cd132f79b39e43",
    sourceTreeSha256: "5f0ccde772d32a9327f4e9533cf45fdb72fc235ef6638d3038b493015b0fce03",
    extensionId: "llngdlffmjbnbmbffkfbknkkddjoknka",
    ...
    browserRestartRecoverySupported: true,
    manifestOriginMatch: true,
  })
  assert.deepEqual(attestEbayOneClickResearchExtensionArtifact({
    extensionVersion: "1.2.27", manifestOriginMatch: true,
  }), {
    extensionVersion: "1.2.27",
    buildId: "e48924c20aa5ec439224a34c0a696e5b85ed19ba",
    browserRestartRecoverySupported: false,
    manifestOriginMatch: true,
  })
  assert.deepEqual(attestEbayOneClickResearchExtensionArtifact({
    extensionVersion: "1.2.26", manifestOriginMatch: true,
  }), {
    extensionVersion: "1.2.26",
    buildId: "10fa05a43791ff4bd42083d0ca88341a5c99a947",
    browserRestartRecoverySupported: false,
    manifestOriginMatch: true,
  })
  assert.deepEqual(EBAY_ONE_CLICK_RESEARCH_CAPTURE_COMPATIBILITY.map(
    ({ version, browserRestartRecoverySupported }) =>
      ({ version, browserRestartRecoverySupported })), [
    { version: "1.2.26", browserRestartRecoverySupported: false },
    { version: "1.2.27", browserRestartRecoverySupported: false },
    { version: "1.2.28", browserRestartRecoverySupported: true },
    { version: "1.2.29", browserRestartRecoverySupported: true },
    { version: "1.2.31", browserRestartRecoverySupported: true },
    { version: "1.2.38", browserRestartRecoverySupported: true },
    { version: "1.2.39", browserRestartRecoverySupported: true },
    { version: "1.2.40", browserRestartRecoverySupported: true },
  ])
  assert.throws(() => attestEbayOneClickResearchExtensionArtifact({
    extensionVersion: "1.2.25", manifestOriginMatch: true,
  }), /ONE_CLICK_RESEARCH_EXTENSION_ARTIFACT_MISMATCH/)
  assert.throws(() => attestEbayOneClickResearchExtensionArtifact({
    extensionVersion: "1.2.28", manifestOriginMatch: false,
  }), /ONE_CLICK_RESEARCH_EXTENSION_ARTIFACT_MISMATCH/)
  assert.throws(() => attestEbayOneClickResearchExtensionArtifact({
    extensionVersion: "1.2.39", extensionId:
      "mjemikjbkpmhobhgmnpchdimnlalmgld", manifestOriginMatch: true,
  }), /ONE_CLICK_RESEARCH_EXTENSION_IDENTITY_MISMATCH/)
})

function createAdminBridgeHarness({
  autoInject = true,
  contextActive = true,
  ...
  window.navigation = { addEventListener: addListener, removeEventListener: removeListener }
  window.top = window
  const document = {}
    ...
    runtime: {
      ...
      sendMessage: (command) => {
        ...
        return Promise.resolve({ success: true, ready: true,
          ...
          extensionVersion: "1.2.28", persistentCredential: false,
  ...
  const sandbox = { chrome, document, MutationObserver: HarnessMutationObserver,
    Promise, window }
  const injectBridge = () => runInNewContext(bridgeSource, sandbox)
  if (autoInject) injectBridge()
  return {
    posted,
    injectBridge,
    ...
    navigate: (nextPathname) => {
      ...
      for (const listener of [...(listeners.get("navigatesuccess") ?? [])]) listener()
      for (const observer of [...mutationObservers]) observer([])
    },
...
function createBackgroundProbeHarness({ tabs = [] } = {}) {
  const listeners = []
  const tabQueries = []
  const injections = []
  ...
  const chrome = {
    ...
    tabs: {
      query: async (filter) => {
        tabQueries.push(structuredClone(filter))
        return existingTabs
      },
    },
    scripting: {
      executeScript: async (input) => { injections.push(structuredClone(input)) },
    },
  }
  ...
  const bootWorker = () => {
    listeners.length = 0
    const sandbox = {
      AbortController, Blob, Date, Intl, Promise, URL, URLSearchParams, chrome,
      clearTimeout, fetch: async () => { throw new Error("UNEXPECTED_NETWORK_CALL") },
      setTimeout,
    }
    sandbox.importScripts = (...files) => {
      for (const file of files) runInNewContext(readFileSync(
        `tools/browser-extensions/ebay-product-research-capture/${file}`, "utf8",
      ), sandbox)
    }
    runInNewContext(backgroundSource, sandbox)
  }
  ...
  return {
    ...
    injections,
    flush,
    workerListenerCount: () => listeners.length,
    restartWorker: async ({ tabs = existingTabs } = {}) => {
      existingTabs = tabs
      bootWorker()
      await flush()
    },
    probe: (pathname, origin = canonicalAdminOrigin) => {
      let response = null
      for (const listener of listeners) listener({
        type: "IMNOVA_EBAY_ONE_CLICK_RESEARCH_PROBE_V1",
      }, {
        frameId: 0,
        tab: { url: `${origin}${pathname}` },
...
test("mixed canonical Sold rows and pack signals are a durable success without replaying settled tasks", () => {
  assert.deepEqual(validateEbayOneClickDurableSoldEvidenceOutcome({
    status: "PASS_WITH_PACK_SIGNALS",
    readbackCount: 7,
    freshSoldRows: 3,
    commercialPackSignalsPreserved: 4,
    evidenceMaxAgeDays: 22,
    displayedVsRealizedGuard: "PASS",
    bestOfferGuard: "PASS",
    marketplaceWrites: 0,
  }), {
  ...
  assert.throws(() => validateEbayOneClickDurableSoldEvidenceOutcome({
    status: "PASS_PACK_SIGNALS_ONLY",
    readbackCount: 4,
    freshSoldRows: 0,
    commercialPackSignalsPreserved: 4,
    evidenceMaxAgeDays: 22,
    ...
    bestOfferGuard: "PASS",
    marketplaceWrites: 0,
  }), /ONE_CLICK_RESEARCH_DURABLE_VALIDATION_FAILED/)

  const resumed = buildEbayOneClickResearchPlan({
    status: "ACTIVE",
    tasks: [
      ...[1, 2, 3, 4].map((ordinal) => ({ id: `settled-${ordinal}`, ordinal,
        search_query: `settled task ${ordinal}`, category_id: null,
        candidate_count: 1, status: "PROCESSED" })),
      { id: "pending-5", ordinal: 5, search_query: "master lock tsa padlock",
        category_id: null, candidate_count: 1, status: "PENDING" },
    ],
  })
  assert.deepEqual(resumed.tasks.map((task) => task.ordinal), [5])
})
...
test("direct load, menu entry, SPA re-entry, refresh, and repetition reuse one bridge", async () => {
  const manifest = JSON.parse(readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/manifest.json", "utf8",
  ))
  const canonicalPath = "/admin/ebay/opportunity-queue/research"
  const adminContentScript = manifest.content_scripts.find((entry) =>
    entry.js.includes("admin-bridge.js"))

  const adminScopeMatches = [
    ...authorizedAdminOrigins.map((origin) => `${origin}/admin/ebay/*`),
    ...authorizedAdminOrigins.map((origin) => `${origin}/admin/ebay-seller-os*`),
  ]
  const adminHostMatches = authorizedAdminOrigins.map(
    (origin) => `${origin}/admin/*`,
  )
  assert.equal(manifest.version, "1.2.40")
  for (const match of adminHostMatches) {
    assert.ok(manifest.host_permissions.includes(match))
  }
  assert.deepEqual(adminContentScript.matches, adminScopeMatches)
  assert.ok(!manifest.host_permissions.some((entry) =>
    /\*\.vercel\.app|<all_urls>/.test(entry)))

  for (const [index, origin] of authorizedAdminOrigins.entries()) {
    const directBridge = createAdminBridgeHarness({ pathname: canonicalPath, origin })
    assert.equal(directBridge.listenerCount(), 1)
    const bridgeAck = await directBridge.probe(
      `77777777-7777-4777-8777-77777777777${index}`,
    )
    assert.equal(bridgeAck.success, true)
    assert.equal(bridgeAck.payload.extensionVersion, "1.2.28")

    const spaBridge = createAdminBridgeHarness({
      pathname: "/admin/ebay/monitor", origin,
    })
    assert.equal(spaBridge.listenerCount(), 0)
    assert.equal(spaBridge.posted.length, 0)
    spaBridge.navigate(canonicalPath)
    assert.equal(spaBridge.listenerCount(), 1)
    assert.equal((await spaBridge.probe(
      `99999999-9999-4999-8999-99999999999${index}`,
    )).payload.extensionVersion, "1.2.28")
    spaBridge.navigate("/admin/ebay/monitor")
    assert.equal(spaBridge.listenerCount(), 0)
    spaBridge.navigate(canonicalPath)
    spaBridge.navigate(canonicalPath)
    assert.equal(spaBridge.listenerCount(), 1)

    const refreshedBridge = createAdminBridgeHarness({ pathname: canonicalPath, origin })
    assert.equal(refreshedBridge.listenerCount(), 1)
    assert.equal((await refreshedBridge.probe(
      `aaaaaaaa-7777-4777-8777-77777777777${index}`,
    )).payload.extensionVersion, "1.2.28")
  }

  const fallbackBridge = createAdminBridgeHarness({ pathname: "/admin/ebay/mobile-review" })
  assert.equal(fallbackBridge.listenerCount(), 1)
  assert.equal((await fallbackBridge.probe(
    "88888888-8888-4888-8888-888888888888",
  )).payload.extensionVersion, "1.2.28")

  const worker = createBackgroundProbeHarness()
  const canonicalWorkerAck = worker.probe(canonicalPath)
  const fallbackWorkerAck = worker.probe("/admin/ebay/mobile-review")
  assert.equal(canonicalWorkerAck?.ready, true)
  assert.equal(canonicalWorkerAck?.extensionVersion, "1.2.28")
  assert.equal(fallbackWorkerAck?.ready, true)
  assert.equal(worker.probe(canonicalPath, dedicatedPreprodOrigin)?.ready, true)
  assert.equal(worker.probe(canonicalPath, dedicatedPreprodAlias)?.ready, true)
  assert.equal(worker.probe("/admin/ebay/opportunity-queue/research-extra"), null)

  const rejectedBridge = createAdminBridgeHarness({
    pathname: "/admin/ebay/opportunity-queue/research-extra",
  })
  assert.equal(rejectedBridge.listenerCount(), 0)
  assert.equal(rejectedBridge.posted.length, 0)
})

test("v1.2.40 worker boot recovers existing tabs and remains idempotent across restarts", async () => {
  const manifest = JSON.parse(readFileSync(
    "tools/browser-extensions/ebay-product-research-capture/manifest.json", "utf8",
  ))
  ...
  assert.match(background, /browserWorkerControl/)
  assert.match(background, /active: false/)
  assert.match(background, /void injectAdminBridgeIntoExistingTabs\(\)/)
  assert.match(background, /Promise\.allSettled/)

  const existingTabs = [
    { id: 41, url: `${canonicalAdminOrigin}/admin/ebay/opportunity-queue/research` },
    { id: 42, url: `${canonicalAdminOrigin}/admin/ebay/monitor` },
    { id: 43, url: "https://unrelated.example/admin/ebay/mobile-review" },
    { id: null, url: `${canonicalAdminOrigin}/admin/ebay/mobile-review` },
  ]
  const worker = createBackgroundProbeHarness({ tabs: existingTabs })
  await worker.flush()
  assert.equal(worker.workerListenerCount(), 2)
  assert.deepEqual(worker.tabQueries, [
    { url: authorizedAdminOrigins.flatMap((origin) => [
      `${origin}/admin/ebay/*`, `${origin}/admin/ebay-seller-os*`]) },
  ])
  assert.deepEqual(worker.injections, [
    { target: { tabId: 41, frameIds: [0] }, files: ["admin-bridge.js"],
      injectImmediately: true },
    { target: { tabId: 42, frameIds: [0] }, files: ["admin-bridge.js"],
      injectImmediately: true },
  ])

  const existingOperationalDocument = createAdminBridgeHarness({
    autoInject: false,
    pathname: "/admin/ebay/opportunity-queue/research",
  })
  assert.equal(existingOperationalDocument.listenerCount(), 0)
  existingOperationalDocument.injectBridge()
  assert.equal(existingOperationalDocument.listenerCount(), 1)
  assert.equal((await existingOperationalDocument.probe(
    "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",
  )).payload.extensionVersion, "1.2.28")

  const existingDormantDocument = createAdminBridgeHarness({
    autoInject: false,
    pathname: "/admin/ebay/monitor",
  })
  existingDormantDocument.injectBridge()
  assert.equal(existingDormantDocument.listenerCount(), 0)
  existingDormantDocument.navigate("/admin/ebay/opportunity-queue/research")
  assert.equal(existingDormantDocument.listenerCount(), 1)
  assert.equal((await existingDormantDocument.probe(
    "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb",
  )).payload.extensionVersion, "1.2.28")

  await worker.restartWorker({ tabs: existingTabs })
  assert.equal(worker.workerListenerCount(), 2)
  assert.equal(worker.tabQueries.length, 2)
  assert.equal(worker.injections.length, 4)
  existingOperationalDocument.injectBridge()
  existingDormantDocument.injectBridge()
  assert.equal(existingOperationalDocument.listenerCount(), 1)
  assert.equal(existingDormantDocument.listenerCount(), 1)
  const restartRequestId = "cccccccc-cccc-4ccc-8ccc-cccccccccccc"
  assert.equal((await existingOperationalDocument.probe(restartRequestId)).success, true)
  const restartAcks = existingOperationalDocument.posted.filter((message) =>
    message.requestId === restartRequestId && message.stage === "ACK_SENT_BY_BRIDGE")
  assert.equal(restartAcks.length, 1)

  const directDocument = createAdminBridgeHarness({
    pathname: "/admin/ebay/mobile-review",
  })
  assert.equal(directDocument.listenerCount(), 1)
  const directRequestId = "dddddddd-dddd-4ddd-8ddd-dddddddddddd"
  assert.equal((await directDocument.probe(directRequestId)).success, true)
  assert.equal(directDocument.posted.filter((message) =>
    message.requestId === directRequestId && message.stage === "ACK_SENT_BY_BRIDGE").length, 1)
})

test("an invalidated extension context is classified and a rebound document boots a new bridge", async () => {
  ...
  assert.equal(failed.error, "CONTENT_SCRIPT_CONTEXT_INVALIDATED")
  ...
  assert.equal(response.success, true)
  assert.equal(response.payload.extensionVersion, "1.2.28")
})

test("extension reload with Seller OS open performs one bounded rebind and proves the current bridge", async () => {
  let probes = 0
  let rebinds = 0
  const proof = await probeEbayCommercialTraceBridgeWithReconnectV1({
    probe: async (timeoutMs) => {
      probes += 1
      assert.equal(timeoutMs,
        EBAY_COMMERCIAL_TRACE_BRIDGE_RECONNECT_BOUNDS.probeTimeoutMs)
      if (probes === 1) {
        throw new Error("CONTENT_SCRIPT_CONTEXT_INVALIDATED")
      }
      return { ready: true,
        extensionVersion: EBAY_ONE_CLICK_RESEARCH_EXTENSION_ARTIFACT.version,
        bridgeVersion: EBAY_ONE_CLICK_RESEARCH_BRIDGE_VERSION,
        bridgeInstanceId: "bridge-current-1.2.38",
        runtimeReachable: true, serviceWorkerResponse: "PROBE_ACK",
        serviceWorkerInstanceId: "worker-current-1.2.38",
        cookieAccess: false, marketplaceWrites: 0 }
    },
    awaitAutoRebind: async (code) => {
      assert.equal(code, "CONTENT_SCRIPT_CONTEXT_INVALIDATED")
      rebinds += 1
    },
    wait: async (delayMs) => assert.equal(delayMs,
      EBAY_COMMERCIAL_TRACE_BRIDGE_RECONNECT_BOUNDS.reconnectDelayMs),
  })
  assert.equal(probes, 2)
  assert.equal(rebinds, 1)
  assert.equal(proof.bridgeVersion, EBAY_ONE_CLICK_RESEARCH_BRIDGE_VERSION)
  assert.equal(proof.runtimeReachable, true)
  assert.equal(proof.serviceWorkerResponse, "PROBE_ACK")
})

test("truly unreachable runtime exhausts the bounded probe and starts zero eBay navigations", async () => {
  let probes = 0
  let rebinds = 0
  let ebayNavigations = 0
  await assert.rejects(() => probeEbayCommercialTraceBridgeWithReconnectV1({
    probe: async () => {
      probes += 1
      throw new Error("EXTENSION_RUNTIME_UNREACHABLE")
    },
    awaitAutoRebind: async () => { rebinds += 1 },
    wait: async () => undefined,
  }), /EXTENSION_RUNTIME_UNREACHABLE/)
  assert.equal(probes,
    EBAY_COMMERCIAL_TRACE_BRIDGE_RECONNECT_BOUNDS.maximumProbeAttempts)
  assert.equal(rebinds, 1)
  assert.equal(ebayNavigations, 0)
})

test("valid bridge proof gates ALL_SOLD before VERIFIED_BROWSER_SOLD persistence", async () => {
  let navigationStarted = false
  const proof = validateEbayCommercialTraceBridgeProofV1({ ready: true,
    extensionVersion: EBAY_ONE_CLICK_RESEARCH_EXTENSION_ARTIFACT.version,
    bridgeVersion: EBAY_ONE_CLICK_RESEARCH_BRIDGE_VERSION,
    bridgeInstanceId: "bridge-valid", runtimeReachable: true,
    serviceWorkerResponse: "PROBE_ACK", serviceWorkerInstanceId: "worker-valid",
    cookieAccess: false, marketplaceWrites: 0 })
  assert.equal(proof.ready, true)
  navigationStarted = true
  const captured = adaptMainSearchSoldCaptureForCommercialTrace({ now,
    requestedQueries: ["microcurrent facial device"], rows: [{
      itemId: "123456789099", title: "Microcurrent Facial Device Massager",
      soldAt: "2026-08-25T12:00:00.000Z",
      capturedAt: "2026-08-26T11:59:00.000Z",
      queryOrResearchIdentity: "microcurrent facial device",
      displayedSoldPriceAmount: 24.99, bestOfferStatus: "UNKNOWN",
      shippingStatus: "UNAVAILABLE", visibleShippingAmount: null,
      sellerIdentityStatus: "UNKNOWN",
      searchBranch: "ALL_SOLD",
    }] })
  assert.equal(navigationStarted, true)
  assert.equal(captured.acceptedRowCount, 1)
  assert.equal(captured.rows[0].soldConfirmationStatus, "SOLD_CONFIRMED")
  assert.equal(captured.rows[0].provenance,
...
test("the same extension owns a bounded page-authorized Product Research and Sold bridge", () => {
  ...
  assert.equal(manifest.version, "1.2.40")
  assert.deepEqual(manifest.permissions, ["scripting", "alarms",
    "webNavigation"])
  ...
  assert.ok(manifest.host_permissions.includes(
    "https://imnova-website-z1qh-canonical-preview.vercel.app/admin/*",
  ))
  assert.ok(manifest.host_permissions.includes(
    "https://imnova-seller-os-preprod.vercel.app/admin/*",
  ))
  assert.ok(manifest.host_permissions.includes(
    "https://imnova-ebay-mobile-preprod.vercel.app/admin/*",
  ))
  ...
  assert.deepEqual(adminContentScript.matches, [
    "https://imnova-website-z1qh-canonical-preview.vercel.app/admin/ebay/*",
    "https://imnova-seller-os-preprod.vercel.app/admin/ebay/*",
    "https://imnova-ebay-mobile-preprod.vercel.app/admin/ebay/*",
    "https://imnova-website-z1qh-canonical-preview.vercel.app/admin/ebay-seller-os*",
    "https://imnova-seller-os-preprod.vercel.app/admin/ebay-seller-os*",
    "https://imnova-ebay-mobile-preprod.vercel.app/admin/ebay-seller-os*",
  ])
  ...
  assert.match(bridge, /OPERATIONAL_PATH/)
  assert.match(bridge, /MutationObserver/)
  assert.match(bridge, /navigatesuccess/)
...
test("the authenticated page ingests through existing routes without starting heavy pipelines", () => {
  ...
  assert.match(soldRoute,
    /durableNoValidSoldEvidence \? "NO_VALID_SOLD_EVIDENCE" : "DURABLE_SOLD_EVIDENCE"/)
  assert.match(soldRoute, /PASS_PACK_SIGNALS_ONLY/)
  assert.match(soldRoute, /ONE_CLICK_RESEARCH_PACK_UNKNOWN_READBACK_MISMATCH/)
...
test("opportunities reuses the certified one-click controller as its command surface", () => {
  const opportunities = readFileSync(
    "app/admin/ebay/mobile-review/opportunity-command-center.tsx", "utf8",
  )
  const controller = readFileSync(
    "app/admin/ebay/mobile-review/loop2-top20-opportunity-pool.tsx", "utf8",
  )

  assert.match(opportunities,
    /import \{ Loop2Top20OpportunityPool \} from "\.\/loop2-top20-opportunity-pool"/)
  assert.match(opportunities,
    /<Loop2Top20OpportunityPool surface="opportunities" \/>/)
  assert.doesNotMatch(opportunities,
    /IMNOVA_EBAY_ONE_CLICK_RESEARCH_QUERY_V1|startOneClickResearch|product-research-capture/)

  assert.equal(controller.match(/const startOneClickResearch = async/g)?.length, 1)
  assert.equal(controller.match(/function OneClickResearchCommandCenter/g)?.length, 1)
  assert.match(controller, /if \(surface === "opportunities"\) return <div/)
  assert.match(controller, /\{oneClickResearchCommandCenter\}/)
  assert.match(controller, /data-commercial-pack-intelligence/)
  assert.equal(controller.match(/function CommercialPackIntelligence/g)?.length, 1)
  assert.match(controller, /EXACT_PACK_COMPARABLE/)
  assert.match(controller, /DIFFERENT_PACK_BUT_COMMERCIALLY_RELEVANT/)
  assert.match(controller, /PACK_UNKNOWN/)
  assert.match(controller, /onClick=\{onStart\}/)
  assert.match(controller, /Consultas pendientes/)
  assert.match(controller, /queryPlan\?\.pendingCount \?\? 0/)
  assert.match(controller, /Estado sesión/)
  assert.match(controller, /NO_VALID_SOLD_EVIDENCE/)
  assert.match(controller, /Resultado terminal por consulta/)
  assert.match(controller,
    /<details data-one-click-handshake-diagnostic className=/)
  assert.doesNotMatch(controller,
    /<details data-one-click-handshake-diagnostic open/)

  const passiveProbe = controller.indexOf("void probeOneClickResearchExtension().then")
  const explicitStart = controller.indexOf("const startOneClickResearch = async")
  const clickBinding = controller.indexOf("onClick={onStart}")
  assert.ok(passiveProbe >= 0 && explicitStart > passiveProbe && clickBinding >= 0)
})

test("canonical opportunities composes one-click, manual research, and pack intelligence", () => {
  const page = readFileSync(
  ...
  assert.match(page,
    /<Loop2Top20OpportunityPool surface="opportunities">/)
  assert.match(page, /data-market-opportunity-research/)
  assert.match(page, /"Analyze Opportunity"/)
  assert.doesNotMatch(page,
    /startOneClickResearch|CommercialPackIntelligence|EBAY_ONE_CLICK_RESEARCH_COMMAND/)

  const oneClick = controller.indexOf("{oneClickResearchCommandCenter}")
  const manualResearch = controller.indexOf("{children}", oneClick)
  const packIntelligence = controller.indexOf(
    "<CommercialPackIntelligence", manualResearch,
  )
  assert.ok(oneClick >= 0 && manualResearch > oneClick &&
    packIntelligence > manualResearch)
  assert.equal(controller.match(/const startOneClickResearch = async/g)?.length, 1)
  assert.equal(controller.match(/function CommercialPackIntelligence/g)?.length, 1)
  assert.match(fallback, /<Loop2Top20OpportunityPool surface="opportunities" \/>/)
})
