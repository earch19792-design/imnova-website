...
function successDependencies(result, overrides = {}) {
  ...
  const dependencies = {
    ...
    postWriteVerify: async () => {
      postWriteCalls += 1
      return {
        POST_WRITE_VERIFICATION_STATUS: "PASS",
        POST_WRITE_LIVE_COUNT: 27,
        POST_WRITE_REGISTRY_RECORD_COUNT: 31,
        POST_WRITE_MATCHED_COUNT: 24,
        POST_WRITE_MISSING_COUNT: 0,
        POST_WRITE_ORPHANED_COUNT: 0,
        POST_WRITE_AMBIGUOUS_COUNT: 0,
        POST_WRITE_COVERAGE_PERCENT: 88.89,
        POST_WRITE_HUMAN_REVIEW_COUNT: 3,
        POST_WRITE_CREATE_RELATIONS_PRESENT: 24,
        POST_WRITE_ENDED_ROWS_CONFIRMED: 4,
        POST_WRITE_DUPLICATE_ITEM_ID_RELATIONS: 0,
        POST_WRITE_PARTITION_VALID: "YES",
        HUMAN_REVIEW_ROWS_MUTATED: 0,
        HUMAN_REVIEW_RELATIONSHIPS_PRESERVED: "YES",
      }
    },
    now: () => NOW,
    ...overrides,
...
test("public DTO and Preview route expose no private action membership", () => {
  ...
  const previewBranch = route.slice(
    route.indexOf('if (action === "preview_registry_repair")'),
    route.indexOf("if (action === EXECUTE_APPROVED_REGISTRY_REPAIR_ACTION)"),
  )
  assert.doesNotMatch(previewBranch,
    /executeApprovedRegistryRepairV1|apply_ebay_registry_repair_v1|executionPlan/)
  assert.doesNotMatch(route, /registryRepairExecutionResult\.executionPlan/)
})
...
test("RPC failure is surfaced after exactly one mock attempt with no retry", async () => {
  const result = planningResult()
  let rpcCalls = 0
  const context = successDependencies(result, {
    invokeRpc: async () => {
      rpcCalls += 1
      throw new Error("SAFE_RPC_FAILURE")
    },
  })
  await assert.rejects(
    ...
    (error) => error instanceof EbayRegistryRepairExecutorError &&
      error.code === "RPC_FAILED" && error.rpcInvocationCount === 1,
  )
  assert.equal(rpcCalls, 1)
  assert.equal(context.postWriteCalls(), 0)
})

test("post-write observation failure does not retry or invent verification", async () => {
  const result = planningResult()
  const context = successDependencies(result, {
    postWriteVerify: async () => {
      throw new Error("SAFE_POST_WRITE_READ_FAILURE")
    },
  })
  const applied = await executeApprovedRegistryRepairV1WithDependencies(
    approval(result),
  ...
  assert.equal(context.rpcCalls(), 1)
  assert.equal(applied.POST_WRITE_VERIFICATION_STATUS, "UNPROVEN")
  assert.equal(applied.POST_WRITE_LIVE_COUNT, "UNPROVEN")
  assert.equal(applied.POST_WRITE_REGISTRY_RECORD_COUNT, "UNPROVEN")
  assert.equal(applied.HUMAN_REVIEW_ROWS_MUTATED, 0)
})
