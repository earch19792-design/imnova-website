...
const {
  categoryMaterialPackageChangedV1,
...
test("authority correction preserves an unchanged material category package", () => {
  assert.equal(categoryMaterialPackageChangedV1({
    currentCategoryId: "123", resolvedCategoryId: "123",
    currentAspects: { Brand: "Unbranded", Type: "Bracelet" },
    resolvedAspects: { Type: "Bracelet", Brand: "Unbranded" },
  }), false)
  assert.equal(categoryMaterialPackageChangedV1({
    currentCategoryId: "123", resolvedCategoryId: "456",
    currentAspects: {}, resolvedAspects: {},
  }), true)
  assert.equal(categoryMaterialPackageChangedV1({
    currentCategoryId: "123", resolvedCategoryId: "123",
    currentAspects: { Type: "Coin" },
    resolvedAspects: { Type: "Fragrance" },
  }), true)
})
...
async function resolveExactCakeCategory(packageData) {
  ...
  const productTruth = buildEbayCategoryResolverProductTruthV1({
    opportunity: cakeOpportunity(), packageData,
  })
  const result = await resolveAndBindEbayListingCategoryV1({
    ...
    exactCanonicalCategory: {
      ...
      semanticCompatibility: {
        ...
        categoryId: "183335",
        familyTypeFingerprint: productTruth.familyTypeFingerprint,
        evidenceClass: "EXACT_PRODUCT_TRUTH_CATEGORY",
      },
    },
...
test("durable full-Luna product type participates in the semantic taxonomy query", async () => {
  const productTruth = buildEbayCategoryResolverProductTruthV1({
    opportunity: {
      product_title: "Model A100",
      best_selling_matches: [],
      assessment: { productTruth: {
        title: "Model A100",
        sourceEvidence: { requiredItemSpecificsTruthV1: {
          lunaExactProductEvidenceSetV1: { productType: "Coffee Grinder" },
        } },
      } },
    },
  })
  const queries = []
  await resolveEbayCategoryV1({ productTruth, learningRows: [],
    taxonomyReader: async (query) => {
      queries.push(query)
      return taxonomy(null, { status: "CATEGORY_NOT_RESOLVED",
        categoryId: null, categoryName: null,
        categoryResolution: "UNRESOLVED", observedAt: null })
    } })
  assert.equal(productTruth.normalizedProductType, "coffee grinder")
  assert.match(queries[0], /coffee grinder/i)
})
...
test("EXACT_CANONICAL_CATEGORY + ASPECT_REFRESH -> CATEGORY_UNCHANGED", async () => {
  ...
  assert.equal(result.packageData.categoryResolverV1
    .explicitReclassificationRequired, false)
...
test("stale learned mappings are revalidated before reuse", async () => {
  ...
  const result = await resolveEbayCategoryV1({
    ...
    taxonomyReader: async (_query, categoryId) => {
      if (categoryId === null) return taxonomy("175757", {
        categoryResolution: "TITLE_SUGGESTION",
      })
...
test("second Window Film package is bound to its own context and persisted mapping", async () => {
  ...
  const result = await resolveAndBindEbayListingCategoryV1({
    ...
    taxonomyReader: async (_query, categoryId) => categoryId === null
      ? taxonomy("175757", { categoryResolution: "TITLE_SUGGESTION" })
      : taxonomy(categoryId),
    now: "2026-08-28T00:00:00Z",
  })
...
test("category existence without semantic proof invalidates exact authority and rematerializes through the normal resolver", async () => {
  const context = {
    marketplaceId: "EBAY_US",
    listingPackageId: "66666666-6666-4666-8666-666666666666",
    opportunityId: "77777777-7777-4777-8777-777777777777",
    candidateKey: `sha256:${"6".repeat(64)}`,
  }
  const opportunity = {
    id: context.opportunityId,
    candidate_key: context.candidateKey,
    product_title: "Portable Cool Mist Humidifier",
    product_type: "Humidifier",
    best_selling_matches: [],
    assessment: { productTruth: {
      title: "Portable Cool Mist Humidifier",
      productType: "Humidifier",
      normalizedProductFamily: "Humidifier",
    } },
  }
  const truth = buildEbayCategoryResolverProductTruthV1({ opportunity })
  const rpcCalls = []
  const supabase = {
    from() {
      const chain = {
    ...
    async rpc(name, args) {
      ...
      return { data: [{
        ...
        account_key: "seller-os-preview", marketplace_id: "EBAY_US",
        normalized_product_family: truth.normalizedProductFamily,
        normalized_product_type: truth.normalizedProductType,
        family_type_fingerprint: truth.familyTypeFingerprint,
        category_id: "222", category_name: "Humidifiers",
        taxonomy_tree_id: "0", taxonomy_tree_version: "142",
        taxonomy_snapshot_digest: args.p_taxonomy_snapshot_digest,
        taxonomy_pass: true, required_aspects: args.p_required_aspects,
        listing_acceptance: "UNKNOWN", confidence_tier: "HIGH_CONFIDENCE",
        confidence_score: args.p_confidence_score,
        last_validated_at: args.p_validated_at,
        revalidate_after: args.p_revalidate_after,
      }], error: null }
  ...
  const result = await resolveAndBindEbayListingCategoryV1({
    supabase, accountKey: "seller-os-preview", context, opportunity,
    exactCanonicalCategory: {
      categoryId: "111", categoryName: "Collectibles",
    ...
    packageData: {
      categoryId: "111", categoryName: "Collectibles",
      aspects: { Type: "Token", Grade: "MS 65" },
      taxonomyPreflight: { categoryId: "111" },
      quickPickMarketTestPackageV1: {
        packageDigest: `sha256:${"a".repeat(64)}`,
      },
      quickPickOwnerReviewV1: {
        status: "CONFIRMED",
        reviewedPackageDigest: `sha256:${"a".repeat(64)}`,
        readyForOwnerPublishAuthorization: true,
        marketplaceWriteAuthorized: false,
      },
      draftConfiguration: {
        aspectValidation: { categoryId: "111" },
        ebayPreflightSnapshot: "old-snapshot",
      },
    },
    taxonomyReader: async (_query, categoryId) => categoryId === null
      ? taxonomy("222", { categoryName: "Humidifiers",
        categoryResolution: "TITLE_SUGGESTION",
        aspects: [aspect("Type", true)],
        requiredAspects: [aspect("Type", true)] })
      : taxonomy(categoryId, { categoryName: categoryId === "222"
        ? "Humidifiers" : "Collectibles",
      aspects: [aspect("Type", true)],
      requiredAspects: [aspect("Type", true)] }),
    now: "2026-09-04T12:00:00Z",
  })
  assert.equal(result.packageData.categoryId, "222")
  assert.equal(result.packageData.categoryName, "Humidifiers")
  assert.equal(result.packageData.categoryResolverV1
    .priorExactCategoryInvalidated, true)
  assert.equal(result.packageData.categoryResolverV1
    .categorySelectionMode, "AUTOMATIC_CATEGORY_RESOLUTION")
  assert.equal(result.packageData.categoryResolverV1
    .semanticCompatibility.evidenceClass, "OFFICIAL_TITLE_SUGGESTION")
  assert.deepEqual(result.packageData.aspects, {})
  assert.equal(result.packageData.quickPickMarketTestPackageV1, null)
  assert.equal(result.packageData.quickPickOwnerReviewV1.status,
    "INVALIDATED_CATEGORY_CHANGE")
  assert.equal(result.packageData.quickPickOwnerReviewV1
    .readyForOwnerPublishAuthorization, false)
  assert.deepEqual(result.packageData.draftConfiguration.aspectValidation, {})
  assert.equal(result.packageData.draftConfiguration.ebayPreflightSnapshot, null)
  assert.equal(result.packageData.categoryDerivedStateInvalidationV1
...
test("unproven exact category fails closed when semantic resolution is unavailable", async () => {
  const context = {
    marketplaceId: "EBAY_US",
    listingPackageId: "99999999-9999-4999-8999-999999999999",
    opportunityId: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",
    candidateKey: `sha256:${"9".repeat(64)}`,
  }
  const result = await resolveAndBindEbayListingCategoryV1({
    supabase: { from() {
      const chain = { select() { return chain }, eq() { return chain },
        order() { return chain }, limit() {
          return Promise.resolve({ data: [], error: null }) } }
      return chain
    } },
    accountKey: "seller-os-preview", context,
    opportunity: { id: context.opportunityId,
      candidate_key: context.candidateKey,
      product_title: "Compact Desktop Air Cleaner",
      product_type: "Air Cleaner", best_selling_matches: [],
      assessment: { productTruth: { title: "Compact Desktop Air Cleaner",
        productType: "Air Cleaner" } } },
    packageData: { categoryId: "333", categoryName: "Unrelated branch",
      aspects: { Certification: "Legacy" } },
    exactCanonicalCategory: { categoryId: "333",
      authorityClass: "EXACT_PREVIOUSLY_CERTIFIED_CATEGORY",
      exactProductIdentityVerified: true, listingAcceptance: "UNKNOWN" },
    taxonomyReader: async () => taxonomy(null, {
      status: "CATEGORY_NOT_RESOLVED", categoryId: null,
      categoryName: null, categoryResolution: "UNRESOLVED", observedAt: null,
    }),
  })
  assert.equal(result.resolution.status, "CATEGORY_EXCEPTION")
  assert.equal(result.packageData.categoryId, null)
  assert.deepEqual(result.packageData.aspects, {})
  assert.equal(result.packageData.categoryResolverV1
    .priorExactCategoryInvalidated, true)
  assert.equal(result.packageData.categoryDerivedStateInvalidationV1
