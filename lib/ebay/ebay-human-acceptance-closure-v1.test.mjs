...
test("Luna readiness card opens the protected activation workspace", () => {
  ...
  assert.match(operationalRoute, /AUTHENTICATED_SERVER_HTTP/)
  assert.match(operationalRoute, /NOT_EVALUATED_SERVER_HTTP_FIRST/)
...
test("Stock Guard exposes scalable portfolio summaries and keeps UNKNOWN distinct from risk", () => {
  for (const label of ["Live listings", "Exact supplier-linked", "Awaiting evidence",
    "Stock risks", "Stale evidence", "Stock unknown", "Actionable", "Needs supplier link",
    "Exact proven", "Stock risk", "Stale", "Unknown"]) assert.match(stock, new RegExp(label))
  assert.match(stock, /NOT A RISK/)
...
test("Decisions, Experiments, and Learning navigation targets usable protected surfaces", () => {
  assert.match(monitor, /\["Decisions", "\/admin\/ebay\/decisions"/)
  assert.match(monitor, /\["Experiments", "\/admin\/ebay\/experiments"/)
  assert.match(monitor, /\["Learning", "\/admin\/ebay\/learning"/)
  ...
  assert.match(intelligenceSurface, /Today.s priorities/)
  assert.match(intelligenceSurface, /DO_NOT_TOUCH/)
  assert.match(intelligenceSurface, /Outcome provenance/)
...
test("Market Opportunity primary UX is automatic and advanced paths remain available", () => {
  ...
  assert.match(marketPage, /Commercial Recommendation V1/)
