...
test("Stock Guard exposes scalable portfolio summaries and keeps UNKNOWN distinct from risk", () => {
  for (const label of ["Publicaciones activas", "Vínculo exacto del proveedor", "Esperando evidencia",
    "Riesgos de stock", "Evidencia vencida", "Stock desconocido", "Accionable", "Necesita vínculo",
    "Exacto comprobado", "Riesgo de stock", "Desconocido"]) assert.match(stock, new RegExp(label))
  assert.match(stock, /No es un riesgo comprobado/)
