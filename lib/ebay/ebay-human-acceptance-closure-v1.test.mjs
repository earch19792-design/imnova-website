...
const marketPage = read("../../app/admin/ebay/opportunity-queue/research/normal-research-page.tsx")
...
test("Stock Guard exposes scalable portfolio summaries and keeps UNKNOWN distinct from risk", () => {
  for (const label of ["Total linked listings", "Monitoring", "Safe",
    "Low stock", "Unknown/stale stock", "Blocked identity",
    "Needs OWNER review"]) assert.match(stock, new RegExp(label))
  assert.match(stock, /stockguard_link_status/)
  assert.match(stock, /UNKNOWN_STALE/)
  assert.match(stock, /currentLiveCertified/)
...
test("legacy Decisions and Learning remain usable while System navigation is de-duplicated", () => {
  assert.doesNotMatch(navigation, /label: "Decisiones"|label: "Aprendizaje"/)
  assert.match(navigation, /item\("settings", "Settings"/)
  assert.match(navigation, /child\("diagnostics", "Diagnóstico"/)
  assert.match(navigation, /child\("experiments", "Experimentos"/)
