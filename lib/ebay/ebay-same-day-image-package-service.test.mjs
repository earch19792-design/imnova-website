...
test("a native medium alternate view can pass its compatible secondary position", async () => {
  const input = fixture()
  ...
  assert.ok(alternateAsset)
  assert.equal(alternateAsset.qa.sourceViewCapabilityPassed, true)
  assert.equal(alternateAsset.qa.automaticStatus, "PASSED")
  disposeTransientSameDayImageAssets(result.transientAssets)
  primary.fill(0)
  alternate.fill(0)
})
