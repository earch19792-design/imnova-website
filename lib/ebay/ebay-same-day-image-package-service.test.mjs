...
test("AI mode makes one dossier-aware scene-board call and zeroes provider pixels", async () => {
  ...
  const detail = result.transientAssets.find((asset) =>
    asset.transformation.visualStrategyPosition?.salesObjective ===
      "QUALITY_DETAIL")
  assert.equal(Boolean(detail), false,
    "a detail crop requires an explicitly classified, sufficiently large source")
  assert.ok(result.transientAssets.every((asset) =>
...
test("a small visible product requests a higher-resolution Luna view before provider spend", async () => {
  ...
  }), /NEEDS_ADDITIONAL_SOURCE_IMAGE:PACK_AND_COUNT/)
  assert.equal(calls, 0)
})

test("a native medium alternate view is skipped instead of being enlarged into a weak secondary", async () => {
  const input = fixture()
  const primary = await sourceFixture()
  const alternate = await sharp(await sourceFixture("#e3bf2d"))
    .resize(1000, 1000)
    .jpeg()
    .toBuffer()
  const result = await generateTransientSameDayImagePackage({
    ...
    authorizedCatalogSources: [{
      ...
      id: "LUNA_AUTHORIZED_SOURCE_2",
      nativeWidth: 1000,
      nativeHeight: 1000,
      effectiveWidth: 1000,
      effectiveHeight: 1000,
      qualityTier: "NATIVE_MEDIUM_RES",
      viewClassification: "ALTERNATE_AUTHORIZED_ANGLE",
      enhancedDerivative: false,
    }],
  })
  const alternateAsset = result.transientAssets.find((asset) =>
    asset.sourceSha256 === sha256(alternate))
  assert.equal(alternateAsset, undefined)
  assert.ok(result.transientAssets.every((asset) =>
    asset.qa.sourceQualityPassed === true &&
    asset.qa.automaticStatus === "PASSED"))
  disposeTransientSameDayImageAssets(result.transientAssets)
  primary.fill(0)
  alternate.fill(0)
})

test("a resolver-bounded 1160px enhancement can serve as the verified primary", async () => {
  const input = fixture()
  const enhancedPrimary = await sharp(await sourceFixture())
    .resize(1160, 1160)
    .jpeg({ quality: 94 })
    .toBuffer()
  const result = await generateTransientSameDayImagePackage({
    ...input,
    aiContext: { enabled: false },
    source: [enhancedPrimary],
    authorizedCatalogSources: [{
      id: "LUNA_AUTHORIZED_ENHANCED_PRIMARY",
      nativeWidth: 580,
      nativeHeight: 580,
      effectiveWidth: 1160,
