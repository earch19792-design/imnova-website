...
async function sourceFixture() {
  return sharp({
    ...
    input: Buffer.from(
      ...
      '<text x="250" y="270" text-anchor="middle" font-size="72">SOURCE</text>' +
      '<text x="250" y="360" text-anchor="middle" font-size="42">3 × 15</text>' +
      '</svg>',
    ),
    left: 200,
    top: 150,
...
test("deterministic runtime creates six transient assets and a metadata-only manifest", async () => {
  const input = fixture()
  ...
  assert.equal(result.persistenceManifest.assets.length, 6)
...
test("AI mode makes exactly one empty-background call and zeroes provider pixels", async () => {
  const input = fixture()
  ...
  const result = await generateTransientSameDayImagePackage({
    ...
    requestBackgroundPlate: async (...args) => {
      ...
      providerPixels = await sharp({
        create: { width: 1600, height: 1600, channels: 3, background: "#dce5ef" },
      }).jpeg().toBuffer()
      return {
  ...
  assert.equal(result.transientAssets.filter((asset) =>
    asset.transformation.generativeAiUsed).length, 1)
  assert.equal(result.transientAssets.find((asset) =>
    asset.transformation.generativeAiUsed)?.slot, "USE_CONTEXT")
  const outbound = JSON.stringify(receivedPlan).toLowerCase()
  assert.doesNotMatch(outbound, /lysol|lemon|wipes|authorized\.example|https?:|base64/)
