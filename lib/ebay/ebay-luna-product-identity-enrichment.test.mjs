...
test("conflicted or invalid evidence never becomes canonical, even below the conflict threshold", () => {
  const result = resolveProductIdentity([
    { attribute: "mpn", rawValue: "UNSAFE-MPN", normalizedValue: "UNSAFE-MPN",
      sourceType: "LUNA_STRUCTURED", sourceIdentifier: "luna:conflicted", observedAt,
      confidence: .2, verifiedByRule: "UPSTREAM_CONFLICT", conflictStatus: "CONFLICTED" },
    { attribute: "model", rawValue: "UNSAFE-MODEL", normalizedValue: "UNSAFE-MODEL",
      sourceType: "LUNA_STRUCTURED", sourceIdentifier: "luna:invalid", observedAt,
      confidence: .2, verifiedByRule: "UPSTREAM_INVALID", conflictStatus: "INVALID" },
  ])
  assert.equal(result.identity.mpn, null)
  assert.equal(result.identity.model, null)
})

test("MPN comparison is case-insensitive while preserving the selected canonical value", () => {
  const result = resolveProductIdentity([
    { attribute: "mpn", rawValue: "AC-16", normalizedValue: "AC-16",
      sourceType: "LUNA_STRUCTURED", sourceIdentifier: "luna:mpn", observedAt,
      confidence: .96, verifiedByRule: "LUNA_EXPLICIT_MPN", conflictStatus: "CLEAR" },
    { attribute: "mpn", rawValue: "ac-16", normalizedValue: "ac-16",
      sourceType: "EBAY_CATALOG", sourceIdentifier: "catalog:mpn", observedAt,
      confidence: .92, verifiedByRule: "EBAY_CATALOG_MPN", conflictStatus: "CLEAR" },
  ])
  assert.deepEqual(result.conflicts, [])
  assert.equal(result.identity.mpn, "AC-16")
})
