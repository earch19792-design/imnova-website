...
test("Media preparation and listing revision have separate bounded counts", () => {
  assert.match(server, /mediaApiWriteCount/)
  assert.match(server, /tradingListingWriteCount: 1/)
  assert.match(server, /MAYEL_TRADING_MEDIA_MAX_CREATE_CALLS = 2/)
  assert.match(server, /MAYEL_TRADING_MEDIA_ATOMIC_CLAIM_LOST/)
  assert.match(server, /ambiguousListingWriteRetryAllowed: false/)
...
test("ambiguous Revise outcome goes to readback and never a second write", () => {
  ...
  // One call site belongs to the historical explicit canary and one to the
  // normal delegated runtime. Both delegate to the same atomic single-write
  // primitive, which rejects a second durable attempt.
  assert.equal((server.match(
    /reviseMayelTradingPicturesOnceV1\(\{/g) ?? []).length, 2)
})
