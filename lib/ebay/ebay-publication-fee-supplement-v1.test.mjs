...
test('missing or denied Finance grant cannot retry or call financial resources',async()=>{
 ...
 try{const calls=[];const r=await readPublicationPayoutCurrencyV1(async(url,options)=>{calls.push({url,method:options.method});return new Response('{}',{status:400})});assert.equal(r.reason,'PAYOUT_OAUTH_GRANT_UNAVAILABLE');assert.equal(calls.length,1);assert.match(calls[0].url,/identity\/v1\/oauth2\/token$/)}finally{for(const k of Object.keys(keys)){if(old[k]===undefined)delete process.env[k];else process.env[k]=old[k]}}
})
