...
test("browser START ticket preserves the exact readonly scope profile", () => {
  const value = ticket({ purpose: "COMMERCIAL_ORDERS_READONLY" })
  assert.deepEqual(verifyEbayCommercialOrdersBrowserStartTicket({
    ticket: value,
    now: NOW,
    host: HOST,
    ...
    expiresAt: NOW + 240_000,
    host: HOST,
    purpose: "COMMERCIAL_ORDERS_READONLY",
  })
})
...
test("browser exchange is deployment/actor bound and exposes bounded readiness only", () => {
  ...
  assert.match(page, /setOperatorStartAvailable\(true\)/)
  assert.match(page, /if \(!\/\^\[A-Za-z0-9\._-\]\{80,2048\}\$\//)
})
...
test("ceremony is fixed-scope and cannot select account, URL, token, or recipient", () => {
  ...
  assert.match(service, /binding\.scopes\.join\(" "\)/)
  assert.match(service, /ticket\.purpose !== binding\.scopeProfile/)
...
test("seller callback preserves legacy encrypted handoff and adds one-time readonly handoff", () => {
  ...
  assert.match(callback, /completed\.handoffMode === "ONE_TIME_OPERATOR"/)
  assert.match(callback, /commercialOrdersTokenSuccessHtml/)
  assert.match(callback, /"EBAY_COMMERCIAL_ORDERS_REFRESH_TOKEN"/)
  assert.doesNotMatch(callback, /NextResponse\.json\([^)]*commercialOrdersRefreshToken/s)

  const service = readFileSync(
    "lib/ebay/ebay-commercial-orders-oauth-authorization.ts",
    "utf8",
  )
  assert.match(service, /encrypted_refresh_token: null/)
  assert.match(service, /handoffMode: "ONE_TIME_OPERATOR"/)
  assert.match(service, /tokenPersisted: false/)
})

test("dedicated operator page is admin-session-bound and cannot request write scopes", () => {
  const page = readFileSync(
    "app/admin/ebay/monitor/commercial-orders-oauth-start/page.tsx",
    "utf8",
  )
  ...
  assert.match(page, /COMMERCIAL_ORDERS_READONLY/)
  assert.match(page, /sell\.fulfillment\.readonly/)
  const readonlyStart = page.slice(
    page.indexOf("async function beginReadonlyAuthorization"),
    page.indexOf("return (", page.indexOf("async function beginReadonlyAuthorization")),
  )
  assert.doesNotMatch(readonlyStart, /commerce\.message|sell\.fulfillment(?!\.readonly)/)
  assert.doesNotMatch(page, /localStorage|sessionStorage/)
  assert.match(page, /START_PATH/)
  assert.match(page, /beginReadonlyAuthorization/)
})
