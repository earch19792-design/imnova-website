...
test("a failed upload is separate from the last valid import and stores no raw report", () => {
  ...
  const attempt = prepareFailedOwnerQualityReportUploadAttemptV1({
    ...
    deploymentId: "dpl_1234567890abcdefghijkl",
  ...
  assert.equal(attempt.failed_stage, "IMPORT_VALIDATION")
  assert.equal(attempt.request_transport_class, "JSON_BASE64")
  assert.equal(attempt.request_content_type_class, "APPLICATION_JSON")
  assert.equal(attempt.file_size_class, "UNDER_256_KIB")
  assert.equal(attempt.mime_type_class, "XLSX_STANDARD")
  assert.equal(attempt.deployment_id, "dpl_1234567890abcdefghijkl")
  assert.equal("content" in attempt, false)
})

test("unexpected XLSX row failure persists its exact sanitized pipeline stage", () => {
  const error = new QualityReportValidationError("OTHER", {
    failedStage: "ROW_PARSE", fileBufferCreated: true,
    xlsxLibraryInvoked: true, workbookOpenPass: true,
    workbookSheetEnumerationReached: true, schemaDiscoveryReached: true,
    rowParseReached: true, worksheetNames: ["Sanitized category"],
    observedHeaderNames: ["Item Id", "Item title", "Number of photos"],
    technicalReasonCode: "QUALITY_REPORT_XLSX_ROW_PARSE_FAILED",
  })
  const attempt = prepareFailedOwnerQualityReportUploadAttemptV1({
    accountKey, attemptedBy: importedBy, format: "XLSX",
    content: Buffer.from("sanitized-fixture").toString("base64"), error,
    mimeType: "application/octet-stream", requestContentType: "application/json",
    deploymentId: "dpl_1234567890abcdefghijkl",
    attemptedAt: "2026-09-02T15:01:00.000Z", correlationSeed: "row-failure" })
  assert.equal(attempt.failed_stage, "ROW_PARSE")
  assert.equal(attempt.technical_reason_code,
    "QUALITY_REPORT_XLSX_ROW_PARSE_FAILED")
  assert.equal(attempt.safe_failure_code, "REPORT_VALIDATION_NOT_PASSED")
  assert.deepEqual(attempt.workbook_sheet_names, ["Sanitized category"])
  assert.deepEqual(attempt.observed_header_names,
    ["Item Id", "Item title", "Number of photos"])
  assert.equal(attempt.mime_type_class, "OCTET_STREAM")
  assert.equal("content" in attempt, false)
})
