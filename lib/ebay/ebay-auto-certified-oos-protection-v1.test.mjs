  ...
  return { listings,
    ...
    liveCertification: { status: "CERTIFIED", marketplaceId: "EBAY_US",
      ...
      oauth: { status: "AVAILABLE", tokenReceived: true, scopes: [{
        ...
        classifications: ["READ_AVAILABLE", "WRITE_CAPABLE_BUT_NOT_USED"],
        evidenceOperation: "TRADING_GET_ITEM_MARKETPLACE",
      }] },
...
test("UNKNOWN, stale, identity mismatch and non-explicit zero write nothing", async () => {
  ...
  for (const entry of cases) {
    ...
    assert.ok(result.blockedCandidates.every((candidate) =>
