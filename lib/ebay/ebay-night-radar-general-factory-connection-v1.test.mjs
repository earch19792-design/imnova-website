...
// RPC boundary is exercised against real Postgres in the CURRENT factory SQL suite.
function currentIntakeRpc(name,args){
 if(name==='reconcile_seller_os_current_candidate_identity_v1'){
  return Promise.resolve({data:{canonicalCandidateId:args.p_expected_canonical_candidate_id,
   canonicalCommercialIdentityMatch:true,radarShippingIdentityDerivationAligned:true,
   duplicateCandidateCount:0,manualIdentityRebind:false,codexRuntimeDependency:false},error:null})
 }
 assert.equal(name,'begin_current_publication_package_v1')
 const id='current:'+args.p_opportunity_id
 const result={data:{status:'CURRENT_CREATED',packageCreated:true,package:{id,account_key:args.p_account_key,
 opportunity_id:args.p_opportunity_id,candidate_key:args.p_candidate_key,package_data:{currentPublicationFactoryV1:{
 version:'SELLER_OS_CURRENT_PUBLICATION_FACTORY_V1',packageId:id,generation:'current-generation',accountKey:args.p_account_key,
 productId:'product',variantId:'variant',supplierSku:'sku',authorityPolicy:'CURRENT_ONLY',reuseLegacyPreparation:false}}}},error:null}
 const query={abortSignal(){return query},retry(){return query},then(resolve,reject){return Promise.resolve(result).then(resolve,reject)}}
 return query
}

function client(queueRows, decisionRows = [], activeRows = []) {
  return { rpc:currentIntakeRpc, from(table) {
  ...
  return { id: `00000000-0000-4000-8000-00000000000${index}`,
    ...
    assessment: {
      currentCommercialCandidateIdentityV1: {
        ...
        accountKey: "account", marketplaceId: "EBAY_US",
        supplierAuthority: "LUNAPORTEX",
        productId: value.lunaProductId, variantId: value.lunaVariantId,
        supplierSku: value.supplierSku,
        storageCandidateKey: `luna-portex:product-${index}:variant-${index}`,
        storageCandidateKeyIsCanonical: false,
        reconciledAliasCandidateIds: [
          `luna-portex:product-${index}:variant-${index}`],
      },
...
test("residual specifics are batched, persisted, and the same candidate resumes readiness", async () => {
  ...
  const supabase = { rpc:currentIntakeRpc, from(table) {
...
test("shipping-only economics blocker becomes a durable waiting browser job without an exception", async () => {
  ...
  assert.equal(result.outcomes[0].opportunityId, queueRow(1).id)
  assert.equal(result.outcomes[0].candidateKey, queueRow(1).candidate_key)
})
...
test("durable shipping result resumes only the same exact Smart Stocking candidate", async () => {
  const base = queueRow(1)
  const canonicalCandidateId = deriveCurrentCommercialCandidateIdentityV1({
    accountKey: "account", productId: base.supplier_product_id,
    variantId: base.supplier_variant_id, supplierSku: base.supplier_sku,
  }).canonicalCandidateId
  const durable = { ...base, candidate_key: `sha256:${"1".repeat(64)}`,
    assessment: { ...base.assessment,
      currentCommercialCandidateIdentityV1: {
        ...base.assessment.currentCommercialCandidateIdentityV1,
        canonicalCandidateId, storageCandidateKey: `sha256:${"1".repeat(64)}`,
        reconciledAliasCandidateIds: [`sha256:${"1".repeat(64)}`],
      },
      radarFactoryCandidateV1: { itemSpecificLogicUsed: false,
        candidateId: canonicalCandidateId },
      radarAutomaticLunaShippingContinuationV1: {
        ...base.assessment.radarAutomaticLunaShippingContinuationV1,
        candidateId: canonicalCandidateId },
    } }
  let materialized = null
  ...
  const result = await resumeRadarFactoryCandidateAfterShippingV1({
    supabase, accountKey: "account", candidateId: canonicalCandidateId,
...
test("existing 09:00 UTC POST scheduler hands Radar to shared Quick Pick intake", () => {
  const route = readFileSync(
  ...
  assert.match(scheduler,
    /MARKET_RADAR_LUNA_SYNC[\s\S]*\/api\/cron\/market-radar-luna-sync[\s\S]*0 9 \* \* \*/)
