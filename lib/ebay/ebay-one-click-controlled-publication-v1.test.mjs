...
test("compensated rearm state remains observable while current readiness is pending", () => {
  const rearmed = classifyRecovery({
    controlledIntentValidation: null,
    readiness: {
      ready: false,
      blockers: ["PACKAGE_SOURCE_STALE", "REQUIRED_ASPECT_MISSING:Brand"],
      payloadHash: recoveryHash,
    },
...
test("authenticated page hydration resumes only prepare and one-shot publish", () => {
  ...
  assert.match(getRoute, /request_hash/)
  assert.match(getRoute, /publish_attempt_count/)
