...
test("Quick Pick owner authorization uses its canonical material digest, never candidate key", () => {
  const canonicalDigest = `sha256:${"e".repeat(64)}`
  const base = listingPackage()
  const quickPick = listingPackage({
    package_data: {
      ...base.package_data,
  ...
  const resolution = ownerAuthorizationPackageDigestV1(quickPick)
  assert.deepEqual(resolution, {
    digest: canonicalDigest,
    source: "QUICK_PICK_MATERIAL_PACKAGE_DIGEST_V1",
    valid: true,
  })
  assert.notEqual(resolution.digest, quickPick.candidate_key)
  const payload = bindOneClickControlledPublicationIntentV1({
    approvedPayload: approvedPayload(),
    actorUserId: actor,
    listingPackage: quickPick,
  })
  assert.equal(payload.controlledPublicationIntent.packageDigest,
    canonicalDigest)
  assert.equal(validateOneClickControlledPublicationIntentV1({
    approvedPayload: payload,
    actorUserId: actor,
    listingPackage: quickPick,
    opportunity: opportunity(),
...
test("Quick Pick digest disagreement fails closed without fallback", () => {
  const quickPick = listingPackage({
    package_data: {
  ...
  assert.deepEqual(ownerAuthorizationPackageDigestV1(quickPick), {
    digest: "",
    source: "QUICK_PICK_MATERIAL_PACKAGE_DIGEST_V1",
    valid: false,
  })
...
test("one-click UI and route preserve machine gates and legacy compatibility", () => {
  ...
  assert.match(workspace,
    /Publicación controlada de \{canonicalOwnerAuthorizationDigest/)
  assert.doesNotMatch(workspace,
    /Publicación controlada de \{opportunity\?\.candidate_key\}/)
  assert.match(route,
...
test("interrupted exact authorization is resumable only after another owner action", () => {
  ...
  assert.equal(refreshesFinalPreflight.autoResume, false)
})
...
test("authenticated page hydration is read-only and never auto-publishes", () => {
  const route = readFileSync(new URL(
    "../../app/api/admin/ebay/draft-only/route.ts",
    import.meta.url,
  ), "utf8")
  ...
  assert.match(getRoute, /readExactUnpublishedPublicationState/)
...
test("owner and runtime authorization both precede their publication paths", () => {
  const route = readFileSync(new URL(
    "../../app/api/admin/ebay/draft-only/route.ts",
    import.meta.url,
  ), "utf8")
  ...
  const handlerStart = route.indexOf("async function handlePost(req: Request)")
  const runtimeActionIndex = route.indexOf(
    'if (action === "batch_runtime")', handlerStart)
  const runtimeAuthIndex = route.indexOf(
    "sellerOsPostRuntimeAuthorizedV1", runtimeActionIndex)
  const runtimeWriteIndex = route.indexOf(
    "resumeSellerOsPublisherBatchV1", runtimeActionIndex)
  const ownerAuthIndex = route.indexOf(
    "const auth = await authenticate(req)", runtimeActionIndex)
  const batchPublishIndex = route.indexOf(
    'if (action === "batch_publish")', ownerAuthIndex)
  assert.ok(runtimeActionIndex > handlerStart)
  assert.ok(runtimeAuthIndex > runtimeActionIndex
    && runtimeAuthIndex < runtimeWriteIndex)
  assert.ok(ownerAuthIndex > runtimeActionIndex
    && ownerAuthIndex < batchPublishIndex)
})
...
test("APPROVE_FAILURE -> UI_PHASE_RESETS_AND_REASON_VISIBLE", () => {
  ...
  const clickEnd = workspace.indexOf("\n  async function approveDraft()", clickStart)
  const click = workspace.slice(clickStart, clickEnd)
...
test("owner-gated clicks stop after UNPUBLISHED and preview stages", () => {
  const workspace = readFileSync(new URL(
    "../../app/admin/ebay/listing-workspace/page.tsx",
    import.meta.url,
  ), "utf8")
  ...
  assert.match(click, /unpublishedDraftCreatedThisClick/)
  assert.match(click, /refreshed\.unpublishedReadback\?\.safe !== true/)
  assert.match(click, /if \(previewPreparedThisClick\)/)
})
