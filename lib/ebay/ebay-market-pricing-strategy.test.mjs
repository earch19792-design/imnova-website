...
test("un precio activo atípico no ensancha el rango ni impide recomendar la ventana de 10%", () => {
  const comparableEvidence = [34.99, 35, 35.5, 36, 107.13].map((price, index) => ({
  ...
  assert.equal(active?.sampleSize, 4)
  assert.equal(active?.excludedOutlierCount, 1)
  assert.equal(active?.minimumLandedPrice, 34.99)
  assert.equal(active?.medianLandedPrice, 35.25)
  assert.equal(active?.maximumLandedPrice, 36)
  ...
  assert.equal(recommendation.controlledRiskActiveMarketFallbackUsed, true)
  assert.equal(recommendation.promotionAllowed, false)
  assert.equal(recommendation.marketReference?.excludedOutlierCount, 1)
})
