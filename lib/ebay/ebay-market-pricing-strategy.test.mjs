...
test("no recomienda publicar pero ofrece una excepción humana si el piso de 10% queda sobre el mercado", () => {
  const recommendation = buildEbayMarketPricingRecommendation({
    minimumOperatorPrice: 41.32,
  ...
  assert.equal(recommendation.recommendedSalePrice, 36)
  assert.equal(recommendation.competitiveness, "NOT_COMPETITIVE")
  assert.equal(recommendation.nonCompetitiveControlledRiskOverrideAvailable, true)
  assert.equal(recommendation.promotionAllowed, false)
  assert.equal(recommendation.minimumNetMarginPercent, 10)
  assert.equal(recommendation.automaticRecommendationUsed, false)
  assert.equal(recommendation.decisionLogic,
