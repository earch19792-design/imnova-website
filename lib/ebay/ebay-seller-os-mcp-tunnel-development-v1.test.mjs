...
test("precompiled local_vm_lab retains the exact private Tunnel auth and metadata decision", () => {
  const environment = tunnelEnvironment({ NODE_ENV: "production", EBAY_PRO_RUNTIME: "local_vm_lab" })
  const state = policy(environment)
  assert.equal(state.nextCompilationMode, "production")
  assert.equal(state.productionContext, false)
  assert.equal(state.requestHandlingAllowed, true)
  assert.equal(state.applicationAuthMode, "TUNNEL_TRANSPORT_ONLY")
  assert.deepEqual(getSellerOsMcpToolSecuritySchemesV1(state.applicationAuthMode), [{ type: "noauth" }])
  const metadata = getSellerOsMcpAuthMetadataPolicyV1(environment)
  assert.deepEqual(metadata.runtime, state)
  assert.equal(metadata.protectedResourceMetadata, "ABSENT_404")
  assert.equal(handleSellerOsMcpProtectedResourceMetadataForRuntimeV1(environment).status, 404)
})

test("precompiled Tunnel cannot use business production, ambiguous labels, Vercel, incorrect bind or write tools", () => {
  const base = tunnelEnvironment({ NODE_ENV: "production", EBAY_PRO_RUNTIME: "local_vm_lab" })
  for (const override of [
    ...[undefined, "", "unknown", "development", "staging", "production", "production_core"].map(EBAY_PRO_RUNTIME => ({ EBAY_PRO_RUNTIME })),
    ...["production", "preview", "development"].map(VERCEL_ENV => ({ VERCEL_ENV })),
    { VERCEL: "1" }, { VERCEL_TARGET_ENV: "preview" },
    ...["", "0.0.0.0", "localhost", "::1"].map(SELLER_OS_MCP_BIND_HOST => ({ SELLER_OS_MCP_BIND_HOST })),
    { SELLER_OS_MCP_DEPLOYMENT_MODE: "unknown" },
  ]) {
    const environment = { ...base, ...override }
    assert.equal(policy(environment).requestHandlingAllowed, false)
    assert.notEqual(getSellerOsMcpAuthMetadataPolicyV1(environment).protectedResourceMetadata, "ABSENT_404")
  }
  assert.equal(policy(base, 1).requestHandlingAllowed, false)
})
