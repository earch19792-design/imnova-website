...
const persistence = readFileSync(new URL("./reference-guided-canary-persistence.ts", import.meta.url), "utf8")
...
test("canary route reserves once before one exact Images Edit request", () => {
  const reserve = route.indexOf("reserve_ebay_reference_guided_replacement_call")
...
test("replacement canary requires the append-only human authorization and total call two", () => {
  assert.match(route, /REPLACEMENT_AFTER_STORAGE_MIME_CONFIGURATION_DEFECT/)
  assert.match(route, /claim_ebay_reference_guided_replacement_canary/)
  assert.match(route, /REFERENCE_GUIDED_REPLACEMENT_HUMAN_AUTHORIZATION_REQUIRED/)
  assert.match(route, /Number\(attempt\.provider_calls\) !== 1/)
  assert.match(route, /budgetReserved = Number\(data\) === 2/)
  assert.match(route, /PROVIDER_SUCCEEDED_PERSISTENCE_FAILED/)
  assert.match(route, /STORAGE_MIME_CONFIGURATION_DEFECT/)
})

test("canary output remains private and cannot auto-approve or publish", () => {
  assert.match(route, /stagingBucket\.public !== false/)
  assert.match(route, /REFERENCE_GUIDED_CANARY_OUTPUT_BUCKET/)
  assert.match(route, /outputStoragePath = proposedStoragePath/)
  assert.match(route, /status: "QA_PENDING"/)
  assert.match(persistence, /humanApprovalRequired: true/)
  assert.match(persistence, /autoApproved: false/)
  assert.match(persistence, /publicationAuthorized: false/)
...
test("semantic identity QA is fail-closed for human visual confirmation", () => {
  for (const check of ["sameWhiteColor", "sameHandles", "sameRim",
    ...
    assert.match(persistence, new RegExp(`${check}: "REQUIRES_HUMAN_CONFIRMATION"`))
  }
  assert.match(persistence, /automaticStatus: "PARTIAL"/)
})
