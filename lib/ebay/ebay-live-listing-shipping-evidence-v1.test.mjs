...
function fakeSupabase(overrides = {}) {
  const tables = []
  const state = { evidence: null, inserts: 0, economicRows: new Map(), economicWrites: 0 }
  const rows = {
  ...
  class Query {
    ...
    upsert(value) { this.operation = "upsert"; this.inserted = value; return this }
    result() {
      if (this.table === "seller_os_live_economic_evidence_v1") {
        if (this.operation === "upsert") {
          state.economicWrites += 1
          if (!state.economicRows.has(this.inserted.evidence_id)) {
            state.economicRows.set(this.inserted.evidence_id, this.inserted)
          }
          return { data: null, error: null }
        }
        const row = [...state.economicRows.values()][0]
        return { data: row ? { ...row, ...overrides.economicReadback } : null, error: null }
      }
...
function presencePair() {
  const base = { ebay_item_id: target.ebayItemId, ebay_sku: "EXACT-EBAY-SKU",
    listing_status: "active", title: "Exact product", ebay_price: 71.99,
    currency: "USD", market_radar_product_id: null,
    supplier_variant_id: null, supplier_sku: null, supplier_cost_at_linking: null }
  return [
    { ...base, id: "current", source: "EBAY_TRADING_GET_MY_EBAY_SELLING",
      last_ebay_sync_at: "2026-08-30T01:00:00Z" },
    { ...base, id: "historical", source: "EBAY_TRADING_GET_ITEM_READONLY",
      last_ebay_sync_at: "2026-08-29T01:00:00Z",
      supplier_variant_id: target.lunaVariantId, supplier_sku: target.sourceSku },
  ]
}

function presenceFake(rows) {
  return fakeSupabase({ ebay_active_listings: rows,
    seller_os_luna_linkage_decisions: [{
      decision_id: `luna-linkage-decision-v1:sha256:${"4".repeat(64)}`,
      decision_version: 1, decision: "APPROVE_EXACT_LINKAGE",
      linkage_id: LINKAGE_ID, ebay_sku: "EXACT-EBAY-SKU",
      luna_product_id: target.lunaProductId,
      luna_variant_id: target.lunaVariantId, luna_sku: target.sourceSku,
    }] })
}

test("current LIVE presence supersedes earlier exact GetItem representation independent of row order", async () => {
  for (const rows of [presencePair(), presencePair().reverse()]) {
    const before = JSON.stringify(rows)
    const fake = presenceFake(rows)
    const result = await server.resolveExactCurrentLiveIdentityV1({
      supabase: fake.client, target })
    assert.equal(result.activeListingRegistryId, "current")
    assert.equal(result.presenceAuthority.reason,
      "CANONICAL_CURRENT_LIVE_OVER_HISTORICAL_GET_ITEM")
    assert.deepEqual(result.presenceAuthority.excludedHistoricalPresenceRowIds,
      ["historical"])
    assert.equal(result.presenceAuthority.historicalRowsPreserved, true)
    assert.equal(JSON.stringify(rows), before)
    assert.equal(fake.state.inserts, 0)
  }
})

test("ambiguous or conflicting LIVE representations fail before shipping acquisition or persistence", async () => {
  const scenarios = [
    rows => { rows[1].ebay_sku = "WRONG-SKU" },
    rows => { rows[1].ebay_item_id = "OTHER-ITEM" },
    rows => { rows[1].supplier_sku = "WRONG-LUNA-SKU" },
    rows => { rows[1].supplier_variant_id = "WRONG-VARIANT" },
    rows => { rows[1].market_radar_product_id = "WRONG-PRODUCT" },
    rows => { rows[1].last_ebay_sync_at = rows[0].last_ebay_sync_at },
    rows => { rows[0].last_ebay_sync_at = "2026-08-28T01:00:00Z" },
    rows => { rows[0].last_ebay_sync_at = "invalid" },
    rows => { rows[0].last_ebay_sync_at = "2999-01-01T00:00:00Z" },
    rows => { rows[1].source = rows[0].source },
    rows => { rows[1].source = "UNKNOWN_AUTHORITY" },
    rows => { rows[1].id = rows[0].id },
    rows => { rows.push({ ...rows[0], id: "second-current" }) },
  ]
  for (const mutate of scenarios) {
    const rows = presencePair(); mutate(rows)
    const fake = presenceFake(rows)
    let calls = 0
    await assert.rejects(server.captureLiveListingShippingEvidenceV1({
      supabase: fake.client, target,
      acquire: async () => { calls += 1; return { status: "AVAILABLE", quote: quote() } },
    }), /LIVE_LISTING_SHIPPING_(EXACT_CURRENT_LIVE_REQUIRED|ACTIVE_LINEAGE_MISMATCH)/)
...
test("fresh manual LIVE shipping reaches Mayel economic source without a second Luna call or renewed freshness", async () => {
  const fake = fakeSupabase()
  fake.state.evidence = domain.buildLiveListingShippingEvidenceV1({
    identity: { ...target, linkageId: LINKAGE_ID }, quote: quote(),
  })
  const before = JSON.stringify(fake.state.evidence)
  const now = Date.parse("2026-08-30T02:00:00Z")
  const first = await server.projectCurrentLiveShippingToEconomicsV1({
    supabase: fake.client, target, expectedEvidenceId: fake.state.evidence.evidence_id, now })
  const replay = await server.projectCurrentLiveShippingToEconomicsV1({
    supabase: fake.client, target, expectedEvidenceId: fake.state.evidence.evidence_id, now: now+1000 })
  assert.equal(first.economicEvidenceId, replay.economicEvidenceId)
  assert.equal(first.shippingCost, 9.99)
  assert.equal(first.capturedAt, "2026-08-30T01:00:00.000Z")
  assert.equal(first.freshUntil, "2026-08-30T07:00:00.000Z")
  assert.equal(first.lunaRequests, 0)
  assert.equal(first.marketplaceWrites, 0)
  assert.equal(fake.state.economicRows.size, 1)
  assert.equal(JSON.stringify(fake.state.evidence), before)
  assert.equal(fake.state.inserts, 0)
  assert.equal(fake.tables.includes("seller_os_economic_evidence_refresh_jobs_v1"), false)
  const row = [...fake.state.economicRows.values()][0]
  assert.equal(row.source_entity_id, fake.state.evidence.evidence_id)
  assert.equal(row.evidence_metadata.identity.sourceSku, target.sourceSku)
  assert.equal(row.evidence_type, "LUNA_CURRENT_SHIPPING")
})

test("economic shipping handoff blocks stale, future, mismatched and unsafe source evidence before writes", async () => {
  const cases = [
    { observed_at: "2026-08-29T01:00:00Z" },
    { observed_at: "2026-08-30T03:00:00Z" },
    { maximum_age_seconds: 999999 },
    { account_key: "other-account" }, { ebay_item_id: "366582586827" },
    { source_sku: "OTHER" }, { luna_variant_id: "OTHER" },
    { linkage_id: "OTHER" }, { marketplace_id: "EBAY_GB" },
    { shipping_currency: "GBP" }, { shipping_cost: -1 },
    { purchase_performed: true }, { credentials_persisted: true },
  ]
  for (const changed of cases) {
    ...
    await assert.rejects(server.projectCurrentLiveShippingToEconomicsV1({
      supabase: fake.client, target, expectedEvidenceId: fake.state.evidence.evidence_id,
      now: Date.parse("2026-08-30T02:00:00Z"),
    }), /LIVE_SHIPPING_ECONOMIC_PROJECTION_CURRENT_QUOTE_REQUIRED/)
    assert.equal(fake.state.economicWrites, 0)
  }
})

test("economic shipping handoff rejects a different durable readback", async () => {
  const fake = fakeSupabase({ economicReadback: { value_amount: 0 } })
  fake.state.evidence = domain.buildLiveListingShippingEvidenceV1({
    identity: { ...target, linkageId: LINKAGE_ID }, quote: quote(),
  })
  await assert.rejects(server.projectCurrentLiveShippingToEconomicsV1({
    supabase: fake.client, target, expectedEvidenceId: fake.state.evidence.evidence_id,
    now: Date.parse("2026-08-30T02:00:00Z"),
  }), /LIVE_SHIPPING_ECONOMIC_PROJECTION_READBACK_MISMATCH/)
})

test('Portex durable quote precedes auxiliary Luna 429 without renewing source time', async () => {
  const { SELLER_OS_CANONICAL_LUNA_SHIPPING_DESTINATION_V1: destination } = await import('./ebay-luna-authoritative-shipping-server-v1.ts')
  const fake = fakeSupabase(), observedAt = '2026-08-30T01:00:00.000Z'
  const saved = { ...quote(observedAt), destinationProfileDigest: destination.profileDigest }
  fake.state.evidence = domain.buildLiveListingShippingEvidenceV1({identity:{...target,linkageId:LINKAGE_ID},quote:saved})
  let auxiliaryCalls = 0
  const result = await server.captureLiveListingShippingEvidenceV1({supabase:fake.client,target,now:Date.parse('2026-08-30T02:00:00Z'),acquire:async()=>{auxiliaryCalls++;throw Error('LUNA_HTTP_429')}})
  assert.equal(auxiliaryCalls,0)
  assert.equal(result.shippingAuthority,'LUNA_PORTEX_SHIPPING_AUTHORITY')
  assert.equal(result.portexUsedWhenValid,true)
