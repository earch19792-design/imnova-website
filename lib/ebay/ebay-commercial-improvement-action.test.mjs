...
test("el precio de mercado activo sólo es ejecutable sobre el piso y con aprobación", () => {
  ...
  assert.match(source, /\.from\("ebay_active_listings"\)/)
  assert.match(source, /controlled_risk_policy: controlledRiskPolicy/)
  assert.match(source, /if \(!publication\?\.listing_package_id\) return/)
  assert.match(source, /select\("controlled_risk_policy"\)/)
  assert.match(source, /activePolicy\.promotion\) === "DO_NOT_PROMOTE"/)
})
