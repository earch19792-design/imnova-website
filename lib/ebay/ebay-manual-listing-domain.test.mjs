...
import { buildOwnerSupplierPolicyApplicationV1,
  lunaNewMerchandisePolicyDigestV1 } from
...
test("owner-certified Luna new-merchandise policy requires exact product identity", () => {
  const policyCore = {
    contractVersion: "OWNER_SUPPLIER_POLICY_APPLICATION_V1",
    policyId: "00000000-0000-4000-8000-000000000001",
    policyDigest: null,
    authorizationReferenceDigest: `sha256:${"c".repeat(64)}`,
    supplier: "LUNA_PORTEX",
    policyCode: "LUNA_ALL_MERCHANDISE_NEW",
    policyVersion: "LUNA_ALL_MERCHANDISE_NEW_V1",
    conditionLabel: "New",
    lunaProductId: "9220840456416",
    lunaVariantId: "48809652158688",
    supplierSku: "EXACT-SKU",
    exactSupplierLineageCertified: true,
    productIdentityExact: true,
    policyAppliedAt: "2026-09-03T01:00:00.000Z",
    factInvented: false,
  }
  const policyApplication = buildOwnerSupplierPolicyApplicationV1({
    policy: {
      id: policyCore.policyId,
      marketplaceAccountKey: "account",
      evidenceDigest: lunaNewMerchandisePolicyDigestV1(),
      certifiedAt: "2026-09-03T00:00:00.000Z",
      authorizationReferenceDigest: policyCore.authorizationReferenceDigest,
    },
    lunaProductId: policyCore.lunaProductId,
    lunaVariantId: policyCore.lunaVariantId,
    supplierSku: policyCore.supplierSku,
    exactSupplierLineageCertified: true,
    productIdentityExact: true,
    appliedAt: policyCore.policyAppliedAt,
  })
  const exact = lunaOwnerCertifiedNewMerchandiseConditionV1({
  ...
  assert.equal(exact?.conditionId, "1000")
  assert.equal(exact?.conditionLabel, "New")
  assert.equal(exact?.factInvented, false)
...
test("el scope normaliza el formato legado alias:huella sin confiar en su huella", () => {
  const officialFingerprint = ebayProductionAccountFingerprint("seller-one")
  const result = getEbaySellerAccountScopeConfiguration({
    EBAY_SELLER_ACCOUNT_KEY: "official-store:UNTRUSTED-LEGACY-SUFFIX",
    EBAY_DRAFT_ONLY_PRODUCTION_EXPECTED_USER_ID: "seller-one",
  })

  assert.equal(result.configured, true)
  assert.equal(result.accountAlias, "official-store")
  assert.equal(result.accountKey, `official-store:${officialFingerprint}`)
  assert.equal(result.identity.expectedAccountFingerprint, officialFingerprint)

  const invalid = getEbaySellerAccountScopeConfiguration({
    EBAY_SELLER_ACCOUNT_KEY: "invalid alias:UNTRUSTED-LEGACY-SUFFIX",
    EBAY_DRAFT_ONLY_PRODUCTION_EXPECTED_USER_ID: "seller-one",
  })
  assert.equal(invalid.configured, false)
  assert.equal(invalid.reason, "ACCOUNT_KEY_INVALID")
})
