...
test("normaliza una solicitud válida sin contenido comercial reutilizable", () => {
  const parsed = parseManualListingRegistrationInput({
    ...
    safeDefaults: {
      ...
      merchantLocationKey: "miami_warehouse",
      condition: "new",
      categoryId: "261003",
      dimensionUnit: "inch",
      weightUnit: "pound",
...
test("un Item propio sólo se vincula si su Custom label coincide con el paquete", () => {
  const expected = "IMNOVA-123E4567E89B42D3A456426614174000"
  assert.deepEqual(
...
test("la clave de plantilla separa categoría y condición", () => {
  assert.equal(
    safeDefaultsTemplateKey({ categoryId: "261003", condition: "NEW" }),
    "EBAY_US:261003:NEW",
  )
  assert.equal(
    safeDefaultsTemplateKey({}),
...
test("la selección prioriza categoría y condición exactas antes de genéricos", () => {
  assert.deepEqual(
    safeDefaultsTemplatePriorityKeys("261003", "NEW"),
    [
      "EBAY_US:261003:NEW",
      "EBAY_US:261003:all-conditions",
      "EBAY_US:all-categories:NEW",
      "EBAY_US:all-categories:all-conditions",
    ],
  )
  assert.throws(
    () => safeDefaultsTemplatePriorityKeys("competitor-category", "NEW"),
