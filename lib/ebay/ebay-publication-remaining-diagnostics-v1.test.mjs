...
test('existing valid binding with no checkout does not invent the cause of an unreported checkout diagnostic without capture authority',()=>{
 assert.equal(project(probe).reason,'CHECKOUT_DIAGNOSTIC_NOT_REPORTED');assert.equal(project({...probe,secret:'never persist',url:'private'}).secret,undefined)
...
test('existing lease gains bounded diagnostic metadata without changing capture or backoff authority',async()=>{
 ...
 const detailed={version:'LUNA_CHECKOUT_OBSERVATION_V1',checkoutTabFound:true,checkoutHostMatch:true,checkoutContentScriptResponded:true,checkoutPageDetected:true,shopPayMarkersEvaluated:true,shopPayRequiredMarkersReady:false,checkoutDomReady:false,observedMarkers:['shipToMarker','subtotalMarker','totalMarker','payNowMarker']}
 const durable=project({...probe,checkoutObservation:detailed})
 await db.query('update public.seller_os_browser_workload_leases_v1 set shipping_probe_diagnostic=$1 where id=1',[durable])
 const {checkoutObservationV1:decode}=await import('./luna-checkout-observation-v1.ts')
 ...
 assert.equal(decode(row.shipping_probe_diagnostic.checkoutObservation).checkoutNotReadyReason,'REQUIRED_MARKERS_MISSING')
 assert.deepEqual(decode(row.shipping_probe_diagnostic.checkoutObservation).missingMarkers,['shippingMarker'])
