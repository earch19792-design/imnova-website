...
test("PASS_READ_CATALOG_PLUS_BOUNDED_RESEARCH_COMMAND", () => {
  assert.equal(SELLER_OS_MCP_EXPECTED_CATALOG_COUNT_V1, 34)
})
const {
...
test("SDK client completes lifecycle, lists every tool, and invokes read-only search", async () => {
  ...
  try {
    ...
    const command = listed.tools.find((tool) =>
      tool.name === "seller_os_request_luna_pre_research")
    assert.ok(command)
    assert.equal(command.annotations?.readOnlyHint, false)
    assert.equal(command.annotations?.destructiveHint, false)
    assert.equal(command.annotations?.openWorldHint, false)
    assert.equal(command.annotations?.idempotentHint, true)
    for (const tool of listed.tools) {
      if (tool.name === "seller_os_request_luna_pre_research") continue
