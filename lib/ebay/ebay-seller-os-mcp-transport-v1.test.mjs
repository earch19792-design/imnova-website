...
test("PASS_STRICTLY_READ_ONLY_CATALOG", () => {
  assert.equal(SELLER_OS_MCP_EXPECTED_CATALOG_COUNT_V1, 33)
})
const {
...
test("SDK client completes lifecycle, lists every tool, and invokes read-only search", async () => {
  ...
  try {
    ...
    assert.equal(listed.tools.some((tool) =>
      tool.name === "seller_os_request_luna_pre_research"), false)
    assert.equal(listed.tools.some((tool) =>
      tool.name === "seller_os_request_luna_pre_research_controlled_rerun"),
    false)
