...
test("Analytics conserva el desglose oficial y su metadata sin mezclar tráfico pagado", () => {
  const result = reconciled(analyticsReport({
    impressions: 100,
    searchImpressions: 60,
    storeImpressions: 15,
    views: 20,
    searchViews: 8,
    directViews: 4,
  ...
  assert.deepEqual({
    impressions: result.observations[0].impressions,
    searchImpressions: result.observations[0].searchImpressions,
    storeImpressions: result.observations[0].storeImpressions,
    views: result.observations[0].views,
    searchViews: result.observations[0].searchViews,
    directViews: result.observations[0].directViews,
    externalViews: result.observations[0].externalViews,
    otherEbayViews: result.observations[0].otherEbayViews,
    storeViews: result.observations[0].storeViews,
  }, {
    impressions: 100,
    searchImpressions: 60,
    storeImpressions: 15,
  ...
  assert.equal(
    result.reportCoverage.lastUpdatedAt,
  ...
  assert.doesNotMatch(service, /paid_(?:impressions|views):/)
})

test("Analytics mantiene null cuando eBay omite el desglose oficial", () => {
  ...
  for (let index = report.header.metrics.length - 1; index >= 0; index -= 1) {
    if (!optionalMetrics.has(report.header.metrics[index].key)) continue
    report.header.metrics.splice(index, 1)
    report.records[0].metricValues.splice(index, 1)
  }
  const observation = reconciled(report).observations[0]
  assert.deepEqual([
    observation.storeImpressions,
    observation.directViews,
    observation.externalViews,
    observation.otherEbayViews,
    observation.storeViews,
