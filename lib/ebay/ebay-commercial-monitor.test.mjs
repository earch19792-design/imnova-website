...
test("Seller Hub no contiene fallback sintético y la UI muestra el modo piloto", () => {
  ...
  const pilotBanner = readFileSync(
    "app/admin/components/seller-os-pilot-banner.tsx",
    "utf8",
  )
  ...
  assert.match(adminLayout, /<SellerOsPilotBanner\s*\/>/)
  assert.match(pilotBanner, /SINGLE_PRODUCT_LAB_BANNER/)
  assert.match(pilotBanner, /pathname === "\/admin"/)
  assert.match(pilotBanner, /pathname === "\/admin\/ebay-seller-os"/)
  assert.match(pilotBanner, /pathname\.startsWith\("\/admin\/ebay\/"\)/)
  assert.match(pilotBanner, /if \(!isSellerOsPath\(pathname\)\) return null/)
  assert.match(pilotBanner, /aria-hidden="true"[\s\S]*h-\[4\.5rem\]/)
  assert.doesNotMatch(pilotBanner, /pathname === "\/admin\/login"/)
  assert.equal(
    SINGLE_PRODUCT_LAB_BANNER,
