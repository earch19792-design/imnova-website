...
test("evidencia manual permanece separada y abre health flag de divergencia", () => {
  ...
  assert.equal(state.healthFlag, ANALYTICS_SOURCE_DIVERGENCE)
  assert.equal(state.analyticsRulesSuspended, true)
...
test("persistencia de divergencia es separada, RLS y reconsulta diaria", () => {
  ...
  assert.match(service, /PILOT_SUPPLIER_SKU = "ITEM3995"/)
  assert.match(service, /\.eq\("supplier_sku", evidence\.sku\)/)
  assert.match(service, /expectedEbayCustomLabel/)
  assert.match(service, /const fulfillmentSku = listing\.supplier_sku \?\? line\.sku/)
