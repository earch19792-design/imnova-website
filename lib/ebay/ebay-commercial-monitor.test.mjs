...
test("conserva ventas pagadas ya cumplidas como historial autoritativo", () => {
  const fulfilled = officialOrderPayload()
  fulfilled.orders[0].orderFulfillmentStatus = "FULFILLED"
  const orders = normalizeCompletedEbayOrders(fulfilled)
...
test("rechaza una orden multi-line completa si una sola línea no es autoritativa", () => {
  const partial = officialOrderPayload()
  partial.orders[0].lineItems.push({
    lineItemId: "line-invalid",
    legacyItemId: "invalid-item",
    listingMarketplaceId: "EBAY_US",
    quantity: 1,
    lineItemCost: { value: "5.00", currency: "USD" },
  })

  assert.deepEqual(normalizeCompletedEbayOrders(partial), [])
})
