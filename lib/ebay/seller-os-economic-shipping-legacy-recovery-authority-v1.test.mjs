...
test("OUT_OF_SCOPE_TERMINAL_PASS", async () => {
  const jobId = await insertLegacyJob({ active: false, attempts: 22 })
  const classified = await classify(jobId)
  assert.equal(classified.classification, "OUT_OF_SCOPE_NO_ACTIVE_LISTING")
  const generation =
    policy.sellerOsEconomicShippingLegacyRecoveryGenerationV1({
      accountKey: ACCOUNT, jobId,
      classificationFingerprint: classified.classificationFingerprint,
    })
  const closed = (await db.query(`select
    public.close_seller_os_economic_shipping_legacy_out_of_scope_v1(
      $1,$2::uuid,$3,false,0,true,true,$4,$5) as receipt`, [ACCOUNT, jobId,
    policy.SELLER_OS_ECONOMIC_SHIPPING_LEGACY_RECOVERY_RUNTIME_SHA_V1,
    classified.classificationFingerprint, generation])).rows[0].receipt
  assert.equal(closed.closed, true)
  assert.equal(closed.historicalAttemptCount, 22)
  const row = (await db.query(`select status,attempt_count,failure_class
    from public.seller_os_economic_evidence_refresh_jobs_v1 where job_id=$1`,
  [jobId])).rows[0]
  assert.deepEqual(row, { status: "FAILED_TERMINAL", attempt_count: 22,
