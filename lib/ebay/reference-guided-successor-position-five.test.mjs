...
const approvalMigration = readFileSync(new URL(
  "../../supabase/migrations/20260722035000_approve_successor_phase_b_position_5.sql",
  import.meta.url), "utf8")
...
test("position 5 human approval is append-only, exact-SHA bound, and isolated", () => {
  assert.match(approvalMigration,
    /ebay_reference_guided_position_5_human_verdict_events/)
  assert.match(approvalMigration,
  ...
  assert.match(approvalMigration, /provider_calls <> 3/)
  assert.match(approvalMigration, /position in \(3,4,6\)/)
