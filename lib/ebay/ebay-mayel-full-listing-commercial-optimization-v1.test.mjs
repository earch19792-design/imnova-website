...
test("all canonical LIVE listings reach Mayel independently of visual tasks", () => {
  ...
  assert.match(page, /MayelRevenueEngine/)
  const revenueRoute = read("../../app/api/admin/ebay/assistant/revenue-engine/route.ts")
  assert.match(revenueRoute, /readMayelListingSelectionV1/)
  const selection = read("../seller-os/mayel-listing-selection-v1.ts")
  assert.match(selection, /readCurrentLiveAuthorityV1/)
  assert.match(selection, /MAX_TREATMENT_LISTINGS/)
  assert.match(selection, /nextCursor/)
  const treatment = read("../seller-os/listing-treatment-runtime-v1.ts")
  assert.match(treatment, /currentLiveListingsForMonitorV1/)
  assert.match(treatment, /EXACT_CURRENT_LIVE_LISTING_REQUIRED/)
...
test("Mayel owns an ordered six-image workflow without technical slots", () => {
  ...
  assert.match(ui, /Tratamiento de esta posición/)
  assert.match(ui, /Reemplazar con propuesta/)
  assert.doesNotMatch(ui, /Eliminar de la propuesta/)
  assert.match(ui, /Guardar Preview de estas posiciones/)
