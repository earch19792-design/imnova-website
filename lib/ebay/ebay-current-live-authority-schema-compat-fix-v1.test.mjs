...
test("fresh official sync preserves exact supplier lineage, but never transfers it to a changed SKU", async () => {
  const database = await schemaDatabase()
  try {
    await database.exec(readFileSync(new URL(
      "../../supabase/migrations/20260911160736_preserve_supplier_lineage_on_live_refresh_v1.sql", import.meta.url), "utf8"))
    const row = { itemId: "123456789012", title: "Own product", sku: "SKU-1", quantity: 1, price: 17.77, currency: "USD" }
    await persistDatabaseRows(database, [row])
    const lineage = { status: "CERTIFIED", accountKey: ACCOUNT, itemId: row.itemId, ebaySku: row.sku,
      productId: "9220851957984", variantId: "53002121347296", sourceSku: "supplier", supersedesItemId: "123456789011" }
    await database.query("update ebay_active_listings set raw_payload=raw_payload||jsonb_build_object('canonicalSupplierLineage',$1::jsonb)", [JSON.stringify(lineage)])
    await persistDatabaseRows(database, [{ ...row, title: "New title", price: 18 }])
    let result = (await database.query("select title,ebay_price,raw_payload->'canonicalSupplierLineage' as lineage from ebay_active_listings")).rows[0]
    assert.deepEqual(result.lineage, lineage)
    assert.equal(result.title, "New title")
    assert.equal(Number(result.ebay_price), 18)
    await persistDatabaseRows(database, [{ ...row, sku: "SKU-2" }])
    result = (await database.query("select raw_payload->'canonicalSupplierLineage' as lineage from ebay_active_listings")).rows[0]
