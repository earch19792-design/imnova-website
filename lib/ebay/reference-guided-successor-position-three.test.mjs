...
const approvalMigration = readFileSync(new URL(
  "../../supabase/migrations/20260722037000_approve_successor_position_3.sql",
  import.meta.url), "utf8")
...
test("position 3 human approval is exact-SHA-bound, append-only, and isolated", () => {
  assert.match(approvalMigration,
    /ebay_reference_guided_position_3_human_verdict_events/)
  assert.match(approvalMigration,
    /before update or delete[\s\S]*prevent_reference_guided_human_evidence_mutation/)
  assert.match(approvalMigration, /force row level security/)
  assert.match(approvalMigration, /to service_role/)
  assert.match(approvalMigration,
    /7a802b4fb4327ba1015a68ee5aa92d41f1892e2e5575ceef4366e321a0ae58da/)
  assert.match(approvalMigration, /HUMAN_CONFIRMED_NON_METRIC_SCALE_CONTEXT/)
  assert.match(approvalMigration, /provider_calls <> 4/)
  assert.match(approvalMigration, /position in \(4,6\)/)
  assert.match(approvalMigration, /set status = 'PASSED'/)
  assert.match(approvalMigration, /'publicationAuthorized', false/)
  assert.doesNotMatch(approvalMigration, /provider_calls\s*=\s*provider_calls\s*\+/)
})
