...
test("explicit compatibility target is a supplier claim, never own Brand/Model", () => {
  const fixture = exactFixture({ title: "CFS Replacement Water Filters 7pk For Watts Premier WPRL-58" })
  const evidence = fullPage.buildLunaExactProductEvidenceSetV1(fixture)
  const resolve = (specificName, e = evidence) => fullPage.resolveLunaFullPageRequiredFactV1({
    opportunity: fixture.opportunity, evidence: e, specificName,
    freeTextAllowed: true, allowedValues: [], allowedValuesComplete: true,
  })
  const compatible = resolve("Compatible Model")
  assert.equal(compatible.value, "Watts Premier WPRL-58")
  assert.equal(compatible.semanticClass, "SUPPLIER_CLAIM")
  assert.equal(compatible.sourceField, "TITLE")
  assert.equal(compatible.sourceExcerpt, "For Watts Premier WPRL-58")
  assert.equal(resolve("Brand"), null)
  assert.equal(resolve("Model"), null)
  for (const title of ["Hot and Cold Bags For Food Delivery", "Filter not for Watts WPRL-58",
    "Filter For Watts WPRL-58 or Other XX-10", "Filter For Most Systems", "Filter For Watts WPRL-58?"]) {
    assert.equal(resolve("Compatible Model", fullPage.buildLunaExactProductEvidenceSetV1(
      exactFixture({ title }))), null, title)
  }
  assert.equal(resolve("Compatible Model", { ...evidence, productIdentityExact: false }), null)
  const generic = resolve("Compatible Model", fullPage.buildLunaExactProductEvidenceSetV1(
    exactFixture({ title: "Replacement Cartridge Compatible With Acme AB-123" })))
  assert.equal(generic.value, "Acme AB-123")
})
...
test("exact structured Luna vendor is preserved as Brand evidence", () => {
  const fixture = exactFixture({ vendor: "Deplux" })
  const { resolution } = resolve(fixture, "Brand")
  assert.equal(resolution.value, "Deplux")
  assert.equal(resolution.source, "EXPLICIT_LUNA_EVIDENCE")
  assert.equal(resolution.sourceField, "SPECS")
})

test("complete gallery capture is distinct from bounded visual review", () => {
  const fixture = exactFixture({ image_urls: Array.from({ length: 6 },
    (_, index) => `https://images.example/${index + 1}.png`) })
  const evidence = fullPage.buildLunaExactProductEvidenceSetV1(fixture)
  assert.equal(evidence.exactImageCount, 6)
  assert.equal(evidence.sectionCoverage.allExactProductImages, "CAPTURED")
  assert.equal(evidence.allExactProductImagesReviewed, false)
  assert.equal(evidence.imageBrandEvidenceStatus, "UNREVIEWED")
})
...
test("exact image overlay facts are durable evidence for free-text dimensions", () => {
  const base = exactFixture({
    image_urls: ["https://images.example/product.png",
      "https://images.example/dimensions.png"],
  })
  const certified = withPolicyAndImageReview(base)
  const imageUrls = certified.catalogRow.image_urls
  const review = fullPage.buildLunaFullPageImageReviewV1({
    lunaProductId: certified.opportunity.supplier_product_id,
    lunaVariantId: certified.opportunity.supplier_variant_id,
    supplierSku: certified.opportunity.supplier_sku, imageUrls,
    brandEvidenceStatus: "NO_EXPLICIT_BRAND",
    exactFacts: [{ specificName: "Item Height", exactValue: "8.6 cm",
      imageIndex: 1, sourceExcerpt: "Dimension graphic: 8,6 cm" }],
    reviewedAt: "2026-09-03T12:10:00.000Z",
  })
  const fixture = { ...certified, opportunity: { ...certified.opportunity,
    assessment: { ...certified.opportunity.assessment,
      lunaFullPageImageReviewV1: review } } }
  const { evidence, resolution } = resolve(fixture, "Item Height")
  assert.equal(evidence.imageExactFacts.length, 1)
  assert.equal(evidence.imageReviewMarkerDigest, review.evidenceDigest)
  assert.equal(evidence.validatedImageReviewEvidenceDigest,
    review.evidenceDigest)
  assert.equal(resolution.value, "8.6 cm")
  assert.equal(resolution.source, "EXPLICIT_LUNA_EVIDENCE")
  assert.equal(resolution.sourceField, "IMAGE")
})

test("exact labeled model and contextual side-sleeper Type resolve without AI", () => {
  const fixture = exactFixture({ body_html: `<p>Product model: JM05.
    Adjustable side sleeper wedge pillow for newborns.</p>` })
  assert.equal(resolve(fixture, "Model").resolution.value, "JM05")
  const type = resolve(fixture, "Type", {
    allowedValues: ["Side Sleeper Pillow"],
  }).resolution
  assert.equal(type.value, "Side Sleeper Pillow")
  assert.equal(type.source, "LUNA_CONTEXTUAL_DERIVATION")
})
