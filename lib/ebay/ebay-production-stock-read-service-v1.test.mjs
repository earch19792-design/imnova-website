...
test('known duplicate SKU keeps canonical A certified and quarantines live B',async()=>{
 const f=fixture(),r=await f.read();assert.equal(f.calls.length,8);assert.equal(r.cohortComplete,true)
...
test('fresh canonical listing case blocks contradictory StockGuard identity',async()=>{
 ...
 f.rows.seller_os_listing_cases_v1=[
  {case_id:'22222222-2222-4222-8222-222222222222',ebay_item_id:itemA,
  ...
  {case_id:'33333333-3333-4333-8333-333333333333',ebay_item_id:itemB,
 ...
 assert.equal(a.limitationCode,'CANONICAL_LISTING_CASE_IDENTITY_BLOCKED')
})
...
test('replay reads durable authority without creating jobs or decisions',async()=>{
 const f=fixture();assert.deepEqual(await f.read(),await f.read());assert.equal(f.calls.length,16)
