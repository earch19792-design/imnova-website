...
test("legacy numeric fees cannot claim proven economics even when timestamps are fresh", () => {
  const result = domain.calculateLiveEconomicsV1({ accountKey, itemId,
  ...
  assert.equal(result.status, "PARTIAL")
  assert.equal(result.expected_profit, null)
  assert.equal(result.margin_percent, null)
  assert.equal(result.roi_percent, null)
  assert.ok(result.missing_economic_inputs.includes("EXPECTED_EBAY_FEE"))
