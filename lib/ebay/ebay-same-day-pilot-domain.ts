...
export function selectSameDayQueue(
  ...
  const excludedOpportunityIds = new Set(Array.from(exclusions.opportunityIds ?? [])
    .map(normalizedKey).filter(Boolean))
  const excludedCandidateKeys = new Set(Array.from(exclusions.candidateKeys ?? [])
    .map(normalizedTrim).filter(Boolean))
  const excludedSupplierVariantIds = new Set(Array.from(exclusions.supplierVariantIds ?? [])
    .map(normalizedIdentifier).filter(Boolean))
  const excludedSupplierSkus = new Set(Array.from(exclusions.supplierSkus ?? [])
    .map(normalizedIdentifier).filter(Boolean))
  const excludedMarketRadarProductIds = new Set(Array.from(exclusions.marketRadarProductIds ?? [])
    .map(normalizedKey).filter(Boolean))
  const excludedFamilyFingerprints = new Set(Array.from(exclusions.familyFingerprints ?? [])
    .map(normalizedTrim).filter(Boolean))
  const evaluated = inputs.map((input) => evaluateSameDayCandidate(input, now))
    ...
    .filter((entry) => {
      ...
      const candidateFamilyFingerprint = normalizedTrim(entry.familyFingerprint)
      const candidateCandidateKey = normalizedKey(entry.candidateKey)
      return !excludedOpportunityIds.has(candidateOpportunityId)
      && !excludedCandidateKeys.has(candidateCandidateKey)
      ...
      && !excludedFamilyFingerprints.has(candidateFamilyFingerprint)
    })
