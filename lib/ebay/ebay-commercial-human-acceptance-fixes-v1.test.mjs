...
test("StockGuard es una sola vista primaria del registro canónico", () => {
  assert.match(navigation,
    /item\("stockguard", "StockGuard", "\/admin\/ebay\/stock-guard"/)
  assert.match(stockPage, /fetch\("\/api\/admin\/ebay\/listings\/registry"/)
  assert.match(stockPage, /fetch\("\/api\/admin\/ebay\/monitor"/)
  assert.doesNotMatch(stockPage, /method:\s*["']POST["']/)
  assert.match(stockPage, /cases\.filter/)
  assert.match(stockPage, /LIVE actual no certificado/)
  assert.match(stockPage, /no autorizan escrituras de cantidad en eBay/)
})
