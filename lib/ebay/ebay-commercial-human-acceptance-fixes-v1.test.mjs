...
test("la navegación secundaria de StockGuard abre su espacio de solo lectura", () => {
  assert.match(navigation,
    /id: "stockguard", label: "StockGuard"[\s\S]*?href: "\/admin\/ebay\/stock-guard"[\s\S]*?tier: "SYSTEM"/)
  assert.match(stockPage, /fetch\("\/api\/admin\/ebay\/monitor"/)
  assert.doesNotMatch(stockPage, /method:\s*["']POST["']/)
  assert.match(stockPage, /supplierLinkage = CERTIFIED/)
  assert.match(stockPage, /No se deriva de stock desconocido/)
  assert.match(stockPage, /No equivale a vínculo exacto/)
  assert.match(stockPage, /vincula automáticamente sólo una identidad exacta y determinista/)
  assert.match(stockPage, /No se infiere identidad por similitud de título/)
