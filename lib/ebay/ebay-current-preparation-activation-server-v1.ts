...
export async function activateCurrentPreparationV1(input:{supabase:SupabaseClient;packageId:string;accountKey:string;actor:string}) {
 ...
 const read=await input.supabase.from('ebay_authorized_listing_publications')
  ...
  .eq('id',preflight.publicationId).eq('actor_user_id',input.actor).eq('marketplace_account_key',input.accountKey).abortSignal(AbortSignal.timeout(8000)).retry(false).single()
 ...
 if(saved.error)throw Error(String(saved.error.message).match(/^[A-Z0-9_]+$/)?saved.error.message:
  /^[A-Z0-9]{5}$/.test(String(saved.error.code))?`CURRENT_ACTIVATION_PERSISTENCE_${saved.error.code}_READBACK_REQUIRED`:'CURRENT_ACTIVATION_PERSISTENCE_UNCONFIRMED_READBACK_REQUIRED')
 ...
 const [execution,approval]=await Promise.all([
  input.supabase.from('ebay_draft_only_execution_ledger').select('id,approval_id,actor_user_id,listing_package_id,account_fingerprint,phase,sku,offer_id,request_hash,sanitized_result')
   .eq('id',after.data.draft_execution_id).abortSignal(AbortSignal.timeout(8000)).retry(false).single(),
  input.supabase.from('ebay_draft_only_approvals').select('id,actor_user_id,listing_package_id,account_fingerprint,status,payload_hash,approved_payload,approval_phrase_version')
   .eq('id',after.data.draft_approval_id).abortSignal(AbortSignal.timeout(8000)).retry(false).single(),
 ])
