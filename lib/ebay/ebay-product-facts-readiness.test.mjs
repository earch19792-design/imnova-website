...
test("verified Luna net content satisfies the eBay Volume requirement", () => {
  const facts = resolveProductFacts([
    ...readyFacts(),
    observation({
      factKey: "netContent",
      value: "3.4 fl. oz / 100mL",
    }),
  ], now).facts
  const [volume] = mapTaxonomyRequirements([
    {
      name: "Volume",
      required: true,
      values: ["3.4 fl oz"],
  ...
  assert.equal(volume.mappedFactKey, "netContent")
  assert.equal(volume.selectedValue, "3.4 fl. oz / 100mL")
  assert.equal(volume.status, "SATISFIED_VERIFIED")
})

test("competitor corroboration cannot invalidate an offer-pack derivation anchored to Luna", () => {
  const lunaUnitCount = observation({
    id: "luna-unit-count",
    factKey: "unitCount",
    value: 1,
    normalizedUnit: "count",
  })
  const competitorUnitCount = observation({
    id: "competitor-unit-count",
    factKey: "unitCount",
    value: 1,
    normalizedUnit: "count",
    sourceType: "EBAY_TRADING_GET_ITEM_READONLY",
    sourceAuthority: "CORROBORATION",
    verificationStatus: "CORROBORATED",
  })
  const lunaPackCount = observation({
    id: "luna-pack-count",
    factScope: "OFFER_PACK",
    factKey: "offerPackCount",
    value: 1,
    normalizedUnit: "count",
  })
  const initial = [lunaUnitCount, competitorUnitCount, lunaPackCount]
  const first = resolveProductFacts(initial, now)
  assert.deepEqual(
    first.facts.find((fact) => fact.factKey === "unitCount")?.authoritativeSupportingObservationIds,
    ["luna-unit-count"],
  )
  const derived = deriveOfferPackFacts({
    candidateId: "candidate-1",
    lunaVariantId: "luna-variant-1",
    facts: first.facts,
    now,
  })
  const resolved = resolveProductFacts([...initial, ...derived], now)
  assert.ok(derived.every((entry) => entry.derivation.sourceObservationIds
    .every((id) => id !== "competitor-unit-count")))
  assert.ok(resolved.facts.filter((fact) => fact.factScope === "OFFER_PACK")
    .every((fact) => fact.verificationStatus === "VERIFIED" ||
      fact.resolutionRule === "AUTHORIZED_DERIVATION"))
...
test("a conflicting competitor quantity cannot override an exact Luna unit count", () => {
  const result = resolveProductFacts([
    observation({ id: "luna-unit", factKey: "unitCount", value: 1,
      normalizedUnit: "count" }),
    observation({ id: "seller-total", factKey: "unitCount", value: 2,
      normalizedUnit: "count", sourceType: "EBAY_TRADING_GET_ITEM_READONLY",
      sourceAuthority: "CORROBORATION", verificationStatus: "CORROBORATED" }),
  ], now)
  const fact = result.facts.find((entry) => entry.factKey === "unitCount")
  assert.equal(fact?.selectedValue, 1)
  assert.equal(fact?.verificationStatus, "VERIFIED")
  assert.equal(result.conflicts.length, 0)
})

test("two of three exact competitor listings can resolve a descriptive color", () => {
  const competitor = (id, value) => observation({
    id,
    factKey: "color",
    value,
    sourceType: "EBAY_TRADING_GET_ITEM_READONLY",
    sourceReference: `EBAY_TRADING_GET_ITEM_READONLY:sha256:${id.padEnd(24, "0").slice(0, 24)}`,
    sourceAuthority: "CORROBORATION",
    verificationStatus: "CORROBORATED",
  })
  const result = resolveProductFacts([
    competitor("seller1", "Black"),
    competitor("seller2", "Black"),
    competitor("seller3", "Metal"),
  ], now)
  const fact = result.facts.find((entry) => entry.factKey === "color")
  assert.equal(fact?.selectedValue, "Black")
  assert.equal(fact?.verificationStatus, "CORROBORATED")
  assert.equal(fact?.resolutionRule, "EXACT_COMPARABLE_MULTI_SELLER_MAJORITY")
  assert.equal(result.conflicts.length, 0)
})
...
test("a reviewed manufacturer brand replaces a legacy Unbranded absence marker", () => {
  const result = resolveProductFacts([
    observation({ id: "legacy-unbranded", factKey: "brand", value: "Unbranded",
      sourceType: "OFFICIAL_LABEL", sourceAuthority: "MANUFACTURER_OR_LABEL",
      confidence: 1 }),
    observation({ id: "tesla-official", factKey: "brand", value: "Tesla",
      sourceType: "MANUFACTURER_OFFICIAL_PUBLIC",
      sourceAuthority: "MANUFACTURER_OR_LABEL", confidence: .96 }),
  ], now)
  const brand = result.facts.find((fact) => fact.factKey === "brand")
  assert.equal(brand?.selectedValue, "Tesla")
  assert.equal(brand?.verificationStatus, "VERIFIED")
  assert.deepEqual(brand?.supportingObservationIds, ["tesla-official"])
  assert.deepEqual(brand?.conflictingObservationIds, ["legacy-unbranded"])
  assert.equal(result.conflicts.length, 0)
})

test("two different positive authoritative brands remain a blocking conflict", () => {
  const result = resolveProductFacts([
    observation({ id: "label-brand", factKey: "brand", value: "Example",
      sourceType: "OFFICIAL_LABEL", sourceAuthority: "MANUFACTURER_OR_LABEL" }),
    observation({ id: "manufacturer-brand", factKey: "brand", value: "Tesla",
      sourceType: "MANUFACTURER_OFFICIAL_PUBLIC",
      sourceAuthority: "MANUFACTURER_OR_LABEL" }),
  ], now)
  assert.equal(result.facts.find((fact) => fact.factKey === "brand")?.verificationStatus,
    "CONFLICTED")
})

test("an exact visible label confirmation resolves older brand disagreement", () => {
  const result = resolveProductFacts([
    observation({ id: "supplier-brand", factKey: "brand", value: "Acme" }),
    observation({ id: "operator-label-brand", factKey: "brand", value: "Tesla",
      sourceType: "OFFICIAL_LABEL", sourceAuthority: "MANUFACTURER_OR_LABEL",
      confidence: 1,
      adapterVersion: "SAME_DAY_SINGLE_FACT_EXCEPTION_V3_2026_07_21" }),
    observation({ id: "manufacturer-brand", factKey: "brand", value: "Tesla",
      sourceType: "MANUFACTURER_OFFICIAL_PUBLIC",
      sourceAuthority: "MANUFACTURER_OR_LABEL" }),
  ], now)
...
test("Taxonomy requirements use the official canonical value for equivalent facts", () => {
  const facts = resolveProductFacts([
    ...readyFacts(),
    observation({ factKey: "type", value: "keychain" }),
  ], now).facts
  const requirements = mapTaxonomyRequirements([
    { name: "Type", required: true, values: ["Key Chain", "Keyring"] },
  ], facts)
  assert.equal(requirements[0].status, "SATISFIED_VERIFIED")
  assert.equal(requirements[0].selectedValue, "Key Chain")
})

test("Taxonomy Manufacturer Part Number reuses a verified canonical MPN", () => {
  const facts = resolveProductFacts([
    ...readyFacts(),
    observation({ factKey: "mpn", value: "1104937-10-B" }),
  ], now).facts
  const [requirement] = mapTaxonomyRequirements([
    { name: "Manufacturer Part Number", required: true },
  ], facts)
  assert.equal(requirement.status, "SATISFIED_VERIFIED")
  assert.equal(requirement.mappedFactKey, "mpn")
  assert.equal(requirement.selectedValue, "1104937-10-B")
})

test("Taxonomy preserves whether suggested values are an enforced selection-only list", () => {
  const facts = resolveProductFacts(readyFacts(), now).facts
  const requirements = mapTaxonomyRequirements([
    { name: "Brand", required: true, values: ["Acme"], aspectMode: "SELECTION_ONLY",
      allowedValuesComplete: true },
    { name: "Material", required: true, values: ["Steel", "Brass"], aspectMode: "FREE_TEXT" },
  ], facts)
  assert.equal(requirements[0].selectionOnly, true)
  assert.equal(requirements[0].allowedValuesComplete, true)
  assert.equal(requirements[1].selectionOnly, false)
  assert.equal(requirements[1].allowedValuesComplete, false)
})

test("a strong exact-comparable majority corrects an isolated incompatible category", () => {
  assert.deepEqual(strongExactComparableCategoryConsensus([
    { categoryId: "262101" }, { categoryId: "262101" }, { categoryId: "1498" },
  ]), {
    categoryId: "262101",
    matchingComparables: 2,
    comparableCount: 3,
    confidence: "STRONG_MAJORITY",
  })
  assert.equal(strongExactComparableCategoryConsensus([
    { categoryId: "262101" }, { categoryId: "1498" }, { categoryId: "293" },
  ]), null)
})

test("only a complete eBay selection list can reject an otherwise authoritative value", () => {
  const facts = resolveProductFacts(readyFacts(), now).facts
  const complete = mapTaxonomyRequirements([{
    name: "Brand", required: true, values: ["Other"], aspectMode: "SELECTION_ONLY",
    allowedValuesComplete: true,
  }], facts)
  const sampled = mapTaxonomyRequirements([{
    name: "Brand", required: true, values: ["Other"], aspectMode: "SELECTION_ONLY",
    allowedValuesComplete: false,
  }], facts)
  assert.equal(complete[0].status, "MISSING_BLOCKING")
  assert.equal(sampled[0].status, "SATISFIED_VERIFIED")
})
...
test("an optional descriptive conflict does not invalidate exact identity", () => {
  const conflictedColorFacts = resolveProductFacts([
    ...readyFacts(),
    observation({ id: "luna-color", factKey: "color", value: "Black" }),
    observation({ id: "catalog-color", factKey: "color", value: "Red",
      sourceType: "EBAY_CATALOG_OFFICIAL_READONLY",
      sourceAuthority: "MANUFACTURER_OR_LABEL" }),
  ], now).facts
  const readiness = calculateReadiness({
    identityExact: true,
    facts: conflictedColorFacts,
    requirements: [{
      aspectName: "Color", required: false, selectionOnly: false,
      allowedValuesComplete: false, mappedFactKey: "color",
      status: "MISSING_OPTIONAL", selectedValue: null, allowedValues: [],
      source: "EBAY_TAXONOMY_OFFICIAL_READONLY",
    }],
    regulated: false,
    taxonomySourceReady: true,
  })
  assert.equal(readiness.gates.IDENTITY_READY, true)
  assert.equal(readiness.gates.PRODUCT_FACTS_READY, true)
  assert.equal(readiness.gates.EBAY_ASPECTS_READY, true)
...
test("M/N/O: source policy has no private browsing, enrichment is bounded, and exceptions are targeted", () => {
  ...
  assert.match(source, /semanticCategoryIdFromTitle/)
  assert.match(source, /explicitAudienceObservations/)
  assert.match(source, /\(\?:woman\|women\|womens\|mom\|mother\|daughter\|sister\)/)
  assert.match(source, /TITLE_SEMANTIC_CATEGORY_GUARD/)
  ...
  assert.match(source, /strongExactComparableCategoryConsensus/)
  assert.match(source, /cachedExactComparableCategoryRows/)
  assert.match(source, /categoryConsensus\?\.categoryId \|\| catalogCategoryId/)
  assert.match(gateway, /if \(!body\.trim\(\)\) return \{\}/)
  assert.match(gateway, /persistentCached\.categoryId === normalizedKnownCategory/)
  assert.match(gateway, /marketplace_id", "EBAY_MOTORS_US"/)
  assert.match(gateway, /allowTitleSuggestionFallback === false/)
  assert.match(gateway, /categoryId: text\(item\.categoryId\) \|\| text\(category\.categoryId\)/)
  assert.match(gateway, /`item-v3:\$\{itemId\}`/)
  assert.match(source, /allowTitleSuggestionFallback: !categoryConsensus/)
