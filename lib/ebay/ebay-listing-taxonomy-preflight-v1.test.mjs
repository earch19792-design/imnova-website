...
test("marketplace fallback resolves a requirement without becoming Product Truth", () => {
  const brand = aspect("Brand", { required: true,
    mode: "SELECTION_ONLY", values: ["Unbranded", "Wilton"] })
  const value = taxonomy({ aspects: [brand], requiredAspects: [brand],
    recommendedAspects: [] })
  const result = buildEbayListingTaxonomyPreflightV1(input({
    taxonomy: value, existingAspects: {}, provenProductValues: {},
    marketplaceRequirementValues: { Brand: "Unbranded" },
    marketplaceRequirementEvidence: { Brand: {
      resolutionClass: "MARKETPLACE_ALLOWED_FALLBACK",
      factInvented: false,
    } },
    knownUnknownAspectNames: ["Brand"],
  }))
  assert.deepEqual(result.marketplaceValuesAutoBound,
    { Brand: "Unbranded" })
  assert.deepEqual(result.provenValuesAutoBound, {})
  assert.deepEqual(result.resolvedAspects, { Brand: "Unbranded" })
  assert.deepEqual(result.unprovenRequiredAspectNames, [])
})

test("every unresolved official required aspect has one explicit product-truth gate", () => {
  const color = aspect("Color", { required: true })
  const value = taxonomy({
    aspects: [color],
    requiredAspects: [color],
    recommendedAspects: [],
  })
  const result = buildEbayListingTaxonomyPreflightV1(input({
    taxonomy: value,
  ...
  assert.deepEqual(result.unprovenRequiredAspectNames, ["Color"])
  assert.deepEqual(result.unprovenAspectEvidenceRequirements, {
    Color: "AUTHORITATIVE_PRODUCT_COLOR_EVIDENCE_REQUIRED",
  })
  assert.deepEqual(result.resolvedAspects, {})
})
...
test("existing draft-only route separates taxonomy preflight from visual gate", async () => {
  ...
  const categoryResolver = await readFile(new URL(
    "./ebay-category-resolver-v1.ts", import.meta.url), "utf8")
  ...
  assert.match(preflightBody, /buildEbayCategoryResolverProductTruthV1/)
  assert.match(preflightBody,
    /provenProductValues: productTruth\.provenProductValues/)
  assert.match(categoryResolver,
