...
test("an active ITEM3155 supersedes durable pre-publication analysis", () => {
  const protection = evaluateActiveListingCandidateReanalysisProtection({
  ...
  assert.equal(protection.blocksReanalysis, true)
  assert.equal(protection.reasonCode, "ALREADY_LISTED_AND_MONITORED")
  assert.equal(
    protection.policyVersion,
    "ACTIVE_LISTING_REANALYSIS_PROTECTION_V2",
  )
})

test("active listing protection does not replace publication readback", () => {
  const protection = evaluateActiveListingCandidateReanalysisProtection({
    ...
    rows: [activeListing({ ebay_sku: "ITEM3155" })],
  })
  assert.equal(protection.excluded, true)
  assert.equal(protection.blocksReanalysis, false)
  assert.equal(protection.reasonCode, null)
})
...
test("same-day selection applies active-listing protection before candidate inputs", () => {
  ...
  assert.match(service, /count === null \|\| count !== rows\.length/)
  ...
  assert.match(service, /reconcileAlreadyListedSameDayCandidates/)
  assert.match(service, /ACTIVE_LISTING_REANALYSIS_SUPERSEDED/)
  assert.match(service, /status: "SUPERSEDED"/)
  assert.match(service, /ebayWrites: 0/)
  assert.ok(
    service.indexOf("reconcileAlreadyListedSameDayCandidates({") <
