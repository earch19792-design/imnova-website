...
test("navigation is touch-first with seven canonical areas", () => {
  ...
  for (const id of ["dashboard", "opportunities", "listings", "stockguard",
    "orders", "analytics", "settings"])
    assert.match(navigation, new RegExp(`item\\("${id}"`))
  assert.doesNotMatch(navigation, /label: "Quick Pick"/)
  assert.match(navigation, /child\("preparation", "Preparación"/)
