...
test("the systemic policy accepts certified CURRENT factory lineage and its canonical category", () => {
  const base = opportunity()
  const policyApplication = buildOwnerSupplierPolicyApplicationV1({
  ...
  }, frontier())?.conditionLabel, "New")
  assert.equal(lunaQuickPickOwnerCertifiedConditionAuthorityV1({
    ...continued, assessment: { ...continued.assessment,
      canonicalMarketplaceReadinessV1: {
        ...continued.assessment.canonicalMarketplaceReadinessV1,
        supplierVariantId: "DIFFERENT",
      },
      quickPickRequiredSpecificsContinuationV1: undefined,
      quickPickExactSoldMarketEnrichmentV1: undefined,
    },
  }, frontier()), null)
})
...
test("CURRENT market-test pricing remains distinct and carries routine delegated exposure", () => {
  ...
  assert.equal(result.marketTestReviewV1.ownerAuthorizationRequired, false)
  assert.equal(result.marketTestReviewV1.routineExposureAuthority,
    "SELLER_OS_CURRENT_FACTORY_ROUTINE_EXPOSURE_AUTHORITY_V1")
  assert.equal(result.packageSeed.pricing.targetPrice, 32.5)
})
