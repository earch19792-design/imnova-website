...
test("UI preserves Product Case pause and never exposes a send control", () => {
  assert.match(page, /Product Case remains paused/)
  assert.match(page, /WhatsApp alert dry run/)
  assert.doesNotMatch(page, />Send WhatsApp<|>Resume Product Case</)
  assert.match(monitor, /Commercial operational readiness/)
  assert.match(monitor, /DRY RUN ONLY/)
})
