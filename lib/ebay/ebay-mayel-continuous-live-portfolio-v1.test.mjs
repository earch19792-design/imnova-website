...
test("Mayel can see the full bounded queue and truthful portfolio states", () => {
  ...
  assert.match(ui, /const taskVisualPresentation = visualTaskStatusV1/)
  assert.match(ui, /taskVisualPresentation\(task\)\.icon/)
  const statusProjection = readFileSync("lib/seller-os/mayel-visual-asset-status-v1.ts", "utf8")
  for (const label of ["Mejora detectada", "QA aprobado", "Guardada en Seller OS", "En revisión", "Pendiente de eBay", "Sincronizada con eBay", "Requiere atención"]) assert.ok(statusProjection.includes(label))
...
test("Mayel workstation bulk-loads output assets without one query per task", () => {
  ...
  // Assets and durable sync states are each bulk reads, independent of task count.
  assert.match(readModel, /databaseReadCount: taskIds\.length \? 3 : 1/)
