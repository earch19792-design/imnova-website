...
const persistFixPath = "supabase/migrations/" +
  "20260920181752_fix_teo_pre_research_plan_persist_recovery_v1.sql"
const persistFixSql = readFileSync(persistFixPath, "utf8")
...
const newerSnapshot = "30000000-0000-4000-8000-000000000004"
...
test("sealed Control batch recovers historical and fresh plans after a newer snapshot", async () => {
  const db = new PGlite({ extensions: { pgcrypto } })
  const historical = candidate(1)
  const fresh = candidate(2)
  const outsideBatch = candidate(3)
  ...
  try {
    ...
    await db.exec(`insert into public.luna_catalog_snapshots_v1 values
      ...
      ('${snapshot}','${fresh.productId}','${fresh.variantId}',
        ...
        '2026-09-20T12:00:00Z','PREFLIGHT_PASS'),
      ('${snapshot}','${outsideBatch.productId}','${outsideBatch.variantId}',
        '${outsideBatch.sku}','${outsideBatch.sourceFingerprint}',
        '2026-09-20T12:00:00Z','PREFLIGHT_PASS');`)

    const createdBatch = await request(db, [historical, fresh])
    const batchId = createdBatch.batchId
    await db.exec(`insert into public.luna_catalog_snapshots_v1 values
      ('${newerSnapshot}','COMPLETE','2026-09-20T13:00:00Z');`)
    await assert.rejects(canonicalPlan(db, historical, snapshot,
      "40000000-0000-4000-8000-000000000010"),
    /LUNA_PRE_RESEARCH_AUTHORITATIVE_CANDIDATE_REQUIRED/)

    await db.exec(persistFixSql)

    await assert.rejects(canonicalPlan(db, outsideBatch, snapshot,
    ...
    const authority = (await db.query(`select
      ...
      (select coalesce(sum(ebay_writes),0)::int
        from public.marketplace_product_research_query_plans
        where source_candidate_key in ($1,$2)) as marketplace_writes,
    ...
    assert.deepEqual(authority, { batches: 1, members: 2,
      canonical_plans: 2, marketplace_writes: 0, source_snapshot_id: snapshot,
...
test("plan persist recovery is bounded to exact active Control authority", () => {
  for (const binding of [
    "batch.marketplace_account_key = p_marketplace_account_key",
    "batch.source_snapshot_id = p_luna_snapshot_id",
    "batch.contract_version =",
    "member.luna_product_id = p_luna_product_id",
    "member.luna_variant_id = p_luna_variant_id",
    "member.luna_sku = p_supplier_sku",
    "member.source_candidate_key = p_identity_key",
    "member.product_truth_fingerprint =",
    "capability.capability_code =",
    "capability.owner_user_id = batch.owner_user_id",
    "capability.command_client_id = batch.command_client_id",
    "capability.allowed_contract_version = batch.contract_version",
    "capability.enabled",
  ]) {
    assert.match(persistFixSql, new RegExp(binding.replaceAll(".", "\\.")))
  }
  assert.match(persistFixSql,
    /not exists[\s\S]*newer[\s\S]*or exists[\s\S]*seller_os_pre_research_batches_v1/)
  assert.doesNotMatch(persistFixSql,
    /alter\s+table|create\s+policy|disable\s+row\s+level|delete\s+from/i)
  assert.doesNotMatch(persistFixSql,
    /publisher|commercial.?trace|marketplace.?write/i)
})
