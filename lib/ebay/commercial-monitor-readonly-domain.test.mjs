...
test("stock sin identidad, parser roto o evidencia stale nunca se vuelve OOS", () => {
  ...
  const approvalRequired = resolveStockEvidence({
    productId: null,
    supplierVariantId: null,
    supplierSku: null,
    identityLimitationCode: "LUNA_HUMAN_APPROVED_LINK_REQUIRED",
    supplies: [baseSupply],
    marketplace,
    identity,
    now: new Date("2026-08-08T12:00:00.000Z"),
  })
  assert.equal(approvalRequired.state, "STOCK_UNKNOWN")
  assert.equal(approvalRequired.limitationCode,
    "LUNA_HUMAN_APPROVED_LINK_REQUIRED")
  assert.equal(parserError.state, "SOURCE_FORMAT_CHANGED")
  assert.equal(stale.state, "STALE")
  for (const result of [unknown, approvalRequired, parserError, stale]) {
