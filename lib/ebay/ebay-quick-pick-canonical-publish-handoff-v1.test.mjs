...
const imagesDigest = `sha256:${"f".repeat(64)}`
...
function fixture() {
  ...
  const listingReview = {
    ...
    category: { id: "123", name: "Canonical category" },
...
test("exact batch authority replaces per-SKU confirmation without mutating package", () => {
  const data = fixture()
  delete data.listingPackage.package_data.quickPickOwnerReviewV1
  data.card.listingReview.ownerReview = {
    ownerReviewConfirmed: false, packageMatch: false,
  }
  data.card.listingReview.publishAuthorizationHandoff
    .finalListingPackageMatch = false
  const result = build({ ...data, batchCommercialAuthorization: {
    contractVersion: "SELLER_OS_PUBLISHER_BATCH_COMMERCIAL_AUTHORITY_V1",
    batchAuthorizationId: "44444444-4444-4444-8444-444444444444",
    batchAuthorizationDigest: `sha256:${"a".repeat(64)}`,
    actorUserId: actor, accountKey, candidateKey,
    listingPackageId: packageId, packageDigest,
    packageMutationAllowed: false,
    authorizationBinding: {
      candidateId: candidateKey, packageId, packageDigest,
      accountId: accountKey, marketplaceId: "EBAY_US",
      price: 56.99, quantity: 1, category: "123",
      condition: "1000", images: 1, imagesDigest,
      businessPolicies: { fulfillmentPolicyId: "fulfillment-1",
        paymentPolicyId: "payment-1", returnPolicyId: "return-1" },
      location: "seller-location-1",
    },
  } })
  assert.equal(result.publishAuthorizationReady, true)
  assert.equal(result.ownerReviewConfirmed, true)
  assert.equal(result.packageMatch, true)
  assert.equal(result.authorization.commercialAuthorizationAuthority,
    "SELLER_OS_PUBLISHER_BATCH_AUTHORIZATION_V1")
  assert.equal(result.authorization.ownerReviewedPackageDigest, packageDigest)
  assert.equal(result.authorization.authorizedImagesDigest, imagesDigest)
  assert.equal(result.authorization.finalHumanAuthorizationRequired, false)
  assert.equal(result.safety.canPublishWithoutNewOwnerClick, true)
  assert.equal(result.safety.packageMutationAllowedAfterAuthorization, false)
})

test("batch execution uses frozen durable market-test authority, not transient owner projection", () => {
  const data = fixture()
  data.card.marketTestReady = false
  data.card.disposition = "RUNNING"
  const result = build({ ...data, batchCommercialAuthorization: {
    contractVersion: "SELLER_OS_PUBLISHER_BATCH_COMMERCIAL_AUTHORITY_V1",
    batchAuthorizationId: "44444444-4444-4444-8444-444444444444",
    batchAuthorizationDigest: `sha256:${"a".repeat(64)}`,
...
test("the existing publisher consumes the canonical handoff without Command Center", () => {
  ...
  const gateway = readFileSync(
    "lib/ebay/ebay-draft-only-gateway.ts", "utf8")
  const manualListing = readFileSync(
    "lib/ebay/ebay-manual-listing-service.ts", "utf8")
  ...
  assert.match(workspace, /EBAY_US · REAL ACCOUNT/)
  ...
  assert.match(publisher, /verifyEbayPublishedOffer/)
  assert.match(publisher, /verification\.connectorListingStatus !== "active"/)
  assert.match(gateway, /status === "PUBLISHED"/)
  assert.match(gateway, /EBAY_PUBLISHED_OFFER_EXACT_PAYLOAD_MISMATCH/)
  assert.match(manualListing, /verifyManualListingOwnershipReadonly/)
})
