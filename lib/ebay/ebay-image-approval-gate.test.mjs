...
test("current V9 deterministic single-source informational sets are reviewable", () => {
...
test("single-source fallback fails closed when any professional contract drifts", () => {
  const mutations = [
    (assets) => { assets[6].transformation.compositorContractVersion = "V8" },
    (assets) => { assets[6].transformation.generativeAiUsed = true },
    (assets) => { assets[6].transformation.competitorImageUsed = true },
    (assets) => { assets[6].source_sha256 = "b".repeat(64) },
    (assets) => { assets[6].qa_result.automaticStatus = "PARTIAL" },
    (assets) => { assets[6].qa_result.opaqueSourceFrameRemoved = false },
    (assets) => { assets[6].qa_result.textLineCount = 1 },
    (assets) => {
      assets[6].transformation.visualStrategyPosition.salesObjective =
        assets[5].transformation.visualStrategyPosition.salesObjective
    },
    (assets) => { assets.pop() },
  ]
