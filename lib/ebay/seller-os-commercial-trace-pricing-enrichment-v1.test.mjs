...
test("PostgreSQL executes the targeted enqueue and SKIP LOCKED claim contract", async () => {
  ...
  await db.query(`update public.seller_os_live_commercial_traces_v1
    set state='RUNNING',updated_at=clock_timestamp() where trace_id=$1`, [trace])
  ...
  /COMMERCIAL_TRACE_PRICING_ENRICHMENT_TRACE_BINDING_INVALID/)
  await db.query(`update public.seller_os_live_commercial_traces_v1
    set state='COMPLETED',updated_at=clock_timestamp() where trace_id=$1`, [trace])
