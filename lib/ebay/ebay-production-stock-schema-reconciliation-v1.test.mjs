...
// Execute the actual five schema-reconciliation queries against PostgreSQL,
// including projection columns, ordering and account predicates. The two
// newer StockGuard authorities are bounded in-memory inputs because they are
// outside the historical migration under test.
function reader(db,authorities=[]){let pending=Promise.resolve();return {from(table){
 if(table==='seller_os_listing_product_link_authorities_v1'||table==='seller_os_listing_identity_quarantines_v1'){
  let rows=table==='seller_os_listing_product_link_authorities_v1'?authorities:[]
  const q={select(){return q},eq(c,v){rows=rows.filter(row=>row[c]===v);return q},order(){return q},then(yes,no){return Promise.resolve({data:rows,error:null}).then(yes,no)}};return q
 }
 assert.ok(tables.includes(table));let columns,filters=[],values=[],order='',limit=1,single=false
 ...
 maybeSingle(){single=true;return q},then(yes,no){const run=pending.then(()=>db.query('select '+columns+' from '+ident(table)+' where '+filters.join(' and ')+order+' limit '+limit,values));pending=run.then(()=>undefined,()=>undefined)
 return run.then(r=>({data:JSON.parse(JSON.stringify(single?r.rows[0]??null:r.rows)),error:null})).then(yes,no)}};return q}}}
...
test('real PostgreSQL authority rows produce stock read; wrong account and unbound legacy row never leak',async()=>{
 ...
 await db.exec('set role service_role')
 const authority={authority_id:'listing-link-authority-v1:sha256:'+digest,account_key:account,marketplace_id:'EBAY_US',ebay_item_id:item,ebay_sku:sku,
  seller_os_product_id:legacyId,luna_product_id:product,luna_variant_id:variant,luna_sku:sku,supplier_quantity_required:1,evidence_maximum_age_seconds:21600,
  components:[{componentIdentityId:component,lunaProductId:product,lunaVariantId:variant,lunaSku:sku,supplierQuantityRequired:1,
   exactProductIdentity:true,exactVariantIdentity:true,exactSupplierSku:true,structuredVariantAttributesComplete:true}],identity_key:component,
  linkage_id:linkage,source_decision_id:'decision',lifecycle_state:'ACTIVE',previous_authority_id:null,transition_reason_code:'FIXTURE',actor_type:'SYSTEM',
  actor_reference:'fixture',identity_preflight_status:'PREFLIGHT_PASS',source_fingerprint:'sha256:'+digest,identity_engine_version:'fixture',
  preflight_contract_version:'fixture',activated_at:now.toISOString(),ended_at:null,created_at:now.toISOString(),updated_at:now.toISOString()}
 const run=accountKey=>readProductionStockGuardV1({supabase:reader(db,[authority]),accountKey,accountAlias:'imnova-ebay-us-primary',itemId:item,now})
