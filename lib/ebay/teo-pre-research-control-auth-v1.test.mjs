...
test("owner consent derives owner/client bindings server-side before service role", () => {
  ...
  const csrfAt = post.indexOf("await consumeControlCsrf(")
...
test("OWNER UI exposes only the existing bounded Marketplace Insights preflight", () => {
  const post = route.slice(route.indexOf("export async function POST"))
  const actionAt = post.indexOf("parseMarketplaceInsightsOwnerPreflightAction")
  const authAt = post.indexOf("validateAdminApiRequest(request)")
  const ownerAt = post.indexOf("assertSellerOsOwnerAdminPreResearchV1(auth)")
  const csrfAt = post.indexOf("await consumeControlCsrf(")
  const runAt = post.indexOf("await runMarketplaceInsightsPreflight()")
  assert.ok(actionAt >= 0 && authAt > actionAt && ownerAt > authAt &&
    csrfAt > ownerAt && runAt > csrfAt)
  assert.match(route, /configuration\.configuredFlag !== "FALSE"/)
  assert.match(route, /projectMarketplaceInsightsOwnerPreflightResult/)
  assert.match(route, /marketplaceWrites: 0/)
  assert.match(route, /credentialsExposed: false/)
  assert.match(route, /tokensExposed: false/)
  assert.match(consent, /Verificar Marketplace Insights/)
  assert.match(consent, /action: "PREFLIGHT_MARKETPLACE_INSIGHTS"/)
  assert.match(consent, /"X-Seller-OS-CSRF": csrf/)
  assert.doesNotMatch(consent, /EBAY_CLIENT_(?:ID|SECRET)|access_token-sensitive/)
})
