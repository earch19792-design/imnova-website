...
test("canonical browser worker is authoritative and server HTTP is non-authoritative", () => {
  assert.equal(resolveLunaWatcherSourcePriorityV1({
    authenticatedBrowserWorkerReady: true,
  }).actualMode, "AUTHENTICATED_WEB_SESSION")
  assert.equal(resolveLunaWatcherSourcePriorityV1({
    authenticatedBrowserWorkerReady: false,
  }).actualMode, "AUTHENTICATED_WEB_SESSION")
  assert.equal(resolveLunaWatcherSourcePriorityV1({
    authenticatedBrowserWorkerReady: false,
  }).sourcePriority[0].status, "REAUTH_REQUIRED")
  assert.equal(resolveLunaWatcherSourcePriorityV1({
    authenticatedBrowserWorkerReady: true,
  }).sourcePriority[1].status, "NON_AUTHORITATIVE_FOR_AUTHENTICATED_STOCK")
