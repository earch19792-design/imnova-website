...
test("canonical stock read accepts the certified Unicode Luna handle without changing identity", async () => {
  const url = "https://lunaportex.com/products/🔥sale🔥3pcs-set-big-sister-little-sister-keychains-set-mother-to-daughters-gifts"
  const result = await resolverFor(linkage("CERTIFIED", {
    components: [component({ canonicalSourceUrl: url })],
  }))({ linkageId: LINKAGE_ID, componentIdentityId: COMPONENT_ID })
  assert.equal(result.canonicalSourceUrl, new URL(url).toString())
  assert.equal(result.lunaProductId, "luna-z6")
  assert.equal(result.lunaVariantId, "luna-z6-black")
})

test("Unicode support preserves credentials, query, encoded separator and path boundaries", async () => {
  for (const url of [
    "https://user:password@lunaportex.com/products/🔥sale",
    "https://lunaportex.com/products/🔥sale?variant=other",
    "https://lunaportex.com/products/🔥sale#other",
    "https://lunaportex.com/products/a%2Fb",
    "https://lunaportex.com/products/a%252Fb",
    "https://lunaportex.com/products/a%00b",
    "https://lunaportex.com/products/../cart",
    "https://lunaportex.com/cart/🔥sale",
    "https://evil.example/products/🔥sale",
  ]) await assert.rejects(resolverFor(linkage("CERTIFIED", {
    components: [component({ canonicalSourceUrl: url })],
  }))({ linkageId: LINKAGE_ID, componentIdentityId: COMPONENT_ID }),
  /LUNA_CANONICAL_SERVER_READ_URL_REJECTED/)
...
test("normal stock claim uses database clock instead of racing database created_at", async () => {
  let called
  const repo = createSellerOsLunaStockObservationRepositoryV1({
    rpc: async (name, args) => {
      called = { name, args }
      return { data: [{ claimed: true, reason: "CLAIMED", attempt_number: 1, lease_expires_at: NOW }], error: null }
    },
  })
  await repo.claimJob({ stockCheckJobId: "job", workerId: "worker" })
  assert.equal(called.name, "claim_seller_os_luna_stock_check_job_v1")
  assert.equal(Object.hasOwn(called.args, "p_now"), false)
  assert.equal(called.args.p_lease_seconds, 180)
})
