...
test("scheduled recovery is generic, POST-only, and contains no batch case", async () => {
  const [route, runtime, scheduler] = await Promise.all([
  ...
  assert.match(route, /SELLER_OS_RUNTIME_RECOVERY_SECRET/)
  ...
  assert.match(scheduler,
    /QUICK_PICK_RUNTIME_RECOVERY[\s\S]*quick-pick-runtime-recovery/)
