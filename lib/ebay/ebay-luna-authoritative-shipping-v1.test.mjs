...
function limited(delay = 60_000) {
  return {
    ...
    rateLimitEvidence: {
      ...
      retryAfterClass: "SECONDS",
      retryAfterSafeValue: String(Math.ceil(delay / 1_000)),
...
test("canonical Luna reader accepts a normalized Unicode Shopify handle without weakening identity", () => {
  const source = {
    ...identity("7"),
    canonicalProductUrl:
      "https://www.lunaportex.com/products/%E2%9C%A8new%E2%9C%A8-luxury-fashion-heart-shape-alloy-artificial-rhinestones-bracelets-1-piece-perfect-gift-for-friends%F0%9F%8E%81?variant=ignored#ignored",
  }
  const normalized = normalizeLunaShippingIdentityV1(source)
  assert.equal(normalized.canonicalProductUrl,
    "https://www.lunaportex.com/products/%E2%9C%A8new%E2%9C%A8-luxury-fashion-heart-shape-alloy-artificial-rhinestones-bracelets-1-piece-perfect-gift-for-friends%F0%9F%8E%81")
  assert.throws(() => normalizeLunaShippingIdentityV1({ ...source,
    canonicalProductUrl: "https://www.lunaportex.com/products/safe%2Fescape" }),
...
test("active Retry-After window never hammers HTTP again", async () => {
  ...
  const second = await acquireAuthoritativeLunaShippingV1({
    identity: candidate, destination,
    httpAcquire: async () => { httpCalls += 1; return limited(120_000) },
    browserAcquire: async () => { browserCalls += 1; return quote() },
  ...
  assert.equal(second.rateLimitEvidence.classificationOrigin,
    "ACTIVE_INTERNAL_COOLDOWN")
  assert.equal(second.rateLimitEvidence.cooldownRemainingClass,
    "UP_TO_5_MINUTES")
  assert.equal(second.rateLimitEvidence.upstreamHttpStatusClass, null)
})

test("SHIPPING_429_BACKOFF_PASS / SCOPED_SHIPPING_RECOVERY_PASS: browser unavailable blocks without inventing a shipping amount", async () => {
  const result = await acquireAuthoritativeLunaShippingV1({
  ...
  assert.equal(result.retryNotBefore, "2026-08-24T17:01:00.000Z")
  assert.equal(result.rateLimitScope, "LUNA_SCOPED_429")
  assert.equal(result.rateLimitEvidence.classificationOrigin,
    "CURRENT_HTTP_429")
  assert.equal(result.rateLimitEvidence.upstreamHttpStatusClass, "HTTP_429")
})
  ...
  const result = await attemptLunaAuthenticatedHttpShippingQuoteV1(
    identity("6"), {
      ...
      fetchImpl: async () => {
        ...
        }, "https://www.lunaportex.com/cart.js?not-retained=secret")
      },
    })
  assert.equal(result.status, "LUNA_RATE_LIMITED")
  assert.equal(result.retryAfterMs, 120_000)
  assert.deepEqual(result.rateLimitEvidence, {
    classificationOrigin: "CURRENT_HTTP_429",
    upstreamHttpStatusClass: "HTTP_429",
    retryAfterPresent: true,
    retryAfterClass: "SECONDS",
    retryAfterSafeValue: "120",
    finalUrlHostClass: "LUNA_WWW",
    finalPathClass: "LUNA_CART_SNAPSHOT",
    cooldownRemainingClass: null,
  })
  assert.doesNotMatch(JSON.stringify(result.rateLimitEvidence), /secret/)
  assert.equal(calls, 1)
})

test("HTTP-date Retry-After is normalized without retaining raw headers", async () => {
  const retryAt = new Date(Date.now() + 120_000).toUTCString()
  const result = await attemptLunaAuthenticatedHttpShippingQuoteV1(
    identity("a"), {
      resolveProtectedSession: async () => "session=opaque-fixture",
      fetchImpl: async () => responseWithUrl(JSON.stringify({}), {
        status: 429, headers: { "Retry-After": retryAt },
      }, "https://www.lunaportex.com/cart/shipping_rates.json?zip=private"),
    })
  assert.equal(result.rateLimitEvidence.retryAfterPresent, true)
  assert.equal(result.rateLimitEvidence.retryAfterClass, "HTTP_DATE")
  assert.match(result.rateLimitEvidence.retryAfterSafeValue,
    /^\d{4}-\d{2}-\d{2}T/)
  assert.equal(result.rateLimitEvidence.finalUrlHostClass, "LUNA_WWW")
  assert.equal(result.rateLimitEvidence.finalPathClass, "LUNA_SHIPPING_RATES")
  assert.doesNotMatch(JSON.stringify(result.rateLimitEvidence), /private/)
})

test("HTTP 429 without Retry-After remains explicitly ABSENT", async () => {
  const result = await attemptLunaAuthenticatedHttpShippingQuoteV1(
    identity("b"), {
      resolveProtectedSession: async () => "session=opaque-fixture",
      fetchImpl: async () => responseWithUrl(JSON.stringify({}), {
        status: 429,
      }, "https://account.lunaportex.com/login?token=not-retained"),
    })
  assert.equal(result.rateLimitEvidence.retryAfterPresent, false)
  assert.equal(result.rateLimitEvidence.retryAfterClass, "ABSENT")
  assert.equal(result.rateLimitEvidence.retryAfterSafeValue, null)
  assert.equal(result.rateLimitEvidence.finalUrlHostClass, "LUNA_ACCOUNT")
  assert.equal(result.rateLimitEvidence.finalPathClass, "LUNA_AUTH")
  assert.doesNotMatch(JSON.stringify(result.rateLimitEvidence), /not-retained/)
})
