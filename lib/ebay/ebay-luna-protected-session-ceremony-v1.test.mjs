...
} = await import("./ebay-luna-browser-context-recovery-gate-v1.ts")
...
test("owner-workstation handoff start is admin-CSRF bound and has no browser ceremony state", () => {
  const h = csrfHarness()
  const issued = h.issue()
  assert.equal(h.consume(issued.csrfToken, {
  ...
  const wrongState = csrfHarness()
  const stateToken = `1.${INSTANCE}.${"A".repeat(43)}`
  const withState = wrongState.issue({ stateToken })
  assert.throws(() => wrongState.consume(withState.csrfToken, {
    action: "OWNER_HANDOFF", stateToken,
  }), /LUNA_CEREMONY_WRONG_STATE/)
})
...
test("worker is visible, bounded, and retains only a completed authenticated context", async () => {
  ...
  assert.match(worker, /reason === "COMPLETED"/)
  assert.match(worker, /promotePendingBrowserWorker\(pending\)/)
  assert.match(worker, /LUNA_CAPTCHA_BLOCKED/)
  assert.match(worker, /LUNA_REAUTH_REQUIRED/)
...
test("UI has no credential/cookie input and delegates only to the owner extension", async () => {
  ...
  assert.match(page, /Abre Luna en tu Chrome normal/)
  assert.match(page, /Seller OS — Luna Owner Session Handoff/)
  assert.match(page, /cifra en memoria/)
  assert.doesNotMatch(page, /createObjectURL|anchor\.download/)
})

test("browser-context recovery gate covers the protected-session state matrix", () => {
  const gate = (protectedSessionStatus, browserContextActive,
  ...
  const initial = gate("SESSION_NOT_CONFIGURED", false)
  assert.equal(initial.startAllowed, true)
  assert.equal(initial.recoveryRequired, false)

  const retained = gate("SESSION_READY", true)
  assert.equal(retained.startAllowed, false)
  assert.equal(retained.recoveryRequired, false)

  const recovery = gate("SESSION_READY", false)
  assert.equal(recovery.startAllowed, true)
  assert.equal(recovery.recoveryRequired, true)
  assert.equal(recovery.startMode, "RECOVER_BROWSER_CONTEXT")

  for (const status of ["SESSION_EXPIRED", "AUTH_REQUIRED", "AUTH_FAILED"]) {
    const reauthentication = gate(status, false)
    assert.equal(reauthentication.startAllowed, true)
    assert.equal(reauthentication.recoveryRequired, false)
    assert.equal(gate(status, true).startAllowed, true)
  }

  assert.equal(gate("SESSION_READY", false, true).startAllowed, false)
})

test("legacy browser recovery remains server-guarded but is retired from owner UI", async () => {
  const page = await readFile(new URL(
    "../../app/admin/ebay/luna-protected-session/page.tsx",
    import.meta.url,
  ), "utf8")
  ...
  assert.doesNotMatch(page, /void act\("START"\)/)
  assert.match(page, /LUNA_OWNER_SESSION_HANDOFF_EXTENSION_V1/)
  assert.match(page, /"X-Seller-OS-CSRF": csrfToken/)
  assert.match(route, /browserContextRecoveryRequired:/)
  assert.match(route, /ceremonyStartAllowed:/)
  assert.match(route, /BROWSER_CONTEXT_RECOVERY_REQUIRED/)
  assert.match(worker, /browserContextActive/)
  assert.match(worker, /health !== "HEALTHY"/)
  assert.match(worker, /promotePendingBrowserWorker\(pending\)/)
  assert.doesNotMatch(page, /setBrowserContext|contextHealthProven:\s*true/)
})
...
test("status refresh is GET-only, rotates no state, and clears stale UI error", async () => {
  ...
  const refreshSource = page.slice(
    ...
    page.indexOf("const prepareOwnerHandoff = useCallback"),
  )
