...
test("owner UI owns upload and reminder while Remote Operator has no upload or raw access", () => {
  ...
  assert.match(owner, /Último reporte/)
  assert.match(owner, /Actualizar reporte/)
  assert.match(owner, /CURRENT/)
  assert.match(owner, /STALE/)
  assert.match(owner, /El último reporte válido sigue disponible/)
  assert.match(owner, /Ver detalles/)
  assert.doesNotMatch(owner, /<details[^>]*open=/)
