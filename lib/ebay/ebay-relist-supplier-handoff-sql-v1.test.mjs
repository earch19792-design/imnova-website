...
test('real SQL relist handoff: certified lineage, identity boundaries, immutable history and idempotent replay',async t=>{
 ...
 await db.exec(read('20260911161035_reconcile_existing_relist_lineage_cache_v1.sql'))
 ...
 await db.exec("alter table ebay_listing_packages add column package_data jsonb default '{}'::jsonb")
 const cleanup=read('20260911200135_current_publication_factory_isolation.sql')
 await db.exec(cleanup.slice(cleanup.indexOf('-- A new, unpublished preparation')))
 await db.exec(cleanup.slice(cleanup.indexOf('-- A new, unpublished preparation')))
 await db.query("insert into ebay_listing_packages(id,account_key,opportunity_id,candidate_key,package_data) values($1,'account',$2,'luna:exact',$3)",
 [uuid(11),uuid(3),JSON.stringify({currentPublicationFactoryV1:{version:'SELLER_OS_CURRENT_PUBLICATION_FACTORY_V1'}})])
 ...
 await t.test('restart after an old sync erased projection reuses the immutable canonical decision',async()=>{
  const before=(await db.query("select to_jsonb(d) as decision from seller_os_luna_linkage_decisions d where ebay_item_id='366643126310'")).rows
  await db.exec("update ebay_active_listings set raw_payload=raw_payload-'canonicalSupplierLineage' where ebay_item_id='366643126310'")
  const recovered=await run();assert.equal(recovered.status,'CERTIFIED');assert.equal(recovered.idempotent,true);assert.equal(recovered.durableReadbackMatch,true)
  assert.deepEqual((await db.query("select to_jsonb(d) as decision from seller_os_luna_linkage_decisions d where ebay_item_id='366643126310'")).rows,before)
  const cache=(await db.query("select raw_payload#>>'{canonicalSupplierLineage,supersedesItemId}' as predecessor from ebay_active_listings where ebay_item_id='366643126310'")).rows[0]
  assert.equal(cache.predecessor,'366553691290')
 })
