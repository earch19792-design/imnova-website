...
test("active category ambiguity stays demand unproven for an exact Luna identity", async () => {
  let persistenceWrites = 0
  const result = await discoverAndPersistSellerOsOnDemandFamilyDemandV1({
    supabase: { rpc() { persistenceWrites += 1
      throw new Error("UNEXPECTED_PERSISTENCE_WRITE") } },
    accountKey: "seller-test",
    lunaCatalogRow: { supplier_product_id: "9265006772448",
      supplier_variant_id: "48903121404128", sku: "ITEM1091",
      title: "Pool Fence Replacement Sleeves and Caps",
      variant_title: "Tan", product_type: "Craft & DIY",
      tags: ["Craft & DIY"], price: 4,
      captured_at: "2026-09-03T09:10:26.483Z" },
    async demandReader(candidate) {
      return buildEbaySellerKeywordDemandValidation({ candidate,
        comparables: [
          { itemId: "111111111111", title: "Pool Fence Sleeve and Cap",
            price: 12.99, currency: "USD", categoryId: "11700",
            categoryName: "Pool Equipment Parts", sellerUsername: "seller-a",
            source: "EBAY_BROWSE_ACTIVE_LISTING" },
          { itemId: "222222222222", title: "Fence Post Sleeve and Cap",
            price: 14.99, currency: "USD", categoryId: "20515",
            categoryName: "Fence Accessories", sellerUsername: "seller-b",
            source: "EBAY_BROWSE_ACTIVE_LISTING" },
        ], insightsAvailability: "NOT_CONFIGURED",
        asOf: "2026-09-03T19:57:34.000Z" })
    },
  })
  assert.equal(persistenceWrites, 0)
  assert.equal(result.status, "FAMILY_DEMAND_UNPROVEN")
  assert.equal(result.reasonCode,
    "ON_DEMAND_MARKETPLACE_INSIGHTS_NOT_CONFIGURED")
  assert.equal(result.demandNegativeEvidencePresent, false)
  assert.equal(result.marketTestRadarFamily?.exactSupplierIdentity?.supplierSku,
    "ITEM1091")
  assert.equal(result.marketTestRadarFamily?.observationSeries?.[0]
    ?.limitations?.includes("MARKETPLACE_CATEGORY_UNPROVEN"), true)
  assert.equal(result.marketTestRadarFamily?.observationSeries?.[0]
    ?.limitations?.includes("MARKETPLACE_CATEGORY_AMBIGUOUS"), true)
})
