...
test("runtime and route have static guards against external/eBay/Luna clients", async () => {
  ...
  assert.match(routeSource, /maxDuration = 60/)
...
test("cron is registered after business-time policy certification", async () => {
  const vercel = JSON.parse(await readFile(new URL("../../vercel.json", import.meta.url)))
  assert.deepEqual((vercel.crons ?? []).find((cron) =>
    cron.path === "/api/cron/daily-dollar-radar-autopilot"), {
