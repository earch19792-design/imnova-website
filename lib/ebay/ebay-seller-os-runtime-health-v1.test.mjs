...
import {
  ...
  attestSellerOsRuntimeCatalogV1,
...
test("healthy allowlisted runtime normalizes to complete HEALTHY evidence", async () => {
  ...
  assert.equal(result.runtimeCatalog.workspaceRuntimeBindingStatus,
    "UNAVAILABLE")
})

test("same-factory registration ledger attests exact bounded runtime catalog", async () => {
  const toolNames = [
    "seller_os_get_runtime_health",
    "seller_os_get_official_orders",
    "seller_os_get_sales_order_events",
    "seller_os_get_recent_sales_feed",
    "seller_os_get_sale_alerts",
    "seller_os_get_whatsapp_sale_alert_status",
    "seller_os_get_buyer_thank_you_status",
    "search",
    "fetch",
  ]
  const result = attestSellerOsRuntimeCatalogV1(await collect(), {
    registeredToolNames: toolNames,
    expectedToolNames: toolNames,
    runtimeWorkingDirectoryMatch: true,
    loadedMcpImplementationVersion: "SELLER_OS_MCP_READONLY_V1_2026_08_12",
  })
  assert.equal(result.runtimeCatalog.contractVersion,
    SELLER_OS_RUNTIME_CATALOG_ATTESTATION_VERSION)
  assert.equal(result.runtimeCatalog.observedAt, NOW.toISOString())
  assert.equal(result.runtimeCatalog.runtimeCatalogCount, 9)
  assert.equal(result.runtimeCatalog.expectedCatalogCount, 9)
  assert.equal(result.runtimeCatalog.officialOrdersToolPresent, true)
  assert.equal(result.runtimeCatalog.salesOrderEventsToolPresent, true)
  assert.equal(result.runtimeCatalog.recentSalesFeedToolPresent, true)
  assert.equal(result.runtimeCatalog.saleAlertsToolPresent, true)
  assert.equal(result.runtimeCatalog.whatsappSaleAlertStatusToolPresent, true)
  assert.equal(result.runtimeCatalog.buyerThankYouStatusToolPresent, true)
  assert.equal(result.runtimeCatalog.exactCatalogMatch, true)
  assert.equal(result.runtimeCatalog.runtimeWorkingDirectoryMatch, true)
  assert.equal(result.runtimeCatalog.workspaceRuntimeBindingStatus, "MATCHED")
  assert.deepEqual(result.runtimeCatalog.limitations, [])
})

test("catalog or workspace mismatch remains explicit without leaking paths", async () => {
  const result = attestSellerOsRuntimeCatalogV1(await collect(), {
    registeredToolNames: ["seller_os_get_runtime_health", "search", "fetch"],
    expectedToolNames: ["seller_os_get_runtime_health",
      "seller_os_get_official_orders", "seller_os_get_sales_order_events",
      "seller_os_get_recent_sales_feed",
      "seller_os_get_sale_alerts",
      "seller_os_get_whatsapp_sale_alert_status",
      "seller_os_get_buyer_thank_you_status",
      "search", "fetch"],
    runtimeWorkingDirectoryMatch: false,
    loadedMcpImplementationVersion: "SELLER_OS_MCP_READONLY_V1_2026_08_12",
  })
  assert.equal(result.runtimeCatalog.runtimeCatalogCount, 3)
  assert.equal(result.runtimeCatalog.expectedCatalogCount, 9)
  assert.equal(result.runtimeCatalog.officialOrdersToolPresent, false)
  assert.equal(result.runtimeCatalog.salesOrderEventsToolPresent, false)
  assert.equal(result.runtimeCatalog.recentSalesFeedToolPresent, false)
  assert.equal(result.runtimeCatalog.saleAlertsToolPresent, false)
  assert.equal(result.runtimeCatalog.whatsappSaleAlertStatusToolPresent, false)
  assert.equal(result.runtimeCatalog.buyerThankYouStatusToolPresent, false)
  assert.equal(result.runtimeCatalog.exactCatalogMatch, false)
  assert.equal(result.runtimeCatalog.workspaceRuntimeBindingStatus, "MISMATCHED")
  assert.ok(result.runtimeCatalog.limitations.includes(
    "RUNTIME_WORKING_DIRECTORY_IDENTITY_MISMATCH"))
  assert.ok(result.runtimeCatalog.limitations.includes(
    "RUNTIME_CATALOG_EXPECTATION_MISMATCH"))
  assert.ok(result.runtimeCatalog.limitations.includes(
  ...
  assert.doesNotMatch(JSON.stringify(result.runtimeCatalog),
    /\/home\/|CONTROL_PLANE_API_KEY|process\.env/i)
  assert.equal(result.runtimeCatalog.safety.fileContentsIncluded, false)
})
