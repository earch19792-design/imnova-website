...
test("strong target identity can corroborate a listing that omits its identifier", () => {
  const result = classifyWinnerComparable(targetIdentity, {
    manufacturerBrand: "Lysol",
    productName: "Lysol Disinfecting Wipes Lemon",
    packCount: 3,
  ...
  assert.ok(result.reasons.includes("TARGET_IDENTIFIER_PLUS_BRAND_NAME_PACK"))
})

test("brand, name and pack cannot make a weak target identity exact", () => {
  const weakTarget = {
    manufacturerBrand: "Lysol",
    productName: "Lysol Disinfecting Wipes Lemon",
    packCount: 3,
  ...
  const result = classifyWinnerComparable(weakTarget, { ...weakTarget })
  assert.notEqual(result.classification, "EXACT_MATCH")
})

test("target corroboration never hides a different pack or variant", () => {
  ...
  assert.equal(classifyWinnerComparable(targetIdentity, {
    ...comparableIdentity, packCount: 2,
  }).classification, "DIFFERENT_PACK")
  assert.equal(classifyWinnerComparable(targetIdentity, {
    ...comparableIdentity, scent: "Lavender",
  }).classification, "DIFFERENT_VARIANT")
  assert.equal(classifyWinnerComparable(targetIdentity, {
    ...comparableIdentity, mpn: "OTHER-MPN",
  }).classification, "INVALID_COMPARABLE")
  assert.equal(classifyWinnerComparable(targetIdentity, {
    ...comparableIdentity, model: "OTHER-MODEL",
  }).classification, "INVALID_COMPARABLE")
})

test("Browse listings default to one offer pack without turning count into pack", () => {
  const single = normalizeWinnerComparableOfferCounts({
    title: "Lysol Disinfecting Wipes Lemon 15 Ct",
  })
  assert.equal(single.packCount, 1)
  assert.equal(single.unitCount, 15)

  const multipack = normalizeWinnerComparableOfferCounts({
    title: "Lysol Disinfecting Wipes Lemon Pack of 3, 15 Ct Each",
  })
  assert.equal(multipack.packCount, 3)
  assert.equal(multipack.unitCount, 15)
})
...
test("pack evidence class is derived from canonical identity and cannot be caller-promoted", () => {
  const result = buildWinnerEvidenceDecisionPackage(input({
    comparables: [
      comparable(),
      comparable({
        sourceListingId: "different-pack",
        identity: { ...targetIdentity, packCount: 2 },
      }),
      comparable({
        source: "EBAY_MAIN_SEARCH_SOLD_BROWSER_CAPTURE",
        sourceListingId: "pack-unknown",
        identity: { ...targetIdentity, packCount: null },
        itemPrice: null,
        shippingCost: null,
        displayedSoldPriceAmount: 29.99,
        ...
        evidenceReviewed: true,
        packEvidenceClassification: "EXACT_PACK_COMPARABLE",
      }),
    ],
  }))
  assert.deepEqual(result.comparables.classified.map((entry) =>
    entry.packEvidenceClassification), [
    "EXACT_PACK_COMPARABLE",
    "DIFFERENT_PACK_BUT_COMMERCIALLY_RELEVANT",
    "PACK_UNKNOWN",
  ])
  assert.equal(result.comparables.classified[2].pricing.landedPrice, null)
  assert.equal(result.comparables.classified[2].pricing.displayedSoldPrice, 29.99)
  assert.equal(result.comparables.classified[2].pricing.realizedPriceStatus, "UNPROVEN")
})
...
test("reviewed imports can be sold evidence and unreviewed imports are invalid", () => {
  const result = buildWinnerEvidenceDecisionPackage(input({
    comparables: [
      comparable({
        ...
        evidenceReviewed: true,
        evidenceScope: "MARKET_WIDE_SOLD_EVIDENCE",
      }),
      comparable({
  ...
  assert.equal(result.comparables.counts.marketWideSoldExact, 1)
  assert.equal(result.comparables.counts.ownAccountSoldExact, 0)
  assert.equal(result.comparables.classified[0].evidenceScope, "MARKET_WIDE_SOLD_EVIDENCE")
})
...
test("official Browse metadata counts galleries without downloading competitor images", () => {
  const metadata = buildOfficialEbayVisualMetadata({
    image: { imageUrl: "https://i.ebayimg.com/images/g/main.jpg" },
    additionalImages: [
      { imageUrl: "https://i.ebayimg.com/images/g/second.jpg" },
      { imageUrl: "https://i.ebayimg.com/images/g/third.jpg" },
    ],
  }, "2026-07-17T12:00:00.000Z")
  assert.equal(metadata.imageCount, 3)
  assert.equal(metadata.sourceType, "OFFICIAL_EBAY_METADATA")
  assert.equal(metadata.imageDownloaded, false)
  assert.equal(metadata.imageCopied, false)
  assert.equal(metadata.pixelAnalysisPerformed, false)
  assert.doesNotMatch(JSON.stringify(metadata), /i\.ebayimg|https?:/i)
})
...
test("visual observations are versioned and change the auditable package hash", () => {
  ...
  assert.equal(lowClarity.packageVersion, "EBAY_WINNER_EVIDENCE_PRODUCT_DECISION_VISUAL_V2_6_2026_07_17")
...
test("Loop 2 intake is signed inside the decision package without competitor content", () => {
  const listingAiIntake = {
    ...
    titleStructurePatterns: ["BRAND_EARLY_IN_EXACT_TITLES"],
    sellerPatterns: {
      activeSellerCount: 3,
      verifiedSoldSellerCount: 2,
      freeShippingPrevalencePercent: 75,
    },
    locale: "en-US",
  }
  const withIntake = buildWinnerEvidenceDecisionPackage(input({ listingAiIntake,
    marketEvidence: { activeSellerCount: 3, verifiedSoldSellerCount: 2,
      evidenceBasis: "CONFIRMED_SOLD_EXACT", discoveryOrigin: "EBAY_FIRST",
      crossSourceCorroborated: true, activeAndSoldSeparated: true } }))
  const withoutIntake = buildWinnerEvidenceDecisionPackage(input())
  assert.equal(withIntake.listingAiIntake.approvedKeywords.length, 2)
  assert.equal(withIntake.listingAiIntake.sellerPatterns.activeSellerCount, 3)
  assert.equal(withIntake.listingAiIntake.sellerPatterns.sellerIdentitiesIncluded, false)
  assert.equal(withIntake.marketEvidence.activeSellerCount, 3)
  assert.equal(withIntake.marketEvidence.competitorSellerIdentitiesStored, false)
