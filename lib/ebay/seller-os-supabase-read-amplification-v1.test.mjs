...
test("owner dashboard uses one bounded queue projection and no count fan-out", async () => {
  ...
  const dashboard = await readFile(new URL(
    ...
    import.meta.url), "utf8")
  const detailedQueue = await readFile(new URL(
    "../../app/admin/ebay/opportunity-queue/page.tsx",
    import.meta.url), "utf8")
  const commandCenter = await readFile(new URL(
    "../../app/admin/ebay/mobile-review/opportunity-command-center.tsx",
    import.meta.url), "utf8")
  ...
  assert.match(readModel, /dashboard_minimum_readiness_current/)
  assert.doesNotMatch(readModel, /hard_gates,evidence_guards,assessment/)
  assert.match(readModel, /assessmentJsonRead: false/)
  assert.match(route, /fullQueue/)
  assert.match(route,
    /fullQueue\s*\?\s*await getEbayFirstLunaQueueDashboard/)
  assert.doesNotMatch(dashboard, /fullQueue=1/)
  assert.match(detailedQueue, /luna-opportunity-queue\?fullQueue=1/)
  assert.match(commandCenter, /luna-opportunity-queue\?fullQueue=1/)
...
test("generated queue fields retain assessment as canonical authority", async () => {
  ...
  const readinessMigration = await readFile(new URL(
    "../../supabase/migrations/20260905064000_seller_os_dashboard_minimum_readiness_projection_v1.sql",
    import.meta.url), "utf8")
  assert.match(readinessMigration, /generated always as/)
  assert.match(readinessMigration, /MINIMUM_TRUTHFUL_LISTING_READINESS_V1/)
