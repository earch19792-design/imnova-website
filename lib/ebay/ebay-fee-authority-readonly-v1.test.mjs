...
test("multiple or unknown Store subscriptions fail closed", () => {
  assert.equal(parseEbaySellerStoreSubscriptionReadonly({ subscriptions: [
    { marketplaceId: "EBAY_US", subscriptionType: "STORE_PLAN", subscriptionLevel: "BASIC" },
    { marketplaceId: "EBAY_US", subscriptionType: "STORE_PLAN", subscriptionLevel: "PREMIUM" },
  ] }).status, "AMBIGUOUS")
  assert.equal(parseEbaySellerStoreSubscriptionReadonly({ subscriptions: [
    { marketplaceId: "EBAY_US", subscriptionType: "STORE_PLAN", subscriptionLevel: "OTHER" },
  ] }).status, "UNPROVEN")
})
...
test("absent or malformed subscriptions never prove NO_STORE", () => {
  for (const input of [{}, null, { subscriptions: null }, { subscriptions: [{}] }]) {
    assert.equal(parseEbaySellerStoreSubscriptionReadonly(input).status, "UNPROVEN")
  }
})


test("unrecognized subscription kinds and incomplete pages cannot prove no Store", () => {
  for (const input of [
    {subscriptions: [{marketplaceId: "EBAY_US", subscriptionType: "STORE", subscriptionLevel: "Basic"}]},
    {subscriptions: [], next: "https://api.ebay.com/sell/account/v1/subscription?offset=1"},
  ]) assert.equal(parseEbaySellerStoreSubscriptionReadonly(input).status, "UNPROVEN")
})
