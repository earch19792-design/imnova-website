...
test("PASS_EXISTING_LOW_QUALITY_COHORT_RECOVERY_IS_BOUNDED_WITHOUT_STARVATION", async () => {
  const cohort = Array.from({ length: 32 }, (_, index) => {
    const hex = (index + 1).toString(16).padStart(64, "0")
    return quickPickRow({
      id: `90477894-9ac3-47ea-9cbd-${(index + 1).toString().padStart(12, "0")}`,
      candidate_key: `sha256:${hex}`,
      supplier_product_id: String(9266387058912 + index),
      supplier_variant_id: String(48907793826016 + index),
      supplier_sku: `ITEM${1046 + index}`,
    })
  })
  const result = await reconcileQuickPickProductResearchHandoffV1({
    supabase: {}, accountKey: "EBAY_US_ACCOUNT",
    dependencies: dependencies({ rows: cohort }),
  })
  assert.equal(result.eligibleCandidateCount, 32)
  assert.equal(result.reconciledPlanCount, 32)
})
