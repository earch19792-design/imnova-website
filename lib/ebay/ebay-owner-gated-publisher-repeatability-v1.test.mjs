...
test("network failures remain classified and never trigger hidden retry", () => {
  ...
  assert.match(route, /errorClass: safe\s*\? null/)
  assert.match(route, /safeNextAction: safe[\s\S]{0,80}OWNER_ACTION_REQUIRED_TO_CONTINUE/)
})

test("one initial authorization reaches preview and final publish stays separate", () => {
  ...
  const readbackBlockStart = action.indexOf(
    "if (unpublishedDraftCreatedThisClick)",
  )
  const prepareBlockStart = action.indexOf(
    "if (!nextPublication?.id || nextPublication.phase !== \"preview_ready\")",
    readbackBlockStart,
  )
  assert.doesNotMatch(
    action.slice(readbackBlockStart, prepareBlockStart),
    /\breturn\b/,
  )
