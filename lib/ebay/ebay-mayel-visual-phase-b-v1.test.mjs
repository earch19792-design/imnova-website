...
const { readOfficialActiveListingBrowseSnapshotV1,
  readOfficialActiveListingImageSnapshotV1,
  buildOfficialTradingPictureReadbackV1,
  EBAY_TRADING_OFFICIAL_IMAGE_SET_DIGEST_VERSION } = await import(
  "./ebay-active-listing-image-revision-service.ts")
const { buildMayelVisualManifestV1,
  buildMayelOrderedVisualManifestV2 } = await import(
...
test("safe rebase does not resurrect a removed Mayel asset already applied as EPS", () => {
  const appliedAssetId = "33333333-3333-4333-8333-333333333333"
  const appliedSource = `${mayel}?asset=applied`
  const appliedEps = "https://i.ebayimg.com/images/g/applied/s-l1600.jpg"
  const selectedAssetId = "44444444-4444-4444-8444-444444444444"
  const selectedSource = `${mayel}?asset=selected`
  const assets = [
    { assetId: appliedAssetId, role: "DETAIL",
      outputSha256: "d".repeat(64), publicUrl: appliedSource },
    { assetId: selectedAssetId, role: "LIFESTYLE",
      outputSha256: "e".repeat(64), publicUrl: selectedSource },
  ]
  const manifest = buildMayelOrderedVisualManifestV2({
    visualTaskId: taskId, ebayItemId: itemId, currentImages: [main], assets,
    finalOrder: [{ kind: "MAYEL_ASSET", assetId: selectedAssetId }],
    productTruthDigest: `sha256:${"b".repeat(64)}`,
    sourceImageSetDigest: `sha256:${"c".repeat(64)}`,
  })
  const rows = assets.map((asset) => ({ id: asset.assetId,
    status: "approved", mayel_approval_status: "APPROVED",
    owner_approval_status: "PENDING", mayel_output_role: asset.role,
    output_sha256: asset.outputSha256, public_url: asset.publicUrl,
    product_truth_digest: `sha256:${"b".repeat(64)}`,
    source_image_set_digest: `sha256:${"c".repeat(64)}` }))
  const rebased = buildMayelVisualPhaseBRebaseV1({
    visualTaskId: taskId, ebayItemId: itemId,
    ...
    taskProductTruthDigest: `sha256:${"b".repeat(64)}`,
    taskSourceImageSetDigest: `sha256:${"c".repeat(64)}`,
    currentOfficialImageUrls: [main, appliedEps], approvedAssets: rows,
    appliedMayelOfficialImages: [{ assetId: appliedAssetId,
      officialUrl: appliedEps }],
    canonicalPublicAssetUrlAllowed: (url) =>
      [appliedSource, selectedSource].includes(url),
  })
  assert.equal(rebased.safe, true)
  assert.deepEqual(rebased.manifest.proposedOrderedImages.map((entry) =>
    entry.publicUrl), [selectedSource])
  assert.equal(rebased.manifest.proposedOrderedImages.some((entry) =>
    entry.publicUrl === appliedEps), false)
})
...
test("canonical account-bound management reader performs only OAuth plus bounded official GET reads", async () => {
  ...
  try {
    ...
    assert.equal(calls.filter((call) => call.href.includes("/sell/inventory/"))
      .every((call) => call.headers?.["Accept-Language"] === "en-US"), true)
  } finally {
...
test("fresh durable account authority verifies a Trading listing from GetItem without consuming GetUser quota", async () => {
  const previousUser = process.env.EBAY_DRAFT_ONLY_PRODUCTION_EXPECTED_USER_ID
  ...
  delete process.env.EBAY_DRAFT_ONLY_PRODUCTION_EXPECTED_CREDENTIAL_FINGERPRINT
  delete process.env.EBAY_DRAFT_ONLY_PRODUCTION_EXPECTED_ACCOUNT_FINGERPRINT
  const fingerprint = createHash("sha256").update("PRODUCTION:seller")
    .digest("hex")
  const calls = []
  const xml = `<?xml version="1.0"?><GetItemResponse xmlns="urn:ebay:apis:eBLBaseComponents"><Ack>Success</Ack><Item><ItemID>${itemId}</ItemID><Seller><UserID>seller</UserID></Seller><SellingStatus><ListingStatus>Active</ListingStatus></SellingStatus><SKU>SKU-1</SKU><ListingType>FixedPriceItem</ListingType><PictureDetails><PictureSource>EPS</PictureSource><PictureURL>${main}</PictureURL></PictureDetails></Item></GetItemResponse>`
  try {
    const snapshot = await readOfficialActiveListingImageSnapshotV1({
      accessToken: "token", itemId, expectedSku: "SKU-1",
      accountKey: `seller:${fingerprint}`,
      durableAccountIdentityProven: true,
      fetchImpl: async (_url, init = {}) => {
        calls.push(init.headers?.["X-EBAY-API-CALL-NAME"])
    ...
    assert.equal(snapshot.sellerUserId, "seller")
    assert.deepEqual(snapshot.pictureUrls, [main])
  } finally {
    const restore = (key, value) => {
      if (value === undefined) delete process.env[key]
      else process.env[key] = value
    }
    restore("EBAY_DRAFT_ONLY_PRODUCTION_EXPECTED_USER_ID", previousUser)
    restore("EBAY_DRAFT_ONLY_PRODUCTION_EXPECTED_CREDENTIAL_FINGERPRINT",
      previousCredential)
    restore("EBAY_DRAFT_ONLY_PRODUCTION_EXPECTED_ACCOUNT_FINGERPRINT",
      previousAccount)
...
test("one Trading PictureURL with an official query remains one digest input", () => {
  const url = `${main}?set_id=880000500F`
  const readback = buildOfficialTradingPictureReadbackV1([url])
  assert.equal(readback.officialPictureCount, 1)
  assert.equal(readback.orderedPictureUrlCount, 1)
  assert.equal(readback.imageDigestInputCount, 1)
  assert.equal(readback.imageDigestCanonicalizationVersion,
    EBAY_TRADING_OFFICIAL_IMAGE_SET_DIGEST_VERSION)
  assert.notEqual(readback.officialImageSetDigest,
    `sha256:${createHash("sha256").update("[]").digest("hex")}`)
  assert.equal(readback.images[0].position, 1)
  assert.equal(readback.images[0].hostClass, "EBAY_EPS")
  assert.equal(readback.images[0].urlPresent, true)
})

test("ordered Trading picture digest changes when two PictureURL values reorder", () => {
  const first = buildOfficialTradingPictureReadbackV1([main, oldSecondary])
  const reversed = buildOfficialTradingPictureReadbackV1(
    [oldSecondary, main])
  assert.notEqual(first.officialImageSetDigest,
    reversed.officialImageSetDigest)
  assert.equal(first.imageDigestInputCount, 2)
  assert.equal(reversed.imageDigestInputCount, 2)
})

test("zero Trading PictureURL values produce only the canonical empty-array digest", () => {
  const readback = buildOfficialTradingPictureReadbackV1([])
  assert.equal(readback.officialPictureCount, 0)
  assert.equal(readback.imageDigestInputCount, 0)
  assert.equal(readback.officialImageSetDigest,
    `sha256:${createHash("sha256").update("[]").digest("hex")}`)
})

test("GalleryURL alone never masquerades as the Trading PictureURL set", async () => {
  const previousUser = process.env.EBAY_DRAFT_ONLY_PRODUCTION_EXPECTED_USER_ID
  const previousCredential =
    process.env.EBAY_DRAFT_ONLY_PRODUCTION_EXPECTED_CREDENTIAL_FINGERPRINT
  const previousAccount =
  ...
  const fingerprint = createHash("sha256").update("PRODUCTION:seller")
    .digest("hex")
  const xml = `<?xml version="1.0"?><GetItemResponse><Ack>Success</Ack><Item><ItemID>${itemId}</ItemID><Seller><UserID>seller</UserID></Seller><SellingStatus><ListingStatus>Active</ListingStatus></SellingStatus><SKU>SKU-1</SKU><ListingType>FixedPriceItem</ListingType><PictureDetails><PictureSource>EPS</PictureSource><GalleryURL>${main}</GalleryURL></PictureDetails></Item></GetItemResponse>`
  try {
    const snapshot = await readOfficialActiveListingImageSnapshotV1({
      accessToken: "token", itemId, expectedSku: "SKU-1",
      accountKey: `seller:${fingerprint}`,
    ...
    assert.deepEqual(snapshot.pictureUrls, [])
    assert.equal(snapshot.galleryUrl, main)
    assert.equal(snapshot.tradingPictureReadback.imageDigestInputCount, 0)
  } finally {
    const restore = (key, value) => {
      if (value === undefined) delete process.env[key]
      else process.env[key] = value
...
test("Browse and manifest images cannot enter the Trading official digest", async () => {
  const phaseBSource = readFileSync(new URL(
    "./ebay-mayel-visual-phase-b-server-v1.ts", import.meta.url), "utf8")
  assert.match(phaseBSource,
    /context\.official\?\.tradingPictureReadback\s*\?\?\s*null/)
  const official = buildOfficialTradingPictureReadbackV1([main])
  const manifestProposal = buildOfficialTradingPictureReadbackV1([main, mayel])
  assert.equal(official.imageDigestInputCount, 1)
  assert.equal(official.manifestImageIncludedInDigest, false)
  assert.notEqual(official.officialImageSetDigest,
    manifestProposal.officialImageSetDigest)
})

test("official Browse fallback proves exact owned active Item and images with read-only calls", async () => {
  const previous = {
  ...
  process.env.EBAY_CLIENT_ID = "browse-client"
  process.env.EBAY_CLIENT_SECRET = "browse-secret"
  process.env.EBAY_DRAFT_ONLY_PRODUCTION_EXPECTED_USER_ID = "seller"
  delete process.env.EBAY_DRAFT_ONLY_PRODUCTION_EXPECTED_CREDENTIAL_FINGERPRINT
  delete process.env.EBAY_DRAFT_ONLY_PRODUCTION_EXPECTED_ACCOUNT_FINGERPRINT
  const fingerprint = createHash("sha256").update("PRODUCTION:seller")
  ...
  const fetchImpl = async (url, init = {}) => {
    ...
    if (href.endsWith("/identity/v1/oauth2/token")) {
      return Response.json({ access_token: "browse-token", expires_in: 7_200 })
    }
    if (href.includes("/buy/browse/v1/item/get_item_by_legacy_id")) {
      return Response.json({ legacyItemId: itemId,
        seller: { username: "seller" }, buyingOptions: ["FIXED_PRICE"],
        image: { imageUrl: main },
        additionalImages: [{ imageUrl: oldSecondary }] })
    }
    throw new Error("UNEXPECTED_REQUEST")
  }
  globalThis.fetch = fetchImpl
  try {
    const snapshot = await readOfficialActiveListingBrowseSnapshotV1({
      itemId, accountKey: `seller:${fingerprint}`,
      fetchImpl,
    })
    assert.equal(snapshot.itemId, itemId)
    assert.equal(snapshot.ebaySku, "")
    assert.equal(snapshot.sourceAuthority,
      "EBAY_BROWSE_GET_ITEM_BY_LEGACY_ID_V1")
    assert.deepEqual(snapshot.pictureUrls, [main, oldSecondary])
    assert.equal(snapshot.tradingPictureReadback, null)
    assert.deepEqual(calls.map((call) => call.method), ["POST", "GET"])
    assert.equal(calls.some((call) => call.method !== "GET"
      && !call.href.endsWith("/identity/v1/oauth2/token")), false)
  } finally {
...
test("Phase B surface is read-only on GET, legacy per-listing owner dispatch is disabled, and Trading is explicitly gated", () => {
  ...
  assert.match(route, /MANAGEMENT_READBACK_V1/)
  assert.match(route, /validation\.authenticationMode === "service_role"/)
  assert.match(route, /safeToExecuteVisualChange: phaseB\.safeToExecuteVisualChange/)
  assert.match(route, /safeRebaseAvailable: phaseB\.safeRebaseAvailable/)
  assert.match(route, /tradingReadFailureClass: phaseB\.tradingReadFailureClass/)
  assert.match(route, /applicationStatus: phaseB\.applicationStatus/)
  assert.match(route, /const tasks = await Promise\.all\(workstation\.tasks\.map/)
  assert.doesNotMatch(route,
    /const tasks = ownerView \? await Promise\.all\(workstation\.tasks\.map/)
  ...
  assert.match(server, /OFFICIAL_TRADING_GET_ITEM_ACTIVE_OWNED_FIXED_PRICE/)
  assert.match(server, /EBAY_BROWSE_GET_ITEM_BY_LEGACY_ID_V1/)
  assert.match(server, /endsWith\("_EBAY_ERROR_518"\)/)
  assert.match(server, /"TRADING_API_RATE_LIMIT"/)
  assert.match(server, /"WAITING_FOR_EBAY"/)
  assert.match(server, /eBay Trading alcanzó temporalmente su límite de llamadas/)
  assert.match(server, /NO_PUBLISHED_INVENTORY_OFFER_LINKAGE/)
  ...
  assert.match(ui, /Modelo de gestión:[\s\S]{0,180}"TRADING_MANAGED" \? "Trading"/)
  assert.match(ui, /Galería de Mayel:[\s\S]{0,100}\? "lista"/)
  assert.match(ui, /Aplicación en eBay:/)
  assert.match(ui, /Mayel decide y aprueba los cambios visuales dentro de su delegación/)
  assert.doesNotMatch(ui,
    /Mayel prepara y aprueba los recursos\. La decisión final pertenece al owner\./)
})
