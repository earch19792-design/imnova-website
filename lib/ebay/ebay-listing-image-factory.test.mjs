...
test("white Calypso-style enamel stays intact while its opaque source frame is removed", async () => {
  ...
  const header = await sharp(pack.output)
    .extract({ left: 990, top: 340, width: 520, height: 100 })
    .raw().toBuffer({ resolveWithObject: true })
  const darkColumns = Array.from({ length: header.info.width }, (_, x) => {
    for (let y = 0; y < header.info.height; y += 1) {
      const offset = (y * header.info.width + x) * header.info.channels
      if (header.data[offset] < 80 && header.data[offset + 1] < 80 &&
        header.data[offset + 2] < 80) return true
    }
    return false
  })
  const glyphRuns = []
  for (let x = 0; x < darkColumns.length;) {
    if (!darkColumns[x]) {
      x += 1
      continue
    }
    const start = x
    while (x < darkColumns.length && darkColumns[x]) x += 1
    glyphRuns.push(x - start)
  }
  assert.ok(glyphRuns.length >= 8 && new Set(glyphRuns).size >= 3 &&
    Math.min(...glyphRuns) * 2 <= Math.max(...glyphRuns),
  "packaged DejaVu glyphs must not collapse to repeated LastResort boxes")
