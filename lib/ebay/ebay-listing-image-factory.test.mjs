...
test("visual compiler reserves a deterministic 3+3 split and never simulates single-source diversity", () => {
  const sourceIds = ["source-main", "source-side"]
  ...
  const chosen = strategy.map((position) =>
    position.authorizedSourceImageIds[0])
  assert.equal(chosen.filter((id) => id === sourceIds[0]).length, 3)
  assert.equal(chosen.filter((id) => id === sourceIds[1]).length, 3)
  assert.ok(strategy.every((position) =>
    position.authorizedSourceImageIds.length === 1))

  assert.throws(() => buildSellerOsEbayVisualStrategyV2(
    validateListingImageFactoryInput({
      ...input(),
      authorizedSourceImageIds: ["source-main"],
      authorizedSourceCapabilities: [{
        id: "source-main",
        nativeWidth: 1600,
        nativeHeight: 1600,
        effectiveWidth: 1600,
