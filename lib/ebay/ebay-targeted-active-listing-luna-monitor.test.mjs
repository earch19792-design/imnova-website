...
test("the cron remains secret-authenticated, feature-gated and boundary-scoped", () => {
  ...
  assert.match(routeSource, /getSellerOsStockGuardRuntimeBoundary\(\)/)
  assert.match(routeSource, /authorizedStockGuardRuntime/)
  assert.match(routeSource, /runtimeBoundary\.dedicatedPreprodAllowed/)
...
test("non-production bearer returns 401 before its StockGuard worker; production requires its separate service identity", () => {
  const authorizationGate = routeSource.indexOf(
    "if (!commercialPreviewCronAuthorized(req))")
  const runtimeGate = routeSource.indexOf(
    "getSellerOsStockGuardRuntimeBoundary()", authorizationGate)
  const worker = routeSource.indexOf(
    "createSellerAutomationRun(supabase")
  // The first pure environment classification selects production service auth;
  // it is not a StockGuard evaluation and cannot start the PREPROD worker.
  const productionAdmission = routeSource.indexOf("if (!await authorizeProductionPopulationV1(req))")
  const productionExecution = routeSource.indexOf("return NextResponse.json(await runProductionCurrentLivePopulationV1")
  assert.ok(productionAdmission > 0 && productionExecution > productionAdmission)
  assert.ok(productionExecution < authorizationGate)
  assert.match(routeSource.slice(productionAdmission, productionExecution), /status: 403/)

  assert.ok(authorizationGate > 0)
  assert.ok(runtimeGate > authorizationGate)
  assert.ok(worker > runtimeGate)
  assert.match(routeSource.slice(authorizationGate, runtimeGate),
    /CRON_UNAUTHORIZED[\s\S]*status: 401/)
})

test("valid dedicated-preprod runtime reaches the existing worker path", () => {
  const runtimeGate = routeSource.indexOf(
  ...
  assert.ok(worker > runtimeGate)
  assert.match(routeSource.slice(runtimeGate, worker),
    /status: "disabled"/)
  assert.doesNotMatch(routeSource.slice(worker),
