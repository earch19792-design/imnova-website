...
test("preprod operator menu uses verified capability without widening admin routes", () => {
  const session = readFileSync(new URL("../../app/api/admin/session/route.ts", import.meta.url), "utf8")
  const home = readFileSync(new URL("../../app/admin/page.tsx", import.meta.url), "utf8")
  const middleware = readFileSync(new URL("../../middleware.ts", import.meta.url), "utf8")
  assert.ok(session.indexOf('validation.authenticationMode !== "seller_os_user"') < session.indexOf('revenueEngineAvailable:'))
  assert.match(session, /revenueEngineAvailable: getEbayProRuntimeBoundary\([\s\S]*?runtime === "seller_os_dedicated_preprod"/)
  assert.match(home, /capabilities\.revenueEngineAvailable === true/)
  assert.match(home, /<MayelRevenueEngine owner=\{false\}/)
  assert.match(home, /legacyDetailsOpen && <RemoteLiveOptimizationOperator/)
  assert.match(middleware, /verification === "REMOTE_OPERATOR" && pathname !== "\/admin"/)
})
