...
test("WORKER_CONTROL_GATE_ONLY_CANNOT_CLAIM_PASS", () => {
  ...
  assert.match(extensionWorker,
    /mayelResearchWorker=auto&browserWorkerControl=1/)
  assert.doesNotMatch(runner,
    /const gateOnly = browserWorkerControl && !planId && !autonomous/)
})

test("WORKER_CONTROL_EXPLICIT_PLAN_IS_SINGLE_PLAN_WITHOUT_QUEUE_FALLBACK", () => {
  assert.match(runner,
    /const maximumPlans = batchControlMode \? 4[\s\S]*?: browserWorkerControl && planId \? 1[\s\S]*?: autonomous \? 4 : 1/)
  assert.match(runner, /\.\.\.\(planId \? \{ planId \} : \{\}\)/)
  const loop = runner.slice(runner.indexOf("const maximumPlans"),
    runner.indexOf("if (browserWorkerControl) {", runner.indexOf("const maximumPlans")))
  assert.equal((loop.match(/CLAIM_AUTONOMOUS_RESEARCH_PLAN/g) ?? []).length, 1)
  assert.doesNotMatch(loop, /planId:\s*(?:null|undefined)/)
  assert.match(loop,
    /if \(batchControlMode\)[\s\S]*nextAuthorizedBatchPlanId\(workerId, leaderSessionId\)/)
})
