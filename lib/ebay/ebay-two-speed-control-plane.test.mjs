...
test("bounded batch discovery can suppress external alerts without changing scan persistence", () => {
  const route = read("app/api/admin/ebay/luna-opportunity-queue/route.ts")
  const cronRoute = read("app/api/cron/ebay-luna-opportunity-scan/route.ts")
  ...
  assert.match(route, /action === "process_next"/)
  assert.match(route, /suppressExternalAlertDelivery: body\.suppressExternalAlerts === true/)
  assert.match(scanner, /suppressExternalAlertDelivery\?: boolean/)
  assert.match(scanner, /options\.suppressExternalAlertDelivery !== true &&[\s\S]*getSellerWhatsAppGatewayConfiguration\(\)\.deliveryAttemptAllowed/)
  assert.match(scanner, /await processClaimedCandidate\(supabase, run, task\)/)
  assert.match(scanner, /await completeSellerScanTask\(supabase, task\.id, workerId, processed\.result\)/)
  assert.match(cronRoute, /searchParams\.get\("suppressExternalAlerts"\) === "true"/)
  assert.match(cronRoute, /suppressExternalAlertDelivery,/)
  assert.match(workflow, /run_luna_discovery_batch:/)
  assert.match(workflow, /inputs\.run_luna_discovery_batch == true/)
  assert.match(workflow, /ebay-luna-opportunity-scan\?suppressExternalAlerts=true/)
  assert.match(workflow, /processed_total.*50/)
})

test("lightweight discovery has one family query and deep detail defaults to five with bounded concurrency", () => {
  ...
  assert.match(gateway, /DEFAULT_DETAIL_SAMPLE_LIMIT = 5/)
  assert.match(gateway, /DETAIL_CONCURRENCY = 2/)
