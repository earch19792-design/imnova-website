...
function fixture() {
  ...
  const opportunity = { id: "opportunity", candidate_key: "candidate",
    ...
    supplier_sku: "supplier", assessment: { productTruth: {
      ...
      requiredItemSpecificsReady: true, requiredItemSpecificsCount: 3,
      requiredItemSpecificsTruth: { resolutions: {
        Brand: { value: "Unbranded" }, Style: { value: "Cuff" },
        Type: { value: "Bracelet" } } } } } }
...
test("CURRENT policy requires the official category specifics, not unrelated optional fields", () => {
  const input = fixture()
  input.listingPackage.package_data.aspects = { Brand: "Unbranded" }
  input.opportunity.assessment.canonicalMarketplaceReadinessV1
    .requiredItemSpecificsCount = 1
  input.opportunity.assessment.canonicalMarketplaceReadinessV1
    .requiredItemSpecificsTruth = { resolutions: {
      Brand: { value: "Unbranded" },
    } }
  assert.equal(evaluate(input).pass, true)
  delete input.listingPackage.package_data.aspects.Brand
  assert.ok(evaluate(input).blockers.includes(
    "CURRENT_CATEGORY_AND_REQUIRED_SPECIFICS_REQUIRED"))
})
