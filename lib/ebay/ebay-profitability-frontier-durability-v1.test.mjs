...
const frontierPath = new URL(
  ...
  import.meta.url)
const captureIdentityPath = new URL(
...
const captureIdentityMigration = readFileSync(captureIdentityPath, "utf8")
...
function captureFrontierId(frontierDigest) {
  return "profitability-frontier-v1:sha256:" + createHash("sha256").update([
    "SELLER_OS_PROFITABILITY_FRONTIER_CAPTURE_ID_V1",
    `market-family-v1:sha256:${"1".repeat(64)}`,
    "9220832329952",
    "48809643376864",
    `sha256:${"2".repeat(64)}`,
    `sha256:${"3".repeat(64)}`,
...
test("capture identity makes exact replays idempotent and distinct observations append-only", () => {
  const firstDigest = `sha256:${"a".repeat(64)}`
  const secondDigest = `sha256:${"b".repeat(64)}`
  assert.equal(captureFrontierId(firstDigest), captureFrontierId(firstDigest))
  assert.notEqual(captureFrontierId(firstDigest), captureFrontierId(secondDigest))
  assert.match(captureIdentityMigration,
    /SELLER_OS_PROFITABILITY_FRONTIER_CAPTURE_ID_V1/)
  assert.match(captureIdentityMigration,
    /shipping_value, frontier_digest, contract_version/)
  assert.match(captureIdentityMigration, /IDEMPOTENT_SUCCESS/)
  assert.match(captureIdentityMigration,
    /SELLER_OS_PROFITABILITY_FRONTIER_REPLAY_CONFLICT/)
  assert.match(captureIdentityMigration, /pg_advisory_xact_lock/)
  assert.match(captureIdentityMigration,
    /drop constraint seller_os_profitability_frontier_snapshot_unique/)
  assert.doesNotMatch(captureIdentityMigration,
    /\b(?:update|delete|truncate)\s+public\.seller_os_profitability_frontier_snapshots\b/i)
  assert.equal(captureIdentityMigration.match(
    /create or replace function public\.put_seller_os_profitability_frontier_v1/g
  )?.length, 1)
})
