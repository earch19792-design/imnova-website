...
test("Quick Pick continuation is one-shot, server-side and reuses the shared resolver", async () => {
  ...
  assert.match(route, /card\.fullLunaBrandEvidenceReviewPending/)
  assert.match(route, /!card\.automaticResolutionContractCurrent/)
  assert.match(route, /card\.automaticResolutionUpgradeHasPriorResidual/)
