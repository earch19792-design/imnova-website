...
test("completed specifics can reenter only to bind an exact CURRENT condition policy", () => {
  const identity = { lunaProductId: "100", lunaVariantId: "200",
    supplierSku: "SKU-1" }
  const policy = { id: "55555555-5555-4555-8555-555555555555",
    marketplaceAccountKey: "account",
    evidenceDigest: lunaNewMerchandisePolicyDigestV1(),
    certifiedAt: "2026-09-13T00:00:00.000Z",
    authorizationReferenceDigest: `sha256:${"1".repeat(64)}` }
  const exactApplication = buildOwnerSupplierPolicyApplicationV1({
    policy, ...identity, exactSupplierLineageCertified: true,
    productIdentityExact: true, appliedAt: "2026-09-13T00:01:00.000Z",
  })
  const base = { trigger: "DEPENDENCY_RECOVERY",
    blockedByCondition: true, currentPolicy: policy, ...identity }
  assert.equal(currentConditionPolicyDependencyRecoveryRequiredV1({
    ...base, currentApplication: null,
  }), true)
  assert.equal(currentConditionPolicyDependencyRecoveryRequiredV1({
    ...base, currentApplication: exactApplication,
  }), false)
  assert.equal(currentConditionPolicyDependencyRecoveryRequiredV1({
    ...base, trigger: "IMMEDIATE", currentApplication: null,
  }), false)
  assert.equal(currentConditionPolicyDependencyRecoveryRequiredV1({
    ...base, blockedByCondition: false, currentApplication: null,
  }), false)
  assert.equal(currentConditionPolicyDependencyRecoveryRequiredV1({
    ...base, currentApplication: { ...exactApplication,
      lunaVariantId: "different" },
  }), true)
})
