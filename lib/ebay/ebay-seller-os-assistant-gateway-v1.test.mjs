import assert from "node:assert/strict"
import { readFileSync } from "node:fs"
import test from "node:test"

import { buildAutomationHealthMetricsV1, buildCommercialEvidenceScoreV1,
  buildPortfolioIntelligenceV1, buildProactiveExceptionQueueV1,
  calculateTimeToFirstSaleV1, evaluateOpportunityRadarV1,
  evaluateReplaceKillIntelligenceV1, planAutomationWorkV1, rankReferenceCandidatesV1,
  assessLearningTransferV1 } from "./ebay-seller-os-portfolio-intelligence-v1.ts"
import { executeSellerOsAssistantToolV1 } from
  "./ebay-seller-os-assistant-gateway-v1.ts"

const route = readFileSync(new URL("../../app/api/admin/ebay/assistant/mcp/route.ts",
  import.meta.url), "utf8")
const assistantSource = readFileSync(new URL("./ebay-seller-os-assistant-gateway-v1.ts",
  import.meta.url), "utf8")
const mcpServerSource = readFileSync(new URL("./ebay-seller-os-mcp-server-v1.ts",
  import.meta.url), "utf8")

function observation(value, availability = "AVAILABLE") {
  return { value, availability, completeness: value === null ? "UNPROVEN" : "COMPLETE",
    source: { system: "TEST", operation: "READ", evidenceReference: "ref" },
    capturedAt: "2026-08-12T00:00:00Z", marketplace: { marketplaceId: "EBAY_US" },
    identity: { itemId: "123456789012" }, grain: "LISTING", reportingWindow: null,
    freshness: { status: "FRESH", ageSeconds: 0, maximumAgeSeconds: 60 }, unit: null,
    limitationCode: null, explicitAuthoritativeZero: value === 0 }
}

function listing(itemId, overrides = {}) {
  return { key: `listing:${itemId}`, identity: { itemId, title: `Listing ${itemId}`, sku: "SKU",
    customLabel: "CUSTOM-LABEL", listedQuantity: 4, listedPrice: 24.99, currency: "USD",
    source: "EBAY_TRADING_GET_MY_EBAY_SELLING", primaryImageUrl: null,
    lastObservedAt: "2026-08-12T00:00:00Z",
    marketplaceCertification: { status: "US_CERTIFIED" } },
  discovery: { livePresence: { status: "LIVE_ACTIVE",
    source: "EBAY_TRADING_GET_MY_EBAY_SELLING",
    observedAt: "2026-08-12T00:00:00Z" } },
  metrics: { impressions: observation(100), ebay_views: observation(5), views: observation(5),
    ctr_calculated: observation(5), ctr_reported: observation(5), ctr: observation(5),
    transactions: observation(0), orders: observation(null, "UNAVAILABLE") },
  stock: { state: "STOCK_UNKNOWN", sourceContractStatus: "UNPROVEN", supplierProductId: null,
    supplierVariantId: null, supplierSku: null, quantity: observation(null, "UNKNOWN"),
    freshness: { status: "UNKNOWN", ageSeconds: null, maximumAgeSeconds: 60 }, limitationCode: "UNPROVEN" },
  composition: { bundleCapacity: observation(null, "UNKNOWN") },
  experiment: { status: "UNAVAILABLE", lifecycleState: null }, blockers: [],
  dataQualityIssues: [], evidenceReferences: [], alertCandidateKeys: [], ...overrides }
}

function decision(itemId, overrides = {}) {
  return { listingKey: `listing:${itemId}`, classification: "CTR", priority: "HIGH",
    evidenceStatus: "AVAILABLE", reasonCodes: ["LOW_CTR_WITH_SUFFICIENT_IMPRESSIONS"],
    recommendedAction: "IMPROVE_CTR", actionBlockedByInsufficientEvidence: false,
    experimentRunning: false, variableFrozen: false, protectionState: "NONE",
    experimentOperationalState: "INACTIVE", frozenVariables: [],
    nextReviewEvidenceRemaining: null, externalSignalCount: null,
    nextReviewCondition: null, nextReviewAt: null, actionExecutionAllowed: false, ...overrides }
}

function monitor(listings = [listing("123456789012")], decisions = [decision("123456789012")]) {
  const liveCount = new Set(listings.filter((row) =>
    row.discovery.livePresence.status === "LIVE_ACTIVE")
    .map((row) => row.identity.itemId)).size
  return { contractVersion: "MONITOR", generatedAt: "2026-08-12T00:00:00Z", listings,
    alertCandidates: [], liveCertification: { status: "CERTIFIED", marketplaceId: "EBAY_US",
      account: { bindingConfigured: true },
      discovery: { status: "AVAILABLE", coverage: "COMPLETE" } },
    productCaseOperatingState: { status: "PAUSED" },
    learning: { status: "UNPROVEN", categoryAdjustments: [], limitationCode: "NO_HISTORY" },
    backend: { decisions, guidanceVsSellerOs: [], listingQualityReport: { status: "UNAVAILABLE_NO_CURRENT_REPORT",
      recommendations: [], limitationCode: "NO_CURRENT_REPORT" }, orders: { status: "AUTH_PENDING",
      fulfillmentStatuses: ["AUTH_PENDING"] }, trafficScopes: { accountTraffic: { status: "UNPROVEN",
        scopeType: "ACCOUNT_TRAFFIC_SCOPE", observedAt: null, sourceUpdatedAt: null,
        upstreamSnapshotAcquisitionCount: 0 },
      currentLivePortfolio: { activeListings: liveCount } },
    kpis: { activeListings: { status: "AVAILABLE", value: liveCount },
      impressions: { status: "AVAILABLE", value: liveCount * 100 },
      ebayViews: { status: "AVAILABLE", value: liveCount * 5 },
      averageCtr: { status: "AVAILABLE", value: 5 },
      quantitySold: { status: "AVAILABLE", value: 0 } },
    capabilities: { registry: { status: "PARTIAL_CERTIFIED", currentLiveCount: liveCount,
      humanReviewCount: 3,
      matchedCount: 24, coveragePercent: 88.89, limitationCodes: ["HUMAN_REVIEW"] },
      inventory: { inventoryItemsResource: "EBAY_REJECTED_25709_UNRESOLVED" } },
    operationalHealth: { runningExperiments: { status: "AVAILABLE" } } } }
}

test("thirteen Assistant tools are private read-only non-destructive closed-world contracts", () => {
  assert.equal(new Set(assistantSource.match(/seller_os_get_[a-z_]+/g)).size, 13)
  assert.match(assistantSource, /readOnlyHint: true/)
  assert.match(assistantSource, /destructiveHint: false/)
  assert.match(assistantSource, /openWorldHint: false/)
  assert.match(assistantSource, /sideEffects: false/)
  assert.match(route, /handleSellerOsMcpRequestV1/)
  assert.match(mcpServerSource, /validateAdminApiRequest\(req\)/)
  assert.doesNotMatch(route + mcpServerSource,
    /sendWhatsApp|apply_ebay_registry_repair|process\.env\.[A-Z_]+\s*=/)
})

test("Assistant commercial and listing reads expose bounded sales and monitor coverage", () => {
  assert.match(assistantSource, /monitorCoverage: monitor\.backend\.monitorCoverage/)
  assert.match(assistantSource, /monitoredItemIdsTruncated:/)
  assert.match(assistantSource, /monitoredItemIds\.length > 25/)
  assert.match(assistantSource, /orderSourceHealth: monitor\.backend\.orderSourceHealth/)
  assert.match(assistantSource, /entries: cap\(assistantRecentSales\(monitor\)\.entries, 10\)/)
  assert.match(assistantSource, /assistantRecentSalesForItem\(monitor, itemId\)/)
  assert.match(assistantSource, /authoritativeItemCount \? entries\.length : null/)
  assert.match(assistantSource, /PARTIAL_RECENT_WINDOW/)
  assert.match(assistantSource, /sale\.itemIds\.includes\(itemId\)/)
  assert.match(assistantSource, /buyerPiiIncluded: false/)
})

test("commercial context exposes the bounded official Orders read without adding a tool", () => {
  assert.match(assistantSource, /officialOrders: assistantOfficialOrders\(monitor\)/)
  assert.match(assistantSource, /createUnavailableSellerOsOfficialOrdersReadV1/)
  assert.doesNotMatch(assistantSource, /seller_os_get_official_orders/)
  assert.doesNotMatch(assistantSource, /buyerEmail|shippingAddress|accessToken|refreshToken/i)
})

test("commercial context retains official Orders evidence and fails closed when absent", () => {
  const current = monitor()
  const unavailable = executeSellerOsAssistantToolV1({
    toolName: "seller_os_get_commercial_context", arguments: {}, monitor: current,
  })
  assert.equal(unavailable.officialOrders.sourceStatus, "UNAVAILABLE")
  assert.equal(unavailable.officialOrders.officialOrderCount, null)

  current.officialOrders = {
    contractVersion: "SELLER_OS_OFFICIAL_ORDERS_READ_V1",
    sourceStatus: "AVAILABLE",
    officialOrderCount: 1,
    officialLineItemQuantity: 2,
    orders: [{ orderId: "ORDER-1", lineItems: [{ lineItemId: "LINE-1",
      itemId: "123456789012", sku: "SKU", quantity: 2 }] }],
    safety: { buyerPiiIncluded: false, marketplaceWrites: 0, databaseWrites: 0 },
  }
  const available = executeSellerOsAssistantToolV1({
    toolName: "seller_os_get_commercial_context", arguments: {}, monitor: current,
  })
  assert.equal(available.officialOrders.sourceStatus, "AVAILABLE")
  assert.equal(available.officialOrders.orders[0].lineItems[0].itemId,
    "123456789012")
  assert.equal(available.officialOrders.safety.buyerPiiIncluded, false)
  assert.equal(available.officialOrders.safety.marketplaceWrites, 0)
})

test("listing sales preserve source status and never turn partial absence into zero", () => {
  const current = monitor()
  current.backend.recentSales = {
    contractVersion: "RECENT_SALES_FEED_V1",
    status: "PARTIAL",
    resultCount: null,
    entries: [],
    maximumEntries: 10,
    truncated: true,
    limitationCodes: ["RECENT_WINDOW_TRUNCATED"],
    source: "PERSISTED_OFFICIAL_EBAY_ORDER_EVENTS",
    buyerPiiIncluded: false,
  }
  const partial = executeSellerOsAssistantToolV1({
    toolName: "seller_os_get_listing_intelligence",
    arguments: { itemId: "123456789012" },
    monitor: current,
  })
  assert.equal(partial.recentSales.status, "PARTIAL")
  assert.equal(partial.recentSales.resultCount, null)
  assert.equal(partial.recentSales.itemSearchCoverage, "PARTIAL_RECENT_WINDOW")

  current.backend.recentSales = {
    ...current.backend.recentSales,
    status: "AVAILABLE",
    truncated: false,
    limitationCodes: [],
  }
  const authoritative = executeSellerOsAssistantToolV1({
    toolName: "seller_os_get_listing_intelligence",
    arguments: { itemId: "123456789012" },
    monitor: current,
  })
  assert.equal(authoritative.recentSales.resultCount, 0)
})

test("listing intelligence projects bounded authoritative current-live facts additively", () => {
  const current = listing("123456789012", { identity: {
    ...listing("123456789012").identity,
    sku: "SKU-EXACT", customLabel: "CUSTOM-EXACT", listedQuantity: 0,
    listedPrice: 19.95, currency: "USD",
  } })
  const result = executeSellerOsAssistantToolV1({
    toolName: "seller_os_get_listing_intelligence",
    arguments: { itemId: "123456789012" },
    monitor: monitor([current], [decision("123456789012")]),
  })
  assert.deepEqual(result.currentLiveFacts, {
    itemId: "123456789012", sku: "SKU-EXACT", customLabel: "CUSTOM-EXACT",
    title: "Listing 123456789012", quantity: 0, price: 19.95, currency: "USD",
    liveStatus: "LIVE_ACTIVE", source: "EBAY_TRADING_GET_MY_EBAY_SELLING",
    observedAt: "2026-08-12T00:00:00Z",
  })
  assert.equal(result.identity.itemId, "123456789012")
  assert.equal(result.safety.databaseWrites, 0)
  assert.equal(result.safety.marketplaceWrites, 0)
  assert.equal(result.safety.rawPayloadIncluded, false)
  assert.equal(result.safety.credentialsIncluded, false)
  assert.doesNotMatch(JSON.stringify(result), /raw_payload|access_token|refresh_token|client_secret/i)
})

test("listing intelligence fails closed for non-live and unaccepted-source items", () => {
  for (const livePresence of [
    { status: "STORED_ONLY_NOT_IN_CURRENT_LIVE_ENUMERATION",
      source: "EBAY_TRADING_GET_MY_EBAY_SELLING",
      observedAt: "2026-08-12T00:00:00Z" },
    { status: "LIVE_ACTIVE", source: "STALE_HISTORICAL_ROW",
      observedAt: "2026-08-12T00:00:00Z" },
  ]) {
    const current = listing("123456789012", { discovery: { livePresence } })
    const result = executeSellerOsAssistantToolV1({
      toolName: "seller_os_get_listing_intelligence",
      arguments: { itemId: "123456789012" },
      monitor: monitor([current], [decision("123456789012")]),
    })
    assert.equal(result.status, "NOT_FOUND")
    assert.equal(result.reason, "NOT_CURRENT_LIVE")
    assert.equal(result.conflict, "CONFLICT")
    assert.equal(result.currentLiveFacts, null)
    assert.equal(result.safety.databaseWrites, 0)
    assert.equal(result.safety.marketplaceWrites, 0)
  }
})

test("listing facts reuse the existing tool and preserve its closed-world contract", () => {
  assert.match(assistantSource, /seller_os_get_listing_intelligence/)
  assert.match(assistantSource, /currentLiveFacts/)
  assert.match(assistantSource, /EBAY_TRADING_GET_MY_EBAY_SELLING/)
  assert.doesNotMatch(assistantSource, /raw_payload|createSellerOs.*Tool|databaseWrites:\s*[1-9]/i)
  assert.equal(new Set(assistantSource.match(/seller_os_get_[a-z_]+/g)).size, 13)
})

test("exception queue is deterministic, deduped and protects active experiments", () => {
  const protectedDecision = decision("123456789012", { experimentRunning: true,
    protectionState: "DO_NOT_TOUCH", experimentOperationalState: "RUNNING" })
  const input = monitor([listing("123456789012")], [protectedDecision])
  const first = buildProactiveExceptionQueueV1({ monitor: input })
  assert.deepEqual(first, buildProactiveExceptionQueueV1({ monitor: input }))
  assert.equal(first.filter((row) => row.entityKey === "123456789012").length, 1)
  assert.equal(first.find((row) => row.entityKey === "123456789012").classification, "DO_NOT_TOUCH")
  assert.equal(first.find((row) => row.entityKey === "123456789012").recommendedAction,
    "WAIT_ACTIVE_EXPERIMENT")
})

test("hard operational override surfaces while ordinary replacement waits for experiment", () => {
  const current = listing("123456789012", { stock: { ...listing("123456789012").stock,
    state: "OUT_OF_STOCK_SIGNAL" } })
  const protectedDecision = decision("123456789012", { experimentRunning: true,
    protectionState: "DO_NOT_TOUCH", reasonCodes: ["HARD_OVERRIDE_REQUIRES_HUMAN_REVIEW"] })
  assert.equal(buildProactiveExceptionQueueV1({ monitor: monitor([current], [protectedDecision]) })
    .find((row) => row.entityKey === "123456789012").classification, "CRITICAL_OPERATIONAL")
  assert.equal(evaluateReplaceKillIntelligenceV1({ listing: current,
    decision: protectedDecision, alternativeOpportunity: { opportunityId: "o", familyLabel: "Family",
      decision: "ADVANCE", score: 90, confidence: "HIGH", activeCompetitionCount: 2,
      keywordEvidenceScore: 80, comparableConfidence: 90, soldHistoryStatus: "UNAVAILABLE",
      observedAt: "2026-08-12T00:00:00Z" } }).status, "WAIT")
})

test("radar never fabricates sold momentum and scheduler is bounded priority-aware", () => {
  const radar = evaluateOpportunityRadarV1({ current: { opportunityId: "o", observedAt: "b",
    activeCompetitionCount: 5, medianActivePrice: 20, keywordEvidenceScore: 80,
    comparableClusterCount: 4, supplierMatchStatus: "UNPROVEN", soldEvidenceStatus: "UNAVAILABLE" },
  previous: { opportunityId: "o", observedAt: "a", activeCompetitionCount: 8,
    medianActivePrice: 20, keywordEvidenceScore: 60, comparableClusterCount: 2,
    supplierMatchStatus: "UNPROVEN", soldEvidenceStatus: "UNAVAILABLE" } })
  assert.equal(radar.state, "STRENGTHENING")
  assert.equal(radar.soldMomentumClaimed, false)
  const plan = planAutomationWorkV1({ entities: Array.from({ length: 1_200 }, (_, index) =>
    ({ entityKey: String(index), critical: index === 0, experimentActive: false,
      evidenceStale: index === 1, newOpportunity: false, healthy: index > 1 })),
  policy: { maximumBatchSize: 100, maximumConcurrency: 5, paginationBudget: 10,
    classWeights: { CRITICAL_WATCH: 100, ACTIVE_EXPERIMENT: 80, STALE_EVIDENCE: 90,
      NEW_OPPORTUNITY_RESEARCH: 70, NORMAL_ACTIVE_LISTING: 50, HEALTHY_LOW_PRIORITY: 10 } } })
  assert.equal(plan.work.length, 100)
  assert.equal(plan.work[0].workClass, "CRITICAL_WATCH")
  assert.equal(plan.uncontrolledFanout, false)
})

test("evidence score is transparent and never claims literal probability", () => {
  const score = buildCommercialEvidenceScoreV1({ components: [
    { name: "MARKET", value: 80, weight: 2, confidence: "HIGH", reason: "OBSERVED" },
    { name: "SOLD", value: null, weight: 3, confidence: "UNPROVEN", reason: "UNAVAILABLE" },
  ] })
  assert.equal(score.score, 80)
  assert.equal(score.literalSaleProbability, false)
  assert.equal(score.components.length, 2)
})

test("time-to-first-sale, learning transfer and reference strategy fail closed", () => {
  assert.equal(calculateTimeToFirstSaleV1({ listingStartedAt: "2026-01-01T00:00:00Z",
    firstAuthoritativeSaleAt: null, source: null }).status, "UNPROVEN")
  assert.equal(assessLearningTransferV1({ comparableCompletedExperiments: 1,
    familyIdentityProven: true, categoryIdentityProven: true }).scope, "LISTING_ONLY")
  const reference = rankReferenceCandidatesV1({ candidates: [{ itemId: "1", comparability: 95,
    categoryCorrect: true, packCompatible: true, formFactorCompatible: true,
    variantCompatible: true, brandModelContamination: false, dataQualityClean: true,
    marketEvidence: 90 }] })[0]
  assert.equal(reference.useAsReferenceRecommendation, "USE_AS_REFERENCE")
  assert.deepEqual(reference.copiedFields, [])
})

test("Assistant context is bounded and contains no secrets or buyer PII", () => {
  assert.match(assistantSource, /SELLER_OS_ASSISTANT_MAX_ITEMS = 100/)
  assert.match(assistantSource, /buyerPiiIncluded: false/)
  assert.match(assistantSource, /marketplaceWrites: 0/)
  assert.match(assistantSource, /stockUnknownIsRisk: false/)
  assert.doesNotMatch(assistantSource, /access_token|refresh_token|client_secret|buyerEmail|shippingAddress/i)
  assert.equal(buildPortfolioIntelligenceV1({ monitor: monitor() }).automaticPortfolioMutationAllowed, false)
  assert.equal(buildAutomationHealthMetricsV1({}).falseAlertRate.status, "NOT_ENOUGH_HISTORY")
})

test("stock tool uses the canonical current-live cohort and reports non-live evidence separately", () => {
  const first = listing("366543596425")
  const second = listing("366592919965")
  second.identity.sku = "IMN-LST-000026"
  const third = listing("366597514990")
  third.identity.sku = "IMN-LST-000026"
  const duplicate = structuredClone(first)
  duplicate.key = "stock-evidence:366543596425:duplicate"
  duplicate.identity.title = "Alternate evidence title"
  const historical = listing("365000000001")
  historical.key = "stock-evidence:365000000001:historical"
  historical.discovery.livePresence.status = "ENDED"
  const current = monitor([first, second, third, duplicate, historical],
    [decision(first.identity.itemId), decision(second.identity.itemId),
      decision(third.identity.itemId)])

  const result = executeSellerOsAssistantToolV1({
    toolName: "seller_os_get_stock_status",
    arguments: { maximumItems: 100 },
    monitor: current,
  })
  assert.equal(result.scope.listingCount, 3)
  assert.equal(result.entries.length, 3)
  assert.equal(new Set(result.entries.map((row) => row.itemId)).size, 3)
  assert.equal(result.historicalOrNonliveEvidence.count, 1)
  assert.deepEqual(result.historicalOrNonliveEvidence.itemIds,
    [historical.identity.itemId])
  assert.equal(result.duplicateItemIds.length, 1)
  assert.equal(result.duplicateItemIds[0].itemId, first.identity.itemId)
  assert.equal(result.stockUnknownIsRisk, false)
  assert.equal(result.marketplaceWrites, 0)
})

test("certified bundle linkage and OOS capacity project through existing Assistant tools", () => {
  const itemId = "366569086086"
  const current = listing(itemId, {
    stock: { ...listing(itemId).stock, state: "CERTIFIED_OOS",
      sourceContractStatus: "HEALTHY", supplierLinkageStatus: "CERTIFIED",
      supplierProductId: null, supplierVariantId: null, supplierSku: null,
      limitationCode: null },
    composition: { status: "AVAILABLE", listingType: "BUNDLE",
      components: [{ componentId: "component-1", supplierSku: "ITEM3752",
        quantityRequired: 1, evidenceReferences: [] },
      { componentId: "component-2", supplierSku: "FL-WF01-BLK",
        quantityRequired: 1, evidenceReferences: [] }],
      limitingComponentId: "component-1", sharedAllocationKnown: true,
      bundleCapacity: observation(0), limitationCode: null },
  })
  const unproven = Array.from({ length: 18 }, (_, index) =>
    listing(String(400000000000 + index)))
  const currentMonitor = monitor([current, ...unproven], [decision(itemId),
    ...unproven.map((row) => decision(row.identity.itemId))])
  const listingResult = executeSellerOsAssistantToolV1({
    toolName: "seller_os_get_listing_intelligence",
    arguments: { itemId }, monitor: currentMonitor,
  })
  assert.equal(listingResult.supplier.linkage, "CERTIFIED")
  assert.equal(listingResult.stockGuard.state, "CERTIFIED_OOS")
  assert.equal(listingResult.stockGuard.safeCapacity.value, 0)
  assert.equal(listingResult.stockGuard.safeCapacity.availability, "AVAILABLE")
  assert.equal(listingResult.stockGuard.safeCapacity.completeness, "COMPLETE")
  const stockResult = executeSellerOsAssistantToolV1({
    toolName: "seller_os_get_stock_status", arguments: {},
    monitor: currentMonitor,
  })
  const targetStock = stockResult.entries.find((row) => row.itemId === itemId)
  assert.equal(targetStock.supplierLinkage, "CERTIFIED")
  assert.equal(targetStock.state, "CERTIFIED_OOS")
  assert.equal(targetStock.safeCapacity, 0)
  const context = executeSellerOsAssistantToolV1({
    toolName: "seller_os_get_commercial_context", arguments: {},
    monitor: currentMonitor,
  })
  assert.equal(context.supplierReadiness.exactLinkedListings, 1)
  assert.equal(context.supplierReadiness.totalLiveListings, 19)
  assert.equal(context.supplierReadiness.totalLiveListings -
    context.supplierReadiness.exactLinkedListings, 18)
})

test("unproven empty Assistant readers expose null result counts while authoritative experiment zero remains zero", () => {
  const current = monitor()
  const opportunities = executeSellerOsAssistantToolV1({
    toolName: "seller_os_get_opportunity_radar", arguments: {}, monitor: current,
  })
  const quality = executeSellerOsAssistantToolV1({
    toolName: "seller_os_get_quality_guidance", arguments: {}, monitor: current,
  })
  const learning = executeSellerOsAssistantToolV1({
    toolName: "seller_os_get_learning_signals", arguments: {}, monitor: current,
  })
  const experiments = executeSellerOsAssistantToolV1({
    toolName: "seller_os_get_experiments", arguments: {}, monitor: current,
  })

  assert.equal(opportunities.entries.length, 0)
  assert.equal(opportunities.resultCount, null)
  assert.equal(quality.recommendations.length, 0)
  assert.equal(quality.resultCount, null)
  assert.equal(learning.categoryAdjustments.length, 0)
  assert.equal(learning.resultCount, null)
  assert.equal(experiments.entries.length, 0)
  assert.equal(experiments.resultCount, 0)
})

test("existing Assistant read tools expose V2 commercial intelligence fields without writes", () => {
  for (const field of ["commercialRecommendation", "keywordRecommendation", "keywordOpportunity",
    "priceOpportunity", "referenceCandidate", "useAsReferenceReadiness", "nextBestEvidence",
    "exceptionPriority", "canonicalFamily"]) assert.match(assistantSource, new RegExp(field))
  assert.match(assistantSource, /seller_os_get_opportunity_case[\s\S]*commercialRecommendation/)
  assert.match(assistantSource, /seller_os_get_opportunity_radar[\s\S]*commercialRecommendation/)
  assert.match(assistantSource, /marketplaceWrites: 0/)
  assert.doesNotMatch(assistantSource, /\.insert\(|\.update\(|\.upsert\(|\.delete\(/)

  assert.match(assistantSource, /\.find\(\(row\) => row\.entityKey === itemId\)/)
})
