...
import { authorizeProductionStockReadV1 as authorize, productionStockReadTransportV1 as transport,
...
const request=(query='capability=STOCKGUARD_EVALUATION',options={})=>new Request('https://imnova-website-z1qh.vercel.app'+path+'?'+query,
 {headers:{authorization:'Bearer '+secret,'x-seller-os-caller':caller},...options})
test('production allows exactly the five read capabilities for the authenticated monitoring caller',()=>{
 for(const capability of capabilities)assert.equal(authorize(request('capability='+capability),env).allowed,true)
})
test('all write verbs and marketplace/unknown capabilities fail closed',()=>{
 for(const method of ['POST','PUT','PATCH','DELETE','HEAD','OPTIONS'])assert.equal(authorize(request(undefined,{method}),env).allowed,false)
 for(const capability of ['publishOffer','reviseItem','endItem','inventory_write','offer_write','ads_write','UNKNOWN'])
  assert.equal(authorize(request('capability='+capability),env).allowed,false)
})
test('wrong service, wrong credential, missing credential and wrong deployment are denied',()=>{
 for(const headers of [{authorization:'Bearer '+secret,'x-seller-os-caller':'PUBLISHER'},
  ...
  assert.equal(authorize(request(undefined,{headers}),env).allowed,false)
 for(const override of [{VERCEL_ENV:'preview'},{VERCEL_PROJECT_ID:'other'},{VERCEL_TARGET_ENV:'preview'},
  {VERCEL_PROJECT_PRODUCTION_URL:'other.vercel.app'},{SUPABASE_SERVICE_ROLE_KEY:''}])
  assert.equal(authorize(request(),{...env,...override}).allowed,false)
})
test('caller controlled target, credential selector, command, account and duplicate query keys are denied',()=>{
 for(const query of ['url=https://api.ebay.com','credential=other','command=publishOffer','account=other','limit=10000',
  ...
  assert.equal(authorize(request('capability=STOCKGUARD_EVALUATION&'+query),env).allowed,false)
 assert.equal(authorize(request('capability=STOCKGUARD_EVALUATION&itemId=366643126310'),env).allowed,true)
})
test('the preprod-only monitor and cloud relay remain denied in production; cron execution is not enabled',()=>{
 for(const pathname of ['/api/admin/ebay/commercial-monitor','/api/seller-os/assistant/cloud-read-relay','/api/admin/ebay/draft-only']){
  assert.equal(authorize(new Request('https://example.test'+pathname),env).allowed,false)
...
const origin='https://vsfthqydfrdzulldbfbe.supabase.co'
...
test('transport permits only bounded reads against installed authority; replay has zero side effects',async()=>{
 const f=fixture()
 for(let n=0;n<2;n++){assert.equal(authorize(request(),env).allowed,true);await f.fetch(readUrl,{method:'GET'})}
...
test('transport rejects marketplace, RPC, alternate host/account/table, unlimited or wildcard reads',async()=>{
 for(const url of ['https://api.ebay.com/sell/inventory/v1/offer',origin+'/rest/v1/rpc/publish',
  readUrl.replace('vsfthqydfrdzulldbfbe','aaaaaaaaaaaaaaaaaaaa'),readUrl.replace('eq.account','eq.other'),
...
test('denied responses never expose secret or environment values',()=>{
 for(const value of [authorize(request(),{...env,VERCEL_PROJECT_ID:'private-project'}),
  authorize(request(undefined,{headers:{}}),env)]){
  assert.ok(!JSON.stringify(value).includes(secret));assert.ok(!JSON.stringify(value).includes('private-project'))
 }
})
test('new route imports the canonical read consumer, never a worker or writer, and does not weaken old boundary',()=>{
