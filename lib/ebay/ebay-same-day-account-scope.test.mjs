...
test("la resolución acepta EBAY_SELLER_ACCOUNT_KEY con formato legado alias:huella", async () => {
  const { resolveSameDayPilotAccountScope } = await scopeServicePromise
  const runtimeFingerprint = userFingerprint("RuntimeSeller")

  await withEnvironment({
    VERCEL_ENV: "preview",
    EBAY_SELLER_ACCOUNT_KEY: "legacy-store:DEADBEEF",
    EBAY_DRAFT_ONLY_PRODUCTION_EXPECTED_USER_ID: "RuntimeSeller",
    EBAY_DRAFT_ONLY_PRODUCTION_EXPECTED_CREDENTIAL_FINGERPRINT: runtimeFingerprint,
  }, async () => {
    const resolution = await resolveSameDayPilotAccountScope()
    assert.equal(resolution.source, "CONFIGURED")
    assert.equal(resolution.fallbackAttempted, false)
    assert.equal(resolution.accountKey, `legacy-store:${runtimeFingerprint}`)
    assert.equal(resolution.scopeResolutionReason, null)
  })
})
