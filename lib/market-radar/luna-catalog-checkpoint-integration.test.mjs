...
test("short final page after catalog growth is authoritative without a total header", () => {
  const coverage = evaluateLunaCollectionCoverage({
    collection: "products",
    expectedTotal: 0,
    pages: [
  ...
  assert.equal(coverage.expectedTotal, 523)
})
...
test("deadline pause preserves checkpoint and the next invocation resumes", () => {
  const persisted = [
    {
      checkpoint: checkpoint(1, 250),
      products: [{ id: "p1" }],
  ...
  const paused = evaluateLunaCatalogExecutionWindow({
    nowMs: 38_500,
    deadlineAtMs: 42_000,
    pagesProcessed: 2,
    maxPages: 4,
  ...
  assert.equal(paused.canStartNextPage, false)
  assert.equal(paused.reason, "DEADLINE")

  const resumed = buildLunaCatalogResumeState(persisted)
  assert.equal(resumed.nextPage, 3)
  assert.deepEqual(
    resumed.products.map(product => product.id),
    ["p1", "p2"],
  )
  assert.equal(evaluateLunaCatalogExecutionWindow({
    nowMs: 0,
    deadlineAtMs: 42_000,
    pagesProcessed: 0,
    maxPages: 4,
    minimumRemainingMs: 5_000,
  }).canStartNextPage, true)
})
...
test("snapshot ingestion identity is stable per run and isolated across runs", () => {
  const input = {
    catalogScanRunId: "11111111-1111-4111-8111-111111111111",
    productId: "22222222-2222-4222-8222-222222222222",
    supplierVariantId: "ITEM3155",
  }
  const first =
    buildLunaSnapshotIngestionKey(input)
  ...
  assert.notEqual(
    buildLunaSnapshotIngestionKey({
      ...input,
      catalogScanRunId: "33333333-3333-4333-8333-333333333333",
    }),
    first,
  )
})
