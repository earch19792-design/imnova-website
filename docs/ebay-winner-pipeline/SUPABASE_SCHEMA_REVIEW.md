# SUPABASE_SCHEMA_REVIEW — Reutilización y brechas

## Tablas/vistas Radar existentes reutilizables

| Objeto | Reutilización para eBay Winner Pipeline |
|---|---|
| `market_radar_sources` | Fuente `lunaportex`, control de health (`last_run_at`, `last_success_at`, `last_error`). |
| `market_radar_products` | Producto proveedor/base candidato; contiene title, vendor, URL, imágenes, tags y metadata. |
| `market_radar_snapshots` | Observaciones históricas por variante: precio, disponibilidad, inventario, colecciones, raw. |
| `market_radar_events` | Señales de cambio con idempotencia; útil para rotación, restocks y auditoría. |
| `market_radar_scores` | Score actual del Radar; input para Winner Score pero no sustituto. |
| `market_radar_latest_snapshots` | Vista de último snapshot por producto-variante. |
...
> Estas son propuestas de migración futura. No se implementaron en este commit.
...
- `candidate_key text not null unique` — hash estable `source_id:product_id:variant_id`
...
Validación auditable antes de draft.
...
Cumplimiento eBay/marca/categoría.
...
Borrador local de listing eBay, sin publicación real.
...
- `ebay_draft_id text null` — reservado; debe permanecer null hasta conector real/sandbox.
- `created_at`, `updated_at`
...
## Migraciones propuestas — principios

- Crear tablas nuevas bajo prefijo `ebay_` o `ebay_winner_`.
- No alterar ni renombrar tablas Radar existentes.
- Referenciar Radar mediante FKs; no copiar todo salvo snapshot de draft necesario para auditoría.
- Usar constraints `check` para estados.
- Activar RLS y políticas admin-only al inicio.
- Agregar índices por `state`, `candidate_key`, `market_radar_product_id`, `created_at` y `winner_score`.
- Diseñar rollbacks con `drop table if exists` en orden inverso solo para tablas nuevas.

## Migración propuesta — esqueleto no ejecutado

> Nombre sugerido futuro: `supabase/migrations/YYYYMMDDHHMM_create_ebay_winner_pipeline.sql`. No se creó ni ejecutó esta migración en la auditoría para cumplir la regla de no implementar todavía.
...
### Constraints mínimas recomendadas
...
- `ebay_winner_audit_log.idempotency_key unique nulls not distinct` si se requiere evitar eventos duplicados incluso con claves nulas; alternativamente, hacer `idempotency_key not null` para eventos de transición.
- Índices por `state`, `winner_score desc`, `created_at desc`, `market_radar_product_id` y `current_snapshot_id`.
...
- No escribir en `products`/`product_images` hasta que exista una decisión formal de convertir un draft eBay a producto IMNOVA.
