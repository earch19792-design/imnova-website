# RADAR_DATA_FLOW — Luna Portex → Radar IMNOVA → Supabase → WhatsApp
...
2. Para sincronizar, el panel envía `POST /api/admin/market-radar` con `action: "sync_lunaportex"`.
...
6. Se descargan productos por colección desde Luna Portex.
...
12. Se crean eventos comparando snapshot previo vs actual: nuevo producto, stock, precio, colección y descuento.
13. Los eventos se upsertean con `idempotency_key` único e `ignoreDuplicates`.
14. Se recalculan scores por producto usando eventos de 7 días y snapshot actual.
15. Se marca `last_success_at` o `last_error`.
16. Para notificar, la misma ruta evalúa productos con `opportunity_score >= 70` y `available === true`, toma máximo 3 y envía WhatsApp.

## Formato real de producto detectado

El formato normalizado disponible para candidatos sale principalmente de la vista `market_radar_latest_products`, compuesta por `market_radar_products`, último snapshot y `market_radar_scores`.

| Campo solicitado | Campo actual | Notas |
|---|---|---|
| SKU | `sku` | Viene del variant Shopify; puede ser null. |
| título | `title` | Requerido en producto Radar; fallback “Producto sin titulo”. |
...
| UPC/GTIN/MPN | No normalizado | Solo podría existir dentro de `raw.product` o tags/body si Luna Portex lo incluye. Requiere enriquecimiento. |
| peso | No normalizado | Shopify JSON público revisado por tipos no modela weight. Requiere enriquecimiento. |
| dimensiones | No disponible | Requiere enriquecimiento manual/API externa. |
| imágenes | `featured_image_url`, `image_urls` | Deduplicadas desde `product.image.src` e `images[].src`. |
| fecha observación | `last_captured_at`, `last_seen_at`, `first_seen_at` | Snapshot y producto. |
...
| señales | `opportunity_score`, scores parciales y conteos 24h/7d | Usadas por dashboard y WhatsApp. |
...
- Snapshots: no tienen clave idempotente; cada sync inserta nueva observación. Esto es correcto para histórico, pero un reintento del mismo minuto puede duplicar snapshots equivalentes.

## Punto de extensión recomendado

Crear el eBay Winner Pipeline como consumidor aguas abajo de `market_radar_latest_products` y `market_radar_events`, sin modificar la sincronización Luna Portex. El pipeline debe crear su propia tabla de candidatos/drafts con referencias a `market_radar_products.product_id` y `supplier_variant_id` para mantener trazabilidad y reversibilidad.
