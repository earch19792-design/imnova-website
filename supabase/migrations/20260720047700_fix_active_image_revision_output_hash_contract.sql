-- Align the ACTIVE-listing apply preflight with the canonical same-day image
-- revision manifest. complete_ebay_same_day_image_revision persists
-- asset_manifest[].outputSha256; package_data.imageAssetManifest[].sha256 is a
-- separate publication projection and must not be read as revision evidence.
