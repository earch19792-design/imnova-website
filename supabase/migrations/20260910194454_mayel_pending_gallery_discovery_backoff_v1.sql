begin;
-- Reuse the existing bounded discovery; pending preparations never create a poller.
create function public.seller_os_pending_gallery_due_v1(signal jsonb,p_now timestamptz) returns boolean
language plpgsql stable security invoker set search_path=public,pg_temp as $$
declare pending jsonb:=signal->'pendingGalleryDecision';
begin
 if pending is null then return true; end if;
 if pending->>'contract' is distinct from 'MAYEL_PENDING_FULL_GALLERY_DECISION_V1' or pending->>'state'='REQUIRES_ATTENTION' then return false; end if;
 if pending->>'nextAttemptAt' is null then return true; end if;
 return (pending->>'nextAttemptAt')::timestamptz<=p_now;
exception when others then return false;
end $$;
revoke all on function public.seller_os_pending_gallery_due_v1(jsonb,timestamptz) from public,anon,authenticated;
grant execute on function public.seller_os_pending_gallery_due_v1(jsonb,timestamptz) to service_role;
...
language sql stable security invoker set search_path=public,pg_temp as $$
 ...
 and public.seller_os_pending_gallery_due_v1(t.selection_signal,now())
