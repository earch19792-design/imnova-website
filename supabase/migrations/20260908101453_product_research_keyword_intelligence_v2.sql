...
-- Same authority, same refresh entry point. Immutable older decisions remain
-- on their existing Research plan, not in another ledger or runtime.
alter table public.marketplace_product_research_query_plans
  add column keyword_intelligence_history jsonb not null default '[]'::jsonb
    check (jsonb_typeof(keyword_intelligence_history)='array');

create function public.keyword_observed_concept_span_v2(
...
declare words text[]; qual text[]:=public.keyword_tokens_v1(p_qualifier);
  head text[]:=public.keyword_tokens_v1(p_head); a integer; b integer; gap text[];
begin
  -- Do not transfer a modifier across accessory/use/brand clauses. An ordered
  -- elision is allowed only for intervening words also present in the current
  -- product title. Elided words receive no attribute authorization or demand.
  ...
  for a in 1..greatest(cardinality(words)-cardinality(qual)+1,0) loop
    ...
    for b in a+cardinality(qual)..least(a+cardinality(qual)+3,cardinality(words)-cardinality(head)+1) loop
      ...
      if not gap<@coalesce(p_allowed_gaps,'{}') then continue; end if;
      return jsonb_build_object('MATCHED',true,'CONTIGUOUS',cardinality(gap)=0,
        ...
        'ELIDED_TOKENS',to_jsonb(gap),'RELATIONSHIP',case when cardinality(gap)=0
          then 'CONTIGUOUS_QUALIFIED_HEAD' else 'ORDERED_SHARED_MODIFIER_ELISION' end);
    end loop;
  end loop;
  return null;
end;
...
create function public.keyword_family_root_v2(p_term text) returns text
language sql immutable parallel safe set search_path=pg_catalog,public as $$
  select case when length(root)>=4 then root else null end from
    (select regexp_replace(public.keyword_phrase_v1(p_term),'(ing|er)$','') root) r
  where public.keyword_phrase_v1(p_term) !~ ' ';
$$;

create function public.derive_product_research_keyword_intelligence_v2(
  p_truth jsonb,p_evidence jsonb,p_queries jsonb,p_prerequisites jsonb
) returns jsonb language plpgsql immutable parallel safe
set search_path=pg_catalog,public,extensions as $$
declare
  v_version constant text:='PRODUCT_RESEARCH_KEYWORD_INTELLIGENCE_V2';
  ...
  facts as (
    ...
    select distinct f.term||' '||v_head,'QUALIFIED_CONCEPT',f.term,f.f
      ...
      where f.kind='ATTRIBUTE_QUALIFIER' and i->>'POSITIVE'='true'
        ...
        and public.keyword_observed_concept_span_v2(i->>'TITLE',f.term,v_head,v_gaps) is not null
    union all
    ...
    select c.*,i,
      case when kind='QUALIFIED_CONCEPT' then public.keyword_observed_concept_span_v2(i->>'TITLE',qualifier,v_head,v_gaps)
...
create function public.preserve_keyword_intelligence_history_v2() returns trigger
language plpgsql security invoker set search_path=pg_catalog,public as $$
begin
  if new.keyword_intelligence_history is distinct from old.keyword_intelligence_history then
    raise exception 'KEYWORD_HISTORY_APPEND_ONLY';
  end if;
  new.keyword_intelligence_history:=old.keyword_intelligence_history;
  if old.keyword_intelligence_decision is not null
    and old.keyword_intelligence_decision is distinct from new.keyword_intelligence_decision
    and not exists(select 1 from jsonb_array_elements(old.keyword_intelligence_history) h
      where h=old.keyword_intelligence_decision) then
    new.keyword_intelligence_history:=old.keyword_intelligence_history||jsonb_build_array(old.keyword_intelligence_decision);
  end if;
  return new;
end;
$$;

create trigger keyword_intelligence_history_v2 before update of keyword_intelligence_decision,keyword_intelligence_history
  on public.marketplace_product_research_query_plans for each row
  execute function public.preserve_keyword_intelligence_history_v2();

revoke all on function public.keyword_observed_concept_span_v2(text,text,text,text[]),
  public.keyword_family_root_v2(text),public.derive_product_research_keyword_intelligence_v2(jsonb,jsonb,jsonb,jsonb),
  public.preserve_keyword_intelligence_history_v2() from public,anon,authenticated;
grant execute on function public.keyword_observed_concept_span_v2(text,text,text,text[]),
  public.keyword_family_root_v2(text),public.derive_product_research_keyword_intelligence_v2(jsonb,jsonb,jsonb,jsonb),
  public.preserve_keyword_intelligence_history_v2() to service_role;
...
begin
  ...
  if p.status is distinct from 'COMPLETED' or p.research_intelligence_status is distinct from 'COMMERCIALLY_SUFFICIENT' then
    d:=public.derive_product_research_keyword_intelligence_v2(t,'[]','[]',
  ...
  d:=public.derive_product_research_keyword_intelligence_v2(t,e,q,jsonb_build_object(
  ...
  if tg_table_name='marketplace_product_research_query_plans' then
    ...
    loop
      ...
      update public.marketplace_product_research_query_plans set keyword_intelligence_decision=
        jsonb_build_object('DECISION_VERSION','PRODUCT_RESEARCH_KEYWORD_INTELLIGENCE_V2',
