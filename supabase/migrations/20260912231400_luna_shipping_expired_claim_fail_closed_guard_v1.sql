-- Stop the legacy lease-only reclaim path before the bounded recovery
-- authority is installed. An expired CLAIMED row remains unavailable until
-- exact durable-result and browser-execution evidence are reconciled.
...
comment on function public.claim_seller_os_luna_shipping_job_v1(
  ...
  'Fail-closed guard: expired in-flight claims require the dedicated durable recovery authority and are never reclaimed by elapsed time alone.';
