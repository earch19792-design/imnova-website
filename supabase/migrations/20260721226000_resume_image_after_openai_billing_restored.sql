-- Create a new append-only handoff/control lane after the operator confirms
-- that OpenAI billing was restored. Both rejected provider attempts and their
-- original control remain immutable history; the new handoff receives a
-- content-bound recovery hash and permits one fresh image generation call.

create or replace function public.resume_same_day_image_after_openai_billing_restored_v1(
  p_account_key text,
  p_actor uuid,
  p_candidate_id uuid,
...
begin
  if coalesce(p_account_key, '') !~ '^[A-Za-z0-9._-]{1,80}:[0-9a-f]{64}$'
    ...
    raise exception 'SAME_DAY_OPENAI_BILLING_RECOVERY_INPUT_INVALID';
  end if;
  ...
  if not found then
    raise exception 'SAME_DAY_OPENAI_BILLING_RECOVERY_CANDIDATE_NOT_FOUND';
  end if;
  ...
  if not found
    ...
    raise exception 'SAME_DAY_OPENAI_BILLING_RECOVERY_RUN_SCOPE_INVALID';
  end if;
  ...
  if not found
    ...
    raise exception 'SAME_DAY_OPENAI_BILLING_RECOVERY_CANDIDATE_SCOPE_INVALID';
  end if;
  ...
  if not exists (
    ...
    raise exception 'SAME_DAY_OPENAI_BILLING_RECOVERY_CAPTURE_INVALID';
  end if;
  ...
  if not found
    ...
    raise exception 'SAME_DAY_OPENAI_BILLING_RECOVERY_FAILED_JOB_INVALID';
  end if;
  ...
  if not found
    ...
    or v_control.last_error_code
      <> 'EBAY_IMAGE_OPENAI_HTTP_400:BILLING_HARD_LIMIT_REACHED'
    ...
    raise exception 'SAME_DAY_OPENAI_BILLING_RECOVERY_CONTROL_INVALID';
  end if;
  ...
  if not found
    ...
    raise exception 'SAME_DAY_OPENAI_BILLING_RECOVERY_HANDOFF_INVALID';
  end if;

  v_recovery_package := v_old_handoff.package_data || jsonb_build_object(
    'imageGenerationRecovery', jsonb_build_object(
      'version', 'OPENAI_BILLING_RESTORED_V1_2026_07_21',
      'previousControlId', p_expected_control_id,
      'previousErrorCode',
        'EBAY_IMAGE_OPENAI_HTTP_400:BILLING_HARD_LIMIT_REACHED',
      'billingRestoredConfirmedByUser', true,
  ...
  if v_recovery_package_hash !~ '^[0-9a-f]{64}$'
    ...
    raise exception 'SAME_DAY_OPENAI_BILLING_RECOVERY_HASH_INVALID';
  end if;

  v_job_key := v_failed_job.idempotency_key
    || ':OPENAI_BILLING_RESTORED_RECOVERY:'
  ...
  if found then
    if v_new_job.run_id <> v_run.id
      ...
      raise exception 'SAME_DAY_OPENAI_BILLING_RECOVERY_IDEMPOTENCY_MISMATCH';
    end if;
  ...
  if not found
    ...
    or v_last_transition.reason_code
      <> 'EBAY_IMAGE_OPENAI_HTTP_400:BILLING_HARD_LIMIT_REACHED' then
    raise exception 'SAME_DAY_OPENAI_BILLING_RECOVERY_TRANSITION_INVALID';
  end if;
  ...
  if v_run.status <> 'ACTIVE'
    ...
    raise exception 'SAME_DAY_OPENAI_BILLING_RECOVERY_STATE_INVALID';
  end if;
  ...
  if exists (
    ...
    raise exception 'SAME_DAY_OPENAI_BILLING_RECOVERY_LANE_NOT_CLEAN';
  end if;
  ...
  insert into public.ebay_same_day_pilot_handoffs (
    ...
    v_old_handoff.handoff_version || ':OPENAI_BILLING_RECOVERY_V1',
  ...
  if not found
    ...
    raise exception 'SAME_DAY_OPENAI_BILLING_RECOVERY_HANDOFF_CREATE_FAILED';
  end if;

  v_checkpoint := v_failed_job.checkpoint || jsonb_build_object(
    'recoveryVersion', 'OPENAI_BILLING_RESTORED_V1_2026_07_21',
    ...
    'previousFailedAttemptsPreserved', 2,
    'openAiCalls', 0,
    'ebayWrites', 0,
    'productionChanged', false
  );
  ...
  v_transition_result := public.advance_same_day_pilot_candidate(
    ...
    'OPENAI_BILLING_RESTORED_BY_USER',
    'USER',
    p_now,
    p_now,
    1,
    ...
    v_evidence_hash,
    v_transition_key,
    'Generar las imágenes con la facturación de OpenAI restablecida.',
    'Ninguna.',
  ...
  if v_transition_result <> 'ADVANCED' then
    raise exception 'SAME_DAY_OPENAI_BILLING_RECOVERY_TRANSITION_BLOCKED';
  end if;
  ...
  set state = 'READY_FOR_CONTENT',
      ...
      manual_handoff_package = candidate.manual_handoff_package
        || jsonb_build_object(
          ...
          'packageHash', v_recovery_package_hash,
          'billingRecovery', jsonb_build_object(
            'version', 'OPENAI_BILLING_RESTORED_V1_2026_07_21',
            'previousControlId', p_expected_control_id,
            'historyPreserved', true
          )
  ...
  if not found then
    raise exception 'SAME_DAY_OPENAI_BILLING_RECOVERY_CANDIDATE_PATCH_FAILED';
  end if;
  ...
  set status = 'ACTIVE',
      ...
      next_automated_action =
        'Procesar la generación después de restablecer facturación.',
      next_human_action = 'Ninguna.',
      updated_at = p_now
  where run.id = v_run.id;
  ...
  where job.idempotency_key = v_job_key;
  if not found then
    raise exception 'SAME_DAY_OPENAI_BILLING_RECOVERY_JOB_MISSING';
  end if;
  ...
  insert into public.ebay_same_day_pilot_events (
    ...
    v_run.id,
    p_candidate_id,
    'SAME_DAY_IMAGE_OPENAI_BILLING_RECOVERY_CREATED',
    jsonb_build_object(
...
revoke all on function public.resume_same_day_image_after_openai_billing_restored_v1(
  text, uuid, uuid, uuid, uuid, uuid, timestamptz
) from public, anon, authenticated;
grant execute on function public.resume_same_day_image_after_openai_billing_restored_v1(
  text, uuid, uuid, uuid, uuid, uuid, timestamptz
) to service_role;

comment on function public.resume_same_day_image_after_openai_billing_restored_v1(
  text, uuid, uuid, uuid, uuid, uuid, timestamptz
) is
  'Creates one append-only recovery handoff after a user confirms OpenAI billing is restored; preserves prior failed controls and makes no eBay write.';
