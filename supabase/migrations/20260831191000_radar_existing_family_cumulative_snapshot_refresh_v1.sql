-- Permit a strictly newer rolling cumulative sold snapshot to refresh the same
-- canonical family even when its source window overlaps the previous rolling
-- window. Replays, backfills and every other adapter remain fail-closed.
...
declare
  ...
  v_limitations text[];
  v_binding jsonb;
  v_cumulative_refresh boolean := false;
begin
  ...
  select coalesce((
    select
      ...
      and previous.aggregation_semantics = 'CUMULATIVE_SNAPSHOT'
      and p_source_adapter =
        'SELLER_OS_PRODUCT_RESEARCH_FAMILY_ADAPTER_V1'
      and p_source_contract_version =
        'EBAY_PRODUCT_RESEARCH_BROWSER_CAPTURE'
      and previous.source_adapter = p_source_adapter
      and previous.source_contract_version = p_source_contract_version
      and p_observation_window_end > previous.observation_window_end
      and p_evidence_observed_at > previous.evidence_observed_at
      and v_demand_evidence_digest is distinct from
        previous.demand_evidence_digest
    from public.seller_os_family_market_observations previous
    where previous.family_id = v_case.family_id
    order by previous.observation_window_end desc,
      previous.evidence_observed_at desc,
      previous.observation_id desc
    limit 1
  ), false) into v_cumulative_refresh;

  if not v_cumulative_refresh and exists (
