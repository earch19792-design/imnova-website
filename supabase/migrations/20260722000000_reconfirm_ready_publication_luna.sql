-- Restore a safe path when Luna freshness expires after content/images are
-- already approved. This only refreshes internal source evidence; it cannot
-- create an Inventory Item, Offer, or eBay publication.
...
begin
  ...
  if jsonb_array_length(v_package.package_data->'imageUrls') <> 6
    or jsonb_array_length(v_package.package_data->'imageAssetManifest') <> 6 then
    raise exception 'SAME_DAY_PUBLICATION_LUNA_RECHECK_PACKAGE_INVALID';
  end if;
  ...
  v_handoff_urls := v_handoff#>'{images,urls}';
  v_approved_urls := v_image_summary->'publicUrls';
  ...
  if jsonb_array_length(v_handoff_urls) <> 6
    or jsonb_array_length(v_approved_urls) <> 6
    ...
    or (select count(distinct value)
        from jsonb_array_elements_text(v_package_urls) as url(value)) <> 6
    or (select count(*)
        from jsonb_array_elements_text(v_package_urls) as url(value)
        where value ~ '^https://') <> 6 then
