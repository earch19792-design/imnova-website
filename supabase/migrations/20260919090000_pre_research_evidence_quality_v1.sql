-- PRE_RESEARCH_EVIDENCE_QUALITY_V1
-- Durable fail-closed validation for Luna Pre-Research dispositions.
-- This migration intentionally preserves the existing service-role-only RPC.
...
declare
  ...
  v_exact_count numeric;
  v_close_count numeric;
  v_family_count numeric;
...
begin
  ...
  if p_evidence ->> 'contractVersion' is distinct from
      ...
      or jsonb_typeof(p_evidence -> 'traceEligible') is distinct from 'boolean' then
    raise exception 'LUNA_PRE_RESEARCH_EVIDENCE_CONTRACT_INVALID';
  end if;

  v_exact_count := (p_evidence ->> 'exactComparableCount')::numeric;
  v_close_count := (p_evidence ->> 'closeVariantComparableCount')::numeric;
  v_family_count := (p_evidence ->> 'familyComparableCount')::numeric;
  v_accepted_count := (p_evidence ->> 'acceptedComparableCount')::numeric;
  v_raw_sold := (p_evidence ->> 'rawObservedSoldQuantity')::numeric;
  v_accepted_sold := (p_evidence ->> 'acceptedComparableSoldQuantity')::numeric;

  if v_accepted_count <> v_exact_count + v_close_count + v_family_count
      or v_accepted_sold > v_raw_sold
      or (p_evidence ->> 'traceEligible')::boolean is distinct from p_trace_eligible
      or p_trace_eligible is distinct from
        (p_result in ('PRE_RESEARCH_HIGH','PRE_RESEARCH_MEDIUM'))
    ...
    raise exception 'LUNA_PRE_RESEARCH_COMPARABLE_AUTHORITY_INVALID';
  end if;
  ...
  if not exists (
    ...
    select 1
    from public.marketplace_product_research_query_tasks task
    where task.plan_id = v_plan.id and task.status = 'PENDING'
  ) then
