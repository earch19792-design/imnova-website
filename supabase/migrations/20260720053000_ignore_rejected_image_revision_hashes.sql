...
begin
  ...
  loop
    ...
    order by case asset.status when 'pending_review' then 0 else 1 end,
      asset.created_at desc, asset.id desc
    limit 1
    for update;
    if found then
      if v_asset.candidate_key is distinct from p_candidate_key
...
) is 'Creates an exact V3 image set, excludes rejected history, and reuses only safe active assets.';
