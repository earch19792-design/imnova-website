-- The current physical ebay_active_listings schema stores optional variation
-- identity in raw_payload. Replace the stale column reference without changing
-- the item-scoped current-live authority or its transaction boundary.
...
comment on function public.record_ebay_current_live_authority_success_v1(
  text, uuid, text, timestamptz, timestamptz, jsonb, jsonb
) is 'Persists an item-scoped certified current-live cohort; optional exact variation identity is stored in raw_payload.variationKey.';
