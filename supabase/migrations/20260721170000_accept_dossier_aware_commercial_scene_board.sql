-- Accept the dossier-aware commercial scene board while preserving review of
-- historical V1 context-plate sets. One OpenAI response may supply the five
-- secondary backgrounds; the exact authorized main remains deterministic.
-- No competitor image, eBay write or Production capability is introduced.
...
begin
  ...
  into v_asset_count, v_slot_count, v_partial_count, v_generative_count,
    v_safe_v1_count, v_safe_v2_count, v_invalid_count
  from unnest(p_asset_ids) requested(asset_id)
...
comment on function public.assert_same_day_pilot_image_set_safe(
  uuid, uuid, uuid[]
) is 'Validates legacy one-context sets or dossier-aware five-secondary commercial scene-board sets; exact authorized main and human review remain mandatory.';
