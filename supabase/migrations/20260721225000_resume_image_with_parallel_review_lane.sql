...
-- deployed. A pending human review for another candidate in the same run is
-- not a conflicting generation lane; only this candidate's controls can block
-- its retry. All existing controls and review assets remain untouched.

create or replace function public.resume_same_day_image_after_safe_openai_error_capture_v2(
  p_account_key text,
  p_actor uuid,
  p_candidate_id uuid,
...
revoke all on function public.resume_same_day_image_after_safe_openai_error_capture_v2(
  text, uuid, uuid, uuid, uuid, uuid, timestamptz
) from public, anon, authenticated;
grant execute on function public.resume_same_day_image_after_safe_openai_error_capture_v2(
  text, uuid, uuid, uuid, uuid, uuid, timestamptz
) to service_role;

comment on function public.resume_same_day_image_after_safe_openai_error_capture_v2(
  text, uuid, uuid, uuid, uuid, uuid, timestamptz
) is
  'Reclassifies one exact no-output HTTP 400 image attempt and permits one final retry without mutating another candidate pending human review.';
