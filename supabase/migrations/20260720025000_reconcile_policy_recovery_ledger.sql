-- Preserve the original fail-closed recovery as an internal core. The public
-- wrapper repairs one exact, audited V6 ledger mismatch and calls the core in
-- the same transaction. Any core failure rolls the reconciliation back.

alter function public.resume_same_day_pilot_candidate_after_account_policy_profile_v1(
  text, uuid, uuid, timestamptz
) rename to resume_same_day_pilot_candidate_after_account_policy_profile_core_v1;

revoke all on function public.resume_same_day_pilot_candidate_after_account_policy_profile_core_v1(
  text, uuid, uuid, timestamptz
