-- Reuse the operational learning ledger as the durable Product Research
-- acquisition authority. One account may have only one live browser claim at
-- a time; an expired lease is safely reclaimable. No marketplace write is
-- performed by either function.

create or replace function public.claim_next_live_listing_product_research_v1(
...
begin
  ...
  -- Serialize claim selection per account so two authenticated admin tabs
  -- cannot acquire different plans concurrently.
...
begin
  if p_marketplace_account_key is null
    ...
    raise exception 'LIVE_PRODUCT_RESEARCH_WORKER_RELEASE_INVALID';
  end if;
      ...
      evidence = ledger.evidence || pg_catalog.jsonb_build_object(
        ...
        coalesce(ledger.evidence -> 'workerAcquisition', '{}'::jsonb) ||
          pg_catalog.jsonb_build_object(
            'claimState', 'RELEASED_RETRY_SAFE',
            'lastErrorCode', p_error_code,
            'releasedAt', pg_catalog.clock_timestamp()
          )
      ),
      lease_owner = null,
  ...
  where ledger.marketplace_account_key = p_marketplace_account_key
    and ledger.id = (
      select plan.request_receipt_id
...
revoke all on function public.claim_next_live_listing_product_research_v1(
  text, text, jsonb, uuid, integer
) from public, anon, authenticated;
revoke all on function public.release_live_listing_product_research_v1(
  text, uuid, text, text
) from public, anon, authenticated;
grant execute on function public.claim_next_live_listing_product_research_v1(
  text, text, jsonb, uuid, integer
) to service_role;
...
comment on function public.claim_next_live_listing_product_research_v1(
  text, text, jsonb, uuid, integer
) is 'Atomically claims one existing LIVE-listing Product Research plan for the authenticated browser worker. Reuses the operational learning ledger, enforces account single-flight and performs zero marketplace writes.';

comment on function public.release_live_listing_product_research_v1(
  text, uuid, text, text
) is 'Releases a retry-safe browser claim while preserving its durable failure evidence. Performs zero marketplace writes.';
