-- A bounded retry must not outrank a plan that has never received its first
-- worker attempt. Preserve FIFO inside each attempt tier while retaining the
-- existing capability, lease, cooldown and idempotency contracts.
...
begin
  ...
  order by case
      when plan.source_context = 'QUICK_PICK_RESEARCH_REQUIRED'
        then plan.worker_claim_count
      else ledger.recovery_attempt_count
    end,
    plan.created_at,
    plan.id
  limit 1
...
) is 'Single attempt-fair Product Research claimer. Never-attempted work precedes bounded retries; stable heartbeat binding, FIFO per attempt tier, cooldowns and single-flight remain enforced.';
