...
begin
  ...
  where child.id = p_child_id
    ...
    and child.status in ('FAILED_BLOCKED', 'AMBIGUOUS_FAIL_CLOSED')
...
comment on function
  ...
  'Atomically rearms exact unchanged batch children after shared preflight authority-selection defects. Existing approval lineage is reused; no package, Offer, Item, or commercial authority is mutated.';
