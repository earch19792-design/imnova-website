...
test("priority-first automation scans the strongest Radar signals before catalog coverage", () => {
  ...
  assert.match(service, /\.order\("seller_scan_priority_score", \{ ascending: false, nullsFirst: false \}\)/)
  ...
  assert.match(cronRoute, /CRON_MAX_BATCHES = 6/)
  assert.match(cronRoute, /CRON_TIME_BUDGET_MS = 47_000/)
