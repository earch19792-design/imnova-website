...
test("mobile command center centralizes scan, queue and candidate review", () => {
  ...
  assert.match(commandCenter, /Iniciar discovery ligero/)
  assert.match(commandCenter, /Acelerar 20 productos/)
  assert.match(commandCenter, /Actualizar Luna/)
  assert.match(commandCenter, /Oportunidades para revisar/)
  assert.match(commandCenter, /Cola canónica \+ Radar actualizado/)
  assert.match(commandCenter, /newRadarSignals/)
  assert.doesNotMatch(mobilePage, /Radar alternativo/)
...
test("priority-first automation scans the strongest Radar signals before catalog coverage", () => {
  ...
  assert.match(adminRoute, /RESET_ALL_1513_BLOCKED/)
  assert.match(adminRoute, /RESUME_FROM_CHECKPOINT/)
...
test("separates eBay candidates from exact comparables and builds a seller fast lane", () => {
  ...
  assert.equal(row.seller_priority_score, 0)
