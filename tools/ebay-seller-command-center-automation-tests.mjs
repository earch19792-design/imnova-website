...
test("active listing protection emits one canonical risk and alert per account + item + SKU", () => {
  ...
  assert.match(service, /resolveExactUniqueCurrentLunaVariant/)
  assert.match(service, /\.eq\("source_key", "lunaportex"\)/)
  assert.match(service, /\.eq\("sku", listing\.supplier_sku\)/)
  assert.match(service, /query = query\.eq\("supplier_variant_id", listing\.supplier_variant_id\)/)
  assert.match(service, /\.in\("id", group\.memberListingIds\)/)
  assert.match(service, /\.is\("market_radar_product_id", null\)/)
  assert.match(service, /listingsAutoLinkedToLuna/)
  assert.match(service, /listingRowsAutoLinkedToLuna/)
...
test("cron cadence preserves priority while respecting the serverless budget", () => {
  ...
  assert.deepEqual(vercel.crons, [
    { path: "/api/cron/ebay-active-listing-luna-monitor", schedule: "*/5 * * * *" },
...
test("dashboard exposes operational health instead of reporting an empty queue as healthy", () => {
  ...
  assert.match(service, /projectActiveListingProtectionMonitorHealth/)
  assert.match(service, /listingProtectionMonitor/)
  assert.match(service, /ebay_active_listing_sync_state/)
  assert.match(service, /targeted_luna_last_success_at/)
  assert.match(service, /\.eq\("account_key", accountKey\)/)
  assert.match(service, /EBAY_TARGETED_LUNA_ACTIVE_MONITOR_ENABLED/)
  assert.match(service, /fullCatalogLastSuccessAt/)
  assert.match(service, /targetedMonitorLastSuccessAt/)
  assert.match(protectionDomain, /TARGETED_LUNA_MONITOR_HEARTBEAT_STALE/)
  assert.match(protectionDomain, /ACTIVE_LISTING_PROTECTION_HEARTBEAT_STALE/)
  assert.match(protectionDomain, /TARGETED_LUNA_MONITOR_FEATURE_DISABLED/)
