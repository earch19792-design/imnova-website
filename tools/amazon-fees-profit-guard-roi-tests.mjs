...
test("builds three bounded fees and profit assessments", async () => {
  ...
  for (const assessment of queue.assessments) {
    ...
    assert.equal(assessment.lunaPrepPackingCostScheduleUsed, true);
    assert.equal(assessment.professionalSellerPlanFeeIncluded, true);
    assert.equal(typeof assessment.referralFeeAmount, "number");
    assert.equal(typeof assessment.netProfitBeforeOperationalAddOns, "number");
    assert.equal(typeof assessment.netProfitAfterOperationalAddOns, "number");
    assert.equal(assessment.netProfitAfterOperationalAddOns <= assessment.netProfitBeforeOperationalAddOns, true);
...
test("DM0628N can calculate ROI but remains blocked from listing package", async () => {
  ...
  assert.equal(dm.lunaFbaPrepCost, 0.5);
...
test("CLI dry-run executes and prints expected numeric output", async () => {
  ...
  assert.equal(summary.lunaPrepPackingCostScheduleUsed, true);
  assert.equal(summary.professionalSellerPlanFeeIncluded, true);
  assert.equal(summary.dm0628nReferralFeeAmount, 3.45);
  assert.equal(summary.dm0628nFnSkuLabelingCost, 0.5);
  assert.equal(summary.dm0628nProfessionalPlanFeePerUnit, 0.8);
  assert.equal(summary.dm0628nTotalOperationalCostAddOn, 1.3);
  assert.equal(summary.dm0628nNetProfitBeforeOperationalAddOns, 3.58);
  assert.equal(summary.dm0628nNetProfitAfterOperationalAddOns, 2.28);
