    ...
    <div id="commercial-dashboard" className="min-h-screen bg-[#eef2f6] text-slate-950 xl:pl-[200px]">
      ...
      <main className="mx-auto max-w-[1680px] space-y-3 p-3 md:p-4">
        ...
        <section aria-label="Integridad del portafolio actual" className="rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5 text-[9px]">{livePortfolioIntegrity.deterministicGuards.map((guard) => <span key={guard.guardCode} title={guard.reasonCode} className={`rounded border px-1.5 py-1 font-bold ${guard.status === "TRIGGERED" || guard.status === "FAIL" ? "border-rose-200 bg-rose-50 text-rose-800" : guard.status === "MITIGATED" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : guard.status === "DEGRADED" ? "border-amber-200 bg-amber-50 text-amber-800" : "border-slate-200 bg-slate-50 text-slate-600"}`}>{guard.guardCode} · {guard.status}</span>)}</div>
        </section>
