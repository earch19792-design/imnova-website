...
function nextAction(task: Task) {
  ...
  if (task.workflow_state === "TRACKING_READY_FOR_SUBMISSION") return "Revisar el preview exacto y confirmar el envío a eBay."
  if (task.workflow_state === "TRACKING_SUBMISSION_QUEUED") return "El worker hará preflight oficial y reconciliación; no edites ni vuelvas a enviar el tracking."
  if (task.workflow_state === "TRACKING_SUBMITTED_TO_EBAY") return "Esperar el read-back oficial antes de considerar la orden enviada."
  if (task.workflow_state === "SHIPPED") return "Tracking confirmado por read-back de eBay; ciclo postventa cerrado."
...
export function MarketplaceFulfillmentPanel() {
  ...
  return <section aria-labelledby="fulfillment-v1a-heading" className="space-y-4 rounded-3xl border border-violet-200/25 bg-violet-200/[0.06] p-4">
    <header>
      <p className="text-xs font-black uppercase tracking-widest text-violet-100/65">Postventa eBay · confirmación humana</p>
      <h2 id="fulfillment-v1a-heading" className="mt-1 text-xl font-black">Venta, tracking y reconciliación</h2>
      <p className="mt-2 text-sm text-white/70">Pega carrier y tracking, revisa el preview y confirma una sola vez. El sistema valida cuenta, orden, line item, Custom Label, pago y estado oficial antes del POST; un resultado incierto queda en revisión manual y nunca se reenvía automáticamente.</p>
    </header>
    ...
    {dashboard?.tasks.map((task) => {
      ...
      return <article key={task.id} className="rounded-3xl border border-white/15 bg-black/25 p-4">
        ...
        <dl className="mt-3 grid gap-2 text-xs sm:grid-cols-3">
          <div><dt className="text-white/45">Order ID</dt><dd className="break-all font-mono">{task.marketplace_order_id}</dd></div>
          <div><dt className="text-white/45">Line Item ID</dt><dd className="break-all font-mono">{task.marketplace_line_item_id}</dd></div>
        ...
        {["WAITING_FOR_TRACKING", "TRACKING_READY_FOR_SUBMISSION"].includes(task.workflow_state) && <form className="mt-4 grid gap-3 rounded-2xl border border-white/10 p-3 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); void submit(task, "tracking") }}>
        ...
        {shipment && <div className="mt-3 rounded-2xl border border-cyan-200/20 bg-cyan-200/[0.06] p-3 text-xs"><p className="font-black">Preview final inmutable · {shipment.approval_status}</p><p className="mt-2 text-cyan-50/80">Orden {task.marketplace_order_id} · line item {task.marketplace_line_item_id} · Item ID {task.listing_id} · Custom Label {task.marketplace_listing_sku ?? "N/D"}. Sólo se enviará el payload mostrado abajo.</p><pre className="mt-2 max-h-56 overflow-auto whitespace-pre-wrap break-all font-mono text-[10px] text-cyan-50/80">{JSON.stringify(shipment.normalized_payload, null, 2)}</pre><p className="mt-2 break-all font-mono">{shipment.payload_hash}</p>{task.workflow_state === "TRACKING_READY_FOR_SUBMISSION" && <div className="mt-3 grid gap-2"><button type="button" disabled={busyTask === task.id} onClick={() => void submit(task, "approve")} className="min-h-12 w-full rounded-xl bg-amber-100 px-3 font-black text-black">Aprobar submission simulada</button>{dashboard.config?.realAdapter?.executable ? <button type="button" disabled={busyTask === task.id} onClick={() => void submit(task, "approve-real")} className="min-h-12 w-full rounded-xl border border-rose-100/40 bg-rose-100/10 px-3 font-black text-rose-50">Confirmar y enviar tracking a eBay</button> : <p className="rounded-xl border border-amber-200/25 p-2 font-black text-amber-50">MANUAL_SELLER_HUB_TRACKING_REQUIRED</p>}</div>}</div>}
