          ...
          <div className="mt-4 space-y-4">
            {pinnedContinuity.pinnedCandidatesOutsideTop5.length === 0 ? (
              ...
              <article key={candidate.pinnedCandidateId} className="rounded-2xl border border-violet-200/15 bg-black/25 p-4">
                ...
                <p className="mt-3 text-xs leading-5 text-white/55">Radar: {candidate.radarPresenceStatus}<br />Última confirmación humana: {formatValue(candidate.lastHumanConfirmationAt)}<br />Mismo producto: {String(candidate.sameProductConfirmed)}<br />Stock: {candidate.stockQuantityConfirmed ?? "pendiente"} · {candidate.stockWarning ?? "sin warning"}<br />Precio Luna: USD {candidate.lunaPrice?.toFixed(2) ?? "pendiente"}<br />Imagen: {String(candidate.imageConfirmed)}<br />Source: {candidate.source}<br />Supplier drift: {String(candidate.supplierDrift.supplierDriftDetected)}<br />Drift blocker: {formatValue(candidate.supplierDrift.driftBlockingReason)}<br />Stock anterior/actual: {formatValue(candidate.supplierDrift.previousStockQuantity)} / {formatValue(candidate.supplierDrift.latestStockQuantity)}<br />Precio anterior/actual: {formatValue(candidate.supplierDrift.previousLunaPrice)} / {formatValue(candidate.supplierDrift.latestLunaPrice)}<br />Disponibilidad anterior/actual: {formatValue(candidate.supplierDrift.previousAvailabilityStatus)} / {formatValue(candidate.supplierDrift.latestAvailabilityStatus)}<br />Próximo paso: {candidate.nextRecommendedRoute}</p>
