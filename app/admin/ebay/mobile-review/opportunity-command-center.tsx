...
export function OpportunityCommandCenter({
  ...
  async function syncActiveListings() {
    ...
    try {
      ...
      await load()
      showMessage(`Listings sincronizados: ${payload.sync?.listingsStored ?? 0}; vinculados inmediatamente: ${payload.immediateLinkage?.autoLinked ?? 0}; vínculos Luna activos: ${payload.sync?.listingsMappedToLuna ?? 0}. La protección ya usa el estado más reciente.`)
    } catch (requestError) {
    ...
    {dashboard && <>
      ...
      <div className="mt-3 space-y-3">{rows.map((row, index) => <article key={row.id} className={`rounded-3xl border p-4 ${cardTone(row.queue_status)}`}>
        ...
        {row.supplier_sku && row.supplier_product_id && row.supplier_variant_id &&
          <a className="mt-2 block rounded-2xl border border-cyan-200/30 p-3 text-center text-xs font-black text-cyan-100"
            href={`/admin/ebay/opportunity-category?opportunityId=${encodeURIComponent(row.id)}&candidateKey=${encodeURIComponent(row.candidate_key)}`}>
            Confirmar categoría antes del paquete
          </a>}
