...
export default function StrategicReviewPage() {
  ...
  const guards = record(bundle.deterministicIntegrityGuards)
  ...
  return <main className="min-h-screen bg-[#07111d] p-4 text-slate-100 md:p-7"><div className="mx-auto max-w-[1500px] space-y-4">
    ...
    <section className="grid gap-4 xl:grid-cols-2"><article className="overflow-hidden rounded-2xl border border-white/10 bg-[#0b1826]"><div className="border-b border-white/10 px-5 py-4"><h2 className="font-black">Strategic Review Queue</h2><p className="mt-1 text-xs text-slate-500">Improve Seller OS—not the operator’s commercial action queue.</p><p className="mt-2 text-[10px] text-slate-400">Active {text(queue.activeViolationCount ?? 0)} · mitigated {text(queue.mitigatedFindingCount ?? 0)} · unresolved identity {text(queue.unresolvedHumanIdentityCount ?? 0)}</p></div><div className="divide-y divide-white/[.06]">{rows(queue.entries).map((entry) => <div key={text(entry.signalId)} className="p-4"><div className="flex items-start justify-between gap-3"><p className="font-bold">{text(entry.signalType)}</p><span className="text-xs text-amber-100">{text(entry.severity)}</span></div><p className="mt-1 text-[10px] font-bold uppercase tracking-wide text-slate-500">{text(entry.strategicClassification)} · {text(entry.invariantLifecycle)}</p><p className="mt-1 text-sm text-slate-300">{text(entry.summary)}</p><p className="mt-2 text-xs text-cyan-100">Next: {text(entry.nextAction)} · evidence {text(entry.evidenceCount)}</p></div>)}{rows(queue.entries).length === 0 ? <p className="p-5 text-sm text-slate-500">No material deterministic strategic signal.</p> : null}</div></article>
    ...
    <section className="rounded-2xl border border-white/10 bg-[#0b1826] p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-[10px] font-black uppercase tracking-wider text-emerald-200">Always-on deterministic guards</p><p className="mt-1 text-xs text-slate-500">Independent of the repeated-evidence automation threshold.</p></div><span className="text-xs text-slate-400">auto mutation: disabled</span></div><div className="mt-3 flex flex-wrap gap-2">{rows(guards.entries).map((entry) => <span key={text(entry.guardCode)} className="rounded-lg border border-white/10 bg-black/20 px-2.5 py-2 text-xs"><strong>{text(entry.guardCode)}</strong> · {text(entry.status)}</span>)}</div></section>
