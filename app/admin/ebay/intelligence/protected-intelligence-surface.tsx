...
export function ProtectedIntelligenceSurface({ mode }: { mode: Mode }) {
  ...
  return <main className="min-h-screen bg-[#07111d] p-4 text-slate-100 md:p-7"><div className="mx-auto max-w-[1500px] space-y-4">
    <header className="flex flex-wrap items-start justify-between gap-4 rounded-2xl border border-white/10 bg-[#0b1826] p-5"><div><Link href="/admin/ebay/monitor" className="inline-flex items-center gap-2 text-xs font-bold text-cyan-300"><ArrowLeft size={14} />Commercial Monitor</Link><div className="mt-3 flex items-center gap-3"><span className="rounded-xl bg-cyan-400/10 p-2.5 text-cyan-300"><Icon size={20} /></span><div><h1 className="text-2xl font-black">{copy.title}</h1><p className="mt-1 text-sm text-slate-400">{copy.subtitle}</p></div></div></div><div className="flex items-center gap-2"><Link href={`/admin/ebay/copilot?surface=${mode === "DECISIONS" ? "DECISION" : mode === "EXPERIMENTS" ? "EXPERIMENT" : "PORTFOLIO"}`} className="rounded-lg border border-violet-300/20 px-3 py-2 text-xs font-black text-violet-200">Ask Copilot</Link><span className="inline-flex items-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1.5 text-xs font-black text-emerald-200"><ShieldCheck size={14} />READ-ONLY</span><button type="button" onClick={() => void load()} className="rounded-lg border border-white/10 p-2" aria-label="Refresh intelligence"><RefreshCw size={15} /></button></div></header>
