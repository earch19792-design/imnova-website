...
type DraftState = {
  ...
  controlledPublication?: {
    ...
    blocker?: string | null
    version?: string | null
    humanAuthorizationCount?: number
    secondHumanAuthorizationRequired?: boolean
  ...
  approvalRequirements?: {
    ...
    singleHumanPublicationEligible?: boolean
    target: "SANDBOX" | "PRODUCTION"
...
type UnpublishedPreflightState =
  | "idle"
  | "loading"
  | "ready_resume"
  | "ready_new"
...
function humanUnpublishedPreflightError(code: string) {
  ...
  const labels: Record<string, string> = {
    ...
    ACTIVE_APPROVAL_PAYLOAD_MISMATCH:
      "La aprobación activa no corresponde al payload vigente. Debe reconciliarse de forma explícita antes de una autorización nueva.",
    FINAL_PREVIEW_HASH_MISMATCH:
      "El preview final vigente no coincide con el snapshot autorizado.",
    TARGET_ACCOUNT_MISMATCH:
      "La cuenta eBay enlazada no coincide con la cuenta de la autorización.",
    AUTHORIZATION_PREVIEW_INTEGRITY_INVALID:
      "El snapshot de autorización no supera la verificación criptográfica y de pantalla.",
    ACTIVE_APPROVAL_INTEGRITY_INVALID:
      "La evidencia persistida de la aprobación no coincide con su payload hash.",
    EBAY_DRAFT_ONLY_RUNTIME_NOT_READY:
...
const LEGACY_V3_IMAGE_MESSAGES = new Set([
  "Aprueba al menos una imagen autorizada.",
  "Optimiza y aprueba al menos una imagen autorizada.",
  "IMAGE_REQUIRED",
...
function ListingWorkspacePageContent() {
  ...
  const [unpublishedAuthorizationError, setUnpublishedAuthorizationError] =
    useState("")
  const [unpublishedPreflightState, setUnpublishedPreflightState] =
  ...
  const [publicationAutomationBusy, setPublicationAutomationBusy] =
    useState(false)
  ...
  const finalReviewCompleted = v3FinalListingReviewCanonicalReady({
    ...
    revisionId: finalReviewRecord.revisionId,
    activeAttemptId: referenceGuidedAttemptId,
    attemptId: finalReviewRecord.attemptId,
    visualPhase: finalReviewRecord.visualPhase,
    finalVisualSetLocked: finalReviewRecord.finalVisualSetLocked,
    generationControlsHidden: finalReviewRecord.generationControlsHidden,
    readyForUnpublishedOfferAuthorization:
      finalReviewRecord.readyForUnpublishedOfferAuthorization,
    providerCalls: finalReviewRecord.providerCalls,
    blockers: finalReviewRecord.blockers,
    gates: finalReviewRecord.gates,
    signedImages: finalListingReview?.signedImages,
  })
  const visibleWorkspaceGateBlockers = useMemo(
    () => visibleWorkspaceBlockers({
      blockers: workspaceGateBlockers,
      canonicalV3FinalReview: finalReviewCompleted,
      source: "workspace",
    }),
    [finalReviewCompleted, workspaceGateBlockers],
  )
  const visibleDraftReadinessBlockers = useMemo(
    () => visibleWorkspaceBlockers({
      blockers: draftState.readiness?.blockers ?? [],
      canonicalV3FinalReview: finalReviewCompleted,
      source: "draft_readiness",
    }),
    [draftState.readiness?.blockers, finalReviewCompleted],
  )
  const visibleDraftReadinessReady = Boolean(draftState.readiness)
    && (draftState.readiness?.ready === true
      || visibleDraftReadinessBlockers.length === 0)
  ...
  const singleHumanPublicationButtonDisabled =
  ...
  async function publishSmartStockingWithSingleAuthorization() {
    if (
      !listingPackage || !opportunity || publicationAutomationBusy
      || !singleHumanPublicationEligible
    ) return
    ...
    setError("")
    setMessage("")
    let nextApproval = draftState.controlledPublication?.authorized
      && (draftState.execution?.id || (
    ...
    try {
      if (nextPublication?.phase === "monitor_registered") {
        ...
        setMessage(
          `Listing ${nextPublication.listing_id} ya está ACTIVE y monitoreado.`,
        )
        return
      }
      if (
        ...
        && ["publish_in_flight", "outcome_unknown",
          "published_pending_verification"].includes(nextPublication.phase)
      ) {
        setPublicationAutomationPhase("verifying")
        setPublicationAutomationStep(
          "Verificando el resultado existente sin repetir publishOffer…",
        )
        const reconciled = await draftRequest({
          action: "reconcile_publish",
          publicationId: nextPublication.id,
        })
        nextPublication = reconciled.publication
        setDraftState((current) => ({
          ...current,
        ...
        setMessage(reconciled.monitoring?.registered
          ? `Listing ${reconciled.listing?.listingId} ACTIVE y monitoreado.`
      ...
      if (!nextApproval?.id) {
        ...
        setPublicationAutomationStep(
          "2/5 · Registrando la única autorización humana, ligada al digest exacto…",
        )
      ...
      if (!nextApproval?.id) {
        throw new Error("EBAY_ONE_CLICK_PUBLICATION_APPROVAL_REQUIRED")
      }
      ...
      if (!nextPublication?.id || nextPublication.phase !== "preview_ready") {
        ...
        setPublicationAutomationStep(
          "4/5 · Comparando Inventory Item y Offer oficiales con el paquete autorizado…",
        )
      ...
      setPublicationAutomationStep(
        "5/5 · Preflight fresco, publicación one-shot y readback ACTIVE…",
      )
      ...
      setMessage(published.monitoring?.registered
        ? `Listing ${published.listing?.listingId} ACTIVE, enlazado y monitoreado.`
  ...
  async function requestV3UnpublishedAuthorization(
    body: Record<string, unknown>,
  ) {
    const { data: sessionData } = await supabase.auth.getSession()
    if (!sessionData.session) throw new Error("ADMIN_SESSION_REQUIRED")
  ...
  async function publishV3Automatically() {
    if (
      !listingPackage
      || !referenceGuidedAttemptId
    ...
    try {
      ...
      if (!nextExecution?.id) {
        ...
        for (let sourceAttempt = 0; sourceAttempt < 2; sourceAttempt += 1) {
          ...
          setPublicationAutomationStep(
            "1/4 · Revalidando Luna, cuenta, policies y las siete imágenes V3…",
          )
        ...
        setUnpublishedPreflightState(
          preparedApproval.id ? "ready_resume" : "ready_new",
        )
        setUnpublishedAuthorizationMode(
          preparedApproval.id
            ? "resume_existing_authorization"
            : "new_authorization",
        )
        const stableImages = Array.isArray(preparedAuthorization.images)
          ? preparedAuthorization.images
            .map((asset) => String(object(asset).url ?? ""))
            .filter(Boolean)
          : []
        if (stableImages.length === 7) {
          setForm((current) => ({ ...current, imageUrls: stableImages }))
        }

        if (preparedApproval.id) {
          nextApproval = {
            id: String(preparedApproval.id),
            status: String(preparedApproval.status ?? "approved"),
            payload_hash: String(preparedApproval.payload_hash
              ?? preparedAuthorization.payloadHash
              ?? ""),
          ...
          if (!confirmationPhrase) {
            throw new Error("EBAY_V3_CONFIRMATION_PHRASE_MISSING")
          }
          const authorized = await requestV3UnpublishedAuthorization({
            action: "authorize",
            authorizationPreviewId: preparedAuthorization.id,
            previewHash: preparedAuthorization.previewHash,
            payloadHash: preparedAuthorization.payloadHash,
            confirmation: confirmationPhrase,
            confirmExactPayload: true,
            confirmWritesButNoPublication: true,
            confirmNoAutomaticRetry: true,
            authorizationSurface: "SELLER_OS_ONE_CLICK_PUBLICATION_V1",
          })
          const authorizedApproval = object(authorized.approval)
          if (!authorizedApproval.id) {
            throw new Error("EBAY_V3_UNPUBLISHED_APPROVAL_FAILED")
          }
          nextApproval = {
            id: String(authorizedApproval.id),
            status: String(authorizedApproval.status ?? "approved"),
            payload_hash: String(authorizedApproval.payload_hash
              ?? preparedAuthorization.payloadHash
              ?? ""),
            expires_at: String(authorizedApproval.expires_at ?? ""),
          }
      ...
      if (!nextExecution?.id || nextExecution.phase !== "completed") {
        throw new Error(
          nextExecution?.last_error_code
          ?? "EBAY_V3_UNPUBLISHED_EXECUTION_NOT_COMPLETED",
        )
      }
      ...
      if (!nextPublication || nextPublication.phase !== "preview_ready") {
        ...
        nextPublication = preparedPublication.publication
        setDraftState((current) => ({
          ...current,
          publication: preparedPublication.publication,
          publicationRequirements:
            preparedPublication.publicationRequirements,
        }))
      }
      if (!nextPublication?.id || nextPublication.phase !== "preview_ready") {
        throw new Error("EBAY_FINAL_PUBLICATION_PREVIEW_NOT_READY")
      ...
      try {
        ...
        if (
          ...
          const reconciled = await draftRequest({
            action: "reconcile_publish",
            publicationId: refreshedPublication.id,
          })
          setDraftState((current) => ({
            ...current,
            publication: reconciled.publication,
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      controller.abort()
      if (current) {
        const code = "EBAY_V3_UNPUBLISHED_PREFLIGHT_TIMEOUT"
        setUnpublishedAuthorizationError(code)
    ...
    setUnpublishedAuthorizationMode(null)
    void (async () => {
      try {
        const { data: sessionData } = await supabase.auth.getSession()
        ...
        const response = await fetch(
          `/api/admin/ebay/unpublished-offer-authorization?${query}`,
          {
            method: "GET",
            cache: "no-store",
            signal: controller.signal,
        ...
        const payload = await response.json() as Record<string, any>
        if (timedOut || !current) return
        const result = String(payload.preflightResult ?? "ERROR")
        if (
          !response.ok
          ...
          throw new Error(String(payload.error
            ?? payload.preflightReason
            ?? "EBAY_V3_UNPUBLISHED_PREFLIGHT_FAILED"))
        }
        if (!current) return
        const authorization = payload.authorization
        setUnpublishedAuthorization(
          authorization && typeof authorization === "object"
            ? authorization
            : null,
        )
        setUnpublishedAuthorizationMode(
          result === "READY_TO_RESUME_EXISTING_AUTHORIZATION"
            ? "resume_existing_authorization"
            : "new_authorization",
        )
        setUnpublishedPreflightReason(String(payload.preflightReason ?? ""))
        setUnpublishedPreflightState(
          result === "READY_TO_RESUME_EXISTING_AUTHORIZATION"
            ? "ready_resume"
            : "ready_new",
  ...
  useEffect(() => {
    if (!finalReviewCompleted) return
    setError((current) => LEGACY_V3_IMAGE_MESSAGES.has(current.trim())
      ? ""
      : current)
  }, [finalReviewCompleted])
    ...
    ? <section id="v3-final-listing-review" className="space-y-4 rounded-3xl border border-emerald-200/35 bg-emerald-200/[0.07] p-4">
      ...
      <section data-v3-one-click-publication data-unpublished-preflight-state={unpublishedPreflightState} className="space-y-3 rounded-2xl border border-rose-200/35 bg-rose-200/[0.07] p-3">
        ...
        {unpublishedAuthorization && <details className="rounded-xl border border-white/10 bg-black/25 p-3 text-xs">
          <summary className="cursor-pointer font-black">Ver resumen técnico del payload exacto</summary>
          <dl className="mt-3 grid gap-2 sm:grid-cols-2">
            <div><dt className="text-white/45">Cuenta destino</dt><dd className="font-black">{String(object(unpublishedAuthorization.targetAccount).environment)} · seller {String(object(unpublishedAuthorization.targetAccount).maskedSellerAccountId)}</dd></div>
            <div><dt className="text-white/45">SKU</dt><dd className="break-all font-black">{String(unpublishedAuthorization.sku)}</dd></div>
            <div className="sm:col-span-2"><dt className="text-white/45">Título</dt><dd className="font-black">{String(unpublishedAuthorization.title)}</dd></div>
            <div><dt className="text-white/45">Precio / cantidad</dt><dd className="font-black">{String(object(unpublishedAuthorization.price).currency)} {String(object(unpublishedAuthorization.price).value)} · {String(unpublishedAuthorization.listingQuantity)}</dd></div>
            <div><dt className="text-white/45">Imágenes</dt><dd className="font-black">{Array.isArray(unpublishedAuthorization.images) ? unpublishedAuthorization.images.length : 0}/7</dd></div>
          </dl>
  ...
  return (
    <main className="min-h-screen bg-[#05070d] px-4 pb-32 pt-4 text-white sm:px-6">
      <section className="mx-auto max-w-xl space-y-4">
        ...
        {!finalReviewCompleted && <section aria-labelledby="ebay-account-configuration-heading" className={`${maintenanceMode ? "hidden" : ""} space-y-4 rounded-3xl border border-cyan-200/25 bg-cyan-200/[0.05] p-4`}>
          <div>
          ...
          {visibleWorkspaceGateBlockers.length > 0 && <div role="status" className="rounded-2xl border border-amber-200/30 bg-amber-200/[0.08] p-3 text-amber-50">
            <strong className="text-sm">El paquete sigue bloqueado por estas guardas:</strong>
            <ul className="mt-2 space-y-2 text-xs">
              {visibleWorkspaceGateBlockers.map((blocker) => <li key={blocker} className="rounded-xl bg-black/20 p-2"><code className="block break-all font-black text-amber-100">{blocker}</code><span className="mt-1 block text-amber-50/75">{humanWorkspaceBlocker(blocker, form.pricing.minimumProfitablePrice)}</span></li>)}
            </ul>
        ...
        {opportunity && !listingPackage && !publicationLunaRecheck
          && visibleWorkspaceGateBlockers.length === 0 && <section className="rounded-3xl border border-amber-200/30 bg-amber-200/[0.07] p-4">
        ...
        {opportunity && listingPackage && !finalReviewCompleted && <>
          ...
          {publicationGateAllowed && !finalReviewCompleted && <section id="seller-os-final-publication" className={`${maintenanceMode ? "hidden" : ""} scroll-mt-28 space-y-4 rounded-3xl border border-cyan-200/25 bg-cyan-200/[0.05] p-4`}>
            <div>
              <p className="text-xs font-black uppercase tracking-widest text-cyan-100/65">Publicación eBay controlada</p>
              {singleHumanPublicationEligible ? <h2 className="mt-1 text-xl font-black">Una intención humana + validación automática completa · {draftTarget}</h2> : <h2 className="mt-1 text-xl font-black">Offer no publicado + autorización final · {draftTarget}</h2>}
              <p className="mt-2 text-sm leading-6 text-white/65">{singleHumanPublicationEligible ? "Un único clic autoriza el candidato, cuenta, package digest, precio, cantidad, policies y marketplace exactos. Seller OS conserva todas las puertas internas, valida el Offer UNPUBLISHED por lectura oficial y sólo entonces ejecuta publishOffer una vez." : "Primero autorizas y creas el Offer UNPUBLISHED. Después Seller OS vuelve a validar todo, muestra el preview exacto y sólo lo publica cuando das una segunda autorización explícita."}</p>
            ...
            <div className={`rounded-2xl border p-3 text-sm ${productionTarget ? "border-rose-200/40 bg-rose-200/[0.09] text-rose-50" : draftState.runtime?.enabled && draftState.runtime?.configured ? "border-emerald-200/25 bg-emerald-200/[0.06] text-emerald-50" : "border-amber-200/25 bg-amber-200/[0.06] text-amber-50"}`}>
              ...
              <p className="mt-1 text-xs opacity-75">Target: {draftTarget} · {singleHumanPublicationEligible ? "autorización comercial exacta de un solo uso" : "publicación final: autorización separada de un solo uso"} · cuenta exacta obligatoria</p>
              {productionTarget && <p className="mt-2 text-xs font-black">{singleHumanPublicationEligible ? "ATENCIÓN: este clic puede publicar en tu cuenta real únicamente después de que todos los readbacks oficiales coincidan con el payload autorizado." : "ATENCIÓN: el primer permiso crea el Offer sin publicarlo; el segundo publica el preview aprobado en tu cuenta real."}</p>}
            ...
            <div className="rounded-2xl border border-sky-200/25 bg-sky-200/[0.05] p-3 text-sm">
              ...
              {!singleHumanPublicationEligible && <button type="button" disabled={draftBusy || !draftState.runtime?.oauthConfigured} onClick={() => void runEbayPreflight()} className="mt-3 min-h-12 w-full rounded-xl border border-sky-200/35 px-3 font-black text-sky-50 disabled:opacity-40">{draftBusy ? "Consultando…" : "Cargar y validar configuración eBay"}</button>}
              {singleHumanPublicationEligible && <p className="mt-3 rounded-xl border border-sky-200/20 p-2 text-xs text-sky-50/75">El botón único obtiene un preflight fresco y fija la cuenta/policies antes de registrar la autorización; este paso ya no requiere otro clic.</p>}
            ...
            {draftState.readiness && <div className={`rounded-2xl border p-3 ${visibleDraftReadinessReady ? "border-emerald-200/30 bg-emerald-200/[0.06]" : "border-amber-200/30 bg-amber-200/[0.06]"}`}><strong>{visibleDraftReadinessReady ? "Listo para tu aprobación" : `${visibleDraftReadinessBlockers.length} bloqueos pendientes`}</strong>{!visibleDraftReadinessReady && <ul className="mt-2 list-disc space-y-1 pl-5 text-xs text-amber-50">{visibleDraftReadinessBlockers.map((blocker) => <li key={blocker}>{humanWorkspaceBlocker(blocker, form.pricing.minimumProfitablePrice)}</li>)}</ul>}</div>}
            {singleHumanPublicationEligible && <div data-one-click-controlled-publication className="space-y-3 rounded-2xl border border-rose-200/35 bg-rose-200/[0.08] p-4">
              ...
              <dl className="grid gap-2 text-xs sm:grid-cols-2"><div className="rounded-xl bg-black/25 p-2"><dt className="text-white/45">Package</dt><dd className="break-all font-black">{listingPackage.id}</dd></div><div className="rounded-xl bg-black/25 p-2"><dt className="text-white/45">Marketplace / cuenta</dt><dd className="font-black">EBAY_US · {draftTarget}</dd></div><div className="rounded-xl bg-black/25 p-2"><dt className="text-white/45">Precio / cantidad</dt><dd className="font-black">USD {Number(form.pricing.targetPrice ?? 0).toFixed(2)} · {effectiveDraftQuantity}</dd></div><div className="rounded-xl bg-black/25 p-2"><dt className="text-white/45">Policies</dt><dd className="break-all font-black">{draftConfiguration.fulfillmentPolicyId || "pendiente"} · {draftConfiguration.paymentPolicyId || "pendiente"} · {draftConfiguration.returnPolicyId || "pendiente"}</dd></div></dl>
