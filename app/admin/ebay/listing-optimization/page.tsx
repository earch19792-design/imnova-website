...
export default function EbayListingOptimizationCommandCenterPage() {
  ...
  async function createVisualVariant(ebayItemId: string, findingCode: string) {
    ...
    try {
      ...
      const body = await response.json() as Json
      if (!response.ok || body.success !== true) {
        throw new Error([stringValue(body.ERROR_CODE, stringValue(body.error, "La variante se detuvo")), stringValue(body.DEPENDENCY_STAGE, ""), stringValue(body.TRACE_ID, "")].filter(Boolean).join(" · "))
      }
      setVisualVariantNotice(
  ...
  async function updateVisualVariant(assetId: string,
    ...
    try {
      ...
      const workflow = record(record(body.visualVariantAction).workflow)
      setVisualVariantNotice(action === "DISCARD"
        ? "Variante descartada; eBay no cambió."
        : workflow.readyWaitingForBaseline === true
          ? "Experimento preparado. Espera un baseline compatible de Analytics; eBay no cambió."
          : "Experimento preparado y listo para aplicar desde Seller OS.")
      await hydrate()
    } catch (caught) {
      setError(caught instanceof Error ? caught.message :
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-2">
            {visualListings.map((listing) => {
              ...
              return <article key={stringValue(listing.ebayItemId)} className="rounded-2xl border border-white/10 bg-black/25 p-4">
                <Link href={`/admin/ebay/listing-optimization/preview?itemId=${stringValue(listing.ebayItemId)}`} className="mb-3 inline-block text-sm font-bold text-cyan-100 underline">Preparar Preview de optimización</Link>
                ...
                {listingVariants.length ? <div className="mt-4 rounded-2xl border border-violet-200/20 bg-violet-200/[0.04] p-3"><p className="text-xs font-black uppercase tracking-wide text-violet-100">Comparar · original y variantes</p><div className="mt-3 grid gap-3 sm:grid-cols-3"><div><div className="aspect-square overflow-hidden rounded-xl bg-white">{imageUrl ? <img src={imageUrl} alt="Original" className="h-full w-full object-contain" /> : null}</div><p className="mt-2 text-center text-xs font-black">ORIGINAL</p></div>{listingVariants.map((variant) => {
                  const workflow = record(variant.workflow)
                  const baseline = record(workflow.baseline)
                  const failedGuards = rows(workflow.guards).filter((guard) =>
                    guard.status !== "PASSED")
                  const prepared = workflow.canonicalLifecycleStatus === "READY"
                  return <div key={stringValue(variant.assetId)}><div className="aspect-square overflow-hidden rounded-xl bg-white">{typeof variant.previewUrl === "string" ? <img src={variant.previewUrl} alt={`Variante ${stringValue(variant.variantLabel)}`} className="h-full w-full object-contain" /> : null}</div><p className="mt-2 text-center text-xs font-black">VARIANTE {stringValue(variant.variantLabel)}</p><p className="mt-1 text-center text-[10px] text-emerald-100">Product Truth preservado</p><div className="mt-2 grid gap-2"><details className="rounded-lg border border-white/15 p-2 text-xs"><summary className="cursor-pointer text-center font-black">Comparar</summary><p className="mt-2 text-white/50">Objetivo: {stringValue(variant.objective)}</p><p className="mt-1 text-white/50">Hipótesis: {stringValue(variant.hypothesis)}</p></details>{failedGuards.length ? <div className="rounded-lg border border-rose-200/20 bg-rose-200/[0.05] p-3 text-xs"><p className="font-black text-rose-50">Experimento detenido por evidencia</p>{failedGuards.map((guard) => <p key={stringValue(guard.code)} className="mt-1 text-white/55">{stringValue(guard.code)} · {guard.status === "UNPROVEN" ? "evidencia pendiente" : "bloqueado"}{guard.reasonCode ? ` · ${stringValue(guard.reasonCode)}` : ""}</p>)}<p className="mt-2 text-white/45">Evidencia pendiente no significa un resultado comercial negativo. La excepción se resuelve en Seller OS; no se envía a eBay como fallback.</p></div> : null}{prepared ? <div className="rounded-lg border border-amber-200/20 bg-amber-200/[0.05] p-3 text-xs"><p className="font-black text-amber-50">{baseline.status === "AVAILABLE" ? "Baseline disponible" : "Esperando baseline de Analytics"}</p><p className="mt-1 leading-5 text-white/55">{baseline.status === "AVAILABLE" ? "La variante podrá aplicarse cuando todos los guards sigan válidos." : "Las métricas ausentes no se convierten en cero. eBay no cambia mientras falte evidencia compatible."}</p><p className="mt-2 font-black text-white/70">Tasa de clics · CTR</p><p className="mt-1 text-white/50">De cada 100 veces que eBay muestra el producto, indica cuántas personas entran a verlo.</p><button type="button" disabled className="mt-3 min-h-10 w-full rounded-lg border border-white/10 text-xs font-black text-white/35">Aplicar variante · {baseline.status === "AVAILABLE" ? "requiere guards finales" : "esperando métricas"}</button></div> : <button type="button" disabled={Boolean(visualVariantBusy) || failedGuards.length > 0} onClick={() => void updateVisualVariant(stringValue(variant.assetId), "USE_IN_EXPERIMENT")} className="min-h-10 rounded-lg border border-emerald-200/25 text-xs font-black text-emerald-100 disabled:opacity-40">Usar en experimento</button>}<button type="button" disabled={Boolean(visualVariantBusy)} onClick={() => void updateVisualVariant(stringValue(variant.assetId), "DISCARD")} className="min-h-10 rounded-lg border border-rose-200/20 text-xs font-black text-rose-100 disabled:opacity-40">Descartar</button></div></div>
                })}</div><div className="mt-4 rounded-xl border border-cyan-200/15 bg-black/20 p-3 text-xs leading-5 text-white/55"><p className="font-black text-cyan-100">Una variable principal · imagen hero</p><p>Mientras el experimento esté activo, Seller OS bloqueará otro experimento conflictivo sobre la imagen principal.</p><p className="mt-1">Una evaluación sólo dirá GANÓ, PERDIÓ o SIN EVIDENCIA SUFICIENTE después de ventanas compatibles; no atribuye causalidad sin evidencia.</p></div></div> : null}
