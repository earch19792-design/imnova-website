...
import {
  ...
  parseControlledLunaPreResearchAdminRequestV1,
} from "@/lib/ebay/luna-pre-research-admin-api-v1"
import { requestControlledLunaPreResearchRerunV1 } from
  "@/lib/ebay/luna-pre-research-rerun-cohort-v1"
import { getSupabaseAdminClient, validateAdminApiRequest } from
  "@/lib/supabase-admin"

const ENDPOINT = "/api/admin/ebay/pre-research/controlled-rerun"
const OPERATION = "CONTROLLED_RERUN"
const CSRF_COOKIE = "seller_os_admin_pre_research_rerun_csrf"
const MAXIMUM_REQUEST_BYTES = 24_576
...
async function readRequest(request: NextRequest) {
  ...
  if (!body || Buffer.byteLength(body, "utf8") > MAXIMUM_REQUEST_BYTES) {
    throw new SellerOsAdminPreResearchErrorV1(
      "ADMIN_PRE_RESEARCH_RERUN_REQUEST_REJECTED",
    )
  }
  try {
    return parseControlledLunaPreResearchAdminRequestV1(JSON.parse(body))
  } catch (cause) {
...
function failure(cause: unknown) {
  const code = cause instanceof SellerOsAdminPreResearchErrorV1
    ? cause.code : cause instanceof Error && /^[A-Z0-9_]{3,160}$/.test(cause.message)
      ? cause.message : "ADMIN_PRE_RESEARCH_RERUN_FAILED_CLOSED"
  const status = code.includes("OWNER_ADMIN") || code.includes("CSRF")
    ? 403 : code.includes("REQUEST_REJECTED") ? 400 :
      code.includes("MISMATCH") ? 409 : 503
