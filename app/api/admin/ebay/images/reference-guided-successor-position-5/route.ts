...
const EXECUTION_CONFIRMATION =
  "RUN_ONE_STAGING_SUCCESSOR_V2_POSITION_5_PROVIDER_CALL_3"
...
function safeCode(error: unknown) {
  ...
  return message.match(/[A-Z][A-Z0-9_:.-]{2,180}/)?.[0]
    ?? "SUCCESSOR_POSITION_5_EXECUTION_FAILED"
}
...
function assertPreviewBoundary() {
  ...
  if (process.env.VERCEL_ENV !== "preview" ||
    ...
    throw new Error("SUCCESSOR_POSITION_5_PREVIEW_STAGING_REQUIRED")
  }
  if (!process.env.OPENAI_API_KEY?.trim() ||
    process.env.OPENAI_IMAGE_MODEL?.trim() !== "gpt-image-2") {
    throw new Error("SUCCESSOR_POSITION_5_PROVIDER_CONFIGURATION_INVALID")
  }
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (body.confirmation !== EXECUTION_CONFIRMATION) {
      throw new Error("SUCCESSOR_POSITION_5_EXPLICIT_CONFIRMATION_REQUIRED")
    }
    // This flag exists only inside this Preview request. The executor turns it
    // off as soon as the sole HTTP request returns and this finally block also
    // closes every pre-HTTP failure path.
    process.env[FEATURE_FLAG] = "true"
    process.env.CANARY_EXECUTION_ENVIRONMENT = "preview"
    // The module accepts no browser plan, position, prompt, source or hash.
