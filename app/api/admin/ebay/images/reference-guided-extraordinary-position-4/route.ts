...
const EXECUTION_CONFIRMATION =
  "RUN_ONE_STAGING_EXTRAORDINARY_POSITION_4_PROVIDER_CALL_7"
...
function safeCode(error: unknown) {
  ...
  return message.match(/[A-Z][A-Z0-9_:.-]{2,180}/)?.[0]
    ?? "EXTRAORDINARY_POSITION_4_EXECUTION_FAILED"
}
...
function assertPreviewBoundary() {
  ...
  if (process.env.VERCEL_ENV !== "preview" ||
    ...
    throw new Error("EXTRAORDINARY_POSITION_4_PREVIEW_STAGING_REQUIRED")
  }
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (body.confirmation !== EXECUTION_CONFIRMATION) {
      throw new Error("EXTRAORDINARY_POSITION_4_EXPLICIT_CONFIRMATION_REQUIRED")
    }
    process.env[FEATURE_FLAG] = "true"
    process.env.CANARY_EXECUTION_ENVIRONMENT = "preview"
    // No prompt, position, plan, amendment, hash, source or budget is accepted
    // from the caller. The deployed server and database resolve all authority.
    const executed = await import(
      "@/scripts/execute-reference-guided-extraordinary-position-4.mjs"
