...
const EXECUTION_CONFIRMATION =
  "RUN_ONE_STAGING_SUCCESSOR_V2_POSITION_3_PROVIDER_CALL_4"
...
function safeCode(error: unknown) {
  ...
  return message.match(/[A-Z][A-Z0-9_:.-]{2,180}/)?.[0]
    ?? "SUCCESSOR_POSITION_3_EXECUTION_FAILED"
}
...
function assertPreviewBoundary() {
  ...
  if (process.env.VERCEL_ENV !== "preview" ||
    ...
    throw new Error("SUCCESSOR_POSITION_3_PREVIEW_STAGING_REQUIRED")
  }
  if (!process.env.OPENAI_API_KEY?.trim() ||
    process.env.OPENAI_IMAGE_MODEL?.trim() !== "gpt-image-2") {
    throw new Error("SUCCESSOR_POSITION_3_PROVIDER_CONFIGURATION_INVALID")
  }
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (body.confirmation !== EXECUTION_CONFIRMATION) {
      throw new Error("SUCCESSOR_POSITION_3_EXPLICIT_CONFIRMATION_REQUIRED")
    }
