...
export async function POST(request: Request) {
  ...
  try {
    if (validation.authenticationMode === "service_role") {
      ...
      if (body?.action === "DISCARD_PROPOSALS_V1" || body?.action === "ARCHIVE_REJECTED_PROPOSALS_V1") {
        const { discardMayelProposalsV1, archiveRejectedMayelProposalsV1 } = await import("@/lib/seller-os/mayel-proposal-discard-server-v1")
        ...
        if (!taskId || !actorId || !ids.length || ids.length > 6 || ids.some(id => !id))
          throw Error("PROPOSAL_DISCARD_BINDING_INVALID")
        if (body.action === "ARCHIVE_REJECTED_PROPOSALS_V1") return json({ success: true, ...await archiveRejectedMayelProposalsV1({
          supabase: getSupabaseAdminClient(), accountKey: accountKey(), actorUserId: actorId, taskId, itemId: String(body.expectedItemId ?? ""), assetIds: ids as string[] }) })
        if (typeof body.expectedVisualManifestDigest !== "string") throw Error("PROPOSAL_DISCARD_BINDING_INVALID")
    ...
    if (action === "DISCARD_PROPOSALS_V1" || action === "ARCHIVE_REJECTED_PROPOSALS_V1") {
      const taskId = uuid(body?.visualTaskId), ids = Array.isArray(body?.assetIds) ? body.assetIds.map(uuid) : []
      if (!taskId || !ids.length || ids.length > 6 || ids.some(id => !id))
        throw Error("PROPOSAL_DISCARD_BINDING_INVALID")
      const { discardMayelProposalsV1, archiveRejectedMayelProposalsV1 } = await import("@/lib/seller-os/mayel-proposal-discard-server-v1")
      if (action === "ARCHIVE_REJECTED_PROPOSALS_V1") return json({ success: true, ...await archiveRejectedMayelProposalsV1({
        supabase: getSupabaseAdminClient(), accountKey: accountKey(), actorUserId: auth.userId, taskId, itemId: String(body?.expectedItemId ?? ""), assetIds: ids as string[] }) })
      if (typeof body?.expectedVisualManifestDigest !== "string") throw Error("PROPOSAL_DISCARD_BINDING_INVALID")
      return json({ success: true, ...await discardMayelProposalsV1({ supabase: getSupabaseAdminClient(), accountKey: accountKey(),
