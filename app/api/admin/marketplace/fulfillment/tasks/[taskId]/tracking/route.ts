...
import { saveMarketplaceFulfillmentTracking } from "@/lib/marketplace/fulfillment-v1a-service"
...
export async function POST(req: Request, context: { params: Promise<{ taskId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      result: await saveMarketplaceFulfillmentTracking(
...
function safeCode(error: unknown) {
  ...
  return /^FULFILLMENT_[A-Z0-9_]+$/.test(value) ? value : "FULFILLMENT_TRACKING_SAVE_FAILED"
}
